# NEBO-BIBLIOTECA-PADRAO-POR-DOMINIOS-LEX-001

unknown or non-canonical std.math namespace or operation name

```text
Identity: NEBO-BIBLIOTECA-PADRAO-POR-DOMINIOS-LEX-001 (DIAGNOSTIC_CODE)
Edition: 1
...
```text
Code: NEBO-BIBLIOTECA-PADRAO-POR-DOMINIOS-LEX-001
Severity: 1
Phase: 3
Message: unknown or non-canonical std.math namespace or operation name
```

## Meaning and remediation

Condition reported: unknown or non-canonical std.math namespace or operation name. Locate the primary span, correct this condition and re-run check before requesting build. For capacity diagnostics reduce the indicated bounded input; for type/receiver errors use the declared domain; for internal invariant failures retain the source and compiler identity for a compiler defect report.
