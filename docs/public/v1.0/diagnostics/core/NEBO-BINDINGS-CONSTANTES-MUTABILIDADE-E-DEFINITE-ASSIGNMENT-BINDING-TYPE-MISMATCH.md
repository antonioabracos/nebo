# NEBO-BINDINGS-CONSTANTES-MUTABILIDADE-E-DEFINITE-ASSIGNMENT-BINDING-TYPE-MISMATCH

binding expression has the wrong public scalar type

```text
Identity: NEBO-BINDINGS-CONSTANTES-MUTABILIDADE-E-DEFINITE-ASSIGNMENT-BINDING-TYPE-MISMATCH (DIAGNOSTIC_CODE)
Edition: 1
...
```text
Code: NEBO-BINDINGS-CONSTANTES-MUTABILIDADE-E-DEFINITE-ASSIGNMENT-BINDING-TYPE-MISMATCH
Severity: 1
Phase: 6
Message: binding expression has the wrong public scalar type
```

## Meaning and remediation

Condition reported: binding expression has the wrong public scalar type. Locate the primary span, correct this condition and re-run check before requesting build. For capacity diagnostics reduce the indicated bounded input; for type/receiver errors use the declared domain; for internal invariant failures retain the source and compiler identity for a compiler defect report.
