# NEBO-CONSOLE-VISUAL-DASHBOARD-E-PLOTS-SECURITY-006

non-canonical integer spelling or authenticated summary integrity failed

```text
Identity: NEBO-CONSOLE-VISUAL-DASHBOARD-E-PLOTS-SECURITY-006 (DIAGNOSTIC_CODE)
Edition: 1
...
```text
Code: NEBO-CONSOLE-VISUAL-DASHBOARD-E-PLOTS-SECURITY-006
Severity: 1
Phase: 14
Message: non-canonical integer spelling or authenticated summary integrity failed
```

## Meaning and remediation

Condition reported: non-canonical integer spelling or authenticated summary integrity failed. Locate the primary span, correct this condition and re-run check before requesting build. For capacity diagnostics reduce the indicated bounded input; for type/receiver errors use the declared domain; for internal invariant failures retain the source and compiler identity for a compiler defect report.
