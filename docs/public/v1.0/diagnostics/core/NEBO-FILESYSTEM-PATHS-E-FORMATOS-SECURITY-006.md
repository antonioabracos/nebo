# NEBO-FILESYSTEM-PATHS-E-FORMATOS-SECURITY-006

path traversal or authenticated lexical summary integrity check failed

```text
Identity: NEBO-FILESYSTEM-PATHS-E-FORMATOS-SECURITY-006 (DIAGNOSTIC_CODE)
Edition: 1
...
```text
Code: NEBO-FILESYSTEM-PATHS-E-FORMATOS-SECURITY-006
Severity: 1
Phase: 14
Message: path traversal or authenticated lexical summary integrity check failed
```

## Meaning and remediation

Condition reported: path traversal or authenticated lexical summary integrity check failed. Locate the primary span, correct this condition and re-run check before requesting build. For capacity diagnostics reduce the indicated bounded input; for type/receiver errors use the declared domain; for internal invariant failures retain the source and compiler identity for a compiler defect report.
