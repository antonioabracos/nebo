# NEBO-IMPORTS-MODULOS-NAMESPACES-E-API-PUBLICA-PARSE-002

malformed module package import symbol or visibility syntax

```text
Identity: NEBO-IMPORTS-MODULOS-NAMESPACES-E-API-PUBLICA-PARSE-002 (DIAGNOSTIC_CODE)
Edition: 1
...
```text
Code: NEBO-IMPORTS-MODULOS-NAMESPACES-E-API-PUBLICA-PARSE-002
Severity: 1
Phase: 4
Message: malformed module package import symbol or visibility syntax
```

## Meaning and remediation

Condition reported: malformed module package import symbol or visibility syntax. Locate the primary span, correct this condition and re-run check before requesting build. For capacity diagnostics reduce the indicated bounded input; for type/receiver errors use the declared domain; for internal invariant failures retain the source and compiler identity for a compiler defect report.
