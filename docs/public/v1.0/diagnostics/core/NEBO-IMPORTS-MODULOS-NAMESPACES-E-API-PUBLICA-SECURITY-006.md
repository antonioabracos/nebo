# NEBO-IMPORTS-MODULOS-NAMESPACES-E-API-PUBLICA-SECURITY-006

module cycle visibility boundary or graph integrity check failed

```text
Identity: NEBO-IMPORTS-MODULOS-NAMESPACES-E-API-PUBLICA-SECURITY-006 (DIAGNOSTIC_CODE)
Edition: 1
...
```text
Code: NEBO-IMPORTS-MODULOS-NAMESPACES-E-API-PUBLICA-SECURITY-006
Severity: 1
Phase: 14
Message: module cycle visibility boundary or graph integrity check failed
```

## Meaning and remediation

Condition reported: module cycle visibility boundary or graph integrity check failed. Locate the primary span, correct this condition and re-run check before requesting build. For capacity diagnostics reduce the indicated bounded input; for type/receiver errors use the declared domain; for internal invariant failures retain the source and compiler identity for a compiler defect report.
