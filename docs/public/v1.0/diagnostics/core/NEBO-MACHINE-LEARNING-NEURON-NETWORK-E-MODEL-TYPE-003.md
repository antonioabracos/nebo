# NEBO-MACHINE-LEARNING-NEURON-NETWORK-E-MODEL-TYPE-003

shape dtype or scalar is outside the local deterministic profile

```text
Identity: NEBO-MACHINE-LEARNING-NEURON-NETWORK-E-MODEL-TYPE-003 (DIAGNOSTIC_CODE)
Edition: 1
...
```text
Code: NEBO-MACHINE-LEARNING-NEURON-NETWORK-E-MODEL-TYPE-003
Severity: 1
Phase: 6
Message: shape dtype or scalar is outside the local deterministic profile
```

## Meaning and remediation

Condition reported: shape dtype or scalar is outside the local deterministic profile. Locate the primary span, correct this condition and re-run check before requesting build. For capacity diagnostics reduce the indicated bounded input; for type/receiver errors use the declared domain; for internal invariant failures retain the source and compiler identity for a compiler defect report.
