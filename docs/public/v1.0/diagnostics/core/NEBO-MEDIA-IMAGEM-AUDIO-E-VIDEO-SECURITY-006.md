# NEBO-MEDIA-IMAGEM-AUDIO-E-VIDEO-SECURITY-006

media privacy capability or authenticated descriptor invariant failed

```text
Identity: NEBO-MEDIA-IMAGEM-AUDIO-E-VIDEO-SECURITY-006 (DIAGNOSTIC_CODE)
Edition: 1
...
```text
Code: NEBO-MEDIA-IMAGEM-AUDIO-E-VIDEO-SECURITY-006
Severity: 1
Phase: 14
Message: media privacy capability or authenticated descriptor invariant failed
```

## Meaning and remediation

Condition reported: media privacy capability or authenticated descriptor invariant failed. Locate the primary span, correct this condition and re-run check before requesting build. For capacity diagnostics reduce the indicated bounded input; for type/receiver errors use the declared domain; for internal invariant failures retain the source and compiler identity for a compiler defect report.
