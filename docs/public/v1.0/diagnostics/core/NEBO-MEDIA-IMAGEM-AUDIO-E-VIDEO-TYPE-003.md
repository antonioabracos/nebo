# NEBO-MEDIA-IMAGEM-AUDIO-E-VIDEO-TYPE-003

media dimensions rate channels or duration are outside the bounded profile

```text
Identity: NEBO-MEDIA-IMAGEM-AUDIO-E-VIDEO-TYPE-003 (DIAGNOSTIC_CODE)
Edition: 1
...
```text
Code: NEBO-MEDIA-IMAGEM-AUDIO-E-VIDEO-TYPE-003
Severity: 1
Phase: 6
Message: media dimensions rate channels or duration are outside the bounded profile
```

## Meaning and remediation

Condition reported: media dimensions rate channels or duration are outside the bounded profile. Locate the primary span, correct this condition and re-run check before requesting build. For capacity diagnostics reduce the indicated bounded input; for type/receiver errors use the declared domain; for internal invariant failures retain the source and compiler identity for a compiler defect report.
