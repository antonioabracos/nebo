# NEBO-MEMORIA-OWNERSHIP-LIFETIMES-E-RECURSOS-SECURITY-006

borrow exclusivity region token or use-after-move check failed

```text
Identity: NEBO-MEMORIA-OWNERSHIP-LIFETIMES-E-RECURSOS-SECURITY-006 (DIAGNOSTIC_CODE)
Edition: 1
...
```text
Code: NEBO-MEMORIA-OWNERSHIP-LIFETIMES-E-RECURSOS-SECURITY-006
Severity: 1
Phase: 14
Message: borrow exclusivity region token or use-after-move check failed
```

## Meaning and remediation

Condition reported: borrow exclusivity region token or use-after-move check failed. Locate the primary span, correct this condition and re-run check before requesting build. For capacity diagnostics reduce the indicated bounded input; for type/receiver errors use the declared domain; for internal invariant failures retain the source and compiler identity for a compiler defect report.
...
### control:callable-borrow-escape; expected NEBO-MEMORIA-OWNERSHIP-LIFETIMES-E-RECURSOS-SECURITY-006

```nebo
callable view(Int.value) capture borrow 9 {capture.return;}start(){view.call(1).return;}
```

Oracle: {"artifact_publication": "REJECTED", "compiler_exit": 1, "diagnostic": "NEBO-MEMORIA-OWNERSHIP-LIFETIMES-E-RECURSOS-SECURITY-006"}
