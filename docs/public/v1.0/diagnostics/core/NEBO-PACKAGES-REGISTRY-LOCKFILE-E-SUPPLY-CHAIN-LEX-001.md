# NEBO-PACKAGES-REGISTRY-LOCKFILE-E-SUPPLY-CHAIN-LEX-001

unknown or non-canonical package manifest or policy name

```text
Identity: NEBO-PACKAGES-REGISTRY-LOCKFILE-E-SUPPLY-CHAIN-LEX-001 (DIAGNOSTIC_CODE)
Edition: 1
...
```text
Code: NEBO-PACKAGES-REGISTRY-LOCKFILE-E-SUPPLY-CHAIN-LEX-001
Severity: 1
Phase: 3
Message: unknown or non-canonical package manifest or policy name
```

## Meaning and remediation

Condition reported: unknown or non-canonical package manifest or policy name. Locate the primary span, correct this condition and re-run check before requesting build. For capacity diagnostics reduce the indicated bounded input; for type/receiver errors use the declared domain; for internal invariant failures retain the source and compiler identity for a compiler defect report.
