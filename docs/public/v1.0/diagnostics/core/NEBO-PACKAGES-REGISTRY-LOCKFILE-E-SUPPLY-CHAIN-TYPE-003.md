# NEBO-PACKAGES-REGISTRY-LOCKFILE-E-SUPPLY-CHAIN-TYPE-003

package dependency version or digest identity is invalid

```text
Identity: NEBO-PACKAGES-REGISTRY-LOCKFILE-E-SUPPLY-CHAIN-TYPE-003 (DIAGNOSTIC_CODE)
Edition: 1
...
```text
Code: NEBO-PACKAGES-REGISTRY-LOCKFILE-E-SUPPLY-CHAIN-TYPE-003
Severity: 1
Phase: 6
Message: package dependency version or digest identity is invalid
```

## Meaning and remediation

Condition reported: package dependency version or digest identity is invalid. Locate the primary span, correct this condition and re-run check before requesting build. For capacity diagnostics reduce the indicated bounded input; for type/receiver errors use the declared domain; for internal invariant failures retain the source and compiler identity for a compiler defect report.
