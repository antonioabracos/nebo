# NEBO-RUNTIME-DISTRIBUIDO-E-STREAMING-DISTRIBUIDO-RUNTIME-005

ordering backpressure retry timeout loss duplicate or ack invariant failed

```text
Identity: NEBO-RUNTIME-DISTRIBUIDO-E-STREAMING-DISTRIBUIDO-RUNTIME-005 (DIAGNOSTIC_CODE)
Edition: 1
...
```text
Code: NEBO-RUNTIME-DISTRIBUIDO-E-STREAMING-DISTRIBUIDO-RUNTIME-005
Severity: 1
Phase: 11
Message: ordering backpressure retry timeout loss duplicate or ack invariant failed
```

## Meaning and remediation

Condition reported: ordering backpressure retry timeout loss duplicate or ack invariant failed. Locate the primary span, correct this condition and re-run check before requesting build. For capacity diagnostics reduce the indicated bounded input; for type/receiver errors use the declared domain; for internal invariant failures retain the source and compiler identity for a compiler defect report.
