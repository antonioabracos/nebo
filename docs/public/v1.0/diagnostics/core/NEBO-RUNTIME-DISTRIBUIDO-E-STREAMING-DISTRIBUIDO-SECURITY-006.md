# NEBO-RUNTIME-DISTRIBUIDO-E-STREAMING-DISTRIBUIDO-SECURITY-006

cleanup or authenticated local simulation invariant failed

```text
Identity: NEBO-RUNTIME-DISTRIBUIDO-E-STREAMING-DISTRIBUIDO-SECURITY-006 (DIAGNOSTIC_CODE)
Edition: 1
...
```text
Code: NEBO-RUNTIME-DISTRIBUIDO-E-STREAMING-DISTRIBUIDO-SECURITY-006
Severity: 1
Phase: 14
Message: cleanup or authenticated local simulation invariant failed
```

## Meaning and remediation

Condition reported: cleanup or authenticated local simulation invariant failed. Locate the primary span, correct this condition and re-run check before requesting build. For capacity diagnostics reduce the indicated bounded input; for type/receiver errors use the declared domain; for internal invariant failures retain the source and compiler identity for a compiler defect report.
