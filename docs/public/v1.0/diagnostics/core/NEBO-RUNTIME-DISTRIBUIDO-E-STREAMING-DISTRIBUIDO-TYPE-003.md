# NEBO-RUNTIME-DISTRIBUIDO-E-STREAMING-DISTRIBUIDO-TYPE-003

queue capacity retry or timeout field is outside the local profile

```text
Identity: NEBO-RUNTIME-DISTRIBUIDO-E-STREAMING-DISTRIBUIDO-TYPE-003 (DIAGNOSTIC_CODE)
Edition: 1
...
```text
Code: NEBO-RUNTIME-DISTRIBUIDO-E-STREAMING-DISTRIBUIDO-TYPE-003
Severity: 1
Phase: 6
Message: queue capacity retry or timeout field is outside the local profile
```

## Meaning and remediation

Condition reported: queue capacity retry or timeout field is outside the local profile. Locate the primary span, correct this condition and re-run check before requesting build. For capacity diagnostics reduce the indicated bounded input; for type/receiver errors use the declared domain; for internal invariant failures retain the source and compiler identity for a compiler defect report.
