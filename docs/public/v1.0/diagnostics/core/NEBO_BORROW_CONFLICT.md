# NEBO_BORROW_CONFLICT

mutation or transfer conflicts with an active lexical borrow

```text
Identity: NEBO_BORROW_CONFLICT (DIAGNOSTIC_CODE)
Edition: 1
...
### sequence:iterator-borrow; expected NEBO_BORROW_CONFLICT

```nebo
start(){List<Int>.new().a;a.iterator().i;a.push(17);23.return;}
```
