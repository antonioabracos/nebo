...
call positional argument count does not match signature

```text
Identity: NEBO_CALL_ARITY (DIAGNOSTIC_CODE)
Edition: 1
...
### control:call-arity; expected NEBO_CALL_ARITY

```nebo
(Int.self)add(Int.delta){(self+delta).return;}start(){17.add().return;}
```

Oracle: {"artifact_publication": "REJECTED", "compiler_exit": 1, "diagnostic": "NEBO_CALL_ARITY"}
