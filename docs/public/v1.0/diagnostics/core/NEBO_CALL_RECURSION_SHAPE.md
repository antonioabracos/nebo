# NEBO_CALL_RECURSION_SHAPE

recursive function must have one exact if/else base case and one self-call site

```text
Identity: NEBO_CALL_RECURSION_SHAPE (DIAGNOSTIC_CODE)
Edition: 1
...
```text
Code: NEBO_CALL_RECURSION_SHAPE
Severity: 1
Phase: 10
Message: recursive function must have one exact if/else base case and one self-call site
```

## Meaning and remediation

Condition reported: recursive function must have one exact if/else base case and one self-call site. Locate the primary span, correct this condition and re-run check before requesting build. For capacity diagnostics reduce the indicated bounded input; for type/receiver errors use the declared domain; for internal invariant failures retain the source and compiler identity for a compiler defect report.
