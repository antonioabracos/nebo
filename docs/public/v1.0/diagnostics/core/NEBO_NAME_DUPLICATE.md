...
```text
Identity: NEBO_NAME_DUPLICATE (DIAGNOSTIC_CODE)
Edition: 1
...
### composite:struct-update-duplicate; expected NEBO_NAME_DUPLICATE

```nebo
struct Point { Int.x; Int.y; }
start() { Point.default().p; p.update(x: 7, x: 9).q; q.x; }

```

Oracle: {"artifact_publication": "REJECTED", "compiler_exit": 1, "diagnostic": "NEBO_NAME_DUPLICATE"}
