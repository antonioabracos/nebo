# NEBO_NESTED_CAPTURE_UNSUPPORTED

nested helper capture is unsupported; the environment must remain zero bytes

```text
Identity: NEBO_NESTED_CAPTURE_UNSUPPORTED (DIAGNOSTIC_CODE)
Edition: 1
...
```text
Code: NEBO_NESTED_CAPTURE_UNSUPPORTED
Severity: 1
Phase: 6
Message: nested helper capture is unsupported; the environment must remain zero bytes
```

## Meaning and remediation

Condition reported: nested helper capture is unsupported; the environment must remain zero bytes. Locate the primary span, correct this condition and re-run check before requesting build. For capacity diagnostics reduce the indicated bounded input; for type/receiver errors use the declared domain; for internal invariant failures retain the source and compiler identity for a compiler defect report.
