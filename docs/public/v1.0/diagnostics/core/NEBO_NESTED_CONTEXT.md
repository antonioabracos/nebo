...
nested helper declaration is only valid directly in a receiver-first outer function body

```text
Identity: NEBO_NESTED_CONTEXT (DIAGNOSTIC_CODE)
Edition: 1
...
```text
Code: NEBO_NESTED_CONTEXT
Severity: 1
Phase: 4
Message: nested helper declaration is only valid directly in a receiver-first outer function body
```

## Meaning and remediation

Condition reported: nested helper declaration is only valid directly in a receiver-first outer function body. Locate the primary span, correct this condition and re-run check before requesting build. For capacity diagnostics reduce the indicated bounded input; for type/receiver errors use the declared domain; for internal invariant failures retain the source and compiler identity for a compiler defect report.
