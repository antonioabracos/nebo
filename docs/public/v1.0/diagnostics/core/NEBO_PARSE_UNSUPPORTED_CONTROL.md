# NEBO_PARSE_UNSUPPORTED_CONTROL

control feature is reserved but unsupported in Nebo v0.1

```text
Identity: NEBO_PARSE_UNSUPPORTED_CONTROL (DIAGNOSTIC_CODE)
Edition: 1
...
### lexical:unavailable-0; expected NEBO_PARSE_UNSUPPORTED_CONTROL

```nebo
start(){switch(7){23.return;}23.return;}
```

Oracle: {"artifact_publication": "REJECTED", "compiler_exit": 1, "diagnostic": "NEBO_PARSE_UNSUPPORTED_CONTROL"}
