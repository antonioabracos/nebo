# NEBO_SOURCE_BIDI_CONTROL

bidirectional control is forbidden in source syntax; remove the code point shown in the note

```text
Identity: NEBO_SOURCE_BIDI_CONTROL (DIAGNOSTIC_CODE)
Edition: 1
...
```text
Code: NEBO_SOURCE_BIDI_CONTROL
Severity: 1
Phase: 14
Message: bidirectional control is forbidden in source syntax; remove the code point shown in the note
```

## Meaning and remediation

Condition reported: bidirectional control is forbidden in source syntax; remove the code point shown in the note. Locate the primary span, correct this condition and re-run check before requesting build. For capacity diagnostics reduce the indicated bounded input; for type/receiver errors use the declared domain; for internal invariant failures retain the source and compiler identity for a compiler defect report.
