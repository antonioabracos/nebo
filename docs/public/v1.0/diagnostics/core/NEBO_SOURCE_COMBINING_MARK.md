# NEBO_SOURCE_COMBINING_MARK

combining mark is forbidden in source syntax; use an exact visible token spelling

```text
Identity: NEBO_SOURCE_COMBINING_MARK (DIAGNOSTIC_CODE)
Edition: 1
...
```text
Code: NEBO_SOURCE_COMBINING_MARK
Severity: 1
Phase: 14
Message: combining mark is forbidden in source syntax; use an exact visible token spelling
```

## Meaning and remediation

Condition reported: combining mark is forbidden in source syntax; use an exact visible token spelling. Locate the primary span, correct this condition and re-run check before requesting build. For capacity diagnostics reduce the indicated bounded input; for type/receiver errors use the declared domain; for internal invariant failures retain the source and compiler identity for a compiler defect report.
