# NEBO_SOURCE_MALFORMED_UTF8

source is not strict UTF-8; replace or remove the invalid byte sequence

```text
Identity: NEBO_SOURCE_MALFORMED_UTF8 (DIAGNOSTIC_CODE)
Edition: 1
...
```text
Code: NEBO_SOURCE_MALFORMED_UTF8
Severity: 1
Phase: 14
Message: source is not strict UTF-8; replace or remove the invalid byte sequence
```

## Meaning and remediation

Condition reported: source is not strict UTF-8; replace or remove the invalid byte sequence. Locate the primary span, correct this condition and re-run check before requesting build. For capacity diagnostics reduce the indicated bounded input; for type/receiver errors use the declared domain; for internal invariant failures retain the source and compiler identity for a compiler defect report.
