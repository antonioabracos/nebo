# NEBO_SOURCE_UNICODE_CONFUSABLE

confusable Unicode source spelling is forbidden; use the canonical ASCII replacement from the note

```text
Identity: NEBO_SOURCE_UNICODE_CONFUSABLE (DIAGNOSTIC_CODE)
Edition: 1
...
```text
Code: NEBO_SOURCE_UNICODE_CONFUSABLE
Severity: 1
Phase: 14
Message: confusable Unicode source spelling is forbidden; use the canonical ASCII replacement from the note
```

## Meaning and remediation

Condition reported: confusable Unicode source spelling is forbidden; use the canonical ASCII replacement from the note. Locate the primary span, correct this condition and re-run check before requesting build. For capacity diagnostics reduce the indicated bounded input; for type/receiver errors use the declared domain; for internal invariant failures retain the source and compiler identity for a compiler defect report.
...
### lexical:unicode-11; expected NEBO_SOURCE_UNICODE_CONFUSABLE
