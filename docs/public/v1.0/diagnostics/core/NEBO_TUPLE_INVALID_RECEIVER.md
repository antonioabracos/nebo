# NEBO_TUPLE_INVALID_RECEIVER

Tuple operation requires a Tuple receiver

```text
Identity: NEBO_TUPLE_INVALID_RECEIVER (DIAGNOSTIC_CODE)
Edition: 1
...
### composite:tuple-to-struct-mismatch; expected NEBO_TUPLE_INVALID_RECEIVER

```nebo
struct Pair { Int.first; Bool.second; }
start() { Tuple.of(4, 9).p; p.toStruct<Pair>().q; q.first; }
