...
Build the admitted native source profile.

```text
Identity: cli:build (CLI_FORM)
Edition: 1
...
```text
neboc build source.no -o output [--edition 1|2] [--no-prelude]
```

## Behavior and usage

Produces an x86_64 Linux System V ELF at -o after validation. Execute the program separately to observe its result; Linux observes the low exit byte.
