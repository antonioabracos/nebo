...
Control --fail-fast for diagnostics.

```text
Identity: cli:--fail-fast (CLI_FORM)
Edition: 1
...
```text
neboc check source.no --fail-fast
```

## Behavior and usage

Stop recovery at the first error. Mutually exclusive with --keep-going; duplicate options reject.
...
neboc check invalid.no --fail-fast
...
### cli:limit-8-fail-fast; expected NEBO_NAME_UNDEFINED
