...
Control --keep-going for diagnostics.

```text
Identity: cli:--keep-going (CLI_FORM)
Edition: 1
...
```text
neboc check source.no --keep-going
```

## Behavior and usage

Continue bounded check recovery up to --max-errors. Mutually exclusive with --fail-fast; duplicate options reject.
...
neboc check invalid.no --keep-going
...
### cli:limit-8-keep-going; expected NEBO_NAME_UNDEFINED
