...
Control --max-errors for diagnostics.

```text
Identity: cli:--max-errors (CLI_FORM)
Edition: 1
...
```text
neboc check source.no --max-errors 1..64
```

## Behavior and usage

Bound the number of check diagnostics. Zero, 65, missing, duplicate and nonnumeric arguments are usage errors.
...
neboc check invalid.no --max-errors 8
...
### cli:limit-1-keep-going; expected NEBO_NAME_UNDEFINED
