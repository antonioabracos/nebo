# neboc --message-format

Control --message-format for diagnostics.

```text
Identity: cli:--message-format (CLI_FORM)
Edition: 1
...
```text
neboc check source.no --message-format human|short|json-lines|sarif
```

## Behavior and usage

Choose a diagnostic projection. JSON-lines schema 1 retains code, severity, phase and byte spans. SARIF 2.1.0 retains neboSchema=1. Human layout is not a machine schema.
...
neboc check invalid.no --message-format json-lines
...
### diagnostics:type-json-lines; expected NEBO_TYPE_MISMATCH
