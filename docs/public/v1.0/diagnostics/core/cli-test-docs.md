...
Compile and execute embedded doc examples and laws.

```text
Identity: cli:test-docs (CLI_FORM)
Edition: 1
...
```text
neboc test-docs app.no --seed 7 --budget 2 --report json
```

## Behavior and usage

Use exit-N and law-exit-N labels in the admitted bounded profile, with N in 0..255. The runner compares real process results; a passing build is insufficient.
