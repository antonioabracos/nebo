...
Supply a source or material interface unit.

```text
Identity: cli:--unit (CLI_FORM)
Edition: 1
...
```text
neboc module-check|link app.no --unit provider.no
```

## Behavior and usage

The admitted .ni source profile carries public Int constants. Source and material-interface consumers must preserve identity and value; metadata-only declarations cannot manufacture a value.
...
neboc module-check app.no --unit core.no --unit util.no
