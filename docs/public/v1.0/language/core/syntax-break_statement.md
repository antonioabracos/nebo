# break_statement syntax

The break_statement production in the admitted structural grammar. Its delimiters and ordering are shown below; type, lifetime and profile restrictions apply after parsing.

```text
Identity: grammar:break_statement (GRAMMAR_PRODUCTION)
Edition: 1
...
### control:loop-break-7; expected 7

```nebo
start(){0.i.mutable;loop{if(i==7){break;}i+=1;}i.return;}
```
