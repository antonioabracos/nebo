# callable_decl syntax

The callable_decl production in the admitted structural grammar. Its delimiters and ordering are shown below; type, lifetime and profile restrictions apply after parsing.

```text
Identity: grammar:callable_decl (GRAMMAR_PRODUCTION)
Edition: 1
...
### control:callable-move-17; expected 24

```nebo
callable work(Int.value) capture move 17 { sum.return; }start(){work.call(7).return;}
```
