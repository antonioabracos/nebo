...
The char production in the admitted structural grammar. Its delimiters and ordering are shown below; type, lifetime and profile restrictions apply after parsing.

```text
Identity: grammar:char (GRAMMAR_PRODUCTION)
Edition: 1
...
### binding:Char-0-implicit; expected 23

```nebo
start(){'Q'.value;value.codepoint().console();23.return;}
```
