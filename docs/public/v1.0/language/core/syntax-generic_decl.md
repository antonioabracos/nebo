...
The generic_decl production in the admitted structural grammar. Its delimiters and ordering are shown below; type, lifetime and profile restrictions apply after parsing.

```text
Identity: grammar:generic_decl (GRAMMAR_PRODUCTION)
Edition: 1
...
### composite:generic-unconstrained; expected 37

```nebo
generic<T> (T.value)hold() { value.return; }
start() { 37.hold(); }
