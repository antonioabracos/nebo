...
The if_statement production in the admitted structural grammar. Its delimiters and ordering are shown below; type, lifetime and profile restrictions apply after parsing.

```text
Identity: grammar:if_statement (GRAMMAR_PRODUCTION)
Edition: 1
...
### control:branch-3; expected 17

```nebo
start(){3.value;if(value<5){17.return;}else{29.return;}}
```
