...
The newtype_decl production in the admitted structural grammar. Its delimiters and ordering are shown below; type, lifetime and profile restrictions apply after parsing.

```text
Identity: grammar:newtype_decl (GRAMMAR_PRODUCTION)
Edition: 1
...
### composite:newtype-int; expected 42

```nebo
newtype UserId(Int);
start() {
    UserId(42).unwrap().return;
}
