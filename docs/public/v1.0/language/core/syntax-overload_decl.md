# overload_decl syntax

The overload_decl production in the admitted structural grammar. Its delimiters and ordering are shown below; type, lifetime and profile restrictions apply after parsing.

```text
Identity: grammar:overload_decl (GRAMMAR_PRODUCTION)
Edition: 1
...
### control:explicit-overload-17-False; expected 37

```nebo
overload (Int.self)pick(Int.value) where Any symbol picked_int {37.return;}overload (Int.self)pick(Bool.value) where Any symbol picked_bool {71.return;}start(){7.pick(17).return;}
```
