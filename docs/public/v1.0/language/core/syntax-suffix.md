...
The suffix production in the admitted structural grammar. Its delimiters and ordering are shown below; type, lifetime and profile restrictions apply after parsing.

```text
Identity: grammar:suffix (GRAMMAR_PRODUCTION)
Edition: 1
...
```text
suffix = call_suffix | field_suffix | postfix_operator | lateral_block ;
```
