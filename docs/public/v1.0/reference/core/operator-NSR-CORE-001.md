# . — flow/member/call/binding dot

The visual and semantic center of Nebo: member access, fluent call, pipeline continuation and value-first binding are decided by parser context.

```text
Identity: NSR-CORE-001 (REGISTRY_ID)
Edition: 1
...
```text
lexeme: .
context: expression, receiver-first call and terminal binding
fixity: postfix/contextual
arity: 1+
precedence: P180_SUFFIX
associativity: left/contextual
canonical_ascii: .
operand_rule: receiver or value followed by a valid member, call, pipeline stage or binding terminal
result_rule: member/call result, transformed receiver, or a new binding
```

## Evaluation and types

receiver exactly once; stages left-to-right. Short-circuit: NO. Operand rule: receiver or value followed by a valid member, call, pipeline stage or binding terminal. Result rule: member/call result, transformed receiver, or a new binding.

## Errors and remediation

type/resolution error before codegen; no hidden fallback. Current native grammar/context only; no implicit activation. Use the exact admitted spelling and operand domain; recognition alone never activates a reserved form.
...
### operator:discard-scope; expected 23

```nebo
start(){17.console()._;if(true){29.console()._;}83.console()._;23.return;}
```

Oracle: {"console_text_utf8": "172983", "independent_builds": 2, "kinds": \[4, 4, 4\], "process_exit": 23, "runtime_sha256": "7eb8719b7b83e47df497d23e2b5286f43e9ae7ebd166bc78c2f2e68cd4a50168"}
...
- [operator-NSR-CORE-005](operator-NSR-CORE-005.md)
