# () — grouping and call delimiters

Groups expressions and invokes callables. Nested calls reuse the general expression grammar.

```text
Identity: NSR-CORE-002 (REGISTRY_ID)
Edition: 1
...
```text
lexeme: ()
context: expressions and argument lists
fixity: delimited/postfix
arity: 0+
precedence: P180_SUFFIX
associativity: left
canonical_ascii: ()
operand_rule: groupable expression or callable plus typed arguments
result_rule: grouped value or callable result
```

## Evaluation and types

callee once, arguments left-to-right exactly once. Short-circuit: NO. Operand rule: groupable expression or callable plus typed arguments. Result rule: grouped value or callable result.

## Errors and remediation

arity/type/effect diagnostics; no call on failure. Current native grammar/context only; no implicit activation. Use the exact admitted spelling and operand domain; recognition alone never activates a reserved form.
