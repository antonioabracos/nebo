# &lt;...&gt; — generic type argument delimiters

Angle brackets in type context are distinct from relational operators in expression context.

```text
Identity: NSR-CORE-005 (REGISTRY_ID)
Edition: 1
...
```text
lexeme: <...>
context: type context only
fixity: delimited/contextual
arity: 1+
precedence: TYPE_GRAMMAR
associativity: N/A
canonical_ascii: <...>
operand_rule: generic type constructor plus valid type arguments
result_rule: instantiated type
```

## Evaluation and types

compile-time only. Short-circuit: NO. Operand rule: generic type constructor plus valid type arguments. Result rule: instantiated type.

## Errors and remediation

generic arity/constraint diagnostic. Current native grammar/context only; no implicit activation. Use the exact admitted spelling and operand domain; recognition alone never activates a reserved form.
