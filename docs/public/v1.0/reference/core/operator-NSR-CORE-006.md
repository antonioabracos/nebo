...
Comma is a separator, never a free expression operator.

```text
Identity: NSR-CORE-006 (REGISTRY_ID)
Edition: 1
...
```text
lexeme: ,
context: arguments, elements, fields, bindings and tuples
fixity: separator
arity: N
precedence: STRUCTURAL
associativity: N/A
canonical_ascii: ,
operand_rule: context-specific list members
result_rule: no value of its own
```

## Evaluation and types

source order. Short-circuit: NO. Operand rule: context-specific list members. Result rule: no value of its own.

## Errors and remediation

missing-element or trailing-comma policy diagnostic. Current native grammar/context only; no implicit activation. Use the exact admitted spelling and operand domain; recognition alone never activates a reserved form.
...
### operator:bytes-construct-four-0; expected 0

```nebo
(Int.v)observe(){v.console();v.return;}start(){Bytes.fromValues(0.observe(),17.observe(),29.observe(),83.observe()).b;b.at(0).console();b.at(3).console();0.return;}
```

Oracle: {"console_text_utf8": "0172983083", "independent_builds": 2, "kinds": \[4, 4, 4, 4, 4, 4\], "process_exit": 0, "runtime_sha256": "d559456850d8f22ef7a27e9dc0db2681fb4ce7d7eb8d36b4131ce68a7fc19c5f"}

## Rejected examples

### operator:bytes-four-arity; expected NEBO_TYPE_MISMATCH

```nebo
(Int.self)invalid(){Bytes.fromValues(17,29,83);23.return;}start(){7.invalid().return;}
```
