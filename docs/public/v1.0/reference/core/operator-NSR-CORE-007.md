...
Terminates statements. It is not an operator and cannot be overloaded.

```text
Identity: NSR-CORE-007 (REGISTRY_ID)
Edition: 1
...
```text
lexeme: ;
context: statement and selected arm termination
fixity: terminator
arity: 0
precedence: STRUCTURAL
associativity: N/A
canonical_ascii: ;
operand_rule: completed statement
result_rule: no value
```

## Evaluation and types

N/A. Short-circuit: NO. Operand rule: completed statement. Result rule: no value.

## Errors and remediation

missing/extra terminator diagnostic. Current native grammar/context only; no implicit activation. Use the exact admitted spelling and operand domain; recognition alone never activates a reserved form.
