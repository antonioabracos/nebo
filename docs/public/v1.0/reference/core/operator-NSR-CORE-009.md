# -&gt; — return-type and match-arm arrow

The current Assembly language uses \`-&gt;\` for bounded match arms and return-type context; it is not pointer member access.

```text
Identity: NSR-CORE-009 (REGISTRY_ID)
Edition: 1
...
```text
lexeme: ->
context: function signatures and match arms
fixity: separator/contextual
arity: 2
precedence: STRUCTURAL
associativity: right/contextual
canonical_ascii: ->
operand_rule: signature type relation or pattern/body relation
result_rule: type declaration or selected arm result
```

## Evaluation and types

compile-time dispatch; selected arm once. Short-circuit: MATCH_ONLY. Operand rule: signature type relation or pattern/body relation. Result rule: type declaration or selected arm result.

## Errors and remediation

invalid-context diagnostic; member-arrow meaning is rejected. Current native grammar/context only; no implicit activation. Use the exact admitted spelling and operand domain; recognition alone never activates a reserved form.
...
### operator:wildcard-_-29; expected 7

```nebo
enum Number { Empty, Value(Int) }start(){Number.Value(29).v;match(v){Empty -> 0.return;_ -> 7.return;}}
```
