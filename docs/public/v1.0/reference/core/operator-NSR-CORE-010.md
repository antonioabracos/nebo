# _ — wildcard and ignored binding

Matches or ignores without introducing a normal binding.

```text
Identity: NSR-CORE-010 (REGISTRY_ID)
Edition: 1
...
```text
lexeme: _
context: patterns and explicitly ignored names
fixity: atomic/contextual
arity: 0
precedence: STRUCTURAL
associativity: N/A
canonical_ascii: _
operand_rule: pattern or binding context
result_rule: no bound value unless a future pattern says otherwise
```

## Evaluation and types

compile-time only. Short-circuit: NO. Operand rule: pattern or binding context. Result rule: no bound value unless a future pattern says otherwise.

## Errors and remediation

invalid-position diagnostic. Current native grammar/context only; no implicit activation. Use the exact admitted spelling and operand domain; recognition alone never activates a reserved form.
...
### operator:discard-effects-29-83; expected 7

```nebo
start(){29.console()._;83.console()._;7.return;}
```

Oracle: {"console_text_utf8": "2983", "independent_builds": 2, "kinds": \[4, 4\], "process_exit": 7, "runtime_sha256": "5fc01f0e12eae85e6b542d74fbc2bb30ba343f71666ec119a5e3a04b02a43253"}
