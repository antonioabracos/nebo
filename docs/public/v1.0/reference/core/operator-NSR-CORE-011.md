...
Begins a line comment. It also permanently prevents \`//\` from becoming integer division.

```text
Identity: NSR-CORE-011 (REGISTRY_ID)
Edition: 1
...
```text
lexeme: //
context: lexer outside Text/Char/raw literals
fixity: lexical
arity: N
precedence: LEXICAL
associativity: N/A
canonical_ascii: //
operand_rule: any source bytes until newline or EOF
result_rule: no token stream output
```

## Evaluation and types

lexical skip. Short-circuit: NO. Operand rule: any source bytes until newline or EOF. Result rule: no token stream output.

## Errors and remediation

none unless encoding/source controls are invalid. Current native grammar/context only; no implicit activation. Use the exact admitted spelling and operand domain; recognition alone never activates a reserved form.
...
### operator:comment-29; expected 7

```nebo
// 99.return; ignored as source comment
start(){"//".console();29.console();7.return;}// comment at EOF
```

Oracle: {"console_text_utf8": "//29", "independent_builds": 2, "kinds": \[2, 4\], "process_exit": 7, "runtime_sha256": "4f176402737b0f6fc2e746f2f01ed6fd85ad2bed4adc2a1f6aaa94ec767350ea"}

## Rejected examples

### operator:comment-does-not-bind; expected NEBO_NAME_UNDEFINED

```nebo
// 17.hidden;
start(){hidden.console();23.return;}
```
