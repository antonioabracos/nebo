...
Adds numeric values. Text concatenation is not implicit and belongs to the domain-gated \`⧺\` operator.

```text
Identity: NSR-CORE-012 (REGISTRY_ID)
Edition: 1
...
associativity: left
canonical_ascii: +
operand_rule: same-type numeric operands or an explicitly registered Add protocol with no implicit unsafe coercion
result_rule: numeric or protocol-declared result
```

## Evaluation and types

left then right exactly once. Short-circuit: NO. Operand rule: same-type numeric operands or an explicitly registered Add protocol with no implicit unsafe coercion. Result rule: numeric or protocol-declared result.

## Errors and remediation

checked overflow by default; invalid type rejected. Current native grammar/context only; no implicit activation. Use the exact admitted spelling and operand domain; recognition alone never activates a reserved form.
...
### operator:binary-2b-6; expected 23
...
### operator:wrong-2b; expected NEBO_TYPE_MISMATCH
