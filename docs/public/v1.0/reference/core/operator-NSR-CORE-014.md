...
Scalar multiplication. Vector cross, Cartesian product and tensor products use distinct domain symbols.

```text
Identity: NSR-CORE-014 (REGISTRY_ID)
Edition: 1
...
associativity: left
canonical_ascii: *
operand_rule: same-type numeric operands or Multiply protocol
result_rule: numeric or protocol-declared result
```

## Evaluation and types

left then right exactly once. Short-circuit: NO. Operand rule: same-type numeric operands or Multiply protocol. Result rule: numeric or protocol-declared result.
...
### operator:binary-2a-6; expected 23
...
### operator:wrong-2a; expected NEBO_TYPE_MISMATCH
