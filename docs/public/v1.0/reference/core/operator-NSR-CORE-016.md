...
In infix position \`%\` means remainder. Template placeholders and postfix percentage are separate contexts.

```text
Identity: NSR-CORE-016 (REGISTRY_ID)
Edition: 1
...
```text
lexeme: %
context: between expressions
fixity: infix
arity: 2
precedence: P140_MULTIPLICATIVE
associativity: left
canonical_ascii: %
operand_rule: Int operands in the current profile; future types require a Remainder protocol
result_rule: Int or protocol-declared result
```

## Evaluation and types

left then right exactly once. Short-circuit: NO. Operand rule: Int operands in the current profile; future types require a Remainder protocol. Result rule: Int or protocol-declared result.

## Errors and remediation

zero divisor checked; result obeys the signed division identity. Current native grammar/context only; no implicit activation. Use the exact admitted spelling and operand domain; recognition alone never activates a reserved form.
...
### operator:binary-25-6; expected 23
...
### operator:wrong-25; expected NEBO_TYPE_MISMATCH
