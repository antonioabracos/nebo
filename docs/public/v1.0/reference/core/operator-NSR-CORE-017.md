...
Canonical exponentiation. It is intentionally not XOR.

```text
Identity: NSR-CORE-017 (REGISTRY_ID)
Edition: 1
...
```text
lexeme: ^
context: numeric or algebraic expression
fixity: infix
arity: 2
precedence: P160_POWER
associativity: right
canonical_ascii: ^
operand_rule: Power protocol; initial core profiles Int^nonnegative-Int and Float-compatible powers
result_rule: type-specific numeric/algebraic result
```

## Evaluation and types

base then exponent exactly once. Short-circuit: NO. Operand rule: Power protocol; initial core profiles Int^nonnegative-Int and Float-compatible powers. Result rule: type-specific numeric/algebraic result.

## Errors and remediation

checked overflow/domain policy; negative Int exponent requires explicit promotion. Current native grammar/context only; no implicit activation. Use the exact admitted spelling and operand domain; recognition alone never activates a reserved form.
...
### operator:binary-5e-6; expected 23
...
### operator:wrong-5e; expected NEBO_TYPE_MISMATCH
