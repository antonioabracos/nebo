...
Explicit numeric identity, primarily useful for generated and mathematical source.

```text
Identity: NSR-CORE-018 (REGISTRY_ID)
Edition: 1
...
associativity: right
canonical_ascii: +
operand_rule: numeric operand
result_rule: same type
```

## Evaluation and types

operand exactly once. Short-circuit: NO. Operand rule: numeric operand. Result rule: same type.

## Errors and remediation

invalid type rejected. Current native grammar/context only; no implicit activation. Use the exact admitted spelling and operand domain; recognition alone never activates a reserved form.
...
### operator:unary-2b-2; expected 23
...
### operator:wrong-unary-2b; expected NEBO_TYPE_MISMATCH
