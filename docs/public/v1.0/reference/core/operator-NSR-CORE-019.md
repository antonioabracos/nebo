...
Negates a value. Power binds more tightly: \`-2 ^ 2\` means \`-(2 ^ 2)\`.

```text
Identity: NSR-CORE-019 (REGISTRY_ID)
Edition: 1
...
```text
lexeme: -
context: prefix numeric expression
fixity: prefix
arity: 1
precedence: P150_PREFIX
associativity: right
canonical_ascii: -
operand_rule: signed numeric operand or Negate protocol
result_rule: same or declared result type
```

## Evaluation and types

operand exactly once. Short-circuit: NO. Operand rule: signed numeric operand or Negate protocol. Result rule: same or declared result type.

## Errors and remediation

checked overflow for minimum signed integer. Current native grammar/context only; no implicit activation. Use the exact admitted spelling and operand domain; recognition alone never activates a reserved form.
...
### operator:unary-2d-2; expected 23
...
### operator:wrong-unary-2d; expected NEBO_TYPE_MISMATCH
