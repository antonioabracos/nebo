...
Strict ordering. Chained comparisons are rejected; use explicit conjunction.

```text
Identity: NSR-CORE-024 (REGISTRY_ID)
Edition: 1
...
canonical_ascii: <
operand_rule: same compatible Comparable domain
result_rule: Bool
```

## Evaluation and types

left then right exactly once. Short-circuit: NO. Operand rule: same compatible Comparable domain. Result rule: Bool.

## Errors and remediation

unsupported comparison rejected. Current native grammar/context only; no implicit activation. Use the exact admitted spelling and operand domain; recognition alone never activates a reserved form.
...
### operator:binary-3c-6; expected 23
...
### operator:wrong-3c; expected NEBO_TYPE_MISMATCH
