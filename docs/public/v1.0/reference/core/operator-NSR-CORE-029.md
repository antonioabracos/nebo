...
Logical negation of typed equality without re-evaluating operands.

```text
Identity: NSR-CORE-029 (REGISTRY_ID)
Edition: 1
...
canonical_ascii: !=
operand_rule: same Eq domain
result_rule: Bool
```

## Evaluation and types

left then right exactly once. Short-circuit: NO. Operand rule: same Eq domain. Result rule: Bool.
...
### operator:binary-213d-6; expected 23
...
### operator:wrong-213d; expected NEBO_TYPE_MISMATCH
