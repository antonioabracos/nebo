...
Assignment never creates a new binding and is not equality.

```text
Identity: NSR-CORE-031 (REGISTRY_ID)
Edition: 1
...
```text
lexeme: =
context: statement to an existing mutable lvalue
fixity: statement operator
arity: 2
precedence: P020_ASSIGNMENT
associativity: right/statement
canonical_ascii: =
operand_rule: mutable lvalue and type-compatible RHS
result_rule: no value
```

## Evaluation and types

target once, RHS once, one store on success. Short-circuit: NO. Operand rule: mutable lvalue and type-compatible RHS. Result rule: no value.

## Errors and remediation

zero store on rejection/failure. Current native grammar/context only; no implicit activation. Use the exact admitted spelling and operand domain; recognition alone never activates a reserved form.
...
### binding:Int-store; expected 71

```nebo
start(){17.value.mutable;value=71;value.return;}
```
