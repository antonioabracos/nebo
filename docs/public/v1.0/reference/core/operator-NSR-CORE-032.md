...
Compound assignment with exactly-once lvalue and RHS evaluation.

```text
Identity: NSR-CORE-032 (REGISTRY_ID)
Edition: 1
...
```text
lexeme: +=
context: statement to an existing mutable numeric lvalue
fixity: statement operator
arity: 2
precedence: P020_ASSIGNMENT
associativity: right/statement
canonical_ascii: +=
operand_rule: mutable lvalue supporting Add and compatible RHS
result_rule: no value
```

## Evaluation and types

target once, old value once, RHS once, one store on success. Short-circuit: NO. Operand rule: mutable lvalue supporting Add and compatible RHS. Result rule: no value.

## Errors and remediation

checked overflow; zero store on failure. Current native grammar/context only; no implicit activation. Use the exact admitted spelling and operand domain; recognition alone never activates a reserved form.
...
### binding:Int-compound-add; expected 23

```nebo
start(){17.value.mutable;value+=11;value.console();23.return;}
```

Oracle: {"console_text_utf8": "28", "independent_builds": 2, "kinds": \[4\], "process_exit": 23, "runtime_sha256": "e4ba6e0415ec7addd98a6b3cbbd4e1e8f361eecb24b97835219ff77bb132fdfd"}
