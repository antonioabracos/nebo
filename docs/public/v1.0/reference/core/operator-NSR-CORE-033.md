# -= — checked subtract assignment

Checked subtraction assignment.

```text
Identity: NSR-CORE-033 (REGISTRY_ID)
Edition: 1
...
canonical_ascii: -=
operand_rule: mutable lvalue supporting Subtract
result_rule: no value
```

## Evaluation and types

target once, old value once, RHS once, one store on success. Short-circuit: NO. Operand rule: mutable lvalue supporting Subtract. Result rule: no value.
...
### binding:Int-compound-subtract; expected 23

```nebo
start(){17.value.mutable;value-=3;value.console();23.return;}
```

Oracle: {"console_text_utf8": "14", "independent_builds": 2, "kinds": \[4\], "process_exit": 23, "runtime_sha256": "d46ef69b558ae64a3e81d5d25ad06d1cc82f3d5ee980cd95ccebb1550169aa86"}
