# /= — checked divide assignment

Future checked division assignment.

```text
Identity: NSR-CORE-035 (REGISTRY_ID)
Edition: 1
...
canonical_ascii: /=
operand_rule: mutable lvalue supporting Divide
result_rule: no value
```

## Evaluation and types

target once, old value once, RHS once, one store on success. Short-circuit: NO. Operand rule: mutable lvalue supporting Divide. Result rule: no value.

## Errors and remediation

zero divisor/overflow checked; zero store on failure. Current native grammar/context only; no implicit activation. Use the exact admitted spelling and operand domain; recognition alone never activates a reserved form.
...
### binding:Int-compound-divide; expected 23

```nebo
start(){17.value.mutable;value/=3;value.console();23.return;}
```

Oracle: {"console_text_utf8": "5", "independent_builds": 2, "kinds": \[4\], "process_exit": 23, "runtime_sha256": "816d3386c44dcbbd80f98b40609269a285c5345a853a942d371ffdfce09d7f03"}
