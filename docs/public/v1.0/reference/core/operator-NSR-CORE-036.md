# %= — checked remainder assignment

Future checked remainder assignment.

```text
Identity: NSR-CORE-036 (REGISTRY_ID)
Edition: 1
...
```text
lexeme: %=
context: statement to an existing mutable Int-like lvalue
fixity: statement operator
arity: 2
precedence: P020_ASSIGNMENT
associativity: right/statement
canonical_ascii: %=
operand_rule: mutable lvalue supporting Remainder
result_rule: no value
```

## Evaluation and types

target once, old value once, RHS once, one store on success. Short-circuit: NO. Operand rule: mutable lvalue supporting Remainder. Result rule: no value.

## Errors and remediation

zero divisor checked; zero store on failure. Current native grammar/context only; no implicit activation. Use the exact admitted spelling and operand domain; recognition alone never activates a reserved form.
...
### binding:Int-compound-remainder; expected 23

```nebo
start(){17.value.mutable;value%=7;value.console();23.return;}
```

Oracle: {"console_text_utf8": "3", "independent_builds": 2, "kinds": \[4\], "process_exit": 23, "runtime_sha256": "31d898c40aa78dc7169f4fabac579cf3c04c2d6a0db4aad3d7360a0b56e9bc3e"}
