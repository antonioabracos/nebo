# ^= — checked power assignment

Power assignment; XOR assignment uses no \`^=\` interpretation.

```text
Identity: NSR-CORE-037 (REGISTRY_ID)
Edition: 1
...
```text
lexeme: ^=
context: statement to an existing mutable power-capable lvalue
fixity: statement operator
arity: 2
precedence: P020_ASSIGNMENT
associativity: right/statement
canonical_ascii: ^=
operand_rule: mutable lvalue supporting Power
result_rule: no value
```

## Evaluation and types

target once, old value once, exponent once, one store on success. Short-circuit: NO. Operand rule: mutable lvalue supporting Power. Result rule: no value.

## Errors and remediation

overflow/domain checked; zero store on failure. Current native grammar/context only; no implicit activation. Use the exact admitted spelling and operand domain; recognition alone never activates a reserved form.

## Availability and ownership

ACTIVE in core.mutation. Exact Registry token and real Program statements; independent value/document/exit oracles, changed operands, associated negatives; Int64/Bool/binary64 or the referenced public typed profile; native checked arithmetic and complete source statement ownership. Int64/Bool/binary64/Unicode scalar values; checked arithmetic; lexical evaluation once The operator itself grants no capability. Owned operands obey the move/borrow rules of their concrete types.

## Executed examples

### binding:Int-compound-power; expected 23

```nebo
start(){17.value.mutable;value^=2;value.console();23.return;}
```

Oracle: {"console_text_utf8": "289", "independent_builds": 2, "kinds": \[4\], "process_exit": 23, "runtime_sha256": "2a66d1d43b6308e1d9feb5af9d11d0855a460f03fad569f4b5fccd21760b7288"}
