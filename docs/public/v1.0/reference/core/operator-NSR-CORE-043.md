...
Uses the single ellipsis U+2026 so ASCII \`..\` remains available for lateral flow.

```text
Identity: NSR-CORE-043 (REGISTRY_ID)
Edition: 1
...
canonical_ascii: Range.inclusive
operand_rule: compatible ordered step-capable endpoints
result_rule: Range<T> inclusive at both ends
```

## Evaluation and types

start then end exactly once. Short-circuit: NO. Operand rule: compatible ordered step-capable endpoints. Result rule: Range&lt;T&gt; inclusive at both ends.

## Errors and remediation

invalid direction/step handled by Range policy. Current native grammar/context only; no implicit activation. Use the exact admitted spelling and operand domain; recognition alone never activates a reserved form.
...
### operator:range-e280a6-0; expected 23
...
### operator:chained-range; expected NEBO_PARSE_UNEXPECTED_TOKEN
