...
```text
Identity: NSR-CORE-045 (REGISTRY_ID)
Edition: 1
...
canonical_ascii: Range.exclusiveStart
operand_rule: compatible ordered step-capable endpoints
result_rule: Range<T> excluding start and including end
```

## Evaluation and types

start then end exactly once. Short-circuit: NO. Operand rule: compatible ordered step-capable endpoints. Result rule: Range&lt;T&gt; excluding start and including end.
...
### operator:range-3ce280a6-0; expected 23
