...
```text
Identity: NSR-CORE-046 (REGISTRY_ID)
Edition: 1
...
canonical_ascii: Range.exclusive
operand_rule: compatible ordered step-capable endpoints
result_rule: Range<T> excluding both ends
```

## Evaluation and types

start then end exactly once. Short-circuit: NO. Operand rule: compatible ordered step-capable endpoints. Result rule: Range&lt;T&gt; excluding both ends.
...
### operator:range-3ce280a63c-0; expected 23
