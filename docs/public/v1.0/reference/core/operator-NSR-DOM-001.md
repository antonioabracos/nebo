...
Square-root operator; negative real input never silently becomes an untyped NaN.

```text
Identity: NSR-DOM-001 (REGISTRY_ID)
Edition: 1
...
associativity: right
canonical_ascii: sqrt()
operand_rule: Sqrt protocol or supported numeric/symbolic type
result_rule: type-specific root or Result
```

## Evaluation and types

operand once. Short-circuit: NO. Operand rule: Sqrt protocol or supported numeric/symbolic type. Result rule: type-specific root or Result.

## Errors and remediation

domain/precision policy is explicit. Native checked exact signed Int roots; nonintegral/invalid real roots trap through the arithmetic domain owner. Use the exact admitted spelling and operand domain; recognition alone never activates a reserved form.
