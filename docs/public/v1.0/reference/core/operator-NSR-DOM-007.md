...
Circle constant equal to two pi in the selected precision domain.

```text
Identity: NSR-DOM-007 (REGISTRY_ID)
Edition: 1
...
```text
lexeme: τ
context: math/angle profile
fixity: literal constant
arity: 0
precedence: LITERAL
associativity: N/A
canonical_ascii: TAU
...
compile-time or canonical constant load. Short-circuit: NO. Operand rule: context supporting the selected precision. Result rule: typed constant.

## Errors and remediation

precision profile explicit. Current native grammar/context only; no implicit activation. Use the exact admitted spelling and operand domain; recognition alone never activates a reserved form.

## Availability and ownership

ACTIVE_BOUNDED in std.math.constants. Exact Registry token and real Program statements; independent value/document/exit oracles, changed operands, associated negatives; Int64/Bool/binary64 or the referenced public typed profile; native checked arithmetic and complete source statement ownership. Native checked scalar/vector mathematics; independent numeric oracle; explicit process return separate from intermediate value The operator itself grants no capability. Owned operands obey the move/borrow rules of their concrete types.

## Executed examples

### operator:constant-tau-7; expected 7

```nebo
start(){τ.v;"%.6f".format(v).console();7.return;}
```

Oracle: {"console_text_utf8": "6.283185", "independent_builds": 2, "kinds": \[2\], "process_exit": 7, "publications": 1, "runtime_sha256": "20391e81f88894d5109074fec38513b43bebe590fe794194faaeea52763f1d5c"}

## Rejected examples

### operator:wrong-domain-cf84; expected NEBO_TYPE_MISMATCH

```nebo
(Int.self)invalid(){(τ + true).return;}start(){7.invalid().return;}
```
