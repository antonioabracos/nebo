...
Summation with a dedicated binder grammar; it never hides nondeterministic parallel reduction.

```text
Identity: NSR-DOM-008 (REGISTRY_ID)
Edition: 1
...
associativity: N/A
canonical_ascii: sum()
operand_rule: finite iterable plus a summable mapped expression
result_rule: type-specific sum or reduction report
```

## Evaluation and types

domain once; iteration order explicit and deterministic by default. Short-circuit: NO. Operand rule: finite iterable plus a summable mapped expression. Result rule: type-specific sum or reduction report.

## Errors and remediation

overflow, cancellation and floating reduction policy explicit. Finite immutable Array&lt;Int,N&gt; domain; canonical checked constant fold in source order; empty identities 0/1; constant overflow is rejected with NEBO_TYPE_CONSTANT_OVERFLOW; later publication and explicit return remain in the shared Program. Use the exact admitted spelling and operand domain; recognition alone never activates a reserved form.
...
### operator:reduce-e28891-\[\]; expected 23
