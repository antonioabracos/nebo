...
Typed negation of approximate equality.

```text
Identity: NSR-DOM-012 (REGISTRY_ID)
Edition: 1
...
operand_rule: same as `≈`
result_rule: Bool
```

## Evaluation and types

operands once; tolerance once. Short-circuit: NO. Operand rule: same as \`≈\`. Result rule: Bool.

## Errors and remediation

hidden epsilon forbidden. Current native grammar/context only; no implicit activation. Use the exact admitted spelling and operand domain; recognition alone never activates a reserved form.
...
### operator:uncertain-e28989-8-2-12-3; expected 23
...
### operator:wrong-domain-e28989; expected NEBO_TYPE_MISMATCH
