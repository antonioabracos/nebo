...
Represents semantic/symbolic equivalence, distinct from runtime typed equality.

```text
Identity: NSR-DOM-013 (REGISTRY_ID)
Edition: 1
...
```text
lexeme: ≡
context: symbolic/formal profile
fixity: infix
arity: 2
precedence: P110_EQUALITY
associativity: non-associative
canonical_ascii: equivalentTo()
operand_rule: symbolic expressions or explicitly registered equivalence domain
result_rule: Bool or ProofObligation
```

## Evaluation and types

operands once. Short-circuit: NO. Operand rule: symbolic expressions or explicitly registered equivalence domain. Result rule: Bool or ProofObligation.

## Errors and remediation

must not collapse to ordinary \`==\`. Current native grammar/context only; no implicit activation. Use the exact admitted spelling and operand domain; recognition alone never activates a reserved form.

## Availability and ownership

ACTIVE_BOUNDED in std.symbolic. Exact Registry token and real Program statements; independent value/document/exit oracles, changed operands, associated negatives; Int64/Bool/binary64 or the referenced public typed profile; native checked arithmetic and complete source statement ownership. Int64/Bool/binary64/Unicode scalar values; checked arithmetic; lexical evaluation once The operator itself grants no capability. Owned operands obey the move/borrow rules of their concrete types.

## Executed examples

### operator:uncertain-e289a1-8-2-12-3; expected 23
...
### operator:wrong-domain-e289a1; expected NEBO_TYPE_MISMATCH
