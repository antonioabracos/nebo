...
Creates or tests a typed proportionality relation.

```text
Identity: NSR-DOM-018 (REGISTRY_ID)
Edition: 1
...
```text
lexeme: ∝
context: symbolic/measurement profile
fixity: infix
arity: 2
...
operands once. Short-circuit: NO. Operand rule: symbolic expressions, quantities or datasets with a declared proportionality model. Result rule: Bool, relation or constraint.

## Errors and remediation

model/units must be explicit. Current native grammar/context only; no implicit activation. Use the exact admitted spelling and operand domain; recognition alone never activates a reserved form.
