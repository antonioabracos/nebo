...
Cartesian product. In vector context the same code point means cross product via type-directed domain resolution.

```text
Identity: NSR-DOM-034 (REGISTRY_ID)
Edition: 1
...
associativity: left
canonical_ascii: cartesianProduct()
operand_rule: finite compatible Sets/collections
result_rule: Set<Tuple<A,B>> or bounded product view
```

## Evaluation and types

left then right once; pair order deterministic. Short-circuit: NO. Operand rule: finite compatible Sets/collections. Result rule: Set&lt;Tuple&lt;A,B&gt;&gt; or bounded product view.

## Errors and remediation

cardinality/budget checked. Current G133 finite immutable Array&lt;Int,N&gt; set profile with distinct values in 0..62; canonical mask/cardinality owner; independent Python set operations. This does not substitute for the separately proved native Dict/Set APIs. Use the exact admitted spelling and operand domain; recognition alone never activates a reserved form.
...
### operator:set-c397-4; expected 23
