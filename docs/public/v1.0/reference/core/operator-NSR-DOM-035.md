...
```text
Identity: NSR-DOM-035 (REGISTRY_ID)
Edition: 1
...
precedence: LITERAL
associativity: N/A
canonical_ascii: Set.empty()
operand_rule: context supplies the element type or an explicit type is required
result_rule: Set<T>
```

## Evaluation and types

compile-time literal. Short-circuit: NO. Operand rule: context supplies the element type or an explicit type is required. Result rule: Set&lt;T&gt;.

## Errors and remediation

ambiguous element type rejected. Current native grammar/context only; no implicit activation. Use the exact admitted spelling and operand domain; recognition alone never activates a reserved form.
...
### operator:set-empty-context-4; expected 23

```nebo
start(){Array<Int,0> [].left;Array<Int,0> [].right;(left ∪ ∅).cardinality().console();23.return;}
```
...
### operator:wrong-set-e288aa; expected NEBO_TYPE_MISMATCH
