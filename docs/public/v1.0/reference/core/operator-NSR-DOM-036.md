...
Dot product or type-declared inner scalar product.

```text
Identity: NSR-DOM-036 (REGISTRY_ID)
Edition: 1
...
associativity: left
canonical_ascii: dot()
operand_rule: compatible vectors or algebraic structures with DotProduct
result_rule: scalar or declared result
```

## Evaluation and types

left then right once. Short-circuit: NO. Operand rule: compatible vectors or algebraic structures with DotProduct. Result rule: scalar or declared result.

## Errors and remediation

shape/type mismatch explicit. Exact immutable Int Vector aggregate grammar, extent 1..4; matrix Int/Complex profile at most four logical cells; vector observers sum, checked rational inverse numerator/denominator, exact integer norm; symbolic/named parity; all source tokens consumed by the canonical current profile, without adjacent statements. Typed infix statement profile additionally supports Int extents 1..64 (cross/parallel exactly 3), tensor result at most 64 lanes; native checked kernels, distinct owned results, ordered operands and explicit program return. Use the exact admitted spelling and operand domain; recognition alone never activates a reserved form.
...
### boundedoperator:dom-036-named-0; expected 60

```nebo
start(){Vector<Int,3> [2,3,4].a;Vector<Int,3> [5,6,8].b;a.dot(b);}
```

Oracle: {"independent_builds": 2, "process_exit": 60, "runtime_sha256": "e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855"}
