...
```text
Identity: NSR-DOM-038 (REGISTRY_ID)
Edition: 1
...
associativity: left
canonical_ascii: hadamard()
operand_rule: equal-shape matrices/tensors
result_rule: same-shape value
```

## Evaluation and types

left then right once. Short-circuit: NO. Operand rule: equal-shape matrices/tensors. Result rule: same-shape value.

## Errors and remediation

shape mismatch explicit. Exact immutable Int Vector aggregate grammar, extent 1..4; matrix Int/Complex profile at most four logical cells; vector observers sum, checked rational inverse numerator/denominator, exact integer norm; symbolic/named parity; all source tokens consumed by the canonical current profile, without adjacent statements. Typed infix statement profile additionally supports Int extents 1..64 (cross/parallel exactly 3), tensor result at most 64 lanes; native checked kernels, distinct owned results, ordered operands and explicit program return. Use the exact admitted spelling and operand domain; recognition alone never activates a reserved form.
...
### boundedoperator:dom-038-named-0; expected 60

```nebo
start(){Vector<Int,3> [2,3,4].a;Vector<Int,3> [5,6,8].b;a.hadamard(b).sum();}
```
