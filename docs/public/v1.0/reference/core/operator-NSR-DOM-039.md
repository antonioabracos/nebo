...
Tensor or Kronecker product according to operand types.

```text
Identity: NSR-DOM-039 (REGISTRY_ID)
Edition: 1
...
associativity: left
canonical_ascii: tensorProduct()
operand_rule: compatible vectors/matrices/tensors
result_rule: higher-rank or Kronecker result
```

## Evaluation and types

left then right once. Short-circuit: NO. Operand rule: compatible vectors/matrices/tensors. Result rule: higher-rank or Kronecker result.

## Errors and remediation

shape/size budget explicit. Exact immutable Int Vector aggregate grammar, extent 1..4; matrix Int/Complex profile at most four logical cells; vector observers sum, checked rational inverse numerator/denominator, exact integer norm; symbolic/named parity; all source tokens consumed by the canonical current profile, without adjacent statements. Typed infix statement profile additionally supports Int extents 1..64 (cross/parallel exactly 3), tensor result at most 64 lanes; native checked kernels, distinct owned results, ordered operands and explicit program return. Use the exact admitted spelling and operand domain; recognition alone never activates a reserved form.
...
Oracle: {"independent_builds": 2, "process_exit": 171, "runtime_sha256": "e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855"}
