...
```text
Identity: NSR-DOM-040 (REGISTRY_ID)
Edition: 1
...
associativity: left
canonical_ascii: directSum()
operand_rule: compatible algebraic structures
result_rule: direct-sum structure
```

## Evaluation and types

left then right once. Short-circuit: NO. Operand rule: compatible algebraic structures. Result rule: direct-sum structure.

## Errors and remediation

shape/budget explicit. Exact immutable Int Vector aggregate grammar, extent 1..4; matrix Int/Complex profile at most four logical cells; vector observers sum, checked rational inverse numerator/denominator, exact integer norm; symbolic/named parity; all source tokens consumed by the canonical current profile, without adjacent statements. Use the exact admitted spelling and operand domain; recognition alone never activates a reserved form.
...
### boundedoperator:dom-040-symbol-0; expected 28

```nebo
start(){Vector<Int,3> [2,3,4].a;Vector<Int,3> [5,6,8].b;(a ⊕ b).sum();}
```

Oracle: {"independent_builds": 2, "process_exit": 28, "runtime_sha256": "e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855"}

## Rejected examples

### boundedoperator:dom-040-negative; expected NEBO_TYPE_CONSTANT_OVERFLOW

```nebo
start(){Vector<Int,1> [1].a;Vector<Int,1> [9223372036854775807].b;(a ⊕ b).sum();}
```

Oracle: {"artifact_publication": "REJECTED", "compiler_exit": 1, "diagnostic": "NEBO_TYPE_CONSTANT_OVERFLOW"}
