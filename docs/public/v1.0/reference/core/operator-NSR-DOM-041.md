...
```text
Identity: NSR-DOM-041 (REGISTRY_ID)
Edition: 1
...
```text
lexeme: ᵀ
context: matrix/tensor profile
fixity: postfix
arity: 1
precedence: P170_POSTFIX
associativity: left
canonical_ascii: transpose()
operand_rule: matrix or transpose-capable tensor/view
result_rule: transposed view/value
```

## Evaluation and types

operand once. Short-circuit: NO. Operand rule: matrix or transpose-capable tensor/view. Result rule: transposed view/value.

## Errors and remediation

view ownership and shape explicit. Exact immutable Int Vector aggregate grammar, extent 1..4; matrix Int/Complex profile at most four logical cells; vector observers sum, checked rational inverse numerator/denominator, exact integer norm; symbolic/named parity; all source tokens consumed by the canonical current profile, without adjacent statements. Use the exact admitted spelling and operand domain; recognition alone never activates a reserved form.
...
### boundedoperator:dom-041-symbol-0-0-1; expected 7

```nebo
import "std.scientific" { Matrix; }.scientific;
start(){Matrix<Int,2,2> [2,5,7,11].m;mᵀ.at(0,1);}
```
...
### boundedoperator:dom-041-negative; expected NEBO_TUPLE_INDEX_OUT_OF_RANGE

```nebo
import "std.scientific" { Matrix; }.scientific;
// The transposed shape is checked before any observed access.
start() {
...
Oracle: {"artifact_publication": "REJECTED", "compiler_exit": 1, "diagnostic": "NEBO_TUPLE_INDEX_OUT_OF_RANGE"}
