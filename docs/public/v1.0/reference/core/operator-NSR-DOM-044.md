...
Mathematical norm delimiters.

```text
Identity: NSR-DOM-044 (REGISTRY_ID)
Edition: 1
...
### boundedoperator:dom-044-negative; expected NEBO_TYPE_UNSUPPORTED_OPERATOR

```nebo
// Exact Int norm refuses an irrational result instead of hiding tolerance.
start() {
    Vector<Int,2> [1,1].vector;
    ‖vector‖;
}
