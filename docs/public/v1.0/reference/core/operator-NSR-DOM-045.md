# ⟨u,v⟩ — inner product delimiters

Inner-product notation.

```text
Identity: NSR-DOM-045 (REGISTRY_ID)
Edition: 1
...
```text
lexeme: ⟨u,v⟩
context: inner-product-space profile
fixity: delimited
arity: 2
precedence: DELIMITER_OPERATOR
associativity: N/A
canonical_ascii: innerProduct()
operand_rule: compatible values in a declared inner-product space
result_rule: scalar
```

## Evaluation and types

u then v once. Short-circuit: NO. Operand rule: compatible values in a declared inner-product space. Result rule: scalar.

## Errors and remediation

domain/shape mismatch explicit. Exact immutable Int Vector aggregate grammar, extent 1..4; matrix Int/Complex profile at most four logical cells; vector observers sum, checked rational inverse numerator/denominator, exact integer norm; symbolic/named parity; all source tokens consumed by the canonical current profile, without adjacent statements. Use the exact admitted spelling and operand domain; recognition alone never activates a reserved form.
...
### boundedoperator:dom-045-symbol-0; expected 65

```nebo
start(){Vector<Int,2> [3,4].a;Vector<Int,2> [7,11].b;⟨a,b⟩;}
```

Oracle: {"independent_builds": 2, "process_exit": 65, "runtime_sha256": "e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855"}

## Rejected examples

### boundedoperator:dom-045-negative; expected NEBO_TYPE_MISMATCH

```nebo
// Inner-product operands must inhabit the same declared finite dimension.
start() {
    Vector<Int,2> [1,2].left;
    Vector<Int,3> [3,4,5].right;
    ⟨left,right⟩;
}
