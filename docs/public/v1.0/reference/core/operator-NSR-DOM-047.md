...
```text
Identity: NSR-DOM-047 (REGISTRY_ID)
Edition: 1
...
```text
lexeme: ⟂
context: linear/geometry profile
fixity: infix
arity: 2
...
left then right once. Short-circuit: NO. Operand rule: vectors/subspaces with explicit tolerance or exact domain. Result rule: Bool.
...
### boundedoperator:dom-047-named-0; expected 0

```nebo
start(){Vector<Int,3> [2,3,4].a;Vector<Int,3> [5,6,8].b;a.isOrthogonalTo(b);}
```
