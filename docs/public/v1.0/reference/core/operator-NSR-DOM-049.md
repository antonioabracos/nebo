...
Single integral. A naked Float result without convergence/error metadata is not the canonical contract.

```text
Identity: NSR-DOM-049 (REGISTRY_ID)
Edition: 1
...
domain and integrand follow the integration plan. Short-circuit: NO. Operand rule: integrable callable/expression, explicit variable/domain, method and tolerance. Result rule: IntegralResult&lt;T&gt;.

## Errors and remediation

convergence/error/status explicit. Explicit finite samples, dimensions, method, tolerance/step and budget in the current checked calculus grammar; exact rational/integer observations; no implicit integration domain, numeric truncation, live solver or hidden callback. Use the exact admitted spelling and operand domain; recognition alone never activates a reserved form.
