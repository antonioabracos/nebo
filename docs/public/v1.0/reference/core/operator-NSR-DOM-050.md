# ∬ — double integral binder

Double integral.

```text
Identity: NSR-DOM-050 (REGISTRY_ID)
Edition: 1
...
precedence: BINDER
associativity: N/A
canonical_ascii: integrate2D()
operand_rule: two-dimensional domain and integrand plan
result_rule: IntegralResult<T>
```

## Evaluation and types

explicit plan. Short-circuit: NO. Operand rule: two-dimensional domain and integrand plan. Result rule: IntegralResult&lt;T&gt;.
...
### boundedoperator:dom-050-e288ac-0; expected 10

```nebo
start(){∬(x,0,2,y,0,2,rectangle,1,16,[1,2,3,4]).value();}
```
...
### boundedoperator:dom-050-negative; expected NEBO_TYPE_UNSUPPORTED_OPERATOR

```nebo
// Independent integration axes cannot bind the same variable twice.
start() { ∬(x, 0, 1, x, 0, 1, rectangle, 1, 8, [1]).value(); }
