# ∭ — triple integral binder

Triple integral.

```text
Identity: NSR-DOM-051 (REGISTRY_ID)
Edition: 1
...
```text
lexeme: ∭
context: multidimensional calculus profile
fixity: binder
arity: 3+
precedence: BINDER
associativity: N/A
canonical_ascii: integrate3D()
operand_rule: three-dimensional domain and integrand plan
result_rule: IntegralResult<T>
```

## Evaluation and types

explicit plan. Short-circuit: NO. Operand rule: three-dimensional domain and integrand plan. Result rule: IntegralResult&lt;T&gt;.

## Errors and remediation

convergence/error explicit. Explicit finite samples, dimensions, method, tolerance/step and budget in the current checked calculus grammar; exact rational/integer observations; no implicit integration domain, numeric truncation, live solver or hidden callback. Use the exact admitted spelling and operand domain; recognition alone never activates a reserved form.
...
### boundedoperator:dom-051-e288ad-0; expected 20

```nebo
start(){∭(x,0,2,y,0,2,z,0,2,rectangle,1,16,[1,2,3,4]).value();}
```

Oracle: {"independent_builds": 2, "process_exit": 20, "runtime_sha256": "e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855"}

## Rejected examples

### boundedoperator:dom-051-negative; expected NEBO_TYPE_UNSUPPORTED_OPERATOR

```nebo
// Finite domains require a strictly increasing bound pair.
start() { ∫(x, 3, 1, trapezoid, 1, 8, [1, 2]).value(); }
