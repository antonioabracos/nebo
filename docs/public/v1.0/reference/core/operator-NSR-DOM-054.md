...
```text
Identity: NSR-DOM-054 (REGISTRY_ID)
Edition: 1
...
```text
lexeme: ∇
context: vector calculus/autodiff profile
fixity: prefix
arity: 1
precedence: P150_PREFIX
associativity: right
canonical_ascii: gradient()
operand_rule: scalar field over a declared coordinate space
result_rule: vector field
```

## Evaluation and types

operand once. Short-circuit: NO. Operand rule: scalar field over a declared coordinate space. Result rule: vector field.
...
### boundedoperator:dom-054-e28887-2-0; expected 0

```nebo
start(){∇(2,unitless,numeric,central,1,64,[0,0,0,0]).component(0);}
```
...
### boundedoperator:dom-054-negative; expected NEBO_TYPE_UNSUPPORTED_OPERATOR

```nebo
// Gradient samples must contain one plus/minus pair per dimension.
start() { gradient(2, unitless, numeric, central, 1, 64, [6, 2]).component(0); }
