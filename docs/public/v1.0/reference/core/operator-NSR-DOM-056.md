...
```text
Identity: NSR-DOM-056 (REGISTRY_ID)
Edition: 1
...
```text
lexeme: ∇×
context: vector calculus profile
fixity: prefix compound
arity: 1
precedence: P150_PREFIX
associativity: right
canonical_ascii: curl()
operand_rule: three-dimensional vector field or declared curl domain
result_rule: vector/scalar domain result
```

## Evaluation and types

operand once. Short-circuit: NO. Operand rule: three-dimensional vector field or declared curl domain. Result rule: vector/scalar domain result.

## Errors and remediation

dimension/domain explicit. Explicit finite samples, dimensions, method, tolerance/step and budget in the current checked calculus grammar; exact rational/integer observations; no implicit integration domain, numeric truncation, live solver or hidden callback. Use the exact admitted spelling and operand domain; recognition alone never activates a reserved form.
...
### boundedoperator:dom-056-e28887c397-0-0; expected 2

```nebo
start(){∇×(3,unitless,numeric,central,64,[0,1,2,3,4,5,6,7,8]).component(0);}
```
...
### boundedoperator:dom-056-negative; expected NEBO_TYPE_UNSUPPORTED_OPERATOR

```nebo
// Curl requires an explicit three-dimensional field.
start() { curl(2, unitless, numeric, central, 64, [0, 1, 2, 3]).component(0); }
