...
Declares or states a distributional relation.

```text
Identity: NSR-DOM-058 (REGISTRY_ID)
Edition: 1
...
fixity: infix declaration/relation
arity: 2
precedence: SPECIAL_PROBABILITY
associativity: non-associative
canonical_ascii: distributedAs()
operand_rule: random variable/model symbol and Distribution
result_rule: typed probabilistic relation/binding
```

## Evaluation and types

left then distribution once. Short-circuit: NO. Operand rule: random variable/model symbol and Distribution. Result rule: typed probabilistic relation/binding.

## Errors and remediation

shape/support mismatch explicit. Explicit bounded model/distribution metadata and finite rational conditional probabilities; independence is an explicit model assumption, not an inferred statistical proof. Use the exact admitted spelling and operand domain; recognition alone never activates a reserved form.
...
### boundedoperator:dom-058-discrete-1-True; expected 1

```nebo
start(){distributedAs(17,3 ∼ bernoulli,discrete,1,discrete,1,0,1,64).shape();}
```
...
### boundedoperator:dom-058-negative; expected NEBO_TYPE_UNSUPPORTED_OPERATOR

```nebo
// Variable and distribution scalar kinds cannot be silently coerced.
start() { distributedAs(11, 3, discrete, 1, bernoulli, continuous, 1, 0, 1, 37).value(); }
