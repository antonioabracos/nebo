# | — conditional probability separator

Separates event and condition only inside a dedicated probability grammar.

```text
Identity: NSR-DOM-060 (REGISTRY_ID)
Edition: 1
...
```text
lexeme: |
context: inside `P(...)` probability grammar only
fixity: contextual separator
arity: 2
precedence: PROBABILITY_GRAMMAR
associativity: N/A
canonical_ascii: conditionalOn()
operand_rule: event/expression pair inside a probability construct
result_rule: conditional event/probability expression
```

## Evaluation and types

context-defined. Short-circuit: NO. Operand rule: event/expression pair inside a probability construct. Result rule: conditional event/probability expression.

## Errors and remediation

outside the probability grammar it is not activated. Explicit bounded model/distribution metadata and finite rational conditional probabilities; independence is an explicit model assumption, not an inferred statistical proof. Use the exact admitted spelling and operand domain; recognition alone never activates a reserved form.
...
### boundedoperator:dom-060-P-0-numerator; expected 1

```nebo
start(){P(17,1,4 | 1,2,83).numerator();}
```
...
### boundedoperator:dom-060-negative; expected NEBO_TYPE_UNSUPPORTED_OPERATOR

```nebo
// Conditional probability rejects a zero-probability conditioning event.
start() { conditionalOn(23, 1, 4, 0, 2, 53).numerator(); }
