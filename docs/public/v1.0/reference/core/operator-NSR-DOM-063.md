# ∄ — non-existence quantifier

States that no witness exists.

```text
Identity: NSR-DOM-063 (REGISTRY_ID)
Edition: 1
...
### boundedoperator:dom-063-e28884-0; expected 0
...
### boundedoperator:dom-063-negative; expected NEBO_TYPE_UNSUPPORTED_OPERATOR

```nebo
// Finite formal domains are capped at one million elements.
start() { exists(1000001, 1, 0, 0, 17).value(); }
