...
Implication, separate from graph arrows and match arrows.

```text
Identity: NSR-DOM-064 (REGISTRY_ID)
Edition: 1
...
```text
lexeme: ⇒
context: formal logic/contract profile
fixity: infix
arity: 2
precedence: P070_IMPLICATION
associativity: right
canonical_ascii: implies
operand_rule: Bool/Constraint operands
result_rule: Bool or Constraint
```

## Evaluation and types

left once; right may be solver-lazy, never ordinary hidden effects. Short-circuit: CONTEXTUAL. Operand rule: Bool/Constraint operands. Result rule: Bool or Constraint.

## Errors and remediation

solver status explicit. Finite domain partition/result grammar, explicit proof/model status and budget; exhaustive Boolean truth tables; unknown and timeout remain distinct outcomes; this does not claim an automatic theorem prover. Use the exact admitted spelling and operand domain; recognition alone never activates a reserved form.
...
### boundedoperator:dom-064-0-0-True; expected 1

```nebo
start(){implies(0 ⇒ 0,7).value();}
```
...
### boundedoperator:dom-064-negative; expected NEBO_TYPE_UNSUPPORTED_OPERATOR

```nebo
// Formal Boolean operators reject numeric truthiness.
start() { nand(2, 1, 7).value(); }
