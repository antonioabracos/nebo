...
```text
Identity: NSR-DOM-065 (REGISTRY_ID)
Edition: 1
...
operands once. Short-circuit: NO. Operand rule: Bool/Constraint operands. Result rule: Bool or Constraint.
...
### boundedoperator:dom-065-0-0-True; expected 1
...
### boundedoperator:dom-065-negative; expected NEBO_PARSE_UNEXPECTED_TOKEN

```nebo
// Equivalence is non-associative and rejects a chained relation.
start() { iff(1 ⇔ 1 ⇔ 1, 7).value(); }
