...
Directed graph edge. It is not the ASCII \`-&gt;\` match/signature separator.

```text
Identity: NSR-DOM-070 (REGISTRY_ID)
Edition: 1
...
canonical_ascii: directedEdgeTo()
operand_rule: compatible nodes/states
result_rule: Edge/Transition
```

## Evaluation and types

left then right once. Short-circuit: NO. Operand rule: compatible nodes/states. Result rule: Edge/Transition.

## Errors and remediation

duplicate/cycle policy graph-specific. Bounded typed edge/transition descriptor and deterministic replay projection; explicit endpoints, schema, guard, effect and capability; no asynchronous external execution. Use the exact admitted spelling and operand domain; recognition alone never activates a reserved form.
...
### boundedoperator:dom-070-endpoint-0-True; expected 29
