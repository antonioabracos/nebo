...
```text
Identity: NSR-DOM-073 (REGISTRY_ID)
Edition: 1
...
```text
lexeme: ⟶
context: statechart/workflow profile
fixity: infix declaration
arity: 2
precedence: GRAPH_GRAMMAR
associativity: non-associative
canonical_ascii: transitionTo()
operand_rule: states and optional transition metadata
result_rule: Transition
```

## Evaluation and types

construction once. Short-circuit: NO. Operand rule: states and optional transition metadata. Result rule: Transition.

## Errors and remediation

guards/effects explicit. Bounded typed edge/transition descriptor and deterministic replay projection; explicit endpoints, schema, guard, effect and capability; no asynchronous external execution. Use the exact admitted spelling and operand domain; recognition alone never activates a reserved form.

## Availability and ownership

ACTIVE_BOUNDED in std.statechart. Exact Registry token and real Program statements; independent value/document/exit oracles, changed operands, associated negatives; Bounded typed edge/transition descriptor and deterministic replay projection; explicit endpoints, schema, guard, effect and capability; no asynchronous external execution. Native checked scalar/vector mathematics; independent numeric oracle; explicit process return separate from intermediate value The operator itself grants no capability. Owned operands obey the move/borrow rules of their concrete types.

## Executed examples

### boundedoperator:dom-073-endpoint-0-True; expected 29

```nebo
start(){transitionTo(17 ⟶ 29,3,5,7,7,1,1,3).replay();}
```
...
### boundedoperator:dom-073-negative; expected NEBO_TYPE_UNSUPPORTED_OPERATOR

```nebo
start() { transitionTo(1 ⟶ 2, 3, 4, 5, 5, 1, 8, 3).effect(); }
