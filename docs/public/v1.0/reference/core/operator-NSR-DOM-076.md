# !~ — pattern/regex non-match

Negated bounded pattern match.

```text
Identity: NSR-DOM-076 (REGISTRY_ID)
Edition: 1
...
### boundedoperator:dom-076-match-3; expected 1
...
### boundedoperator:dom-076-negative; expected NEBO_TYPE_UNSUPPORTED_OPERATOR

```nebo
start() { !matches("abc" =~ Pattern("abc", 0, 20)).matched(); }
