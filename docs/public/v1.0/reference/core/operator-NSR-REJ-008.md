...
REJECTED: Rejected because \`??\` has a precise Option meaning. The operand/result description records the rejected or reserved proposal; it is not executable permission.

```text
Identity: NSR-REJ-008 (REGISTRY_ID)
Edition: 1
...
fixity: binary/ternary shorthand
arity: 2
precedence: N/A
associativity: right
canonical_ascii: ?? for Option or explicit conditional
operand_rule: N/A
result_rule: N/A
```
...
REJECTED in core.option. Current native grammar/context only; no implicit activation. The operator itself grants no capability. Owned operands obey the move/borrow rules of their concrete types.
...
### lexical:protected-30; expected NEBO_LEX_REJECTED_FORM
