...
RESERVED: Historical parse-only idea; no executable semantics are frozen in this registry. The operand/result description records the rejected or reserved proposal; it is not executable permission.

```text
Identity: NSR-RES-001 (REGISTRY_ID)
Edition: 1
...
```text
lexeme: <.
context: future flow expression
fixity: infix
arity: 2
...
### lexical:protected-0; expected NEBO_LEX_RESERVED_SYMBOL
