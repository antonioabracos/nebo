# &lt;.&gt; — bidirectional flow/binding

RESERVED: Reserved until synchronization, ownership, causality and failure semantics are separately frozen. The operand/result description records the rejected or reserved proposal; it is not executable permission.

```text
Identity: NSR-RES-002 (REGISTRY_ID)
Edition: 1
...
```text
lexeme: <.>
context: future endpoint relation
fixity: infix
arity: 2
...
unfrozen. Short-circuit: NO. Operand rule: unfrozen. Result rule: unfrozen.
...
RESERVED in future.flow. Current native grammar/context only; no implicit activation. The operator itself grants no capability. Owned operands obey the move/borrow rules of their concrete types.
...
### lexical:protected-1; expected NEBO_LEX_RESERVED_SYMBOL
