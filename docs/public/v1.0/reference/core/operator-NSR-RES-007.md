# $ — dollar syntax outside interpolation

RESERVED: Only \`${...}\` inside an interpolation-enabled Text literal may eventually use dollar. The operand/result description records the rejected or reserved proposal; it is not executable permission.

```text
Identity: NSR-RES-007 (REGISTRY_ID)
Edition: 1
...
### lexical:protected-6; expected NEBO_LEX_RESERVED_SYMBOL
