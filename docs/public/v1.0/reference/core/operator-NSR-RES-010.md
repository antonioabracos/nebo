...
RESERVED: Existing method-based bitwise API is preserved while \`&amp;\` also remains available for a future unsafe/reference decision. The operand/result description records the rejected or reserved proposal; it is not executable permission.

```text
Identity: NSR-RES-010 (REGISTRY_ID)
Edition: 1
...
associativity: left
canonical_ascii: bitAnd()
operand_rule: Int/Bytes candidate
result_rule: candidate
```

## Evaluation and types

left then right once. Short-circuit: NO. Operand rule: Int/Bytes candidate. Result rule: candidate.
...
### lexical:protected-8; expected NEBO_LEX_RESERVED_SYMBOL
