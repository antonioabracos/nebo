...
RESERVED: Existing bounded method semantics are not automatically exposed as an infix operator. The operand/result description records the rejected or reserved proposal; it is not executable permission.

```text
Identity: NSR-RES-013 (REGISTRY_ID)
Edition: 1
...
### lexical:protected-11; expected NEBO_LEX_RESERVED_SYMBOL
