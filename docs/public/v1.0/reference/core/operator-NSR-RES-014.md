...
RESERVED: Reserved until signed/logical right-shift profiles are unified beyond the current method contract. The operand/result description records the rejected or reserved proposal; it is not executable permission.

```text
Identity: NSR-RES-014 (REGISTRY_ID)
Edition: 1
...
associativity: left
canonical_ascii: shiftRight()
operand_rule: Int and bounded shift count candidate
result_rule: Int
```

## Evaluation and types

left then count once. Short-circuit: NO. Operand rule: Int and bounded shift count candidate. Result rule: Int.
...
### lexical:protected-12; expected NEBO_LEX_RESERVED_SYMBOL
