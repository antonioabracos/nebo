...
U+2212 is an exact source alias for ASCII hyphen-minus in operator context.

```text
Identity: NSR-UA-001 (REGISTRY_ID)
Edition: 1
...
```text
lexeme: −
context: prefix or infix numeric expression
fixity: alias
arity: 1 or 2
precedence: SAME_AS_ASCII
associativity: same as `-`
canonical_ascii: -
operand_rule: same as ASCII `-`
result_rule: same as ASCII `-`
```

## Evaluation and types

identical lowering. Short-circuit: NO. Operand rule: same as ASCII \`-\`. Result rule: same as ASCII \`-\`.

## Errors and remediation

identical failure policy. Current native grammar/context only; no implicit activation. Use the exact admitted spelling and operand domain; recognition alone never activates a reserved form.
