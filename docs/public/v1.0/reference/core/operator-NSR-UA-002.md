...
U+00F7 aliases \`/\`; U+2215 and other slash confusables do not.

```text
Identity: NSR-UA-002 (REGISTRY_ID)
Edition: 1
...
```text
lexeme: ÷
context: infix numeric expression
fixity: alias
arity: 2
...
identical lowering. Short-circuit: NO. Operand rule: same as \`/\`. Result rule: same as \`/\`.
...
### operator:binary-c3b7-6; expected 23
...
### operator:wrong-c3b7; expected NEBO_TYPE_MISMATCH
