...
Exact alias for \`!=\`; it is not symbolic inequivalence beyond typed inequality.

```text
Identity: NSR-UA-005 (REGISTRY_ID)
Edition: 1
...
identical lowering. Short-circuit: NO. Operand rule: same as \`!=\`. Result rule: Bool.

## Errors and remediation

identical diagnostics. Current native grammar/context only; no implicit activation. Use the exact admitted spelling and operand domain; recognition alone never activates a reserved form.
...
### operator:binary-e289a0-6; expected 23
...
### operator:wrong-e289a0; expected NEBO_TYPE_MISMATCH
