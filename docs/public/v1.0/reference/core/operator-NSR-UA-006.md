...
Exact short-circuit alias for \`&amp;&amp;\`.

```text
Identity: NSR-UA-006 (REGISTRY_ID)
Edition: 1
...
associativity: left
canonical_ascii: &&
operand_rule: Bool and Bool
result_rule: Bool
```

## Evaluation and types

same short-circuit as \`&amp;&amp;\`. Short-circuit: YES_RIGHT_LAZY. Operand rule: Bool and Bool. Result rule: Bool.
...
### operator:short-unicode-and; expected 23

```nebo
start(){(false ∧ (1/0==0)).console();23.return;}
```
...
### operator:wrong-logic-int; expected NEBO_TYPE_MISMATCH

```nebo
start(){(true && 17).console();23.return;}
```
