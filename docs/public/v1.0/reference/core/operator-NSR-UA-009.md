...
Exact alias for the canonical ASCII keyword \`xor\`.

```text
Identity: NSR-UA-009 (REGISTRY_ID)
Edition: 1
...
```text
lexeme: ⊻
context: Bool, Int or Bytes expression
fixity: alias
arity: 2
precedence: SAME_AS_ASCII
associativity: left
canonical_ascii: xor
operand_rule: same as `xor`
result_rule: same as `xor`
```

## Evaluation and types

identical lowering. Short-circuit: NO. Operand rule: same as \`xor\`. Result rule: same as \`xor\`.

## Errors and remediation

same diagnostics. Current native grammar/context only; no implicit activation. Use the exact admitted spelling and operand domain; recognition alone never activates a reserved form.
...
### operator:binary-e28abb-6; expected 23
...
### operator:wrong-e28abb; expected NEBO_TYPE_MISMATCH
