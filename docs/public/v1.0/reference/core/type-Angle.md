...
The native angle quantity identity.

```text
Identity: type:Angle (INTRINSIC_TYPE)
Edition: 1
...
```text
Angle
Native TypeId: 18 (not SymbolId)
```

## Logical layout

Unit-bearing scalar quantity; it remains distinct from dimensionless Int.

## Construction and members

The registered angle profile controls constructors and operations; presence of a TypeId grants no wider domain syntax.

## Protocols and ownership

The Angle value preserves its concrete semantic identity through bindings and arguments. Arithmetic/comparison does not grant I/O authority; console is an explicit effect. Only the documented comparisons and generic constraint profiles apply. A TypeId does not synthesize a user-defined Eq/Ord, clone or drop implementation.
...
### operator:angle-17-False; expected 23

```nebo
start(){(17°).q;flow.assert((q+7°)==24°);23.return;}
```
...
[
  {
    "base": 0,
    "case_id": "types:type-18",
    "category": "native-type",
    "flags": 2,
    "kind": 18,
    "observed_sha256": "2a9f236e4774edbcb9a5d8838332074e0edfee14fc2ebd5e484f0d5bc88397fb",
    "type_id": 18
  }
]
```
