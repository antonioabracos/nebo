...
A distinct fixed-point ratio quantity.

```text
Identity: type:Percent (INTRINSIC_TYPE)
Edition: 1
...
```text
Percent
Native TypeId: 15 (not SymbolId)
```

## Logical layout

Uses the canonical ratio scale in the current runtime while preserving a separate semantic type.

## Construction and members

Postfix percent in the admitted quantity profile; do not confuse postfix % with integer remainder.

## Protocols and ownership

The Percent value preserves its concrete semantic identity through bindings and arguments. Arithmetic/comparison does not grant I/O authority; console is an explicit effect. Only the documented comparisons and generic constraint profiles apply. A TypeId does not synthesize a user-defined Eq/Ord, clone or drop implementation.
...
### operator:ratio-25-7-False; expected 23

```nebo
start(){(7%).q;flow.assert(q == 700‱);23.return;}
```
...
[
  {
    "base": 0,
    "case_id": "types:type-15",
    "category": "native-type",
    "flags": 2,
    "kind": 15,
    "observed_sha256": "9f8e99e2d70daec65a65dabc14ee569c9eab811e9ad027bc3987d3cc654d0b6a",
    "type_id": 15
  }
]
```
