# ai-agents — prompt.withSystem(text)

Offline tokenizer/generation/agent reference model; no service, network or foundation-model claim; original observation: system text returns a new prompt without changing the original; SDK bounds: {"MAX_CONTEXT":4096,"MAX_EVALUATION_CASES":256,"MAX_GENERATED_TOKENS":256,"MAX_MANIFEST_BYTES":65536,"MAX_PLAN_STEPS":32,"MAX_SCHEMA_FIELDS":64,"MAX_TEXT_BYTES":8192,"MAX_TOOLS":32,"MAX_TOOL_ARGUMENTS":16,"MAX_VOCABULARY":512}

```text
Identity: owner:G023:G023-S04-02 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Prompt.withSystem(self, text: str) -> 'Prompt'
```
...
[
  ...
  {
    "excerpt": "221:     surface(\"G023-S04-01\", base.render() == \"[user] h\", base.render())\n222:     with_system = base.withSystem(\"hello\")\n223:     surface(\"G023-S04-02\", with_system.parts[0][\"role\"] == \"system\" and base.parts[0][\"role\"] == \"user\", with_system.render())\n224:     context_text = \"world\"\n225:     context = with_system.withContext({\"text\": context_text, \"provenance\": {\"source\": \"fixture:context\", \"digest\": hashlib.sha256(context_text.encode()).hexdigest()}})",
