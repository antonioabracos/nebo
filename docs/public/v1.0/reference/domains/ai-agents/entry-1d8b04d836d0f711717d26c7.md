# ai-agents — toolCall.requireApproval()

Offline tokenizer/generation/agent reference model; no service, network or foundation-model claim; original observation: reversible call exposes stable approval request without execution; SDK bounds: {"MAX_CONTEXT":4096,"MAX_EVALUATION_CASES":256,"MAX_GENERATED_TOKENS":256,"MAX_MANIFEST_BYTES":65536,"MAX_PLAN_STEPS":32,"MAX_SCHEMA_FIELDS":64,"MAX_TEXT_BYTES":8192,"MAX_TOOLS":32,"MAX_TOOL_ARGUMENTS":16,"MAX_VOCABULARY":512}

```text
Identity: owner:G023:G023-S05-07 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
ToolCall.requireApproval(self) -> ApprovalRequest
```
...
[
  ...
  {
    "excerpt": "282:     approval_call = ToolCall(\"store\", {\"value\": 9}).validate(reversible_registry)\n283:     approval = approval_call.requireApproval()\n284:     surface(\"G023-S05-07\", approval.required and approval.callId == approval_call.call_id and reversible_state == [], approval)\n285: \n286:     changed = ToolCall(\"add\", {\"left\": 4, \"right\": 5}).validate(registry).execute(ToolExecutionContext((\"compute\",), (), 1))",
