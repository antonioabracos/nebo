# ai-agents — agent.auditTrail()

Offline tokenizer/generation/agent reference model; no service, network or foundation-model claim; original observation: ordered immutable trail records plan generation tool pause and rollback; SDK bounds: {"MAX_CONTEXT":4096,"MAX_EVALUATION_CASES":256,"MAX_GENERATED_TOKENS":256,"MAX_MANIFEST_BYTES":65536,"MAX_PLAN_STEPS":32,"MAX_SCHEMA_FIELDS":64,"MAX_TEXT_BYTES":8192,"MAX_TOOLS":32,"MAX_TOOL_ARGUMENTS":16,"MAX_VOCABULARY":512}

```text
Identity: owner:G023:G023-S06-07 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Agent.auditTrail(self) -> tuple[Mapping[str, Any], ...]
```
...
[
  ...
  {
    "excerpt": "343:     agent.pause()\n344:     surface(\"G023-S06-05\", agent._paused is True, agent.auditTrail()[-1])\n345:     reverted = agent.rollback(0)\n346:     surface(\"G023-S06-06\", reverted == 1 and state == [], reverted)\n347:     audit = agent.auditTrail()",
