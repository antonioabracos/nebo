# ai-agents — AiPolicy.network(enabled)

Offline tokenizer/generation/agent reference model; no service, network or foundation-model claim; original observation: strict Bool controls network capability without performing network IO; SDK bounds: {"MAX_CONTEXT":4096,"MAX_EVALUATION_CASES":256,"MAX_GENERATED_TOKENS":256,"MAX_MANIFEST_BYTES":65536,"MAX_PLAN_STEPS":32,"MAX_SCHEMA_FIELDS":64,"MAX_TEXT_BYTES":8192,"MAX_TOOLS":32,"MAX_TOOL_ARGUMENTS":16,"MAX_VOCABULARY":512}

```text
Identity: owner:G023:G023-S07-04 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
AiPolicy.network(self, enabled: bool) -> 'AiPolicy'
```
...
[
  ...
  {
    "excerpt": "313:     negative(\"CAPABILITY_DENIED\", \"toolCall.execute\", lambda: ToolCall(\"add\", {\"left\": 1, \"right\": 2}).validate(registry).execute(ToolExecutionContext((), (), 1)))\n314:     negative(\"INVALID_BOOLEAN\", \"ToolExecutionContext\", lambda: ToolExecutionContext((\"network\",), (), 1, network=\"false\"))\n315:     negative(\"APPROVAL_REQUIRED\", \"toolCall.execute\", lambda: approval_call.execute(ToolExecutionContext((\"compute\",), (), 1)))\n316:     negative(\"TOOL_BUDGET\", \"toolCall.execute\", lambda: ToolCall(\"add\", {\"left\": 1, \"right\": 2}).validate(registry).execute(ToolExecutionContext((\"compute\",), (), 0)))\n317:     wrong_tool = Tool.fromFunction(wrong_return, {\"arguments\": {\"value\": \"int\"}, \"returns\": \"int\", \"capabilities\": (), \"effect\": \"pure\"})",
