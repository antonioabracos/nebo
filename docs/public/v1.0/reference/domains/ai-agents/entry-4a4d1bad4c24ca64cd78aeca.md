# ai-agents — prompt.withContext(document)

Offline tokenizer/generation/agent reference model; no service, network or foundation-model claim; original observation: context requires matching source and text digest provenance; SDK bounds: {"MAX_CONTEXT":4096,"MAX_EVALUATION_CASES":256,"MAX_GENERATED_TOKENS":256,"MAX_MANIFEST_BYTES":65536,"MAX_PLAN_STEPS":32,"MAX_SCHEMA_FIELDS":64,"MAX_TEXT_BYTES":8192,"MAX_TOOLS":32,"MAX_TOOL_ARGUMENTS":16,"MAX_VOCABULARY":512}

```text
Identity: owner:G023:G023-S04-03 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Prompt.withContext(self, document: Mapping[str, Any]) -> 'Prompt'
```
...
[
  ...
  {
    "excerpt": "224:     context_text = \"world\"\n225:     context = with_system.withContext({\"text\": context_text, \"provenance\": {\"source\": \"fixture:context\", \"digest\": hashlib.sha256(context_text.encode()).hexdigest()}})\n226:     surface(\"G023-S04-03\", context.parts[-1][\"role\"] == \"context\" and context.parts[-1][\"provenance\"][\"source\"] == \"fixture:context\", context.render())\n227:     schema = OutputSchema.fromType({\"answer\": \"int\", \"safe\": \"bool\"})\n228:     surface(\"G023-S04-04\", tuple(schema.fields) == (\"answer\", \"safe\") and len(schema.digest) == 64, schema.fields)",
