# ai-agents — tokenizer.specialToken(name)

Offline tokenizer/generation/agent reference model; no service, network or foundation-model claim; original observation: case-normalized BOS and EOS names resolve stable IDs; SDK bounds: {"MAX_CONTEXT":4096,"MAX_EVALUATION_CASES":256,"MAX_GENERATED_TOKENS":256,"MAX_MANIFEST_BYTES":65536,"MAX_PLAN_STEPS":32,"MAX_SCHEMA_FIELDS":64,"MAX_TEXT_BYTES":8192,"MAX_TOOLS":32,"MAX_TOOL_ARGUMENTS":16,"MAX_VOCABULARY":512}

```text
Identity: owner:G023:G023-S01-05 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Tokenizer.specialToken(self, name: str) -> int
```
