# ai-agents — Evaluation.run(suite,model)

Offline tokenizer/generation/agent reference model; no service, network or foundation-model claim; original observation: versioned local corpus measures independent top-token expectations; SDK bounds: {"MAX_CONTEXT":4096,"MAX_EVALUATION_CASES":256,"MAX_GENERATED_TOKENS":256,"MAX_MANIFEST_BYTES":65536,"MAX_PLAN_STEPS":32,"MAX_SCHEMA_FIELDS":64,"MAX_TEXT_BYTES":8192,"MAX_TOOLS":32,"MAX_TOOL_ARGUMENTS":16,"MAX_VOCABULARY":512}

```text
Identity: owner:G023:G023-S07-05 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Evaluation.run(suite: Mapping[str, Any], model: CausalLanguageModel) -> EvaluationResult
```
...
[
  ...
  {
    "excerpt": "395:     suite = {\"version\": 1, \"name\": \"tiny-local-evaluation\", \"promptDigest\": prompt_digest, \"toolchain\": \"neboc-1.0.0+g023-sdk-v1\", \"cases\": cases}\n396:     evaluation = Evaluation.run(suite, model)\n397:     surface(\"G023-S07-05\", evaluation.values[\"cases\"] == 2 and evaluation.values[\"accuracy\"] == 1.0, evaluation.values)\n398:     metrics = evaluation.metrics()\n399:     surface(\"G023-S07-06\", metrics[\"safetyRate\"] == 1.0 and metrics[\"performanceClaim\"] == \"logical-operations-only\", metrics)",
