# ai-agents — Tool.fromFunction(function,schema)

Offline tokenizer/generation/agent reference model; no service, network or foundation-model claim; original observation: public callable binds exact argument return effect and version schema; SDK bounds: {"MAX_CONTEXT":4096,"MAX_EVALUATION_CASES":256,"MAX_GENERATED_TOKENS":256,"MAX_MANIFEST_BYTES":65536,"MAX_PLAN_STEPS":32,"MAX_SCHEMA_FIELDS":64,"MAX_TEXT_BYTES":8192,"MAX_TOOLS":32,"MAX_TOOL_ARGUMENTS":16,"MAX_VOCABULARY":512}

```text
Identity: owner:G023:G023-S05-01 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Tool.fromFunction(cls, function: Callable[..., Any], schema: Mapping[str, Any]) -> 'Tool'
```
...
[
  ...
  {
    "excerpt": "260:     schema = {\"arguments\": {\"left\": \"int\", \"right\": \"int\"}, \"returns\": \"int\", \"capabilities\": (\"compute\",), \"effect\": \"pure\", \"version\": 1}\n261:     tool = Tool.fromFunction(add, schema)\n262:     surface(\"G023-S05-01\", tool.name == \"add\" and tool.arguments[\"left\"] == \"int\", tool.name)\n263:     surface(\"G023-S05-02\", tool.requiredCapabilities() == frozenset({\"compute\"}), tool.requiredCapabilities())\n264:     registry = ToolRegistry((\"add\",)).register(tool)",
