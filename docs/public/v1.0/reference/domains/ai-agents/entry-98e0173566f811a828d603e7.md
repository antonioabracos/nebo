# ai-agents — toolResult.provenance()

Offline tokenizer/generation/agent reference model; no service, network or foundation-model claim; original observation: result links tool call effect version arguments and value identity; SDK bounds: {"MAX_CONTEXT":4096,"MAX_EVALUATION_CASES":256,"MAX_GENERATED_TOKENS":256,"MAX_MANIFEST_BYTES":65536,"MAX_PLAN_STEPS":32,"MAX_SCHEMA_FIELDS":64,"MAX_TEXT_BYTES":8192,"MAX_TOOLS":32,"MAX_TOOL_ARGUMENTS":16,"MAX_VOCABULARY":512}

```text
Identity: owner:G023:G023-S05-06 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Tokenizer.provenance(self) -> Mapping[str, Any]
CausalLanguageModel.provenance(self) -> Mapping[str, Any]
GenerationResult.provenance(self) -> Mapping[str, Any]
StructuredOutput.provenance(self) -> Mapping[str, Any]
ToolResult.provenance(self) -> Mapping[str, Any]
EvaluationResult.provenance(self) -> Mapping[str, Any]
```
...
[
  ...
  {
    "excerpt": "90:     tokenizer = load_tokenizer()\n91:     surface(\"G023-S01-01\", tokenizer.checksum.startswith(\"658fceb1\") and tokenizer.merges == (), tokenizer.provenance())\n92:     encoded = tokenizer.encode(\"hello world!\", {\"maximumTokens\": 8, \"errorPolicy\": \"reject\"})\n93:     surface(\"G023-S01-02\", encoded == (4, 6, 5, 7), encoded)\n94:     decoded = tokenizer.decode(encoded, {\"specialPolicy\": \"skip\", \"errorPolicy\": \"reject\"})",
