# ai-agents — output.validationErrors()

Offline tokenizer/generation/agent reference model; no service, network or foundation-model claim; original observation: valid constrained output exposes an empty immutable error list; SDK bounds: {"MAX_CONTEXT":4096,"MAX_EVALUATION_CASES":256,"MAX_GENERATED_TOKENS":256,"MAX_MANIFEST_BYTES":65536,"MAX_PLAN_STEPS":32,"MAX_SCHEMA_FIELDS":64,"MAX_TEXT_BYTES":8192,"MAX_TOOLS":32,"MAX_TOOL_ARGUMENTS":16,"MAX_VOCABULARY":512}

```text
Identity: owner:G023:G023-S04-06 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
StructuredOutput.validationErrors(self) -> tuple[str, ...]
```
...
[
  ...
  {
    "excerpt": "230:     surface(\"G023-S04-05\", set(structured.value) == {\"answer\", \"safe\"} and isinstance(structured.value[\"answer\"], int) and isinstance(structured.value[\"safe\"], bool), structured.value)\n231:     surface(\"G023-S04-06\", structured.validationErrors() == (), structured.validationErrors())\n232: \n233:     changed = base.withSystem(\"world\")\n234:     claim(\"metamorphic\", changed.render() != with_system.render(), \"system text governs rendered prompt\")",
