# ai-agents — ToolRegistry.register(tool)

Offline tokenizer/generation/agent reference model; no service, network or foundation-model claim; original observation: deny-default registry accepts only explicitly allowlisted tool; SDK bounds: {"MAX_CONTEXT":4096,"MAX_EVALUATION_CASES":256,"MAX_GENERATED_TOKENS":256,"MAX_MANIFEST_BYTES":65536,"MAX_PLAN_STEPS":32,"MAX_SCHEMA_FIELDS":64,"MAX_TEXT_BYTES":8192,"MAX_TOOLS":32,"MAX_TOOL_ARGUMENTS":16,"MAX_VOCABULARY":512}

```text
Identity: owner:G023:G023-S05-03 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
ToolRegistry.register(self, tool: Tool) -> 'ToolRegistry'
```
...
[
  ...
  {
    "excerpt": "263:     surface(\"G023-S05-02\", tool.requiredCapabilities() == frozenset({\"compute\"}), tool.requiredCapabilities())\n264:     registry = ToolRegistry((\"add\",)).register(tool)\n265:     surface(\"G023-S05-03\", registry._tools[\"add\"] is tool, tuple(registry._tools))\n266:     call = ToolCall(\"add\", {\"left\": 2, \"right\": 5}).validate(registry)\n267:     surface(\"G023-S05-04\", call._tool is tool and len(call.call_id) == 64, call.call_id)",
