# ai-agents — agent.rollback(checkpoint)

Offline tokenizer/generation/agent reference model; no service, network or foundation-model claim; original observation: rollback invokes only registered compensation back to checkpoint; SDK bounds: {"MAX_CONTEXT":4096,"MAX_EVALUATION_CASES":256,"MAX_GENERATED_TOKENS":256,"MAX_MANIFEST_BYTES":65536,"MAX_PLAN_STEPS":32,"MAX_SCHEMA_FIELDS":64,"MAX_TEXT_BYTES":8192,"MAX_TOOLS":32,"MAX_TOOL_ARGUMENTS":16,"MAX_VOCABULARY":512}

```text
Identity: owner:G023:G023-S06-06 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Agent.rollback(self, checkpoint: int) -> int
```
...
[
  ...
  {
    "excerpt": "344:     surface(\"G023-S06-05\", agent._paused is True, agent.auditTrail()[-1])\n345:     reverted = agent.rollback(0)\n346:     surface(\"G023-S06-06\", reverted == 1 and state == [], reverted)\n347:     audit = agent.auditTrail()\n348:     surface(\"G023-S06-07\", tuple(event[\"event\"] for event in audit) == (\"plan-start\", \"generated\", \"tool\", \"plan-complete\", \"paused\", \"rollback\"), audit)",
