# ai-agents — AiPolicy.maxToolCalls(value)

Offline tokenizer/generation/agent reference model; no service, network or foundation-model claim; original observation: immutable policy update sets independent tool-call budget; SDK bounds: {"MAX_CONTEXT":4096,"MAX_EVALUATION_CASES":256,"MAX_GENERATED_TOKENS":256,"MAX_MANIFEST_BYTES":65536,"MAX_PLAN_STEPS":32,"MAX_SCHEMA_FIELDS":64,"MAX_TEXT_BYTES":8192,"MAX_TOOLS":32,"MAX_TOOL_ARGUMENTS":16,"MAX_VOCABULARY":512}

```text
Identity: owner:G023:G023-S07-02 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
AiPolicy.maxToolCalls(self, value: int) -> 'AiPolicy'
```
...
[
  ...
  {
    "excerpt": "330:     registry = ToolRegistry((\"store\",)).register(tool)\n331:     policy = AiPolicy.maxTokens(2).maxToolCalls(1).allowedTools((\"store\",)).network(False)\n332:     model = load_model()\n333:     agent = Agent.new(model, registry, policy)\n334:     surface(\"G023-S06-01\", agent.model is model and agent.policy.maximum_tool_calls == 1, True)",
