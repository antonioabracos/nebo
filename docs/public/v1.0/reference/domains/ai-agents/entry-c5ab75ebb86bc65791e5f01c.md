...
Offline tokenizer/generation/agent reference model; no service, network or foundation-model claim; original observation: accuracy safety logical operations and claim scope are immutable; SDK bounds: {"MAX_CONTEXT":4096,"MAX_EVALUATION_CASES":256,"MAX_GENERATED_TOKENS":256,"MAX_MANIFEST_BYTES":65536,"MAX_PLAN_STEPS":32,"MAX_SCHEMA_FIELDS":64,"MAX_TEXT_BYTES":8192,"MAX_TOOLS":32,"MAX_TOOL_ARGUMENTS":16,"MAX_VOCABULARY":512}

```text
Identity: owner:G023:G023-S07-06 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
EvaluationResult.metrics(self) -> Mapping[str, Any]
```
...
[
  ...
  {
    "excerpt": "397:     surface(\"G023-S07-05\", evaluation.values[\"cases\"] == 2 and evaluation.values[\"accuracy\"] == 1.0, evaluation.values)\n398:     metrics = evaluation.metrics()\n399:     surface(\"G023-S07-06\", metrics[\"safetyRate\"] == 1.0 and metrics[\"performanceClaim\"] == \"logical-operations-only\", metrics)\n400:     provenance = evaluation.provenance()\n401:     surface(\"G023-S07-07\", provenance[\"model\"] == model.checksum and len(provenance[\"data\"]) == 64 and provenance[\"prompt\"] == prompt_digest and provenance[\"toolchain\"] == \"neboc-1.0.0+g023-sdk-v1\", provenance)",
