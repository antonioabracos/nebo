# ai-agents — tokenizer.count(text)

Offline tokenizer/generation/agent reference model; no service, network or foundation-model claim; original observation: count scans the text without retaining encoded output; SDK bounds: {"MAX_CONTEXT":4096,"MAX_EVALUATION_CASES":256,"MAX_GENERATED_TOKENS":256,"MAX_MANIFEST_BYTES":65536,"MAX_PLAN_STEPS":32,"MAX_SCHEMA_FIELDS":64,"MAX_TEXT_BYTES":8192,"MAX_TOOLS":32,"MAX_TOOL_ARGUMENTS":16,"MAX_VOCABULARY":512}

```text
Identity: owner:G023:G023-S01-06 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Tokenizer.count(self, text: str) -> int
```
...
[
  ...
  {
    "excerpt": "97:     surface(\"G023-S01-05\", tokenizer.specialToken(\"EOS\") == 2 and tokenizer.specialToken(\"bos\") == 1, 2)\n98:     surface(\"G023-S01-06\", tokenizer.count(\"hello world!\") == 4, tokenizer.count(\"hello world!\"))\n99: \n100:     claim(\"composition\", tokenizer.decode(tokenizer.encode(\"hello!\"), None) == \"hello!\", \"tokenizer roundtrip composes\")\n101:     claim(\"metamorphic\", tokenizer.encode(\"world!\") != encoded, \"text governs token IDs\")",
