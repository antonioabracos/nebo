# ai-agents — Sampler.topP(probability)

Offline tokenizer/generation/agent reference model; no service, network or foundation-model claim; original observation: nucleus probability remains within the closed budget; SDK bounds: {"MAX_CONTEXT":4096,"MAX_EVALUATION_CASES":256,"MAX_GENERATED_TOKENS":256,"MAX_MANIFEST_BYTES":65536,"MAX_PLAN_STEPS":32,"MAX_SCHEMA_FIELDS":64,"MAX_TEXT_BYTES":8192,"MAX_TOOLS":32,"MAX_TOOL_ARGUMENTS":16,"MAX_VOCABULARY":512}

```text
Identity: owner:G023:G023-S03-04 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Sampler.topP(cls, probability: float) -> 'Sampler'
```
...
[
  ...
  {
    "excerpt": "182:     surface(\"G023-S03-03\", top_k.mode == \"top-k\" and top_k.parameter == 3, top_k.parameter)\n183:     top_p = Sampler.topP(0.8)\n184:     surface(\"G023-S03-04\", top_p.mode == \"top-p\" and top_p.parameter == 0.8, top_p.parameter)\n185:     penalized = greedy.repetitionPenalty(1.5)\n186:     surface(\"G023-S03-05\", penalized.penalty == 1.5 and greedy.penalty == 1.0, penalized.penalty)",
