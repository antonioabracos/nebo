# ai-agents — cache.truncate(length)

Offline tokenizer/generation/agent reference model; no service, network or foundation-model claim; original observation: safe rewind removes suffix and advances cache generation; SDK bounds: {"MAX_CONTEXT":4096,"MAX_EVALUATION_CASES":256,"MAX_GENERATED_TOKENS":256,"MAX_MANIFEST_BYTES":65536,"MAX_PLAN_STEPS":32,"MAX_SCHEMA_FIELDS":64,"MAX_TEXT_BYTES":8192,"MAX_TOOLS":32,"MAX_TOOL_ARGUMENTS":16,"MAX_VOCABULARY":512}

```text
Identity: owner:G023:G023-S02-05 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
KvCache.truncate(self, length: int) -> 'KvCache'
```
...
[
  ...
  {
    "excerpt": "142:     generation_before = cache.generation\n143:     cache.truncate(2)\n144:     surface(\"G023-S02-05\", cache.length() == 2 and cache.generation == generation_before + 1, cache.tokens)\n145:     surface(\"G023-S02-06\", model.contextLength() == 64, model.contextLength())\n146: ",
