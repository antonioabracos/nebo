# ai-agents — tokenizer.decode(tokens,options)

Offline tokenizer/generation/agent reference model; no service, network or foundation-model claim; original observation: explicit special/error policy reconstructs the source text; SDK bounds: {"MAX_CONTEXT":4096,"MAX_EVALUATION_CASES":256,"MAX_GENERATED_TOKENS":256,"MAX_MANIFEST_BYTES":65536,"MAX_PLAN_STEPS":32,"MAX_SCHEMA_FIELDS":64,"MAX_TEXT_BYTES":8192,"MAX_TOOLS":32,"MAX_TOOL_ARGUMENTS":16,"MAX_VOCABULARY":512}

```text
Identity: owner:G023:G023-S01-03 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Tokenizer.decode(self, tokens: Sequence[int], options: Mapping[str, Any] | None=None) -> str
```
...
[
  ...
  {
    "excerpt": "93:     surface(\"G023-S01-02\", encoded == (4, 6, 5, 7), encoded)\n94:     decoded = tokenizer.decode(encoded, {\"specialPolicy\": \"skip\", \"errorPolicy\": \"reject\"})\n95:     surface(\"G023-S01-03\", decoded == \"hello world!\", decoded)\n96:     surface(\"G023-S01-04\", tokenizer.vocabularySize() == 15, tokenizer.vocabularySize())\n97:     surface(\"G023-S01-05\", tokenizer.specialToken(\"EOS\") == 2 and tokenizer.specialToken(\"bos\") == 1, 2)",
