# crypto-policy — share.refresh(session)

Synthetic data only; SHA/HMAC functional checks are not implementation security review; no physical zeroization assurance; original observation: share refresh stays unavailable without an approved protocol; SDK bounds: {"MAX_CIRCUIT_INPUTS":64,"MAX_CONSTRAINTS":256,"MAX_MPC_OPERATIONS":128,"MAX_PARAMETER_SETS":16,"MAX_PARTICIPANTS":16,"MAX_RANGE_BITS":256,"MAX_TEXT_BYTES":4096,"MAX_TRANSCRIPT_EVENTS":256,"MAX_VAULT_KEYS":64}

```text
Identity: owner:G044:G044-S04-03 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Share.refresh(self, session: Any) -> None
```
...
[
  ...
  {
    "excerpt": "119:          \"EXTERNAL-REVIEW-REQUIRED\", \"SecretShare.combine(shares)\")\n120: rejected(lambda: Share().refresh(object()), \"EXTERNAL-REVIEW-REQUIRED\", \"share.refresh(session)\")\n121: session = MpcSession.new((\"alice\", \"bob\", \"carol\"), \"LOCAL-TRANSCRIPT-V1\",\n122:                          {\"mode\": \"simulation\", \"maxOperations\": 8})\n123: passed(\"MpcSession.new(participants,protocol,policy)\", len(session.participants) == 3)",
