# crypto-policy — security.compilerBarrier()

Synthetic data only; SHA/HMAC functional checks are not implementation security review; no physical zeroization assurance; original observation: reference profile states barrier insertion unavailable instead of claiming emission; SDK bounds: {"MAX_CIRCUIT_INPUTS":64,"MAX_CONSTRAINTS":256,"MAX_MPC_OPERATIONS":128,"MAX_PARAMETER_SETS":16,"MAX_PARTICIPANTS":16,"MAX_RANGE_BITS":256,"MAX_TEXT_BYTES":4096,"MAX_TRANSCRIPT_EVENTS":256,"MAX_VAULT_KEYS":64}

```text
Identity: owner:G044:G044-S06-07 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
_SecurityService.compilerBarrier(self) -> Mapping[str, Any]
```
...
[
  ...
  {
    "excerpt": "174:        timing[\"dynamicTests\"] == \"NOT_EXECUTED\", dict(timing))\n175: barrier = security.compilerBarrier()\n176: passed(\"security.compilerBarrier()\", not barrier[\"inserted\"] and\n177:        barrier[\"targetContract\"] == \"UNAVAILABLE\", dict(barrier))\n178: ",
