# crypto-policy — CryptoProtocol.compose(steps)

Synthetic data only; SHA/HMAC functional checks are not implementation security review; no physical zeroization assurance; original observation: only exact reviewed SHA-256 plus HMAC construction is allowlisted; SDK bounds: {"MAX_CIRCUIT_INPUTS":64,"MAX_CONSTRAINTS":256,"MAX_MPC_OPERATIONS":128,"MAX_PARAMETER_SETS":16,"MAX_PARTICIPANTS":16,"MAX_RANGE_BITS":256,"MAX_TEXT_BYTES":4096,"MAX_TRANSCRIPT_EVENTS":256,"MAX_VAULT_KEYS":64}

```text
Identity: owner:G044:G044-S07-05 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
CryptoProtocol.compose(steps: Sequence[str]) -> 'CryptoProtocol'
```
...
[
  ...
  {
    "excerpt": "193:        dict(revoked))\n194: protocol = CryptoProtocol.compose((\"SHA-256\", \"HMAC-SHA256\"))\n195: passed(\"CryptoProtocol.compose(steps)\", protocol.steps == (\"SHA-256\", \"HMAC-SHA256\"))\n196: review = protocol.reviewStatus()\n197: passed(\"protocol.reviewStatus()\", review[\"status\"] == \"approved-base-only\" and",
