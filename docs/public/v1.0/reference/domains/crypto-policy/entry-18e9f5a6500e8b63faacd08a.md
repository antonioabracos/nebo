# crypto-policy — vault.store(key,metadata)

Synthetic data only; SHA/HMAC functional checks are not implementation security review; no physical zeroization assurance; original observation: opaque live key is retained only in memory with explicit non-persistence; SDK bounds: {"MAX_CIRCUIT_INPUTS":64,"MAX_CONSTRAINTS":256,"MAX_MPC_OPERATIONS":128,"MAX_PARAMETER_SETS":16,"MAX_PARTICIPANTS":16,"MAX_RANGE_BITS":256,"MAX_TEXT_BYTES":4096,"MAX_TRANSCRIPT_EVENTS":256,"MAX_VAULT_KEYS":64}

```text
Identity: owner:G044:G044-S07-02 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
KeyVault.store(self, key: Key, metadata: Mapping[str, Any]) -> Mapping[str, Any]
```
...
[
  ...
  {
    "excerpt": "183: first = Key.generate(hmac_algorithm, new_entropy(0x21, \"vault-first\"))\n184: stored = vault.store(first, {\"identity\": \"audit-key\", \"purpose\": \"synthetic-test\"})\n185: passed(\"vault.store(key,metadata)\", not stored[\"persistent\"] and stored[\"version\"] == 1,\n186:        dict(stored))\n187: replacement = Key.generate(hmac_algorithm, new_entropy(0x22, \"vault-replacement\"))",
