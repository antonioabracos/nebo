# crypto-policy — sealed.unseal(context)

Synthetic data only; SHA/HMAC functional checks are not implementation security review; no physical zeroization assurance; original observation: no fabricated sealed value can be recovered from simulation; SDK bounds: {"MAX_CIRCUIT_INPUTS":64,"MAX_CONSTRAINTS":256,"MAX_MPC_OPERATIONS":128,"MAX_PARAMETER_SETS":16,"MAX_PARTICIPANTS":16,"MAX_RANGE_BITS":256,"MAX_TEXT_BYTES":4096,"MAX_TRANSCRIPT_EVENTS":256,"MAX_VAULT_KEYS":64}

```text
Identity: owner:G044:G044-S05-05 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Sealed.unseal(self, context: SecureContext) -> None
```
...
[
  ...
  {
    "excerpt": "145:          \"secret.seal(context,policy)\")\n146: rejected(lambda: Sealed().unseal(context), \"HARDWARE-UNAVAILABLE\", \"sealed.unseal(context)\")\n147: measurement = context.measurement()\n148: passed(\"context.measurement()\", not measurement[\"hardware\"] and not measurement[\"attested\"],\n149:        dict(measurement))",
