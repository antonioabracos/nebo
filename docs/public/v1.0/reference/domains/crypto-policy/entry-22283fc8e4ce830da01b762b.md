# crypto-policy — vault.revoke(identity,reason)

Synthetic data only; SHA/HMAC functional checks are not implementation security review; no physical zeroization assurance; original observation: revocation destroys current key and exposes only reason digest; SDK bounds: {"MAX_CIRCUIT_INPUTS":64,"MAX_CONSTRAINTS":256,"MAX_MPC_OPERATIONS":128,"MAX_PARAMETER_SETS":16,"MAX_PARTICIPANTS":16,"MAX_RANGE_BITS":256,"MAX_TEXT_BYTES":4096,"MAX_TRANSCRIPT_EVENTS":256,"MAX_VAULT_KEYS":64}

```text
Identity: owner:G044:G044-S07-04 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
KeyVault.revoke(self, identity: str, reason: str) -> Mapping[str, Any]
```
...
[
  ...
  {
    "excerpt": "190:        rotated[\"previousDestroyed\"], dict(rotated))\n191: revoked = vault.revoke(\"audit-key\", \"scheduled test completion\")\n192: passed(\"vault.revoke(identity,reason)\", revoked[\"revoked\"] and revoked[\"keyDestroyed\"],\n193:        dict(revoked))\n194: protocol = CryptoProtocol.compose((\"SHA-256\", \"HMAC-SHA256\"))",
