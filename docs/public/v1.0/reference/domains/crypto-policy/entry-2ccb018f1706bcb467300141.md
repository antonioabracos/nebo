# crypto-policy — attestation.provenance()

Synthetic data only; SHA/HMAC functional checks are not implementation security review; no physical zeroization assurance; original observation: hardware firmware verifier roots and claim absence are explicit; SDK bounds: {"MAX_CIRCUIT_INPUTS":64,"MAX_CONSTRAINTS":256,"MAX_MPC_OPERATIONS":128,"MAX_PARAMETER_SETS":16,"MAX_PARTICIPANTS":16,"MAX_RANGE_BITS":256,"MAX_TEXT_BYTES":4096,"MAX_TRANSCRIPT_EVENTS":256,"MAX_VAULT_KEYS":64}

```text
Identity: owner:G044:G044-S05-08 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
_AttestationService.provenance(self) -> Mapping[str, Any]
```
...
[
  ...
  {
    "excerpt": "151: passed(\"context.destroy()\", closed[\"destroyed\"] and closed[\"secretsInvalidated\"], dict(closed))\n152: provenance = attestation.provenance()\n153: passed(\"attestation.provenance()\", not provenance[\"attestationClaim\"] and\n154:        provenance[\"hardware\"] == \"UNAVAILABLE\", dict(provenance))\n155: ",
