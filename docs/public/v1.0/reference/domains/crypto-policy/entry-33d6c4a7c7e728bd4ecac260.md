# crypto-policy — CryptoAlgorithm.lookup(id,version)

Synthetic data only; SHA/HMAC functional checks are not implementation security review; no physical zeroization assurance; original observation: exact versioned registry identity governs resolution; SDK bounds: {"MAX_CIRCUIT_INPUTS":64,"MAX_CONSTRAINTS":256,"MAX_MPC_OPERATIONS":128,"MAX_PARAMETER_SETS":16,"MAX_PARTICIPANTS":16,"MAX_RANGE_BITS":256,"MAX_TEXT_BYTES":4096,"MAX_TRANSCRIPT_EVENTS":256,"MAX_VAULT_KEYS":64}

```text
Identity: owner:G044:G044-S01-01 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
CryptoAlgorithm.lookup(identifier: str, version: str) -> 'CryptoAlgorithm'
```
...
[
  ...
  {
    "excerpt": "52: # S01: approved base registry/key lifecycle plus factual self-tests.\n53: sha = CryptoAlgorithm.lookup(\"SHA-256\", \"FIPS-180-4\")\n54: passed(\"CryptoAlgorithm.lookup(id,version)\", sha.algorithm_id == \"SHA-256\", sha.algorithm_id)\n55: level = sha.securityLevel()\n56: passed(\"algorithm.securityLevel()\", level[\"targetBits\"] == 128, dict(level))",
