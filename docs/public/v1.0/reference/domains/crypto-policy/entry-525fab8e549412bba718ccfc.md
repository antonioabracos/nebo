# crypto-policy — key.destroy()

Synthetic data only; SHA/HMAC functional checks are not implementation security review; no physical zeroization assurance; original observation: material is overwritten and handle invalidation is observable without physical guarantee; SDK bounds: {"MAX_CIRCUIT_INPUTS":64,"MAX_CONSTRAINTS":256,"MAX_MPC_OPERATIONS":128,"MAX_PARAMETER_SETS":16,"MAX_PARTICIPANTS":16,"MAX_RANGE_BITS":256,"MAX_TEXT_BYTES":4096,"MAX_TRANSCRIPT_EVENTS":256,"MAX_VAULT_KEYS":64}

```text
Identity: owner:G044:G044-S01-07 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Key.destroy(self) -> Mapping[str, Any]
SecureContext.destroy(self) -> Mapping[str, Any]
```
...
[
  ...
  {
    "excerpt": "63: rejected(key.publicPart, \"KEY-TYPE\", \"key.publicPart()\")\n64: destroyed = key.destroy()\n65: passed(\"key.destroy()\", destroyed[\"destroyed\"] and not destroyed[\"physicalZeroizationGuaranteed\"],\n66:        dict(destroyed))\n67: self_test = crypto.selfTest()",
