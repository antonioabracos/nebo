# crypto-policy — kem.decapsulate(ciphertext,privateKey)

Synthetic data only; SHA/HMAC functional checks are not implementation security review; no physical zeroization assurance; original observation: decapsulation cannot fall back to an unreviewed construction; SDK bounds: {"MAX_CIRCUIT_INPUTS":64,"MAX_CONSTRAINTS":256,"MAX_MPC_OPERATIONS":128,"MAX_PARAMETER_SETS":16,"MAX_PARTICIPANTS":16,"MAX_RANGE_BITS":256,"MAX_TEXT_BYTES":4096,"MAX_TRANSCRIPT_EVENTS":256,"MAX_VAULT_KEYS":64}

```text
Identity: owner:G044:G044-S02-03 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
PqcKem.decapsulate(self, ciphertext: Any, privateKey: Any) -> None
```
...
[
  ...
  {
    "excerpt": "77:          \"kem.encapsulate(publicKey,random)\")\n78: rejected(lambda: kem.decapsulate(b\"ciphertext\", b\"private\"), \"EXTERNAL-REVIEW-REQUIRED\",\n79:          \"kem.decapsulate(ciphertext,privateKey)\")\n80: rejected(lambda: PqcSignature.keyPair(\"ML-DSA-65\"), \"EXTERNAL-REVIEW-REQUIRED\",\n81:          \"PqcSignature.keyPair(parameterSet)\")",
