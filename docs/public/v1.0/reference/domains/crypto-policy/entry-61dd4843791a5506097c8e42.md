# crypto-policy — circuit.publicInput(name,type)

Synthetic data only; SHA/HMAC functional checks are not implementation security review; no physical zeroization assurance; original observation: unique bounded public input identity and type are retained; SDK bounds: {"MAX_CIRCUIT_INPUTS":64,"MAX_CONSTRAINTS":256,"MAX_MPC_OPERATIONS":128,"MAX_PARAMETER_SETS":16,"MAX_PARTICIPANTS":16,"MAX_RANGE_BITS":256,"MAX_TEXT_BYTES":4096,"MAX_TRANSCRIPT_EVENTS":256,"MAX_VAULT_KEYS":64}

```text
Identity: owner:G044:G044-S03-02 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
ProofCircuit.publicInput(self, name: str, type: str) -> 'ProofCircuit'
```
...
[
  ...
  {
    "excerpt": "94: passed(\"ProofCircuit.new(field,limits)\", circuit.field == \"BN254-METADATA\", circuit.field)\n95: circuit.publicInput(\"total\", \"u64\")\n96: passed(\"circuit.publicInput(name,type)\", circuit._inputs[\"total\"] == (\"public\", \"u64\"))\n97: circuit.privateInput(\"witness\", \"u64\")\n98: passed(\"circuit.privateInput(name,type)\", circuit._inputs[\"witness\"] == (\"private\", \"u64\"))",
