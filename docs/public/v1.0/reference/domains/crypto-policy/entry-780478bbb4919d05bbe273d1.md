# crypto-policy — secret.declassify(reason,capability)

Synthetic data only; SHA/HMAC functional checks are not implementation security review; no physical zeroization assurance; original observation: exact capability releases synthetic bytes and records only reason digest; SDK bounds: {"MAX_CIRCUIT_INPUTS":64,"MAX_CONSTRAINTS":256,"MAX_MPC_OPERATIONS":128,"MAX_PARAMETER_SETS":16,"MAX_PARTICIPANTS":16,"MAX_RANGE_BITS":256,"MAX_TEXT_BYTES":4096,"MAX_TRANSCRIPT_EVENTS":256,"MAX_VAULT_KEYS":64}

```text
Identity: owner:G044:G044-S06-05 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Secret.declassify(self, reason: str, capability: Capability) -> bytes
```
...
[
  ...
  {
    "excerpt": "166:        not cache[\"cacheIsolationAvailable\"], dict(cache))\n167: revealed = secret.declassify(\"bounded oracle equality check\",\n168:                              Capability.issue(\"secret.declassify\", \"oracle-s06\"))\n169: passed(\"secret.declassify(reason,capability)\",\n170:        hashlib.sha256(revealed).hexdigest() == hashlib.sha256(b\"synthetic-secret-44\").hexdigest()",
