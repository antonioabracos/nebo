# crypto-policy — session.abort(reason)

Synthetic data only; SHA/HMAC functional checks are not implementation security review; no physical zeroization assurance; original observation: abort hashes reason clears active state and prevents further compute; SDK bounds: {"MAX_CIRCUIT_INPUTS":64,"MAX_CONSTRAINTS":256,"MAX_MPC_OPERATIONS":128,"MAX_PARAMETER_SETS":16,"MAX_PARTICIPANTS":16,"MAX_RANGE_BITS":256,"MAX_TEXT_BYTES":4096,"MAX_TRANSCRIPT_EVENTS":256,"MAX_VAULT_KEYS":64}

```text
Identity: owner:G044:G044-S04-07 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
MpcSession.abort(self, reason: str) -> Mapping[str, Any]
```
...
[
  ...
  {
    "excerpt": "128:        not computed[\"mpcSecurityClaim\"], dict(computed))\n129: aborted = session.abort(\"oracle-complete\")\n130: passed(\"session.abort(reason)\", aborted[\"aborted\"] and aborted[\"stateCleared\"], dict(aborted))\n131: transcript = session.transcript()\n132: passed(\"session.transcript()\", transcript[\"redacted\"] and len(transcript[\"events\"]) == 3 and",
