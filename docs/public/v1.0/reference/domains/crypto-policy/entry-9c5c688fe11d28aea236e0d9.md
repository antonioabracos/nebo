# crypto-policy — signature.verify(message,signature,publicKey)

Synthetic data only; SHA/HMAC functional checks are not implementation security review; no physical zeroization assurance; original observation: verification never returns unconditional true and remains unavailable; SDK bounds: {"MAX_CIRCUIT_INPUTS":64,"MAX_CONSTRAINTS":256,"MAX_MPC_OPERATIONS":128,"MAX_PARAMETER_SETS":16,"MAX_PARTICIPANTS":16,"MAX_RANGE_BITS":256,"MAX_TEXT_BYTES":4096,"MAX_TRANSCRIPT_EVENTS":256,"MAX_VAULT_KEYS":64}

```text
Identity: owner:G044:G044-S02-06 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
PqcSignature.verify(self, message: Any, signature: Any, publicKey: Any) -> None
Proof.verify(self, publicInputs: Any, verificationKey: Any) -> None
Attestation.verify(report: Any, policy: Mapping[str, Any], roots: Sequence[Any]) -> None
```
...
[
  ...
  {
    "excerpt": "83:          \"EXTERNAL-REVIEW-REQUIRED\", \"signature.sign(message,privateKey,random)\")\n84: rejected(lambda: signature.verify(b\"message\", b\"signature\", b\"public\"),\n85:          \"EXTERNAL-REVIEW-REQUIRED\", \"signature.verify(message,signature,publicKey)\")\n86: rejected(lambda: HybridKex.combine(b\"classical\", b\"post-quantum\"),\n87:          \"EXTERNAL-REVIEW-REQUIRED\", \"HybridKex.combine(classical,postQuantum)\")",
