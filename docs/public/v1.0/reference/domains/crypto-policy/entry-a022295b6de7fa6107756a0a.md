# crypto-policy — vault.rotate(identity,schedule)

Synthetic data only; SHA/HMAC functional checks are not implementation security review; no physical zeroization assurance; original observation: explicit replacement and epoch increment version and destroy predecessor; SDK bounds: {"MAX_CIRCUIT_INPUTS":64,"MAX_CONSTRAINTS":256,"MAX_MPC_OPERATIONS":128,"MAX_PARAMETER_SETS":16,"MAX_PARTICIPANTS":16,"MAX_RANGE_BITS":256,"MAX_TEXT_BYTES":4096,"MAX_TRANSCRIPT_EVENTS":256,"MAX_VAULT_KEYS":64}

```text
Identity: owner:G044:G044-S07-03 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
KeyVault.rotate(self, identity: str, schedule: Mapping[str, Any]) -> Mapping[str, Any]
```
...
[
  ...
  {
    "excerpt": "187: replacement = Key.generate(hmac_algorithm, new_entropy(0x22, \"vault-replacement\"))\n188: rotated = vault.rotate(\"audit-key\", {\"replacement\": replacement, \"effectiveEpoch\": 44})\n189: passed(\"vault.rotate(identity,schedule)\", rotated[\"version\"] == 2 and\n190:        rotated[\"previousDestroyed\"], dict(rotated))\n191: revoked = vault.revoke(\"audit-key\", \"scheduled test completion\")",
