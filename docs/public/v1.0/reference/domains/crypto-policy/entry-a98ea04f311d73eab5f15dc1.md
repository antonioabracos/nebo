# crypto-policy — MpcSession.new(participants,protocol,policy)

Synthetic data only; SHA/HMAC functional checks are not implementation security review; no physical zeroization assurance; original observation: distinct participants explicit simulation and operation bound govern local state; SDK bounds: {"MAX_CIRCUIT_INPUTS":64,"MAX_CONSTRAINTS":256,"MAX_MPC_OPERATIONS":128,"MAX_PARAMETER_SETS":16,"MAX_PARTICIPANTS":16,"MAX_RANGE_BITS":256,"MAX_TEXT_BYTES":4096,"MAX_TRANSCRIPT_EVENTS":256,"MAX_VAULT_KEYS":64}

```text
Identity: owner:G044:G044-S04-04 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
MpcSession.new(participants: Sequence[str], protocol: str, policy: Mapping[str, Any]) -> 'MpcSession'
```
...
[
  ...
  {
    "excerpt": "92: # S03: bounded constraint metadata is executable; proof generation remains gated.\n93: circuit = ProofCircuit.new(\"BN254-METADATA\", {\"inputs\": 4, \"constraints\": 5, \"rangeBits\": 64})\n94: passed(\"ProofCircuit.new(field,limits)\", circuit.field == \"BN254-METADATA\", circuit.field)\n95: circuit.publicInput(\"total\", \"u64\")\n96: passed(\"circuit.publicInput(name,type)\", circuit._inputs[\"total\"] == (\"public\", \"u64\"))",
