# crypto-policy — circuit.privateInput(name,type)

Synthetic data only; SHA/HMAC functional checks are not implementation security review; no physical zeroization assurance; original observation: private input identity is retained only in circuit metadata; SDK bounds: {"MAX_CIRCUIT_INPUTS":64,"MAX_CONSTRAINTS":256,"MAX_MPC_OPERATIONS":128,"MAX_PARAMETER_SETS":16,"MAX_PARTICIPANTS":16,"MAX_RANGE_BITS":256,"MAX_TEXT_BYTES":4096,"MAX_TRANSCRIPT_EVENTS":256,"MAX_VAULT_KEYS":64}

```text
Identity: owner:G044:G044-S03-03 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
ProofCircuit.privateInput(self, name: str, type: str) -> 'ProofCircuit'
```
...
[
  ...
  {
    "excerpt": "96: passed(\"circuit.publicInput(name,type)\", circuit._inputs[\"total\"] == (\"public\", \"u64\"))\n97: circuit.privateInput(\"witness\", \"u64\")\n98: passed(\"circuit.privateInput(name,type)\", circuit._inputs[\"witness\"] == (\"private\", \"u64\"))\n99: circuit.assertEqual(\"total\", \"witness\")\n100: passed(\"circuit.assertEqual(a,b)\", len(circuit._constraints) == 1)",
