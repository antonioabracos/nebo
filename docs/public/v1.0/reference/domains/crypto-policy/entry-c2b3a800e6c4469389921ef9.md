# crypto-policy — circuit.rangeCheck(value,bits)

Synthetic data only; SHA/HMAC functional checks are not implementation security review; no physical zeroization assurance; original observation: input identity and configured bit ceiling govern range constraint; SDK bounds: {"MAX_CIRCUIT_INPUTS":64,"MAX_CONSTRAINTS":256,"MAX_MPC_OPERATIONS":128,"MAX_PARAMETER_SETS":16,"MAX_PARTICIPANTS":16,"MAX_RANGE_BITS":256,"MAX_TEXT_BYTES":4096,"MAX_TRANSCRIPT_EVENTS":256,"MAX_VAULT_KEYS":64}

```text
Identity: owner:G044:G044-S03-05 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
ProofCircuit.rangeCheck(self, value: str, bits: int) -> 'ProofCircuit'
```
...
[
  ...
  {
    "excerpt": "100: passed(\"circuit.assertEqual(a,b)\", len(circuit._constraints) == 1)\n101: circuit.rangeCheck(\"witness\", 17)\n102: passed(\"circuit.rangeCheck(value,bits)\", circuit._constraints[-1][\"bits\"] == 17)\n103: compiled = circuit.compile(\"CONSTRAINTS-ONLY-V1\")\n104: passed(\"circuit.compile(backend)\", not compiled[\"proofCapable\"] and len(compiled[\"digest\"]) == 64,",
