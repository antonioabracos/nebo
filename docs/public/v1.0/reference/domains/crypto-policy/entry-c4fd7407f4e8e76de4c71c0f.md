# crypto-policy — signature.sign(message,privateKey,random)

Synthetic data only; SHA/HMAC functional checks are not implementation security review; no physical zeroization assurance; original observation: signing rejects without inspecting or logging secret arguments; SDK bounds: {"MAX_CIRCUIT_INPUTS":64,"MAX_CONSTRAINTS":256,"MAX_MPC_OPERATIONS":128,"MAX_PARAMETER_SETS":16,"MAX_PARTICIPANTS":16,"MAX_RANGE_BITS":256,"MAX_TEXT_BYTES":4096,"MAX_TRANSCRIPT_EVENTS":256,"MAX_VAULT_KEYS":64}

```text
Identity: owner:G044:G044-S02-05 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
PqcSignature.sign(self, message: Any, privateKey: Any, random: Any) -> None
```
...
[
  ...
  {
    "excerpt": "81:          \"PqcSignature.keyPair(parameterSet)\")\n82: rejected(lambda: signature.sign(b\"message\", b\"private\", b\"random\"),\n83:          \"EXTERNAL-REVIEW-REQUIRED\", \"signature.sign(message,privateKey,random)\")\n84: rejected(lambda: signature.verify(b\"message\", b\"signature\", b\"public\"),\n85:          \"EXTERNAL-REVIEW-REQUIRED\", \"signature.verify(message,signature,publicKey)\")",
