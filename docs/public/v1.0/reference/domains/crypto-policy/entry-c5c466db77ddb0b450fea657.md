# crypto-policy — security.timingReport()

Synthetic data only; SHA/HMAC functional checks are not implementation security review; no physical zeroization assurance; original observation: target rules dynamic absence confidence and limitations are factual; SDK bounds: {"MAX_CIRCUIT_INPUTS":64,"MAX_CONSTRAINTS":256,"MAX_MPC_OPERATIONS":128,"MAX_PARAMETER_SETS":16,"MAX_PARTICIPANTS":16,"MAX_RANGE_BITS":256,"MAX_TEXT_BYTES":4096,"MAX_TRANSCRIPT_EVENTS":256,"MAX_VAULT_KEYS":64}

```text
Identity: owner:G044:G044-S06-06 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
_SecurityService.timingReport(self) -> Mapping[str, Any]
```
...
[
  ...
  {
    "excerpt": "171:        and len(secret.audit()) == 1, {\"auditRecords\": len(secret.audit())})\n172: timing = security.timingReport()\n173: passed(\"security.timingReport()\", not timing[\"constantTimeProof\"] and\n174:        timing[\"dynamicTests\"] == \"NOT_EXECUTED\", dict(timing))\n175: barrier = security.compilerBarrier()",
