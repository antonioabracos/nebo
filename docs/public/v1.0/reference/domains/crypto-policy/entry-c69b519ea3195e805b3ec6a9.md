# crypto-policy — KeyVault.local(capability,policy)

Synthetic data only; SHA/HMAC functional checks are not implementation security review; no physical zeroization assurance; original observation: exact capability finite key budget and memory-only policy govern vault; SDK bounds: {"MAX_CIRCUIT_INPUTS":64,"MAX_CONSTRAINTS":256,"MAX_MPC_OPERATIONS":128,"MAX_PARAMETER_SETS":16,"MAX_PARTICIPANTS":16,"MAX_RANGE_BITS":256,"MAX_TEXT_BYTES":4096,"MAX_TRANSCRIPT_EVENTS":256,"MAX_VAULT_KEYS":64}

```text
Identity: owner:G044:G044-S07-01 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
KeyVault.local(capability: Capability, policy: Mapping[str, Any]) -> 'KeyVault'
```
...
[
  ...
  {
    "excerpt": "179: # S07: memory-only opaque lifecycle and one reviewed base composition.\n180: vault = KeyVault.local(Capability.issue(\"vault.local\", \"oracle-s07\"),\n181:                        {\"storage\": \"memory-only\", \"maxKeys\": 3})\n182: passed(\"KeyVault.local(capability,policy)\", vault.policy[\"storage\"] == \"memory-only\")\n183: first = Key.generate(hmac_algorithm, new_entropy(0x21, \"vault-first\"))",
