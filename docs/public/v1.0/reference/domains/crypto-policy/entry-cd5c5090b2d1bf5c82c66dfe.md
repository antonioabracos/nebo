# crypto-policy — context.measurement()

Synthetic data only; SHA/HMAC functional checks are not implementation security review; no physical zeroization assurance; original observation: content-derived simulation identity exposes hardware and attested false; SDK bounds: {"MAX_CIRCUIT_INPUTS":64,"MAX_CONSTRAINTS":256,"MAX_MPC_OPERATIONS":128,"MAX_PARAMETER_SETS":16,"MAX_PARTICIPANTS":16,"MAX_RANGE_BITS":256,"MAX_TEXT_BYTES":4096,"MAX_TRANSCRIPT_EVENTS":256,"MAX_VAULT_KEYS":64}

```text
Identity: owner:G044:G044-S05-06 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
SecureContext.measurement(self) -> Mapping[str, Any]
```
...
[
  ...
  {
    "excerpt": "140:          \"Attestation.request(context,nonce)\")\n141: rejected(lambda: Attestation.verify({}, {\"measurement\": \"expected\"}, (\"root\",)),\n142:          \"HARDWARE-UNAVAILABLE\", \"Attestation.verify(report,policy,roots)\")\n143: secret = Secret.synthetic(b\"synthetic-secret-44\")\n144: rejected(lambda: secret.seal(context, {\"bindMeasurement\": True}), \"HARDWARE-UNAVAILABLE\",",
