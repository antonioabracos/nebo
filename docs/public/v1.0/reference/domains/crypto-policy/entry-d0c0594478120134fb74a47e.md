# crypto-policy — SecureContext.open(backend,policy)

Synthetic data only; SHA/HMAC functional checks are not implementation security review; no physical zeroization assurance; original observation: only explicitly requested simulation identity opens and claims no TEE; SDK bounds: {"MAX_CIRCUIT_INPUTS":64,"MAX_CONSTRAINTS":256,"MAX_MPC_OPERATIONS":128,"MAX_PARAMETER_SETS":16,"MAX_PARTICIPANTS":16,"MAX_RANGE_BITS":256,"MAX_TEXT_BYTES":4096,"MAX_TRANSCRIPT_EVENTS":256,"MAX_VAULT_KEYS":64}

```text
Identity: owner:G044:G044-S05-01 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
SecureContext.open(backend: str, policy: Mapping[str, Any]) -> 'SecureContext'
```
...
[
  ...
  {
    "excerpt": "136: # S05: simulation identity is inspectable, while hardware-dependent operations fail closed.\n137: context = SecureContext.open(\"SIMULATED-NO-TEE\", {\"allowSimulation\": True})\n138: passed(\"SecureContext.open(backend,policy)\", context.backend == \"SIMULATED-NO-TEE\")\n139: rejected(lambda: Attestation.request(context, b\"nonce-44\"), \"HARDWARE-UNAVAILABLE\",\n140:          \"Attestation.request(context,nonce)\")",
