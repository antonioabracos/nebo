# crypto-policy — LeakageTest.cacheTrace(function,corpus)

Synthetic data only; SHA/HMAC functional checks are not implementation security review; no physical zeroization assurance; original observation: missing cache-isolation backend is reported without a leakage claim; SDK bounds: {"MAX_CIRCUIT_INPUTS":64,"MAX_CONSTRAINTS":256,"MAX_MPC_OPERATIONS":128,"MAX_PARAMETER_SETS":16,"MAX_PARTICIPANTS":16,"MAX_RANGE_BITS":256,"MAX_TEXT_BYTES":4096,"MAX_TRANSCRIPT_EVENTS":256,"MAX_VAULT_KEYS":64}

```text
Identity: owner:G044:G044-S06-04 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
LeakageTest.cacheTrace(function: Callable[..., Any], corpus: Sequence[bytes]) -> Mapping[str, Any]
```
...
[
  ...
  {
    "excerpt": "163:        dudect[\"status\"] == \"ENVIRONMENT_LIMITED\", dict(dudect))\n164: cache = LeakageTest.cacheTrace(xor_words, (b\"c\", b\"d\"))\n165: passed(\"LeakageTest.cacheTrace(function,corpus)\", not cache[\"executed\"] and\n166:        not cache[\"cacheIsolationAvailable\"], dict(cache))\n167: revealed = secret.declassify(\"bounded oracle equality check\",",
