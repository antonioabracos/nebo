# crypto-policy — Key.generate(algorithm,capability)

Synthetic data only; SHA/HMAC functional checks are not implementation security review; no physical zeroization assurance; original observation: approved HMAC and one-shot synthetic entropy govern opaque key creation; SDK bounds: {"MAX_CIRCUIT_INPUTS":64,"MAX_CONSTRAINTS":256,"MAX_MPC_OPERATIONS":128,"MAX_PARAMETER_SETS":16,"MAX_PARTICIPANTS":16,"MAX_RANGE_BITS":256,"MAX_TEXT_BYTES":4096,"MAX_TRANSCRIPT_EVENTS":256,"MAX_VAULT_KEYS":64}

```text
Identity: owner:G044:G044-S01-05 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Key.generate(algorithm: CryptoAlgorithm, capability: Capability) -> 'Key'
```
...
[
  ...
  {
    "excerpt": "60: hmac_algorithm = CryptoAlgorithm.lookup(\"HMAC-SHA256\", \"RFC-2104\")\n61: key = Key.generate(hmac_algorithm, new_entropy(0x44, \"oracle-s01\"))\n62: passed(\"Key.generate(algorithm,capability)\", len(key.identity) == 64, key.identity)\n63: rejected(key.publicPart, \"KEY-TYPE\", \"key.publicPart()\")\n64: destroyed = key.destroy()",
