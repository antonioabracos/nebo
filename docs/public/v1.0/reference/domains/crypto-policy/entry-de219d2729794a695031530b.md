# crypto-policy — protocol.reviewStatus()

Synthetic data only; SHA/HMAC functional checks are not implementation security review; no physical zeroization assurance; original observation: review references version and advanced-family gates are immutable; SDK bounds: {"MAX_CIRCUIT_INPUTS":64,"MAX_CONSTRAINTS":256,"MAX_MPC_OPERATIONS":128,"MAX_PARAMETER_SETS":16,"MAX_PARTICIPANTS":16,"MAX_RANGE_BITS":256,"MAX_TEXT_BYTES":4096,"MAX_TRANSCRIPT_EVENTS":256,"MAX_VAULT_KEYS":64}

```text
Identity: owner:G044:G044-S07-06 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
CryptoProtocol.reviewStatus(self) -> Mapping[str, Any]
```
...
[
  ...
  {
    "excerpt": "195: passed(\"CryptoProtocol.compose(steps)\", protocol.steps == (\"SHA-256\", \"HMAC-SHA256\"))\n196: review = protocol.reviewStatus()\n197: passed(\"protocol.reviewStatus()\", review[\"status\"] == \"approved-base-only\" and\n198:        review[\"advancedFamilies\"] == \"GATED\", dict(review))\n199: valid_transcript = protocol.makeTranscript({\"message\": \"synthetic\", \"sequence\": 44})",
