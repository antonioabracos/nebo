# crypto-policy — pqc.vectorReport()

Synthetic data only; SHA/HMAC functional checks are not implementation security review; no physical zeroization assurance; original observation: standards parameter sets missing vectors implementation and review are factual; SDK bounds: {"MAX_CIRCUIT_INPUTS":64,"MAX_CONSTRAINTS":256,"MAX_MPC_OPERATIONS":128,"MAX_PARAMETER_SETS":16,"MAX_PARTICIPANTS":16,"MAX_RANGE_BITS":256,"MAX_TEXT_BYTES":4096,"MAX_TRANSCRIPT_EVENTS":256,"MAX_VAULT_KEYS":64}

```text
Identity: owner:G044:G044-S02-08 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
_PqcService.vectorReport(self) -> Mapping[str, Any]
```
...
[
  ...
  {
    "excerpt": "87:          \"EXTERNAL-REVIEW-REQUIRED\", \"HybridKex.combine(classical,postQuantum)\")\n88: vector_report = pqc.vectorReport()\n89: passed(\"pqc.vectorReport()\", not vector_report[\"quantumSafeClaim\"] and\n90:        vector_report[\"ML-KEM-768\"][\"implementation\"] == \"UNAVAILABLE\", dict(vector_report))\n91: ",
