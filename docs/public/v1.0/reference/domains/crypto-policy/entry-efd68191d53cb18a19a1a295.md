# crypto-policy — algorithm.parameterSet(name)

Synthetic data only; SHA/HMAC functional checks are not implementation security review; no physical zeroization assurance; original observation: parameter name selects bounded versioned values; SDK bounds: {"MAX_CIRCUIT_INPUTS":64,"MAX_CONSTRAINTS":256,"MAX_MPC_OPERATIONS":128,"MAX_PARAMETER_SETS":16,"MAX_PARTICIPANTS":16,"MAX_RANGE_BITS":256,"MAX_TEXT_BYTES":4096,"MAX_TRANSCRIPT_EVENTS":256,"MAX_VAULT_KEYS":64}

```text
Identity: owner:G044:G044-S01-03 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
CryptoAlgorithm.parameterSet(self, name: str) -> Mapping[str, Any]
```
...
[
  ...
  {
    "excerpt": "56: passed(\"algorithm.securityLevel()\", level[\"targetBits\"] == 128, dict(level))\n57: params = sha.parameterSet(\"SHA-256\")\n58: passed(\"algorithm.parameterSet(name)\", params[\"digestBytes\"] == 32, dict(params))\n59: passed(\"algorithm.status()\", sha.status() == \"approved\", sha.status())\n60: hmac_algorithm = CryptoAlgorithm.lookup(\"HMAC-SHA256\", \"RFC-2104\")",
