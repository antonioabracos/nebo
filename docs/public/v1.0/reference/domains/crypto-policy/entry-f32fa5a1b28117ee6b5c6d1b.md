# crypto-policy — SecretShare.combine(shares)

Synthetic data only; SHA/HMAC functional checks are not implementation security review; no physical zeroization assurance; original observation: sequence shape is checked and no invented reconstruction runs; SDK bounds: {"MAX_CIRCUIT_INPUTS":64,"MAX_CONSTRAINTS":256,"MAX_MPC_OPERATIONS":128,"MAX_PARAMETER_SETS":16,"MAX_PARTICIPANTS":16,"MAX_RANGE_BITS":256,"MAX_TEXT_BYTES":4096,"MAX_TRANSCRIPT_EVENTS":256,"MAX_VAULT_KEYS":64}

```text
Identity: owner:G044:G044-S04-02 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
SecretShare.combine(shares: Any) -> None
```
...
[
  ...
  {
    "excerpt": "85:          \"EXTERNAL-REVIEW-REQUIRED\", \"signature.verify(message,signature,publicKey)\")\n86: rejected(lambda: HybridKex.combine(b\"classical\", b\"post-quantum\"),\n87:          \"EXTERNAL-REVIEW-REQUIRED\", \"HybridKex.combine(classical,postQuantum)\")\n88: vector_report = pqc.vectorReport()\n89: passed(\"pqc.vectorReport()\", not vector_report[\"quantumSafeClaim\"] and",
