# crypto-policy — circuit.compile(backend)

Synthetic data only; SHA/HMAC functional checks are not implementation security review; no physical zeroization assurance; original observation: constraints-only descriptor is content-addressed and explicitly not proof-capable; SDK bounds: {"MAX_CIRCUIT_INPUTS":64,"MAX_CONSTRAINTS":256,"MAX_MPC_OPERATIONS":128,"MAX_PARAMETER_SETS":16,"MAX_PARTICIPANTS":16,"MAX_RANGE_BITS":256,"MAX_TEXT_BYTES":4096,"MAX_TRANSCRIPT_EVENTS":256,"MAX_VAULT_KEYS":64}

```text
Identity: owner:G044:G044-S03-06 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
ProofCircuit.compile(self, backend: str) -> Mapping[str, Any]
```
...
[
  ...
  {
    "excerpt": "102: passed(\"circuit.rangeCheck(value,bits)\", circuit._constraints[-1][\"bits\"] == 17)\n103: compiled = circuit.compile(\"CONSTRAINTS-ONLY-V1\")\n104: passed(\"circuit.compile(backend)\", not compiled[\"proofCapable\"] and len(compiled[\"digest\"]) == 64,\n105:        dict(compiled))\n106: rejected(lambda: circuit.prove({\"witness\": 9}, b\"random\"), \"EXTERNAL-REVIEW-REQUIRED\",",
