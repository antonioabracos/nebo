# crypto-policy — protocol.verifyTranscript(transcript)

Synthetic data only; SHA/HMAC functional checks are not implementation security review; no physical zeroization assurance; original observation: version sequence payload digest and transcript digest govern verification; SDK bounds: {"MAX_CIRCUIT_INPUTS":64,"MAX_CONSTRAINTS":256,"MAX_MPC_OPERATIONS":128,"MAX_PARAMETER_SETS":16,"MAX_PARTICIPANTS":16,"MAX_RANGE_BITS":256,"MAX_TEXT_BYTES":4096,"MAX_TRANSCRIPT_EVENTS":256,"MAX_VAULT_KEYS":64}

```text
Identity: owner:G044:G044-S07-07 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
CryptoProtocol.verifyTranscript(self, transcript: Mapping[str, Any]) -> bool
```
...
[
  ...
  {
    "excerpt": "199: valid_transcript = protocol.makeTranscript({\"message\": \"synthetic\", \"sequence\": 44})\n200: passed(\"protocol.verifyTranscript(transcript)\", protocol.verifyTranscript(valid_transcript),\n201:        dict(valid_transcript))\n202: \n203: assert len(surfaces) == 55, (len(surfaces), surfaces)",
