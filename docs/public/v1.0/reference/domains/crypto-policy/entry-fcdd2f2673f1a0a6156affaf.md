# crypto-policy — crypto.selfTest()

Synthetic data only; SHA/HMAC functional checks are not implementation security review; no physical zeroization assurance; original observation: FIPS SHA-256 and RFC HMAC known-answer vectors are recomputed; SDK bounds: {"MAX_CIRCUIT_INPUTS":64,"MAX_CONSTRAINTS":256,"MAX_MPC_OPERATIONS":128,"MAX_PARAMETER_SETS":16,"MAX_PARTICIPANTS":16,"MAX_RANGE_BITS":256,"MAX_TEXT_BYTES":4096,"MAX_TRANSCRIPT_EVENTS":256,"MAX_VAULT_KEYS":64}

```text
Identity: owner:G044:G044-S01-08 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
_CryptoService.selfTest(self) -> Mapping[str, Any]
```
...
[
  ...
  {
    "excerpt": "66:        dict(destroyed))\n67: self_test = crypto.selfTest()\n68: passed(\"crypto.selfTest()\", self_test[\"passed\"] == 2 and self_test[\"advanced\"] == \"GATED\",\n69:        dict(self_test))\n70: ",
