...
Legacy data owner record; current typed Table declarations retain their independent public tier; original observation: Query returns the current bounded byte count

```text
Identity: owner:G018:console.size() (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
console.size()
Owner: runtime/console/document/linear_console_document.asm::nebo_console_size_query
```
