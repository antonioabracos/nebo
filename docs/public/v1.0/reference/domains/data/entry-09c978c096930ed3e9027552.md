...
Legacy data owner record; current typed Table declarations retain their independent public tier; original observation: Typed event kind is observed from the queue

```text
Identity: owner:G018:event.kind() (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
event.kind()
Owner: runtime/window/headless/headless_window.asm::nebo_headless_window_event_kind
```
