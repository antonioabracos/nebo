# data — canvas.rectangle(rect, paint)

Legacy data owner record; current typed Table declarations retain their independent public tier; original observation: Validated rectangle records deterministic bounds

```text
Identity: owner:G018:canvas.rectangle(rect, paint) (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
canvas.rectangle(rect, paint)
Owner: runtime/canvas/canvas.asm::nebo_canvas_rectangle
```
