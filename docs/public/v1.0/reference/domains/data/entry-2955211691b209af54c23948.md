...
Legacy data owner record; current typed Table declarations retain their independent public tier; original observation: Close, stream release and reclaim occur exactly once

```text
Identity: owner:G018:window.close() (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
window.close()
Owner: runtime/window/headless/headless_window.asm::nebo_headless_window_close
```
