# data — Chart.bar(categories, values)

Legacy data owner record; current typed Table declarations retain their independent public tier; original observation: Category labels and values lower in source order

```text
Identity: owner:G018:Chart.bar(categories, values) (DOMAIN_CONTRACT_ID)
Edition: 1
...
Owner: runtime/charts/charts.asm::nebo_chart_bar
```
