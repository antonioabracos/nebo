...
Legacy data owner record; current typed Table declarations retain their independent public tier; original observation: Key payload preserves the submitted key code

```text
Identity: owner:G018:event.key() (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
event.key()
Owner: runtime/window/headless/headless_window.asm::nebo_headless_window_event_key
```
