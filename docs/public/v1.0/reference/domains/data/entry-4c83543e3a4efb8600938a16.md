...
Legacy data owner record; current typed Table declarations retain their independent public tier; original observation: Validated dimensions publish a typed resize event

```text
Identity: owner:G018:window.resize(width, height) (DOMAIN_CONTRACT_ID)
Edition: 1
...
Owner: runtime/window/headless/headless_window.asm::nebo_headless_window_resize
```
