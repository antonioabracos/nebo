...
Legacy data owner record; current typed Table declarations retain their independent public tier; original observation: An owner-authenticated nonblocking stream is returned

```text
Identity: owner:G018:window.events() (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
window.events()
Owner: runtime/window/headless/headless_window.asm::nebo_headless_window_events
```
