# data — console.title(text)

Legacy data owner record; current typed Table declarations retain their independent public tier; original observation: Owned title bytes are replaced atomically

```text
Identity: owner:G018:console.title(text) (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
console.title(text)
Owner: runtime/console/document/linear_console_document.asm::nebo_console_set_title
```
