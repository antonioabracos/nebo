# data — window.present(canvas)

Legacy data owner record; current typed Table declarations retain their independent public tier; original observation: Shown Window observes the Canvas state hash

```text
Identity: owner:G018:window.present(canvas) (DOMAIN_CONTRACT_ID)
Edition: 1
...
Owner: runtime/canvas/canvas.asm::nebo_canvas_present_headless
```
