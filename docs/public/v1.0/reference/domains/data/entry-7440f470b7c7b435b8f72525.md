# data — Layout.row(children)

Legacy data owner record; current typed Table declarations retain their independent public tier; original observation: Children receive exact horizontal integer extents

```text
Identity: owner:G018:Layout.row(children) (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Layout.row(children)
Owner: runtime/widgets/widgets.asm::nebo_ui_row
```
