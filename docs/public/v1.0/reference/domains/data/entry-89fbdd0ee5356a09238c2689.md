# data — Window.create(options)

Legacy data owner record; current typed Table declarations retain their independent public tier; original observation: Caller-owned options create an authenticated headless handle

```text
Identity: owner:G018:Window.create(options) (DOMAIN_CONTRACT_ID)
Edition: 1
...
Owner: runtime/window/headless/headless_window.asm::nebo_headless_window_create
```
