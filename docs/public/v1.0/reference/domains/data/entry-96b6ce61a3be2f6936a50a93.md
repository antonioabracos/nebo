...
Legacy data owner record; current typed Table declarations retain their independent public tier; original observation: Visible state and event generation advance deterministically

```text
Identity: owner:G018:window.show() (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
window.show()
Owner: runtime/window/headless/headless_window.asm::nebo_headless_window_show
```
