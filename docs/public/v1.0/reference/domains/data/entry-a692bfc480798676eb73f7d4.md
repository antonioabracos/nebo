# data — canvas.text(text, position, font, paint)

Legacy data owner record; current typed Table declarations retain their independent public tier; original observation: Built-in glyph path records bounded UTF-8 text

```text
Identity: owner:G018:canvas.text(text, position, font, paint) (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
canvas.text(text, position, font, paint)
Owner: runtime/canvas/canvas.asm::nebo_canvas_text
```
...
- runtime/canvas/canvas.asm — SHA-256 1962715c6483de7cd2ed4a25db667bd17d605e74e24a052eb9ded4b778ae3056
