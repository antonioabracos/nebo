...
Legacy data owner record; current typed Table declarations retain their independent public tier; original observation: Pointer payload preserves bounded coordinates

```text
Identity: owner:G018:event.pointer() (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
event.pointer()
Owner: runtime/window/headless/headless_window.asm::nebo_headless_window_event_pointer
```
