# data — chart.render(canvas)

Legacy data owner record; current typed Table declarations retain their independent public tier; original observation: Chart geometry publishes Canvas commands and state hash

```text
Identity: owner:G018:chart.render(canvas) (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
chart.render(canvas)
Owner: runtime/charts/charts.asm::nebo_chart_render
```
