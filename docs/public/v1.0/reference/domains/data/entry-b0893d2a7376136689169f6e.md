...
Legacy data owner record; current typed Table declarations retain their independent public tier; original observation: Every pixel receives the canonical color

```text
Identity: owner:G018:canvas.clear(color) (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
canvas.clear(color)
Owner: runtime/canvas/canvas.asm::nebo_canvas_clear
```
