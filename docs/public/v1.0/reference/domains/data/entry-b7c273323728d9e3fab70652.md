...
Legacy data owner record; current typed Table declarations retain their independent public tier; original observation: Console acquisition uses an authenticated handle and pending identity

```text
Identity: owner:G018:Console.scan() (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Console.scan()
Owner: runtime/core/runtime_core.asm::nebo_runtime_scan_console_handle
```
