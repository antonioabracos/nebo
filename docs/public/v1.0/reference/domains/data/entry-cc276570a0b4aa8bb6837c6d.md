# data — event.textInput()

Legacy data owner record; current typed Table declarations retain their independent public tier; original observation: UTF-8 input payload is length checked

```text
Identity: owner:G018:event.textInput() (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
event.textInput()
Owner: runtime/window/headless/headless_window.asm::nebo_headless_window_event_text_input
```
