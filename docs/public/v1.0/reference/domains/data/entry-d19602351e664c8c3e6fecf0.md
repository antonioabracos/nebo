# data — Button.new(label)

Legacy data owner record; current typed Table declarations retain their independent public tier; original observation: Button owns bounded label metadata

```text
Identity: owner:G018:Button.new(label) (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Button.new(label)
Owner: runtime/widgets/widgets.asm::nebo_ui_button
```
