...
Legacy data owner record; current typed Table declarations retain their independent public tier; original observation: Chart owns validated UTF-8 title bytes

```text
Identity: owner:G018:chart.title(text) (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
chart.title(text)
Owner: runtime/charts/charts.asm::nebo_chart_title
```
