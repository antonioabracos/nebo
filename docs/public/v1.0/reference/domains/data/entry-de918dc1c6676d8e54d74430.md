...
Legacy data owner record; current typed Table declarations retain their independent public tier; original observation: Resize payload reports the new dimensions

```text
Identity: owner:G018:event.resize() (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
event.resize()
Owner: runtime/window/headless/headless_window.asm::nebo_headless_window_event_resize
```
