# data — Canvas.create(width, height)

Legacy data owner record; current typed Table declarations retain their independent public tier; original observation: Bounded pixels and command storage initialize atomically

```text
Identity: owner:G018:Canvas.create(width, height) (DOMAIN_CONTRACT_ID)
Edition: 1
...
Owner: runtime/canvas/canvas.asm::nebo_canvas_create
```
