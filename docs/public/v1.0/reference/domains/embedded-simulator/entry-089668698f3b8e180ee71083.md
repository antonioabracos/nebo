# embedded-simulator — Spi.open(config,capability)

Simulator only; no physical board, interrupt latency or real-time certification; original observation: SPI bus is bound to its own transfer authority; SDK bounds: {"MAX_IMAGE_BYTES":262144,"MAX_TASKS":32,"MAX_TRACE_EVENTS":4096,"MAX_TRANSFER_BYTES":4096}

```text
Identity: owner:G038:G038-S04-05 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Spi.open(config: Mapping[str, Any], capability: Capability) -> Bus
```
