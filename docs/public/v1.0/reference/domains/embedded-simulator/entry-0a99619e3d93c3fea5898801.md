# embedded-simulator — interrupt.enable()

Simulator only; no physical board, interrupt latency or real-time certification; original observation: vector becomes injectable and trace records enable; SDK bounds: {"MAX_IMAGE_BYTES":262144,"MAX_TASKS":32,"MAX_TRACE_EVENTS":4096,"MAX_TRANSFER_BYTES":4096}

```text
Identity: owner:G038:G038-S03-02 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
InterruptBinding.enable(self) -> 'InterruptBinding'
```
...
[
  ...
  {
    "excerpt": "89: ok(\"positive\", \"Interrupt.bind\", binding.vector == 5 and not binding.enabled)\n90: ok(\"positive\", \"interrupt.enable\", binding.enable() is binding and binding.enabled)\n91: ok(\"positive\", \"interrupt.priority\", binding.priority(7) is binding and binding.priority_level == 7)\n92: ok(\"positive\", \"interrupt.latencyBudget\",\n93:    binding.latencyBudget(250) is binding and binding.budget_ns == 250)",
