# embedded-simulator — interrupt.priority(level)

Simulator only; no physical board, interrupt latency or real-time certification; original observation: bounded priority seven retained; SDK bounds: {"MAX_IMAGE_BYTES":262144,"MAX_TASKS":32,"MAX_TRACE_EVENTS":4096,"MAX_TRANSFER_BYTES":4096}

```text
Identity: owner:G038:G038-S03-04 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
InterruptBinding.priority(self, level: int) -> 'InterruptBinding'
RealTimeTask.priority(self, level: int) -> 'RealTimeTask'
```
