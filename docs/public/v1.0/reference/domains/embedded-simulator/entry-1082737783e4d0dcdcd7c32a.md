# embedded-simulator — Exception.handler(kind,function)

Simulator only; no physical board, interrupt latency or real-time certification; original observation: fault handler produces independently checked value five; SDK bounds: {"MAX_IMAGE_BYTES":262144,"MAX_TASKS":32,"MAX_TRACE_EVENTS":4096,"MAX_TRANSFER_BYTES":4096}

```text
Identity: owner:G038:G038-S03-07 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Exception.handler(self, kind: str, function: Callable[..., Any]) -> ExceptionBinding
```
...
[
  ...
  {
    "excerpt": "100: ok(\"positive\", \"CriticalSection.run\", critical_state == (False, 73) and simulator.interrupts_enabled)\n101: exception_binding = FirmwareException(simulator).handler(\"fault\", lambda code: code + 1)\n102: ok(\"positive\", \"Exception.handler\", exception_binding.function(4) == 5)\n103: transcript.append((observed_irqs, tuple(dict(event) for event in simulator.trace()[-8:])))\n104: ",
