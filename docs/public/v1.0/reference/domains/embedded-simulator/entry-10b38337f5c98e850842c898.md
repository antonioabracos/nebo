# embedded-simulator — Timer.periodic(period,handler)

Simulator only; no physical board, interrupt latency or real-time certification; original observation: period-four handler fires twice in ten logical units; SDK bounds: {"MAX_IMAGE_BYTES":262144,"MAX_TASKS":32,"MAX_TRACE_EVENTS":4096,"MAX_TRANSFER_BYTES":4096}

```text
Identity: owner:G038:G038-S05-05 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Timer.periodic(self, period: int, handler: Callable[[], Any]) -> TimerHandle
```
...
[
  ...
  {
    "excerpt": "139: ok(\"positive\", \"Timer.after\", one_shot.due_ns == simulator.time_ns + 10)\n140: periodic = timer.periodic(4, lambda: timer_events.append(\"periodic\"))\n141: ok(\"positive\", \"Timer.periodic\", periodic.period_ns == 4)\n142: simulator.advance(10)\n143: ok(\"composition\", \"timer-execution\", timer_events == [\"periodic\", \"periodic\", \"once\"])",
