# embedded-simulator — Pin.output(id,options)

Simulator only; no physical board, interrupt latency or real-time certification; original observation: GPIO pin three configured output under capability; SDK bounds: {"MAX_IMAGE_BYTES":262144,"MAX_TASKS":32,"MAX_TRACE_EVENTS":4096,"MAX_TRANSFER_BYTES":4096}

```text
Identity: owner:G038:G038-S04-01 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Pin.output(id: int, options: Mapping[str, Any]) -> PinHandle
```
...
[
  ...
  {
    "excerpt": "106: gpio_cap = Capability.new(simulator, \"gpio\", 3_145_728, 4096, (\"configure\", \"read\", \"write\"))\n107: pin_out = Pin.output(3, {\"simulator\": simulator, \"capability\": gpio_cap})\n108: ok(\"positive\", \"Pin.output\", pin_out.direction == \"output\")\n109: pin_in = Pin.input(4, {\"simulator\": simulator, \"capability\": gpio_cap})\n110: ok(\"positive\", \"Pin.input\", pin_in.direction == \"input\")",
