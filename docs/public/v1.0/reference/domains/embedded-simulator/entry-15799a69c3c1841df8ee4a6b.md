# embedded-simulator — DmaChannel.open(id,capability)

Simulator only; no physical board, interrupt latency or real-time certification; original observation: channel two owns explicit DMA authority; SDK bounds: {"MAX_IMAGE_BYTES":262144,"MAX_TASKS":32,"MAX_TRACE_EVENTS":4096,"MAX_TRANSFER_BYTES":4096}

```text
Identity: owner:G038:G038-S05-01 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
DmaChannel.open(id: int, capability: Capability) -> 'DmaChannel'
```
