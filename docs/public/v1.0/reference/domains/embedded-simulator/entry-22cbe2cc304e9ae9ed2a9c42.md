# embedded-simulator — Pin.input(id,options)

Simulator only; no physical board, interrupt latency or real-time certification; original observation: GPIO pin four configured input under capability; SDK bounds: {"MAX_IMAGE_BYTES":262144,"MAX_TASKS":32,"MAX_TRACE_EVENTS":4096,"MAX_TRANSFER_BYTES":4096}

```text
Identity: owner:G038:G038-S04-02 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Pin.input(id: int, options: Mapping[str, Any]) -> PinHandle
```
...
[
  ...
  {
    "excerpt": "108: ok(\"positive\", \"Pin.output\", pin_out.direction == \"output\")\n109: pin_in = Pin.input(4, {\"simulator\": simulator, \"capability\": gpio_cap})\n110: ok(\"positive\", \"Pin.input\", pin_in.direction == \"input\")\n111: pin_out.write(True)\n112: ok(\"positive\", \"pin.write\", simulator._pins[3] is True)",
