# embedded-simulator — firmware.update(slot,image,policy)

Simulator only; no physical board, interrupt latency or real-time certification; original observation: hashed candidate commits atomically to inactive B slot; SDK bounds: {"MAX_IMAGE_BYTES":262144,"MAX_TASKS":32,"MAX_TRACE_EVENTS":4096,"MAX_TRANSFER_BYTES":4096}

```text
Identity: owner:G038:G038-S07-03 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
FirmwareState.update(self, slot: str, image: bytes, policy: Mapping[str, Any]) -> Mapping[str, Any]
```
...
[
  ...
  {
    "excerpt": "180:     before = (dict(manifest.slots), manifest.active_slot, manifest.rollback_counter)\n181:     cut = manifest.update(\"B\", candidate, {\"version\": 2, \"powerCutAfter\": power_cut})\n182:     ok(\"failure_atomicity\", f\"power-cut-{power_cut}\", not cut[\"committed\"]\n183:        and before == (dict(manifest.slots), manifest.active_slot, manifest.rollback_counter))\n184: updated = manifest.update(\"B\", candidate, {\"version\": 2})",
