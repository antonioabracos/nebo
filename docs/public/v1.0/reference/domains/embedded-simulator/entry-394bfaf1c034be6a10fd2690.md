# embedded-simulator — interrupt.acknowledge()

Simulator only; no physical board, interrupt latency or real-time certification; original observation: pending vector is cleared and acknowledged in trace; SDK bounds: {"MAX_IMAGE_BYTES":262144,"MAX_TASKS":32,"MAX_TRACE_EVENTS":4096,"MAX_TRANSFER_BYTES":4096}

```text
Identity: owner:G038:G038-S03-05 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
InterruptBinding.acknowledge(self) -> None
```
