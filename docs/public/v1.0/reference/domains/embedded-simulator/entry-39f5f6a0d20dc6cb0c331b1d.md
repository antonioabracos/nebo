# embedded-simulator — scheduler.run()

Simulator only; no physical board, interrupt latency or real-time certification; original observation: admitted fixed-priority order returns 11 then 22; SDK bounds: {"MAX_IMAGE_BYTES":262144,"MAX_TASKS":32,"MAX_TRACE_EVENTS":4096,"MAX_TRANSFER_BYTES":4096}

```text
Identity: owner:G038:G038-S06-06 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
CriticalSection.run(self, callable: Callable[[], T]) -> T
Scheduler.run(self) -> tuple[Any, ...]
```
...
[
  ...
  {
    "excerpt": "98: ok(\"positive\", \"interrupt.disable\", binding.disable() is binding and not binding.enabled)\n99: critical_state = CriticalSection(simulator).run(lambda: (simulator.interrupts_enabled, 73))\n100: ok(\"positive\", \"CriticalSection.run\", critical_state == (False, 73) and simulator.interrupts_enabled)\n101: exception_binding = FirmwareException(simulator).handler(\"fault\", lambda code: code + 1)\n102: ok(\"positive\", \"Exception.handler\", exception_binding.function(4) == 5)",
...
- runtime/embedded/realtime_admission.asm — SHA-256 f153150d273a3ffa4fea378447ed03401d143544342652331d97928d18650480
