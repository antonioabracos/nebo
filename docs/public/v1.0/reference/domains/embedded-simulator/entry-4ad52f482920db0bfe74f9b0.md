# embedded-simulator — timer.cancel()

Simulator only; no physical board, interrupt latency or real-time certification; original observation: active periodic handle transitions once to cancelled; SDK bounds: {"MAX_IMAGE_BYTES":262144,"MAX_TASKS":32,"MAX_TRACE_EVENTS":4096,"MAX_TRANSFER_BYTES":4096}

```text
Identity: owner:G038:G038-S05-06 (DOMAIN_CONTRACT_ID)
Edition: 1
...
[
  ...
  {
    "excerpt": "143: ok(\"composition\", \"timer-execution\", timer_events == [\"periodic\", \"periodic\", \"once\"])\n144: ok(\"positive\", \"timer.cancel\", periodic.cancel() and periodic.cancelled)\n145: watchdog = Watchdog(simulator).start(12)\n146: ok(\"positive\", \"Watchdog.start\", watchdog.timeout_ns == 12 and watchdog.resets == 0)\n147: first_deadline = watchdog.feed()",
