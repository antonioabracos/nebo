# embedded-simulator — RealTimeScheduler.edf(tasks)

Simulator only; no physical board, interrupt latency or real-time certification; original observation: explicit EDF policy owns immutable task tuple; SDK bounds: {"MAX_IMAGE_BYTES":262144,"MAX_TASKS":32,"MAX_TRACE_EVENTS":4096,"MAX_TRANSFER_BYTES":4096}

```text
Identity: owner:G038:G038-S06-05 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
RealTimeScheduler.edf(tasks: Iterable[RealTimeTask]) -> Scheduler
```
...
[
  ...
  {
    "excerpt": "160: ok(\"positive\", \"RealTimeScheduler.fixedPriority\", fixed.policy == \"fixed-priority\")\n161: edf = RealTimeScheduler.edf((slow, fast))\n162: ok(\"positive\", \"RealTimeScheduler.edf\", edf.policy == \"edf\")\n163: ok(\"positive\", \"scheduler.admissionTest\",\n164:    fixed.admissionTest()[\"admitted\"] and fixed.admissionTest()[\"utilization\"] == 0.4)",
