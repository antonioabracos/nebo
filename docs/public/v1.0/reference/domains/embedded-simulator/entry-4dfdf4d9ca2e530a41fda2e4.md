# embedded-simulator — HardwareSimulator.new(boardSpec)

Simulator only; no physical board, interrupt latency or real-time certification; original observation: new isolated simulator owns reference board and logical clock; SDK bounds: {"MAX_IMAGE_BYTES":262144,"MAX_TASKS":32,"MAX_TRACE_EVENTS":4096,"MAX_TRANSFER_BYTES":4096}

```text
Identity: owner:G038:G038-S07-05 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
HardwareSimulator.new(boardSpec: Board | Mapping[str, Any]) -> 'HardwareSimulator'
```
