# embedded-simulator — Register.at(address,capability)

Simulator only; no physical board, interrupt latency or real-time certification; original observation: aligned 32-bit register is bound to explicit MMIO authority; SDK bounds: {"MAX_IMAGE_BYTES":262144,"MAX_TASKS":32,"MAX_TRACE_EVENTS":4096,"MAX_TRANSFER_BYTES":4096}

```text
Identity: owner:G038:G038-S02-01 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Register.at(address: int, capability: Capability, bits: int=32) -> 'Register[int]'
```
...
[
  ...
  {
    "excerpt": "72: ok(\"positive\", \"MemoryMapped.region\", region.base == 3_145_728 and region.length == 4096)\n73: register = Register.at(3_145_728, mmio_cap)\n74: ok(\"positive\", \"Register.at\", register.address == 3_145_728 and register.bits == 32)\n75: ok(\"positive\", \"register.read\", register.read() == 0)\n76: register.write(0x12)",
