# embedded-simulator — Firmware.linkerScript()

Simulator only; no physical board, interrupt latency or real-time certification; original observation: ROM 0x100000 and RAM 0x200000 emitted deterministically; SDK bounds: {"MAX_IMAGE_BYTES":262144,"MAX_TASKS":32,"MAX_TRACE_EVENTS":4096,"MAX_TRANSFER_BYTES":4096}

```text
Identity: owner:G038:G038-S01-05 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Firmware.linkerScript(self) -> str
```
...
[
  ...
  {
    "excerpt": "55:    firmware.vectorTable((\"board_reset\", \"timer_irq\", \"uart_irq\")) is firmware)\n56: linker = firmware.linkerScript()\n57: ok(\"positive\", \"Firmware.linkerScript\",\n58:    \"ORIGIN=0x100000\" in linker and \"ORIGIN=0x200000\" in linker)\n59: image = firmware.image(\"raw-bin\")",
