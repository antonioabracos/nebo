# embedded-simulator — mmio.fence(order)

Simulator only; no physical board, interrupt latency or real-time certification; original observation: seq-cst fence is ordered after volatile register effects; SDK bounds: {"MAX_IMAGE_BYTES":262144,"MAX_TASKS":32,"MAX_TRACE_EVENTS":4096,"MAX_TRANSFER_BYTES":4096}

```text
Identity: owner:G038:G038-S02-08 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
MemoryRegion.fence(self, order: str) -> int
```
