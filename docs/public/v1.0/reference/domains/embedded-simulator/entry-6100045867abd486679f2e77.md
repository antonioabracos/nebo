# embedded-simulator — Interrupt.bind(vector,handler)

Simulator only; no physical board, interrupt latency or real-time certification; original observation: vector five binds once under interrupt capability; SDK bounds: {"MAX_IMAGE_BYTES":262144,"MAX_TASKS":32,"MAX_TRACE_EVENTS":4096,"MAX_TRANSFER_BYTES":4096}

```text
Identity: owner:G038:G038-S03-01 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Interrupt.bind(self, vector: int, handler: Callable[[], Any]) -> InterruptBinding
```
...
[
  ...
  {
    "excerpt": "85: # S03 — interrupt state, acknowledgement, critical restoration and handlers.\n86: irq_cap = Capability.new(simulator, \"interrupt\", 0, board.vectors, (\"bind\", \"control\"))\n87: observed_irqs: list[str] = []\n88: binding = Interrupt(simulator, irq_cap).bind(5, lambda: observed_irqs.append(\"irq5\") or 38)\n89: ok(\"positive\", \"Interrupt.bind\", binding.vector == 5 and not binding.enabled)",
