# embedded-simulator — MemoryMapped.region(base,length,capability)

Simulator only; no physical board, interrupt latency or real-time certification; original observation: 4 KiB region is bounded by read capability; SDK bounds: {"MAX_IMAGE_BYTES":262144,"MAX_TASKS":32,"MAX_TRACE_EVENTS":4096,"MAX_TRANSFER_BYTES":4096}

```text
Identity: owner:G038:G038-S02-07 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
MemoryMapped.region(base: int, length: int, capability: Capability) -> MemoryRegion
```
