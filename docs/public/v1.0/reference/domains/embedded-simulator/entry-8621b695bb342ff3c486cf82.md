# embedded-simulator — Timer.after(duration,handler)

Simulator only; no physical board, interrupt latency or real-time certification; original observation: one-shot due time is derived from logical clock; SDK bounds: {"MAX_IMAGE_BYTES":262144,"MAX_TASKS":32,"MAX_TRACE_EVENTS":4096,"MAX_TRANSFER_BYTES":4096}

```text
Identity: owner:G038:G038-S05-04 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Timer.after(self, duration: int, handler: Callable[[], Any]) -> TimerHandle
```
...
[
  ...
  {
    "excerpt": "137: timer = Timer(simulator)\n138: one_shot = timer.after(10, lambda: timer_events.append(\"once\"))\n139: ok(\"positive\", \"Timer.after\", one_shot.due_ns == simulator.time_ns + 10)\n140: periodic = timer.periodic(4, lambda: timer_events.append(\"periodic\"))\n141: ok(\"positive\", \"Timer.periodic\", periodic.period_ns == 4)",
