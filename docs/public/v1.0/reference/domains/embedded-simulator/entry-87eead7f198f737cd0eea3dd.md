# embedded-simulator — watchdog.feed()

Simulator only; no physical board, interrupt latency or real-time certification; original observation: feed advances deadline by exactly twelve logical units; SDK bounds: {"MAX_IMAGE_BYTES":262144,"MAX_TASKS":32,"MAX_TRACE_EVENTS":4096,"MAX_TRANSFER_BYTES":4096}

```text
Identity: owner:G038:G038-S05-08 (DOMAIN_CONTRACT_ID)
Edition: 1
...
[
  ...
  {
    "excerpt": "146: ok(\"positive\", \"Watchdog.start\", watchdog.timeout_ns == 12 and watchdog.resets == 0)\n147: first_deadline = watchdog.feed()\n148: ok(\"positive\", \"watchdog.feed\", first_deadline == simulator.time_ns + 12)\n149: transcript.append((bytes(destination).hex(), timer_events, watchdog.deadline_ns))\n150: ",
