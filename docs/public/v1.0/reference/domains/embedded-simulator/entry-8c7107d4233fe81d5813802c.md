# embedded-simulator — Target.bareMetal(triple,board)

Simulator only; no physical board, interrupt latency or real-time certification; original observation: explicit x86_64 target with simulator-only maturity; SDK bounds: {"MAX_IMAGE_BYTES":262144,"MAX_TASKS":32,"MAX_TRACE_EVENTS":4096,"MAX_TRANSFER_BYTES":4096}

```text
Identity: owner:G038:G038-S01-01 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Target.bareMetal(triple: str, board: Board) -> 'Target'
```
...
[
  ...
  {
    "excerpt": "49: ok(\"positive\", \"Board.define\", board.id == BOARD_ID and board.vectors == 16)\n50: target = Target.bareMetal(TARGET_TRIPLE, board)\n51: ok(\"positive\", \"Target.bareMetal\", target.maturity == \"SIMULATOR_ONLY\" and target.abi.endswith(\"V1\"))\n52: firmware = Firmware(target)\n53: ok(\"positive\", \"Firmware.entry\", firmware.entry(\"board_reset\") is firmware)",
