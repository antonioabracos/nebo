# embedded-simulator — scheduler.deadlineReport()

Simulator only; no physical board, interrupt latency or real-time certification; original observation: report says simulator-only hardware-unmeasured and no hard RT claim; SDK bounds: {"MAX_IMAGE_BYTES":262144,"MAX_TASKS":32,"MAX_TRACE_EVENTS":4096,"MAX_TRANSFER_BYTES":4096}

```text
Identity: owner:G038:G038-S06-07 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Scheduler.deadlineReport(self) -> Mapping[str, Any]
```
...
[
  ...
  {
    "excerpt": "165: ok(\"positive\", \"scheduler.run\", fixed.run() == (11, 22) and executed == [\"fast\", \"slow\"])\n166: report = fixed.deadlineReport()\n167: ok(\"positive\", \"scheduler.deadlineReport\",\n168:    report[\"environment\"] == \"SIMULATOR_ONLY\" and not report[\"hardwareMeasured\"]\n169:    and not report[\"hardRealTimeClaim\"])",
