# embedded-simulator — simulator.trace()

Simulator only; no physical board, interrupt latency or real-time certification; original observation: immutable ordered enable enter exit trace is published; SDK bounds: {"MAX_IMAGE_BYTES":262144,"MAX_TASKS":32,"MAX_TRACE_EVENTS":4096,"MAX_TRANSFER_BYTES":4096}

```text
Identity: owner:G038:G038-S07-07 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
HardwareSimulator.trace(self) -> tuple[Mapping[str, Any], ...]
```
