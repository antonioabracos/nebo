# embedded-simulator — firmware.verify()

Simulator only; no physical board, interrupt latency or real-time certification; original observation: manifest verified with explicit simulator-only and secure-boot-false claims; SDK bounds: {"MAX_IMAGE_BYTES":262144,"MAX_TASKS":32,"MAX_TRACE_EVENTS":4096,"MAX_TRANSFER_BYTES":4096}

```text
Identity: owner:G038:G038-S07-02 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
FirmwareState.verify(self) -> Mapping[str, Any]
```
...
[
  ...
  {
    "excerpt": "176: ok(\"positive\", \"FirmwareManifest.new\", manifest.version == 1 and manifest.active_slot == \"A\")\n177: verification = manifest.verify()\n178: ok(\"positive\", \"firmware.verify\", verification[\"verified\"] and not verification[\"secureBoot\"])\n179: for power_cut in range(4):\n180:     before = (dict(manifest.slots), manifest.active_slot, manifest.rollback_counter)",
