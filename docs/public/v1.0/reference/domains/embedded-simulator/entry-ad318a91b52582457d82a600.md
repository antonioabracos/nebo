# embedded-simulator — register.setBits(mask)

Simulator only; no physical board, interrupt latency or real-time certification; original observation: mask 0x40 produces 0x55; SDK bounds: {"MAX_IMAGE_BYTES":262144,"MAX_TASKS":32,"MAX_TRACE_EVENTS":4096,"MAX_TRANSFER_BYTES":4096}

```text
Identity: owner:G038:G038-S02-05 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Register.setBits(self, mask: int) -> int
```
