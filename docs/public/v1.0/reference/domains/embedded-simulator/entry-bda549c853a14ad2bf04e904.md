# embedded-simulator — task.worstCaseBudget(duration)

Simulator only; no physical board, interrupt latency or real-time certification; original observation: bounded reference budget twenty retained; SDK bounds: {"MAX_IMAGE_BYTES":262144,"MAX_TASKS":32,"MAX_TRACE_EVENTS":4096,"MAX_TRANSFER_BYTES":4096}

```text
Identity: owner:G038:G038-S06-02 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
RealTimeTask.worstCaseBudget(self, duration: int) -> 'RealTimeTask'
```
...
[
  ...
  {
    "excerpt": "154: ok(\"positive\", \"RealTime.task\", fast.period_ns == 100 and fast.deadline_ns == 80)\n155: ok(\"positive\", \"task.worstCaseBudget\", fast.worstCaseBudget(20) is fast and fast.budget_ns == 20)\n156: ok(\"positive\", \"task.priority\", fast.priority(9) is fast and fast.priority_level == 9)\n157: slow = RealTime.task(\"slow\", 200, 150, lambda: executed.append(\"slow\") or 22)\n158: slow.worstCaseBudget(40).priority(3)",
