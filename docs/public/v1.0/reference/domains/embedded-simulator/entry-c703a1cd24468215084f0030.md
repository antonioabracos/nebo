# embedded-simulator — register.modify(transform)

Simulator only; no physical board, interrupt latency or real-time certification; original observation: atomic transform changes 0x12 to 0x15; SDK bounds: {"MAX_IMAGE_BYTES":262144,"MAX_TASKS":32,"MAX_TRACE_EVENTS":4096,"MAX_TRANSFER_BYTES":4096}

```text
Identity: owner:G038:G038-S02-04 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Register.modify(self, transform: Callable[[int], int]) -> int
```
