# embedded-simulator — firmware.memoryMap()

Simulator only; no physical board, interrupt latency or real-time certification; original observation: immutable 256 KiB ROM and RAM map published; SDK bounds: {"MAX_IMAGE_BYTES":262144,"MAX_TASKS":32,"MAX_TRACE_EVENTS":4096,"MAX_TRANSFER_BYTES":4096}

```text
Identity: owner:G038:G038-S01-07 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Firmware.memoryMap(self) -> Mapping[str, Any]
```
...
[
  ...
  {
    "excerpt": "61:    image[:8] == b\"NEBOFW01\" and image[-32:] == hashlib.sha256(image[:-32]).digest())\n62: memory = firmware.memoryMap()\n63: ok(\"positive\", \"firmware.memoryMap\",\n64:    memory[\"rom\"][\"bytes\"] == 262_144 and memory[\"ram\"][\"origin\"] == 2_097_152)\n65: transcript.append((board.id, target.triple, linker, image.hex(), dict(memory)))",
