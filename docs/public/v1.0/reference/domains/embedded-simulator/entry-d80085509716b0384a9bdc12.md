# embedded-simulator — Firmware.entry(function)

Simulator only; no physical board, interrupt latency or real-time certification; original observation: board_reset entry retained in image metadata; SDK bounds: {"MAX_IMAGE_BYTES":262144,"MAX_TASKS":32,"MAX_TRACE_EVENTS":4096,"MAX_TRANSFER_BYTES":4096}

```text
Identity: owner:G038:G038-S01-03 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Firmware.entry(self, function: str | Callable[..., Any]) -> 'Firmware'
```
