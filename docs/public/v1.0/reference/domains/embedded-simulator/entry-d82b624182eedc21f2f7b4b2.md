# embedded-simulator — RealTimeScheduler.fixedPriority(tasks)

Simulator only; no physical board, interrupt latency or real-time certification; original observation: explicit fixed-priority policy owns immutable task tuple; SDK bounds: {"MAX_IMAGE_BYTES":262144,"MAX_TASKS":32,"MAX_TRACE_EVENTS":4096,"MAX_TRANSFER_BYTES":4096}

```text
Identity: owner:G038:G038-S06-04 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
RealTimeScheduler.fixedPriority(tasks: Iterable[RealTimeTask]) -> Scheduler
```
...
[
  ...
  {
    "excerpt": "158: slow.worstCaseBudget(40).priority(3)\n159: fixed = RealTimeScheduler.fixedPriority((slow, fast))\n160: ok(\"positive\", \"RealTimeScheduler.fixedPriority\", fixed.policy == \"fixed-priority\")\n161: edf = RealTimeScheduler.edf((slow, fast))\n162: ok(\"positive\", \"RealTimeScheduler.edf\", edf.policy == \"edf\")",
