# embedded-simulator — RealTime.task(name,period,deadline,function)

Simulator only; no physical board, interrupt latency or real-time certification; original observation: fast task retains period 100 and deadline 80; SDK bounds: {"MAX_IMAGE_BYTES":262144,"MAX_TASKS":32,"MAX_TRACE_EVENTS":4096,"MAX_TRANSFER_BYTES":4096}

```text
Identity: owner:G038:G038-S06-01 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
RealTime.task(name: str, period: int, deadline: int, function: Callable[[], Any]) -> RealTimeTask
```
...
[
  ...
  {
    "excerpt": "152: executed: list[str] = []\n153: fast = RealTime.task(\"fast\", 100, 80, lambda: executed.append(\"fast\") or 11)\n154: ok(\"positive\", \"RealTime.task\", fast.period_ns == 100 and fast.deadline_ns == 80)\n155: ok(\"positive\", \"task.worstCaseBudget\", fast.worstCaseBudget(20) is fast and fast.budget_ns == 20)\n156: ok(\"positive\", \"task.priority\", fast.priority(9) is fast and fast.priority_level == 9)",
