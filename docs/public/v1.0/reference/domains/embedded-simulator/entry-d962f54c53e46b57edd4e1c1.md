# embedded-simulator — firmware.rollback()

Simulator only; no physical board, interrupt latency or real-time certification; original observation: previous bootable A slot becomes active again; SDK bounds: {"MAX_IMAGE_BYTES":262144,"MAX_TASKS":32,"MAX_TRACE_EVENTS":4096,"MAX_TRANSFER_BYTES":4096}

```text
Identity: owner:G038:G038-S07-04 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
FirmwareState.rollback(self) -> str
```
...
[
  ...
  {
    "excerpt": "186:    and manifest.slots[\"B\"] == candidate)\n187: ok(\"positive\", \"firmware.rollback\", manifest.rollback() == \"A\" and manifest.active_slot == \"A\")\n188: second_simulator = HardwareSimulator.new(reference_board_spec())\n189: ok(\"positive\", \"HardwareSimulator.new\", second_simulator.board.id == BOARD_ID)\n190: second_cap = Capability.new(second_simulator, \"interrupt\", 0, 16, (\"bind\",))",
