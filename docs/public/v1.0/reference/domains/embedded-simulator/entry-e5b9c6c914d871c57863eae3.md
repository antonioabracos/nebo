# embedded-simulator — Firmware.vectorTable(entries)

Simulator only; no physical board, interrupt latency or real-time certification; original observation: three named vectors retained within sixteen-vector limit; SDK bounds: {"MAX_IMAGE_BYTES":262144,"MAX_TASKS":32,"MAX_TRACE_EVENTS":4096,"MAX_TRANSFER_BYTES":4096}

```text
Identity: owner:G038:G038-S01-04 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Firmware.vectorTable(self, entries: Iterable[str | Callable[..., Any]]) -> 'Firmware'
```
...
[
  ...
  {
    "excerpt": "53: ok(\"positive\", \"Firmware.entry\", firmware.entry(\"board_reset\") is firmware)\n54: ok(\"positive\", \"Firmware.vectorTable\",\n55:    firmware.vectorTable((\"board_reset\", \"timer_irq\", \"uart_irq\")) is firmware)\n56: linker = firmware.linkerScript()\n57: ok(\"positive\", \"Firmware.linkerScript\",",
