# embedded-simulator — pin.read()

Simulator only; no physical board, interrupt latency or real-time certification; original observation: output high and untouched input low are observed; SDK bounds: {"MAX_IMAGE_BYTES":262144,"MAX_TASKS":32,"MAX_TRACE_EVENTS":4096,"MAX_TRANSFER_BYTES":4096}

```text
Identity: owner:G038:G038-S04-04 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Register.read(self) -> int
PinHandle.read(self) -> bool
```
