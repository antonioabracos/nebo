# embedded-simulator — Board.define(spec)

Simulator only; no physical board, interrupt latency or real-time certification; original observation: reference board validates non-overlapping bounded memory and claims; SDK bounds: {"MAX_IMAGE_BYTES":262144,"MAX_TASKS":32,"MAX_TRACE_EVENTS":4096,"MAX_TRANSFER_BYTES":4096}

```text
Identity: owner:G038:G038-S01-02 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Board.define(spec: Mapping[str, Any]) -> 'Board'
```
...
[
  ...
  {
    "excerpt": "47: spec = reference_board_spec()\n48: board = Board.define(spec)\n49: ok(\"positive\", \"Board.define\", board.id == BOARD_ID and board.vectors == 16)\n50: target = Target.bareMetal(TARGET_TRIPLE, board)\n51: ok(\"positive\", \"Target.bareMetal\", target.maturity == \"SIMULATOR_ONLY\" and target.abi.endswith(\"V1\"))",
