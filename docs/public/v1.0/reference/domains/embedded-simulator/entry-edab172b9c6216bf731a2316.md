# embedded-simulator — Watchdog.start(timeout)

Simulator only; no physical board, interrupt latency or real-time certification; original observation: logical watchdog starts with timeout twelve; SDK bounds: {"MAX_IMAGE_BYTES":262144,"MAX_TASKS":32,"MAX_TRACE_EVENTS":4096,"MAX_TRANSFER_BYTES":4096}

```text
Identity: owner:G038:G038-S05-07 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Watchdog.start(self, timeout: int) -> WatchdogHandle
```
