# embedded-simulator — FirmwareManifest.new(version,target,hashes)

Simulator only; no physical board, interrupt latency or real-time certification; original observation: version-one two-slot manifest validates lowercase SHA-256; SDK bounds: {"MAX_IMAGE_BYTES":262144,"MAX_TASKS":32,"MAX_TRACE_EVENTS":4096,"MAX_TRANSFER_BYTES":4096}

```text
Identity: owner:G038:G038-S07-01 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
FirmwareManifest.new(version: int, target: Target, hashes: Mapping[str, str]) -> FirmwareState
```
...
[
  ...
  {
    "excerpt": "67: # S02 — every volatile read/write remains in the trace and needs a bounded cap.\n68: simulator = HardwareSimulator.new(board)\n69: mmio_cap = Capability.new(simulator, \"mmio\", 3_145_728, 4096,\n70:                           (\"read\", \"write\", \"modify\", \"fence\"))\n71: region = MemoryMapped.region(3_145_728, 4096, mmio_cap)",
...
- runtime/embedded/firmware_update.asm — SHA-256 f1601a238e3c767634bb75fd7410dfd0ed24b43134d350af60c13a2481242d8a
