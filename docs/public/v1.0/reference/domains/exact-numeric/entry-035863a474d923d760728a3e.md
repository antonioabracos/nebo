# exact-numeric — BigDecimal.fromText(text, context)

Native bounded exact-number owners; legacy whole-file source probe is not public typed lowering; original observation: BigDecimal.fromText(text, context)

```text
Identity: owner:G035:G035-S02-01 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
BigDecimal.fromText(text, context)
Owner: G035 source vertical + exact decimal witness
```
