# exact-numeric — unit literal suffix

Native bounded exact-number owners; legacy whole-file source probe is not public typed lowering; original observation: unit literal suffix

```text
Identity: owner:G035:G035-S04-09 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
unit literal suffix
Owner: G035 source vertical contextual suffix
```
