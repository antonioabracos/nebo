# exact-numeric — expression.differentiate(symbol)

Native bounded exact-number owners; legacy whole-file source probe is not public typed lowering; original observation: expression.differentiate(symbol)

```text
Identity: owner:G035:G035-S06-06 (DOMAIN_CONTRACT_ID)
Edition: 1
...
Owner: G035 source vertical + exact dual evaluation
```
