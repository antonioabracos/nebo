# exact-numeric — interval.add(other)

Native bounded exact-number owners; legacy whole-file source probe is not public typed lowering; original observation: interval.add(other)

```text
Identity: owner:G035:G035-S05-06 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
interval.add(other)
Owner: G035 source vertical + nebo_interval_add_f64
```
