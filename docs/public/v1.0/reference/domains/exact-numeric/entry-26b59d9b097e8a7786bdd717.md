# exact-numeric — expression.substitute(symbol, value)

Native bounded exact-number owners; legacy whole-file source probe is not public typed lowering; original observation: expression.substitute(symbol, value)

```text
Identity: owner:G035:G035-S06-05 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
expression.substitute(symbol, value)
