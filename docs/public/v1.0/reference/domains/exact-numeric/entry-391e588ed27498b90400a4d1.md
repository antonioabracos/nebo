# exact-numeric — expression.evaluate(bindings, context)

Native bounded exact-number owners; legacy whole-file source probe is not public typed lowering; original observation: expression.evaluate(bindings, context)

```text
Identity: owner:G035:G035-S06-09 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
expression.evaluate(bindings, context)
Owner: G035 source vertical + nebo_symbolic_eval_dual_i64
```
