# exact-numeric — expression.integrate(symbol, options)

Native bounded exact-number owners; legacy whole-file source probe is not public typed lowering; original observation: expression.integrate(symbol, options)

```text
Identity: owner:G035:G035-S06-07 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
expression.integrate(symbol, options)
Owner: G035 source vertical + exact linear integration witness
```
