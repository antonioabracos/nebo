# exact-numeric — big.subtract(other)

Native bounded exact-number owners; legacy whole-file source probe is not public typed lowering; original observation: big.subtract(other)

```text
Identity: owner:G035:G035-S01-05 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
big.subtract(other)
Owner: G035 source vertical + nebo_g035_exact_sub_i64
```
