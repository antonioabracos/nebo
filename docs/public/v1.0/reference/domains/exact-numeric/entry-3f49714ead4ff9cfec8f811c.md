# exact-numeric — rational.reduce()

Native bounded exact-number owners; legacy whole-file source probe is not public typed lowering; original observation: rational.reduce()

```text
Identity: owner:G035:G035-S02-06 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
rational.reduce()
Owner: G035 source vertical + rational normalization
```
