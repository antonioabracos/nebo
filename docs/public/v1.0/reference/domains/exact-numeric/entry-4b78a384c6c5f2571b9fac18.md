# exact-numeric — interval.refine(predicate)

Native bounded exact-number owners; legacy whole-file source probe is not public typed lowering; original observation: interval.refine(predicate)

```text
Identity: owner:G035:G035-S05-09 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
interval.refine(predicate)
Owner: G035 source vertical + bounded refinement witness
```
