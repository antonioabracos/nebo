# exact-numeric — quantity.multiply(other)

Native bounded exact-number owners; legacy whole-file source probe is not public typed lowering; original observation: quantity.multiply(other)

```text
Identity: owner:G035:G035-S04-07 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
quantity.multiply(other)
Owner: G035 source vertical + nebo_dimension_combine_i8
```
