# exact-numeric — BigInt.zero()

Native bounded exact-number owners; legacy whole-file source probe is not public typed lowering; original observation: BigInt.zero()

```text
Identity: owner:G035:G035-S01-01 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
BigInt.zero()
Owner: G035 source vertical + BigInt core
```
