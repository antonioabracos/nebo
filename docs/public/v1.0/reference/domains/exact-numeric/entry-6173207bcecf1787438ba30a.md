# exact-numeric — big.bitLength()

Native bounded exact-number owners; legacy whole-file source probe is not public typed lowering; original observation: big.bitLength()

```text
Identity: owner:G035:G035-S01-09 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
big.bitLength()
Owner: G035 source vertical + nebo_bigint_bit_length
```
