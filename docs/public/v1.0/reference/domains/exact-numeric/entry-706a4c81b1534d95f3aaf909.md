# exact-numeric — BigInt.parse(text, radix)

Native bounded exact-number owners; legacy whole-file source probe is not public typed lowering; original observation: BigInt.parse(text, radix)

```text
Identity: owner:G035:G035-S01-03 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
BigInt.parse(text, radix)
Owner: G035 source vertical + nebo_g035_bigint_parse_i64
```
