# exact-numeric — Interval.closed(min, max)

Native bounded exact-number owners; legacy whole-file source probe is not public typed lowering; original observation: Interval.closed(min, max)

```text
Identity: owner:G035:G035-S05-01 (DOMAIN_CONTRACT_ID)
Edition: 1
...
Owner: G035 source vertical + interval descriptor
```
