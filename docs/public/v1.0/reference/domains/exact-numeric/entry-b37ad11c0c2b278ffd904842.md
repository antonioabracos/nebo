# exact-numeric — big.add(other)

Native bounded exact-number owners; legacy whole-file source probe is not public typed lowering; original observation: big.add(other)

```text
Identity: owner:G035:G035-S01-04 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
big.add(other)
Owner: G035 source vertical + nebo_bigint_add_abs
```
