# exact-numeric — Unit.define(name, dimension, scale, offset)

Native bounded exact-number owners; legacy whole-file source probe is not public typed lowering; original observation: Unit.define(name, dimension, scale, offset)

```text
Identity: owner:G035:G035-S04-02 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Unit.define(name, dimension, scale, offset)
Owner: G035 source vertical + unit descriptor
```
