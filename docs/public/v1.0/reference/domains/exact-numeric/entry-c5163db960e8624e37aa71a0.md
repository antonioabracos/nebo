# exact-numeric — decimal.quantize(scale, mode)

Native bounded exact-number owners; legacy whole-file source probe is not public typed lowering; original observation: decimal.quantize(scale, mode)

```text
Identity: owner:G035:G035-S02-03 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
decimal.quantize(scale, mode)
Owner: G035 source vertical + nebo_decimal_round_u64
```
