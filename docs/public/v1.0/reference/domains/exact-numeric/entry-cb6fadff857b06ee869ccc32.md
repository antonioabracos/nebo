# exact-numeric — expression.simplify(assumptions)

Native bounded exact-number owners; legacy whole-file source probe is not public typed lowering; original observation: expression.simplify(assumptions)

```text
Identity: owner:G035:G035-S06-04 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
expression.simplify(assumptions)
Owner: G035 source vertical + bounded symbolic classifier
```
