# exact-numeric — interval.width()

Native bounded exact-number owners; legacy whole-file source probe is not public typed lowering; original observation: interval.width()

```text
Identity: owner:G035:G035-S05-08 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
interval.width()
Owner: G035 source vertical + interval width witness
```
