# exact-numeric — complex.magnitude()

Native bounded exact-number owners; legacy whole-file source probe is not public typed lowering; original observation: complex.magnitude()

```text
Identity: owner:G035:G035-S03-05 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
complex.magnitude()
Owner: G035 source vertical + nebo_math_hypot_f64
```
