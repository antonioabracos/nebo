# exact-numeric — quantity.dimension()

Native bounded exact-number owners; legacy whole-file source probe is not public typed lowering; original observation: quantity.dimension()

```text
Identity: owner:G035:G035-S04-05 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
quantity.dimension()
Owner: G035 source vertical + nebo_dimension_equal_i8
```
