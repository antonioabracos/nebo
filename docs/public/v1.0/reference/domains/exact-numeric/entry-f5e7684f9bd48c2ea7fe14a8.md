# exact-numeric — exact.toFloat(rounding)

Native bounded exact-number owners; legacy whole-file source probe is not public typed lowering; original observation: exact.toFloat(rounding)

```text
Identity: owner:G035:G035-S02-08 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
exact.toFloat(rounding)
Owner: G035 source vertical + nebo_exact_i64_to_f64
```
