# exact-numeric — expression.solveFor(symbol, domain)

Native bounded exact-number owners; legacy whole-file source probe is not public typed lowering; original observation: expression.solveFor(symbol, domain)

```text
Identity: owner:G035:G035-S06-08 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
expression.solveFor(symbol, domain)
Owner: G035 source vertical + exact linear solve witness
```
