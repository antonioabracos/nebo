# extensions-ffi — sandbox.load(plugin)

Capability and ABI reference policy; no arbitrary dlopen, external plugin execution or sandbox certification; original observation: load seals policy and initializes only an authorized plugin; SDK bounds: {"MAX_CALLS":4096,"MAX_DURATION":1000000,"MAX_EXPORTS":64,"MAX_MANIFEST_BYTES":65536,"MAX_NODES":16,"MAX_PARTITIONS":1024,"MAX_REPLICAS":3,"MAX_RETRIES":8,"MAX_RETRY_DELAY":1024,"MAX_TASKS":64,"MAX_TRUST_ENTRIES":32}

```text
Identity: owner:G026:G026-S02-02 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
PluginManifest.load(cls, path: str | Path) -> 'PluginManifest'
Plugin.load(cls, path: str | Path, policy: Mapping[str, Any]) -> 'Plugin'
Sandbox.load(self, plugin: Plugin) -> Mapping[str, Any]
```
...
[
  ...
  {
    "excerpt": "123:     path, digest = write_manifest(root, identity)\n124:     return Plugin.load(path, load_policy(root, digest)), path, digest\n125: \n126: \n127: def make_nodes() -> tuple[Any, Any, Cluster]:",
