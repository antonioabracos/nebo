# extensions-ffi — workflow.compensate(step)

Capability and ABI reference policy; no arbitrary dlopen, external plugin execution or sandbox certification; original observation: explicit unique compensations execute in reverse order; SDK bounds: {"MAX_CALLS":4096,"MAX_DURATION":1000000,"MAX_EXPORTS":64,"MAX_MANIFEST_BYTES":65536,"MAX_NODES":16,"MAX_PARTITIONS":1024,"MAX_REPLICAS":3,"MAX_RETRIES":8,"MAX_RETRY_DELAY":1024,"MAX_TASKS":64,"MAX_TRUST_ENTRIES":32}

```text
Identity: owner:G026:G026-S06-06 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Workflow.compensate(self, step: Mapping[str, Any]) -> 'Workflow'
```
...
[
  ...
  {
    "excerpt": "402:     events: list[str] = []\n403:     workflow = Workflow().compensate({\"name\": \"release\", \"action\": lambda: events.append(\"release\")}).compensate({\"name\": \"refund\", \"action\": lambda: events.append(\"refund\")})\n404:     applied = workflow.runCompensations()\n405:     surface(\"G026-S06-06\", applied == (\"refund\", \"release\") and tuple(events) == applied and workflow.runCompensations() == (), applied)\n406:     placement = cluster.rebalance()",
