# extensions-ffi — sandbox.terminate()

Capability and ABI reference policy; no arbitrary dlopen, external plugin execution or sandbox certification; original observation: termination shuts down plugin and remains monotonic; SDK bounds: {"MAX_CALLS":4096,"MAX_DURATION":1000000,"MAX_EXPORTS":64,"MAX_MANIFEST_BYTES":65536,"MAX_NODES":16,"MAX_PARTITIONS":1024,"MAX_REPLICAS":3,"MAX_RETRIES":8,"MAX_RETRY_DELAY":1024,"MAX_TASKS":64,"MAX_TRUST_ENTRIES":32}

```text
Identity: owner:G026:G026-S02-07 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Sandbox.terminate(self) -> Mapping[str, Any]
```
...
[
  ...
  {
    "excerpt": "222:     claim(\"failure_atomicity\", sandbox._calls == call_count, \"rejected call does not consume budget\")\n223:     report = sandbox.terminate()\n224:     surface(\"G026-S02-07\", report[\"state\"] == \"terminated\" and sandbox.terminate() == report, report)\n225:     claim(\"ownership\", loaded[\"state\"] == \"loaded\" and report[\"calls\"] == 3, \"load report is detached from termination\")\n226:     boundary_plugin, _, _ = make_plugin(root, \"boundary-plugin\")",
