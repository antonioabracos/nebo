# extensions-ffi — remote.cancel()

Capability and ABI reference policy; no arbitrary dlopen, external plugin execution or sandbox certification; original observation: pending cancellation is idempotent and releases inflight accounting; SDK bounds: {"MAX_CALLS":4096,"MAX_DURATION":1000000,"MAX_EXPORTS":64,"MAX_MANIFEST_BYTES":65536,"MAX_NODES":16,"MAX_PARTITIONS":1024,"MAX_REPLICAS":3,"MAX_RETRIES":8,"MAX_RETRY_DELAY":1024,"MAX_TASKS":64,"MAX_TRUST_ENTRIES":32}

```text
Identity: owner:G026:G026-S05-04 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
RemoteExecution.cancel(self) -> bool
```
...
[
  ...
  {
    "excerpt": "346:     cancelled = cluster.submit(task, 12, {\"retry\": retry, \"idempotencyKey\": \"job-12\"})\n347:     surface(\"G026-S05-04\", cancelled.cancel() and not cancelled.cancel() and cancelled.state == \"cancelled\", cancelled.state)\n348:     data = DistributedData((\"alpha\", \"beta\", \"gamma\"))\n349:     partitioned = data.partition({\"kind\": \"hash\", \"partitions\": 3})\n350:     surface(\"G026-S05-05\", partitioned.partitions == 3 and len(partitioned.assignment) == 3, partitioned.assignment)",
