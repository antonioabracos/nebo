# extensions-ffi — data.partition(strategy)

Capability and ABI reference policy; no arbitrary dlopen, external plugin execution or sandbox certification; original observation: hash or range strategy and partition count govern assignments; SDK bounds: {"MAX_CALLS":4096,"MAX_DURATION":1000000,"MAX_EXPORTS":64,"MAX_MANIFEST_BYTES":65536,"MAX_NODES":16,"MAX_PARTITIONS":1024,"MAX_REPLICAS":3,"MAX_RETRIES":8,"MAX_RETRY_DELAY":1024,"MAX_TASKS":64,"MAX_TRUST_ENTRIES":32}

```text
Identity: owner:G026:G026-S05-05 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
DistributedData.partition(self, strategy: Mapping[str, Any] | str) -> 'DistributedData'
```
...
[
  ...
  {
    "excerpt": "348:     data = DistributedData((\"alpha\", \"beta\", \"gamma\"))\n349:     partitioned = data.partition({\"kind\": \"hash\", \"partitions\": 3})\n350:     surface(\"G026-S05-05\", partitioned.partitions == 3 and len(partitioned.assignment) == 3, partitioned.assignment)\n351:     replicated = partitioned.replicate(2)\n352:     ranged = data.partition({\"kind\": \"range\", \"partitions\": 3})",
