# extensions-ffi — cluster.health()

Capability and ABI reference policy; no arbitrary dlopen, external plugin execution or sandbox certification; original observation: health derives from actual ready and total node states; SDK bounds: {"MAX_CALLS":4096,"MAX_DURATION":1000000,"MAX_EXPORTS":64,"MAX_MANIFEST_BYTES":65536,"MAX_NODES":16,"MAX_PARTITIONS":1024,"MAX_REPLICAS":3,"MAX_RETRIES":8,"MAX_RETRY_DELAY":1024,"MAX_TASKS":64,"MAX_TRUST_ENTRIES":32}

```text
Identity: owner:G026:G026-S04-05 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Cluster.health(self) -> Mapping[str, Any]
```
