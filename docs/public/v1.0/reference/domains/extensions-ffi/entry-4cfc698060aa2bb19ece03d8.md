# extensions-ffi — remote.await()

Capability and ABI reference policy; no arbitrary dlopen, external plugin execution or sandbox certification; original observation: await executes bounded registered work and validates output type; SDK bounds: {"MAX_CALLS":4096,"MAX_DURATION":1000000,"MAX_EXPORTS":64,"MAX_MANIFEST_BYTES":65536,"MAX_NODES":16,"MAX_PARTITIONS":1024,"MAX_REPLICAS":3,"MAX_RETRIES":8,"MAX_RETRY_DELAY":1024,"MAX_TASKS":64,"MAX_TRUST_ENTRIES":32}

```text
Identity: owner:G026:G026-S05-03 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
RemoteExecution.await_(self) -> Any
```
...
[
  ...
  {
    "excerpt": "309:     draining = drain_node.shutdown()\n310:     drain_result = getattr(drain_remote, \"await\")()\n311:     surface(\"G026-S04-06\", draining[\"state\"] == \"draining\" and drain_result == 8 and drain_node.state == \"shutdown\", (draining, drain_node.state))\n312:     node_b.shutdown()\n313: ",
