# extensions-ffi — NodeRuntime.start(config)

Capability and ABI reference policy; no arbitrary dlopen, external plugin execution or sandbox certification; original observation: local node starts with synthetic identity protocol trust and limits; SDK bounds: {"MAX_CALLS":4096,"MAX_DURATION":1000000,"MAX_EXPORTS":64,"MAX_MANIFEST_BYTES":65536,"MAX_NODES":16,"MAX_PARTITIONS":1024,"MAX_REPLICAS":3,"MAX_RETRIES":8,"MAX_RETRY_DELAY":1024,"MAX_TASKS":64,"MAX_TRUST_ENTRIES":32}

```text
Identity: owner:G026:G026-S04-01 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
NodeRuntime.start(cls, config: Mapping[str, Any]) -> Node
```
...
[
  ...
  {
    "excerpt": "127: def make_nodes() -> tuple[Any, Any, Cluster]:\n128:     node_a = NodeRuntime.start({\"identity\": \"node-a\", \"protocolVersion\": 1, \"authToken\": \"local-trust\", \"maxTasks\": 8, \"capabilities\": (\"compute\", \"checkpoint\")})\n129:     node_b = NodeRuntime.start({\"identity\": \"node-b\", \"protocolVersion\": 1, \"authToken\": \"local-trust\", \"maxTasks\": 8, \"capabilities\": (\"compute\", \"storage\")})\n130:     return node_a, node_b, Cluster.connect((node_a, node_b))\n131: ",
