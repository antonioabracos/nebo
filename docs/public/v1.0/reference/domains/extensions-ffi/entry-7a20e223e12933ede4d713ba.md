# extensions-ffi — plugin.shutdown()

Capability and ABI reference policy; no arbitrary dlopen, external plugin execution or sandbox certification; original observation: shutdown is monotonic idempotent and reports completed calls; SDK bounds: {"MAX_CALLS":4096,"MAX_DURATION":1000000,"MAX_EXPORTS":64,"MAX_MANIFEST_BYTES":65536,"MAX_NODES":16,"MAX_PARTITIONS":1024,"MAX_REPLICAS":3,"MAX_RETRIES":8,"MAX_RETRY_DELAY":1024,"MAX_TASKS":64,"MAX_TRUST_ENTRIES":32}

```text
Identity: owner:G026:G026-S01-06 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Plugin.shutdown(self) -> Mapping[str, Any]
Node.shutdown(self) -> Mapping[str, Any]
```
...
[
  ...
  {
    "excerpt": "163:     surface(\"G026-S01-05\", initialized[\"state\"] == \"ready\" and plugin._initialized, initialized)\n164:     stopped = plugin.shutdown()\n165:     surface(\"G026-S01-06\", stopped[\"state\"] == \"shutdown\" and plugin.shutdown() == stopped, stopped)\n166: \n167:     other_path, other_digest = write_manifest(root, \"arithmetic-v2\")",
