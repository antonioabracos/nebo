# extensions-ffi — data.replicate(factor)

Capability and ABI reference policy; no arbitrary dlopen, external plugin execution or sandbox certification; original observation: replication returns detached bounded data placement; SDK bounds: {"MAX_CALLS":4096,"MAX_DURATION":1000000,"MAX_EXPORTS":64,"MAX_MANIFEST_BYTES":65536,"MAX_NODES":16,"MAX_PARTITIONS":1024,"MAX_REPLICAS":3,"MAX_RETRIES":8,"MAX_RETRY_DELAY":1024,"MAX_TASKS":64,"MAX_TRUST_ENTRIES":32}

```text
Identity: owner:G026:G026-S05-06 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
DistributedData.replicate(self, factor: int) -> 'DistributedData'
```
...
[
  ...
  {
    "excerpt": "350:     surface(\"G026-S05-05\", partitioned.partitions == 3 and len(partitioned.assignment) == 3, partitioned.assignment)\n351:     replicated = partitioned.replicate(2)\n352:     ranged = data.partition({\"kind\": \"range\", \"partitions\": 3})\n353:     ranged_replica = ranged.replicate(2)\n354:     surface(\"G026-S05-06\", replicated.replicas == 2 and partitioned.replicas == 1 and ranged_replica.assignment == ranged.assignment, replicated.replicas)",
