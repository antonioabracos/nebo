# extensions-ffi — library.symbol(name, signature)

Capability and ABI reference policy; no arbitrary dlopen, external plugin execution or sandbox certification; original observation: name argument types return type and capability match exactly; SDK bounds: {"MAX_CALLS":4096,"MAX_DURATION":1000000,"MAX_EXPORTS":64,"MAX_MANIFEST_BYTES":65536,"MAX_NODES":16,"MAX_PARTITIONS":1024,"MAX_REPLICAS":3,"MAX_RETRIES":8,"MAX_RETRY_DELAY":1024,"MAX_TASKS":64,"MAX_TRUST_ENTRIES":32}

```text
Identity: owner:G026:G026-S03-02 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
ForeignLibrary.symbol(self, name: str, signature: Mapping[str, Any]) -> 'ForeignSymbol'
```
...
[
  ...
  {
    "excerpt": "251:     signature = {\"args\": (\"Int\", \"Int\"), \"returns\": \"Int\", \"capability\": \"compute\"}\n252:     symbol = library.symbol(\"add_i64\", signature)\n253:     surface(\"G026-S03-02\", symbol.name == \"add_i64\" and tuple(symbol.signature[\"args\"]) == (\"Int\", \"Int\"), symbol.name)\n254:     value = symbol.call((13, 29))\n255:     surface(\"G026-S03-03\", value == 42, value)",
