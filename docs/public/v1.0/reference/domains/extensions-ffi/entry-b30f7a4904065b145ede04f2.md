# extensions-ffi — remote.provenance()

Capability and ABI reference policy; no arbitrary dlopen, external plugin execution or sandbox certification; original observation: node task schema input key and attempts bind execution lineage; SDK bounds: {"MAX_CALLS":4096,"MAX_DURATION":1000000,"MAX_EXPORTS":64,"MAX_MANIFEST_BYTES":65536,"MAX_NODES":16,"MAX_PARTITIONS":1024,"MAX_REPLICAS":3,"MAX_RETRIES":8,"MAX_RETRY_DELAY":1024,"MAX_TASKS":64,"MAX_TRUST_ENTRIES":32}

```text
Identity: owner:G026:G026-S05-07 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
RemoteExecution.provenance(self) -> Mapping[str, Any]
```
...
[
  ...
  {
    "excerpt": "354:     surface(\"G026-S05-06\", replicated.replicas == 2 and partitioned.replicas == 1 and ranged_replica.assignment == ranged.assignment, replicated.replicas)\n355:     provenance = remote.provenance()\n356:     surface(\"G026-S05-07\", provenance[\"node\"] in {\"node-a\", \"node-b\"} and provenance[\"attempts\"] == 1 and provenance[\"inputDigest\"] == remote.input_digest, provenance)\n357: \n358:     changed = cluster.submit(task, 13, {\"retry\": retry, \"idempotencyKey\": \"job-13\"})",
