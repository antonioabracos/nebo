# extensions-ffi — remote.failure()

Capability and ABI reference policy; no arbitrary dlopen, external plugin execution or sandbox certification; original observation: failure preserves kind retryability attempts and selected node; SDK bounds: {"MAX_CALLS":4096,"MAX_DURATION":1000000,"MAX_EXPORTS":64,"MAX_MANIFEST_BYTES":65536,"MAX_NODES":16,"MAX_PARTITIONS":1024,"MAX_REPLICAS":3,"MAX_RETRIES":8,"MAX_RETRY_DELAY":1024,"MAX_TASKS":64,"MAX_TRUST_ENTRIES":32}

```text
Identity: owner:G026:G026-S06-05 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
RemoteExecution.failure(self) -> RemoteFailure | None
```
...
[
  ...
  {
    "excerpt": "382:     negative(\"REMOTE_FAILED\", \"remote.await\", lambda: getattr(application_remote, \"await\")())\n383:     require(application_remote.failure() is not None and application_remote.failure().kind == \"application\", \"application failure is isolated\")\n384:     negative(\"UNKNOWN_FIELD\", \"cluster.submit\", lambda: cluster.submit(task, 14, {\"retry\": retry, \"codeShipping\": True}))\n385: \n386: ",
