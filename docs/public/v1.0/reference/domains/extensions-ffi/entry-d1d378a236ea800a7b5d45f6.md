# extensions-ffi — ForeignType.layout(spec)

Capability and ABI reference policy; no arbitrary dlopen, external plugin execution or sandbox certification; original observation: alignment offsets size and layout identity derive from field types; SDK bounds: {"MAX_CALLS":4096,"MAX_DURATION":1000000,"MAX_EXPORTS":64,"MAX_MANIFEST_BYTES":65536,"MAX_NODES":16,"MAX_PARTITIONS":1024,"MAX_REPLICAS":3,"MAX_RETRIES":8,"MAX_RETRY_DELAY":1024,"MAX_TASKS":64,"MAX_TRUST_ENTRIES":32}

```text
Identity: owner:G026:G026-S03-04 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
ForeignType.layout(cls, spec: Sequence[Mapping[str, Any]]) -> ForeignLayout
```
...
[
  ...
  {
    "excerpt": "255:     surface(\"G026-S03-03\", value == 42, value)\n256:     layout = ForeignType.layout(({\"name\": \"tag\", \"type\": \"u8\"}, {\"name\": \"value\", \"type\": \"i64\"}))\n257:     surface(\"G026-S03-04\", layout.size == 16 and layout.alignment == 8 and layout.fields[1][\"offset\"] == 8, (layout.size, layout.alignment, layout.identity))\n258:     lifetime = ForeignLifetime(\"borrow-1\")\n259:     buffer = ForeignBuffer.borrow(b\"nebo\", 3, lifetime)",
