# extensions-ffi — Sandbox.new(policy)

Capability and ABI reference policy; no arbitrary dlopen, external plugin execution or sandbox certification; original observation: deny-first local sandbox begins with finite resource budgets; SDK bounds: {"MAX_CALLS":4096,"MAX_DURATION":1000000,"MAX_EXPORTS":64,"MAX_MANIFEST_BYTES":65536,"MAX_NODES":16,"MAX_PARTITIONS":1024,"MAX_REPLICAS":3,"MAX_RETRIES":8,"MAX_RETRY_DELAY":1024,"MAX_TASKS":64,"MAX_TRUST_ENTRIES":32}

```text
Identity: owner:G026:G026-S02-01 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Sandbox.new(cls, policy: Mapping[str, Any]) -> 'Sandbox'
```
...
[
  ...
  {
    "excerpt": "103:         document.update(extra)\n104:     document[\"signature\"] = hmac.new(key, canonical(document), hashlib.sha256).hexdigest()\n105:     raw = canonical(document)\n106:     path = root / f\"{identity}.plugin.json\"\n107:     path.write_bytes(raw)",
