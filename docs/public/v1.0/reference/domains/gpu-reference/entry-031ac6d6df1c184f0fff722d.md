# gpu-reference — memory.pinned(length)

CPU reference only; zero authenticated GPU devices; no accelerator backend; original observation: memory.pinned(length); SDK bounds: {"MAX_BUFFER_BYTES":262144,"MAX_CONTEXT_BYTES":1048576,"MAX_LAUNCH_DIMENSION":65535,"MAX_LAUNCH_WORK_ITEMS":1048576,"MAX_PENDING_COMMANDS":256,"MAX_PINNED_BYTES":262144,"MAX_STREAMS":32,"MAX_TRACE_EVENTS":512}

```text
Identity: owner:G022:G022-S02-07 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
_Memory.pinned(self, length: int) -> PinnedMemory
```
...
[
  ...
  {
    "excerpt": "84:    restored.device() is None)\n85: pinned = memory.pinned(23)\n86: ok(\"positive\", \"memory.pinned\", len(pinned.data) == 23 and not pinned.pinned and\n87:    \"unavailable\" in pinned.fallback)\n88: ",
