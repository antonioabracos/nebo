# gpu-reference — kernel.requiredFeatures()

CPU reference only; zero authenticated GPU devices; no accelerator backend; original observation: kernel.requiredFeatures(); SDK bounds: {"MAX_BUFFER_BYTES":262144,"MAX_CONTEXT_BYTES":1048576,"MAX_LAUNCH_DIMENSION":65535,"MAX_LAUNCH_WORK_ITEMS":1048576,"MAX_PENDING_COMMANDS":256,"MAX_PINNED_BYTES":262144,"MAX_STREAMS":32,"MAX_TRACE_EVENTS":512}

```text
Identity: owner:G022:G022-S03-05 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
GpuKernel.requiredFeatures(self) -> tuple[str, ...]
```
...
[
  ...
  {
    "excerpt": "100: ok(\"positive\", \"kernel.launch\", receipt.grid == (1,) and receipt.block == (2,))\n101: ok(\"positive\", \"kernel.requiredFeatures\", kernel.requiredFeatures() == (\"f64\",))\n102: cache_key = module.cacheKey()\n103: ok(\"positive\", \"module.cacheKey\", len(cache_key) == 64 and\n104:    cache_key == hashlib.sha256(json.dumps(module._manifest, sort_keys=True,",
