# gpu-reference — tensor.toDevice(device)

CPU reference only; zero authenticated GPU devices; no accelerator backend; original observation: tensor.toDevice(device); SDK bounds: {"MAX_BUFFER_BYTES":262144,"MAX_CONTEXT_BYTES":1048576,"MAX_LAUNCH_DIMENSION":65535,"MAX_LAUNCH_WORK_ITEMS":1048576,"MAX_PENDING_COMMANDS":256,"MAX_PINNED_BYTES":262144,"MAX_STREAMS":32,"MAX_TRACE_EVENTS":512}

```text
Identity: owner:G022:G022-S02-05 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
GpuTensor.toDevice(self, device: GpuDevice) -> 'GpuTensor'
```
...
[
  ...
  {
    "excerpt": "79: host_tensor = GpuTensor.fromHost(HostTensor([3.5, -4.25], (2,)))\n80: placed = host_tensor.toDevice(device)\n81: ok(\"positive\", \"tensor.toDevice\", placed.device() is device)\n82: restored = placed.toHost()\n83: ok(\"positive\", \"tensor.toHost\", restored.host.values == (3.5, -4.25) and",
