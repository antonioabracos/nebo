# gpu-reference — buffer.download(hostBuffer)

CPU reference only; zero authenticated GPU devices; no accelerator backend; original observation: buffer.download(hostBuffer); SDK bounds: {"MAX_BUFFER_BYTES":262144,"MAX_CONTEXT_BYTES":1048576,"MAX_LAUNCH_DIMENSION":65535,"MAX_LAUNCH_WORK_ITEMS":1048576,"MAX_PENDING_COMMANDS":256,"MAX_PINNED_BYTES":262144,"MAX_STREAMS":32,"MAX_TRACE_EVENTS":512}

```text
Identity: owner:G022:G022-S02-03 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
DeviceBuffer.download(self, hostBuffer: MutableSequence[int] | memoryview) -> int
```
...
[
  ...
  {
    "excerpt": "48:     target = bytearray(value.nbytes)\n49:     value.download(target)\n50:     return struct.unpack(\"<\" + \"d\" * (len(target) // 8), target)\n51: \n52: ",
