# gpu-reference — kernel.signature()

CPU reference only; zero authenticated GPU devices; no accelerator backend; original observation: kernel.signature(); SDK bounds: {"MAX_BUFFER_BYTES":262144,"MAX_CONTEXT_BYTES":1048576,"MAX_LAUNCH_DIMENSION":65535,"MAX_LAUNCH_WORK_ITEMS":1048576,"MAX_PENDING_COMMANDS":256,"MAX_PINNED_BYTES":262144,"MAX_STREAMS":32,"MAX_TRACE_EVENTS":512}

```text
Identity: owner:G022:G022-S03-02 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
GpuKernel.signature(self) -> tuple[str, ...]
```
...
[
  ...
  {
    "excerpt": "92: ok(\"positive\", \"GpuKernel.load\", isinstance(kernel, GpuKernel))\n93: ok(\"positive\", \"kernel.signature\", kernel.signature() ==\n94:    (\"buffer\", \"buffer\", \"buffer\"))\n95: right.upload(pack((7.75, 6.5)))\n96: bound = kernel.bind((left, right, output))",
