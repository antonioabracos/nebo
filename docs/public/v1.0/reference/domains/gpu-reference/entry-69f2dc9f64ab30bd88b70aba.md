# gpu-reference — buffer.upload(hostBytes)

CPU reference only; zero authenticated GPU devices; no accelerator backend; original observation: buffer.upload(hostBytes); SDK bounds: {"MAX_BUFFER_BYTES":262144,"MAX_CONTEXT_BYTES":1048576,"MAX_LAUNCH_DIMENSION":65535,"MAX_LAUNCH_WORK_ITEMS":1048576,"MAX_PENDING_COMMANDS":256,"MAX_PINNED_BYTES":262144,"MAX_STREAMS":32,"MAX_TRACE_EVENTS":512}

```text
Identity: owner:G022:G022-S02-02 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
DeviceBuffer.upload(self, hostBytes: bytes | bytearray | memoryview) -> int
```
...
[
  ...
  {
    "excerpt": "72: ok(\"positive\", \"DeviceBuffer.allocate\", context.memoryUsed() == 48)\n73: ok(\"positive\", \"buffer.upload\", left.upload(pack((1.25, -2.5))) == 16)\n74: downloaded = bytearray(16)\n75: ok(\"positive\", \"buffer.download\", left.download(downloaded) == 16 and\n76:    bytes(downloaded) == pack((1.25, -2.5)))",
