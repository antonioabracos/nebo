# gpu-reference — planner.fallbackToCpu()

CPU reference only; zero authenticated GPU devices; no accelerator backend; original observation: planner.fallbackToCpu(); SDK bounds: {"MAX_BUFFER_BYTES":262144,"MAX_CONTEXT_BYTES":1048576,"MAX_LAUNCH_DIMENSION":65535,"MAX_LAUNCH_WORK_ITEMS":1048576,"MAX_PENDING_COMMANDS":256,"MAX_PINNED_BYTES":262144,"MAX_STREAMS":32,"MAX_TRACE_EVENTS":512}

```text
Identity: owner:G022:G022-S05-04 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
GpuPlanner.fallbackToCpu(self) -> Any
```
...
[
  ...
  {
    "excerpt": "132: planner.selectDevice(unsupported, context)\n133: ok(\"positive\", \"planner.fallbackToCpu\", planner.fallbackToCpu() == (3.0, 5.0))\n134: session = GpuSession()\n135: ok(\"positive\", \"session.pinDevice\", session.pinDevice(device) is device)\n136: ok(\"positive\", \"operation.supportedOn\", add_operation.supportedOn(device) and",
