# gpu-reference — kernel.bind(arguments)

CPU reference only; zero authenticated GPU devices; no accelerator backend; original observation: kernel.bind(arguments); SDK bounds: {"MAX_BUFFER_BYTES":262144,"MAX_CONTEXT_BYTES":1048576,"MAX_LAUNCH_DIMENSION":65535,"MAX_LAUNCH_WORK_ITEMS":1048576,"MAX_PENDING_COMMANDS":256,"MAX_PINNED_BYTES":262144,"MAX_STREAMS":32,"MAX_TRACE_EVENTS":512}

```text
Identity: owner:G022:G022-S03-03 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
GpuKernel.bind(self, arguments: Sequence[Any]) -> 'GpuKernel'
```
...
[
  ...
  {
    "excerpt": "95: right.upload(pack((7.75, 6.5)))\n96: bound = kernel.bind((left, right, output))\n97: ok(\"positive\", \"kernel.bind\", bound.signature() == kernel.signature())\n98: stream = GpuStream.create(context)\n99: receipt = bound.launch((1,), (2,), stream)",
