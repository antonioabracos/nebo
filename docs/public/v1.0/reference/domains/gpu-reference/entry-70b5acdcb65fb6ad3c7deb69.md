# gpu-reference — stream.recordEvent()

CPU reference only; zero authenticated GPU devices; no accelerator backend; original observation: stream.recordEvent(); SDK bounds: {"MAX_BUFFER_BYTES":262144,"MAX_CONTEXT_BYTES":1048576,"MAX_LAUNCH_DIMENSION":65535,"MAX_LAUNCH_WORK_ITEMS":1048576,"MAX_PENDING_COMMANDS":256,"MAX_PINNED_BYTES":262144,"MAX_STREAMS":32,"MAX_TRACE_EVENTS":512}

```text
Identity: owner:G022:G022-S04-02 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
GpuStream.recordEvent(self) -> GpuEvent
```
...
[
  ...
  {
    "excerpt": "110: ok(\"positive\", \"GpuStream.create\", stream._context is context)\n111: finished = stream.recordEvent()\n112: ok(\"positive\", \"stream.recordEvent\", finished._completed_tick is None)\n113: ok(\"positive\", \"event.wait\", finished.wait() > 0 and unpack(output) == (9.0, 4.0))\n114: dependent = GpuStream.create(context)",
