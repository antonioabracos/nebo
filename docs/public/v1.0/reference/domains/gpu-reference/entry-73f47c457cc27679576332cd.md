# gpu-reference — context.setMemoryBudget(bytes)

CPU reference only; zero authenticated GPU devices; no accelerator backend; original observation: context.setMemoryBudget(bytes); SDK bounds: {"MAX_BUFFER_BYTES":262144,"MAX_CONTEXT_BYTES":1048576,"MAX_LAUNCH_DIMENSION":65535,"MAX_LAUNCH_WORK_ITEMS":1048576,"MAX_PENDING_COMMANDS":256,"MAX_PINNED_BYTES":262144,"MAX_STREAMS":32,"MAX_TRACE_EVENTS":512}

```text
Identity: owner:G022:G022-S06-06 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
GpuContext.setMemoryBudget(self, bytes: int) -> int
```
...
[
  ...
  {
    "excerpt": "157: ok(\"positive\", \"context.memoryUsed\", context.memoryUsed() == 48)\n158: ok(\"positive\", \"context.setMemoryBudget\", context.setMemoryBudget(96) == 96)\n159: trace = json.loads(profiler.exportTrace())\n160: ok(\"positive\", \"profiler.exportTrace\", trace[\"schema\"] ==\n161:    \"nebo-g022-logical-trace-v1\" and trace[\"timing\"].endswith(\"not-benchmark-time\") and",
