# gpu-reference — GpuContext.create(device, options)

CPU reference only; zero authenticated GPU devices; no accelerator backend; original observation: GpuContext.create(device, options); SDK bounds: {"MAX_BUFFER_BYTES":262144,"MAX_CONTEXT_BYTES":1048576,"MAX_LAUNCH_DIMENSION":65535,"MAX_LAUNCH_WORK_ITEMS":1048576,"MAX_PENDING_COMMANDS":256,"MAX_PINNED_BYTES":262144,"MAX_STREAMS":32,"MAX_TRACE_EVENTS":512}

```text
Identity: owner:G022:G022-S01-03 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
GpuContext.create(device: GpuDevice, options: Mapping[str, Any] | None=None) -> 'GpuContext'
```
...
[
  ...
  {
    "excerpt": "60:    properties[\"accelerated\"] is False and properties[\"driverContract\"] == \"none\")\n61: probe = GpuContext.create(device, {\"memoryBudget\": 64})\n62: ok(\"positive\", \"GpuContext.create\", probe.memoryUsed() == 0)\n63: ok(\"positive\", \"context.device\", probe.device() is device)\n64: ok(\"positive\", \"context.synchronize\", probe.synchronize() == 0)",
