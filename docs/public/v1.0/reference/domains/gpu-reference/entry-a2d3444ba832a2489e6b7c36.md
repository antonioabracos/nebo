# gpu-reference — operation.supportedOn(device)

CPU reference only; zero authenticated GPU devices; no accelerator backend; original observation: operation.supportedOn(device); SDK bounds: {"MAX_BUFFER_BYTES":262144,"MAX_CONTEXT_BYTES":1048576,"MAX_LAUNCH_DIMENSION":65535,"MAX_LAUNCH_WORK_ITEMS":1048576,"MAX_PENDING_COMMANDS":256,"MAX_PINNED_BYTES":262144,"MAX_STREAMS":32,"MAX_TRACE_EVENTS":512}

```text
Identity: owner:G022:G022-S05-06 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
GpuOperation.supportedOn(self, device: GpuDevice) -> bool
```
...
[
  ...
  {
    "excerpt": "135: ok(\"positive\", \"session.pinDevice\", session.pinDevice(device) is device)\n136: ok(\"positive\", \"operation.supportedOn\", add_operation.supportedOn(device) and\n137:    not unsupported.supportedOn(device))\n138: \n139: # S06: stable errors, logical profiling, and atomic memory budgets.",
