# gpu-reference — kernel.launch(grid, block, stream)

CPU reference only; zero authenticated GPU devices; no accelerator backend; original observation: kernel.launch(grid, block, stream); SDK bounds: {"MAX_BUFFER_BYTES":262144,"MAX_CONTEXT_BYTES":1048576,"MAX_LAUNCH_DIMENSION":65535,"MAX_LAUNCH_WORK_ITEMS":1048576,"MAX_PENDING_COMMANDS":256,"MAX_PINNED_BYTES":262144,"MAX_STREAMS":32,"MAX_TRACE_EVENTS":512}

```text
Identity: owner:G022:G022-S03-04 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
GpuKernel.launch(self, grid: Sequence[int], block: Sequence[int], stream: 'GpuStream') -> LaunchReceipt
```
...
[
  ...
  {
    "excerpt": "98: stream = GpuStream.create(context)\n99: receipt = bound.launch((1,), (2,), stream)\n100: ok(\"positive\", \"kernel.launch\", receipt.grid == (1,) and receipt.block == (2,))\n101: ok(\"positive\", \"kernel.requiredFeatures\", kernel.requiredFeatures() == (\"f64\",))\n102: cache_key = module.cacheKey()",
