# gpu-reference — buffer.copyTo(other)

CPU reference only; zero authenticated GPU devices; no accelerator backend; original observation: buffer.copyTo(other); SDK bounds: {"MAX_BUFFER_BYTES":262144,"MAX_CONTEXT_BYTES":1048576,"MAX_LAUNCH_DIMENSION":65535,"MAX_LAUNCH_WORK_ITEMS":1048576,"MAX_PENDING_COMMANDS":256,"MAX_PINNED_BYTES":262144,"MAX_STREAMS":32,"MAX_TRACE_EVENTS":512}

```text
Identity: owner:G022:G022-S02-04 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
DeviceBuffer.copyTo(self, other: 'DeviceBuffer') -> int
```
...
[
  ...
  {
    "excerpt": "76:    bytes(downloaded) == pack((1.25, -2.5)))\n77: ok(\"positive\", \"buffer.copyTo\", left.copyTo(right) == 16 and\n78:    unpack(right) == (1.25, -2.5))\n79: host_tensor = GpuTensor.fromHost(HostTensor([3.5, -4.25], (2,)))\n80: placed = host_tensor.toDevice(device)",
