# gpu-reference — planner.transferPlan()

CPU reference only; zero authenticated GPU devices; no accelerator backend; original observation: planner.transferPlan(); SDK bounds: {"MAX_BUFFER_BYTES":262144,"MAX_CONTEXT_BYTES":1048576,"MAX_LAUNCH_DIMENSION":65535,"MAX_LAUNCH_WORK_ITEMS":1048576,"MAX_PENDING_COMMANDS":256,"MAX_PINNED_BYTES":262144,"MAX_STREAMS":32,"MAX_TRACE_EVENTS":512}

```text
Identity: owner:G022:G022-S05-03 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
GpuPlanner.transferPlan(self) -> Mapping[str, Any]
```
...
[
  ...
  {
    "excerpt": "127:    planner.selectDevice(add_operation, context) is device)\n128: plan = planner.transferPlan()\n129: ok(\"positive\", \"planner.transferPlan\", plan[\"backend\"] == \"cpu-reference\" and\n130:    plan[\"deviceTransfers\"] == 0 and plan[\"accelerated\"] is False)\n131: unsupported = GpuOperation(\"fft\", 16, lambda: (3.0, 5.0), False)",
