# gpu-reference — planner.selectDevice(operation, context)

CPU reference only; zero authenticated GPU devices; no accelerator backend; original observation: planner.selectDevice(operation, context); SDK bounds: {"MAX_BUFFER_BYTES":262144,"MAX_CONTEXT_BYTES":1048576,"MAX_LAUNCH_DIMENSION":65535,"MAX_LAUNCH_WORK_ITEMS":1048576,"MAX_PENDING_COMMANDS":256,"MAX_PINNED_BYTES":262144,"MAX_STREAMS":32,"MAX_TRACE_EVENTS":512}

```text
Identity: owner:G022:G022-S05-02 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
GpuPlanner.selectDevice(self, operation: GpuOperation, context: GpuContext) -> GpuDevice | None
```
...
[
  ...
  {
    "excerpt": "125: add_operation = GpuOperation(\"vector-add\", 16, lambda: (9.0, 4.0), True)\n126: ok(\"positive\", \"planner.selectDevice\",\n127:    planner.selectDevice(add_operation, context) is device)\n128: plan = planner.transferPlan()\n129: ok(\"positive\", \"planner.transferPlan\", plan[\"backend\"] == \"cpu-reference\" and",
...
- runtime/gpu/gpu_fallback.asm — SHA-256 c9f1754e196b45a6f89ffa5c7c7a1e06c64aa8d4e0491848c82512ab8855d307
