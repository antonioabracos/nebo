# gpu-reference — context.close()

CPU reference only; zero authenticated GPU devices; no accelerator backend; original observation: context.close(); SDK bounds: {"MAX_BUFFER_BYTES":262144,"MAX_CONTEXT_BYTES":1048576,"MAX_LAUNCH_DIMENSION":65535,"MAX_LAUNCH_WORK_ITEMS":1048576,"MAX_PENDING_COMMANDS":256,"MAX_PINNED_BYTES":262144,"MAX_STREAMS":32,"MAX_TRACE_EVENTS":512}

```text
Identity: owner:G022:G022-S01-06 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
GpuContext.close(self) -> int
DeviceBuffer.close(self) -> int
GpuTensor.close(self) -> int
```
...
[
  ...
  {
    "excerpt": "64: ok(\"positive\", \"context.synchronize\", probe.synchronize() == 0)\n65: ok(\"positive\", \"context.close\", probe.close() == 0)\n66: \n67: # S02: owned host/reference memory and exact byte transfers.\n68: context = GpuContext.create(device, {\"memoryBudget\": 256})",
