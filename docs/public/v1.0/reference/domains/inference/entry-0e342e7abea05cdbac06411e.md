# inference — model.verifyIntegrity()

Bounded CPU model, synthetic local weights only; no downloaded model or GPU claim; original observation: model.verifyIntegrity(); SDK bounds: {"MAX_BATCH":16,"MAX_DIMENSION":64,"MAX_ELEMENTS":4096,"MAX_LAYERS":64,"MAX_MODEL_BYTES":1048576,"MAX_WARMUP":16,"MAX_WORKSPACE_BYTES":1048576}

```text
Identity: owner:G020:G020-S05-07 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Model.verifyIntegrity(self) -> bool
```
...
[
  ...
  {
    "excerpt": "172:    manifest[\"requirements\"] == [\"cpu\", \"scalar-f64\"] and len(manifest[\"tensors\"]) == 6)\n173: ok(\"positive\", \"model.verifyIntegrity\", model.verifyIntegrity())\n174: \n175: # S06: bounded CPU sessions, buffer reuse, warmup, batching, memory, and profile.\n176: session = InferenceSession.new(model, {\"maxBatch\": 4, \"maxWorkspaceBytes\": 4096,",
