# inference — model.exportManifest()

Bounded CPU model, synthetic local weights only; no downloaded model or GPU claim; original observation: model.exportManifest(); SDK bounds: {"MAX_BATCH":16,"MAX_DIMENSION":64,"MAX_ELEMENTS":4096,"MAX_LAYERS":64,"MAX_MODEL_BYTES":1048576,"MAX_WARMUP":16,"MAX_WORKSPACE_BYTES":1048576}

```text
Identity: owner:G020:G020-S05-06 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Model.exportManifest(self) -> dict[str, Any]
```
...
[
  ...
  {
    "excerpt": "169: ok(\"positive\", \"model.loadState\", state_result == {\"missing\": [], \"extra\": [], \"loaded\": 6})\n170: manifest = model.exportManifest()\n171: ok(\"positive\", \"model.exportManifest\", manifest[\"format\"] == \"NMF1\" and\n172:    manifest[\"requirements\"] == [\"cpu\", \"scalar-f64\"] and len(manifest[\"tensors\"]) == 6)\n173: ok(\"positive\", \"model.verifyIntegrity\", model.verifyIntegrity())",
