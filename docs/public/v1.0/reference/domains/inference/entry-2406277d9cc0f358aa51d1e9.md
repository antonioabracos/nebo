...
Bounded CPU model, synthetic local weights only; no downloaded model or GPU claim; original observation: model.eval(); SDK bounds: {"MAX_BATCH":16,"MAX_DIMENSION":64,"MAX_ELEMENTS":4096,"MAX_LAYERS":64,"MAX_MODEL_BYTES":1048576,"MAX_WARMUP":16,"MAX_WORKSPACE_BYTES":1048576}

```text
Identity: owner:G020:G020-S01-04 (DOMAIN_CONTRACT_ID)
Edition: 1
...
[
  ...
  {
    "excerpt": "69: batch_norm = BatchNorm.new(2, 1e-5)\n70: model = Model.new([linear, batch_norm], {}).eval()\n71: \n72: # S01: model construction, state enumeration, inference mode, and validation.\n73: ok(\"positive\", \"Model.new\", isinstance(model, Model) and len(model.graph) == 2)",
