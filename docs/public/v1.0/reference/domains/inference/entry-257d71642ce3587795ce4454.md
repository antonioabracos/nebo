# inference — model.parameterCount()

Bounded CPU model, synthetic local weights only; no downloaded model or GPU claim; original observation: model.parameterCount(); SDK bounds: {"MAX_BATCH":16,"MAX_DIMENSION":64,"MAX_ELEMENTS":4096,"MAX_LAYERS":64,"MAX_MODEL_BYTES":1048576,"MAX_WARMUP":16,"MAX_WORKSPACE_BYTES":1048576}

```text
Identity: owner:G020:G020-S01-06 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Model.parameterCount(self) -> dict[str, int]
```
...
[
  ...
  {
    "excerpt": "81:    summary.parameters == 12 and summary.buffers == 4)\n82: count = model.parameterCount()\n83: ok(\"positive\", \"model.parameterCount\", count == {\"elements\": 12, \"bytes\": 96})\n84: ok(\"positive\", \"model.validate\", model.validate() ==\n85:    {\"valid\": True, \"layers\": 2, \"parameters\": 6})",
