# inference — session.run(inputs)

Bounded CPU model, synthetic local weights only; no downloaded model or GPU claim; original observation: session.run(inputs); SDK bounds: {"MAX_BATCH":16,"MAX_DIMENSION":64,"MAX_ELEMENTS":4096,"MAX_LAYERS":64,"MAX_MODEL_BYTES":1048576,"MAX_WARMUP":16,"MAX_WORKSPACE_BYTES":1048576}

```text
Identity: owner:G020:G020-S06-02 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
InferenceSession.run(self, inputs: Tensor | Mapping[str, Tensor]) -> Tensor
```
...
[
  ...
  {
    "excerpt": "178: ok(\"positive\", \"InferenceSession.new\", session.max_batch == 4 and session.deterministic)\n179: session_result = session.run({\"input\": sample})\n180: ok(\"positive\", \"session.run\", close_values(session_result, model.forward(sample)))\n181: output_buffer = {\"output\": Tensor([0, 0], (2,))}\n182: ok(\"positive\", \"session.runInto\",",
