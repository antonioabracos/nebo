# inference — Activation.relu(input)

Bounded CPU model, synthetic local weights only; no downloaded model or GPU claim; original observation: Activation.relu(input); SDK bounds: {"MAX_BATCH":16,"MAX_DIMENSION":64,"MAX_ELEMENTS":4096,"MAX_LAYERS":64,"MAX_MODEL_BYTES":1048576,"MAX_WARMUP":16,"MAX_WORKSPACE_BYTES":1048576}

```text
Identity: owner:G020:G020-S02-03 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Activation.relu(input: Tensor) -> Tensor
```
...
[
  ...
  {
    "excerpt": "91:    close_values(affine, [2.0, -2.0]))\n92: ok(\"positive\", \"Activation.relu\",\n93:    Activation.relu(Tensor([-2, 0, 3], (3,))).values == (0.0, 0.0, 3.0))\n94: ok(\"positive\", \"Activation.gelu\",\n95:    close_values(Activation.gelu(Tensor([0, 1], (2,)), \"exact\"),",
