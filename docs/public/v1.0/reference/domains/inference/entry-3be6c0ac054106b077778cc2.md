# inference — Activation.tanh(input)

Bounded CPU model, synthetic local weights only; no downloaded model or GPU claim; original observation: Activation.tanh(input); SDK bounds: {"MAX_BATCH":16,"MAX_DIMENSION":64,"MAX_ELEMENTS":4096,"MAX_LAYERS":64,"MAX_MODEL_BYTES":1048576,"MAX_WARMUP":16,"MAX_WORKSPACE_BYTES":1048576}

```text
Identity: owner:G020:G020-S02-06 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Activation.tanh(input: Tensor) -> Tensor
```
...
[
  ...
  {
    "excerpt": "99:    sigmoid.values[1] == 0.5 and sigmoid.values[2] == 1.0)\n100: ok(\"positive\", \"Activation.tanh\",\n101:    close_values(Activation.tanh(Tensor([-1, 0, 1], (3,))),\n102:                 [-math.tanh(1), 0.0, math.tanh(1)]))\n103: softmax = Softmax.forward(Tensor([1000, 1001], (2,)), -1)",
