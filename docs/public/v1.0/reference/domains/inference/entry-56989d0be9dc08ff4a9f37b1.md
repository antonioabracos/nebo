# inference — Activation.sigmoid(input)

Bounded CPU model, synthetic local weights only; no downloaded model or GPU claim; original observation: Activation.sigmoid(input); SDK bounds: {"MAX_BATCH":16,"MAX_DIMENSION":64,"MAX_ELEMENTS":4096,"MAX_LAYERS":64,"MAX_MODEL_BYTES":1048576,"MAX_WARMUP":16,"MAX_WORKSPACE_BYTES":1048576}

```text
Identity: owner:G020:G020-S02-05 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Activation.sigmoid(input: Tensor) -> Tensor
```
...
[
  ...
  {
    "excerpt": "96:                 [0.0, 0.8413447460685429], 1e-12))\n97: sigmoid = Activation.sigmoid(Tensor([-1000, 0, 1000], (3,)))\n98: ok(\"positive\", \"Activation.sigmoid\", sigmoid.values[0] == 0.0 and\n99:    sigmoid.values[1] == 0.5 and sigmoid.values[2] == 1.0)\n100: ok(\"positive\", \"Activation.tanh\",",
