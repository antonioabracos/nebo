# inference — session.runInto(inputs, outputs)

Bounded CPU model, synthetic local weights only; no downloaded model or GPU claim; original observation: session.runInto(inputs, outputs); SDK bounds: {"MAX_BATCH":16,"MAX_DIMENSION":64,"MAX_ELEMENTS":4096,"MAX_LAYERS":64,"MAX_MODEL_BYTES":1048576,"MAX_WARMUP":16,"MAX_WORKSPACE_BYTES":1048576}

```text
Identity: owner:G020:G020-S06-03 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
InferenceSession.runInto(self, inputs: Tensor | Mapping[str, Tensor], outputs: MutableMapping[str, Tensor]) -> MutableMapping[str, Tensor]
```
...
[
  ...
  {
    "excerpt": "181: output_buffer = {\"output\": Tensor([0, 0], (2,))}\n182: ok(\"positive\", \"session.runInto\",\n183:    close_values(session.runInto(sample, output_buffer)[\"output\"], model.forward(sample)))\n184: warmup = session.warmup(sample)\n185: ok(\"positive\", \"session.warmup\", warmup[\"warmups\"] == 1 and len(warmup[\"digest\"]) == 64)",
