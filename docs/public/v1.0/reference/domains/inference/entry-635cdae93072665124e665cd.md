# inference — quantizer.calibrate(dataset)

Bounded CPU model, synthetic local weights only; no downloaded model or GPU claim; original observation: quantizer.calibrate(dataset); SDK bounds: {"MAX_BATCH":16,"MAX_DIMENSION":64,"MAX_ELEMENTS":4096,"MAX_LAYERS":64,"MAX_MODEL_BYTES":1048576,"MAX_WARMUP":16,"MAX_WORKSPACE_BYTES":1048576}

```text
Identity: owner:G020:G020-S07-02 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Quantizer.calibrate(self, dataset: Sequence[Tensor]) -> dict[str, float | int]
```
