# inference — model.loadState(dictionary, strict)

Bounded CPU model, synthetic local weights only; no downloaded model or GPU claim; original observation: model.loadState(dictionary, strict); SDK bounds: {"MAX_BATCH":16,"MAX_DIMENSION":64,"MAX_ELEMENTS":4096,"MAX_LAYERS":64,"MAX_MODEL_BYTES":1048576,"MAX_WARMUP":16,"MAX_WORKSPACE_BYTES":1048576}

```text
Identity: owner:G020:G020-S05-05 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Model.loadState(self, dictionary: Mapping[str, Tensor], strict: bool) -> dict[str, Any]
```
...
[
  ...
  {
    "excerpt": "167: staged = {name: value for name, value in model._parameters.items()}\n168: state_result = model.loadState(staged, True)\n169: ok(\"positive\", \"model.loadState\", state_result == {\"missing\": [], \"extra\": [], \"loaded\": 6})\n170: manifest = model.exportManifest()\n171: ok(\"positive\", \"model.exportManifest\", manifest[\"format\"] == \"NMF1\" and",
