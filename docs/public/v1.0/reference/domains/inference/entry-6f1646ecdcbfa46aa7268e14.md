# inference — Padding2D.new(spec)

Bounded CPU model, synthetic local weights only; no downloaded model or GPU claim; original observation: Padding2D.new(spec); SDK bounds: {"MAX_BATCH":16,"MAX_DIMENSION":64,"MAX_ELEMENTS":4096,"MAX_LAYERS":64,"MAX_MODEL_BYTES":1048576,"MAX_WARMUP":16,"MAX_WORKSPACE_BYTES":1048576}

```text
Identity: owner:G020:G020-S04-06 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Padding2D.new(spec: int | Sequence[int]) -> 'Padding2D'
```
