# inference — model.save(path, options)

Bounded CPU model, synthetic local weights only; no downloaded model or GPU claim; original observation: model.save(path, options); SDK bounds: {"MAX_BATCH":16,"MAX_DIMENSION":64,"MAX_ELEMENTS":4096,"MAX_LAYERS":64,"MAX_MODEL_BYTES":1048576,"MAX_WARMUP":16,"MAX_WORKSPACE_BYTES":1048576}

```text
Identity: owner:G020:G020-S05-03 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Model.save(self, path: str | os.PathLike[str], options: Mapping[str, Any] | None=None) -> int
```
...
[
  ...
  {
    "excerpt": "156: model_path = scratch / \"tiny.nmf\"\n157: saved_bytes = model.save(model_path, {})\n158: raw_model = model_path.read_bytes()\n159: ok(\"positive\", \"model.save\", saved_bytes == len(raw_model) and saved_bytes < 1_048_576)\n160: inspected = ModelFormat.inspect(raw_model)",
