# inference — session.warmup(inputs)

Bounded CPU model, synthetic local weights only; no downloaded model or GPU claim; original observation: session.warmup(inputs); SDK bounds: {"MAX_BATCH":16,"MAX_DIMENSION":64,"MAX_ELEMENTS":4096,"MAX_LAYERS":64,"MAX_MODEL_BYTES":1048576,"MAX_WARMUP":16,"MAX_WORKSPACE_BYTES":1048576}

```text
Identity: owner:G020:G020-S06-04 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
InferenceSession.warmup(self, inputs: Tensor | Mapping[str, Tensor]) -> dict[str, Any]
```
...
[
  ...
  {
    "excerpt": "183:    close_values(session.runInto(sample, output_buffer)[\"output\"], model.forward(sample)))\n184: warmup = session.warmup(sample)\n185: ok(\"positive\", \"session.warmup\", warmup[\"warmups\"] == 1 and len(warmup[\"digest\"]) == 64)\n186: batched = session.batch([sample, Tensor([1, 2, 3], (3,))])\n187: ok(\"positive\", \"session.batch\", len(batched) == 2 and batched[0].shape == (2,))",
