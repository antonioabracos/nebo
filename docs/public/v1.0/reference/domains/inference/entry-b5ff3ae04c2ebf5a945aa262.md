# inference — ModelFormat.inspect(bytes)

Bounded CPU model, synthetic local weights only; no downloaded model or GPU claim; original observation: ModelFormat.inspect(bytes); SDK bounds: {"MAX_BATCH":16,"MAX_DIMENSION":64,"MAX_ELEMENTS":4096,"MAX_LAYERS":64,"MAX_MODEL_BYTES":1048576,"MAX_WARMUP":16,"MAX_WORKSPACE_BYTES":1048576}

```text
Identity: owner:G020:G020-S05-01 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
ModelFormat.inspect(bytes: bytes | bytearray) -> dict[str, Any]
```
...
[
  ...
  {
    "excerpt": "159: ok(\"positive\", \"model.save\", saved_bytes == len(raw_model) and saved_bytes < 1_048_576)\n160: inspected = ModelFormat.inspect(raw_model)\n161: ok(\"positive\", \"ModelFormat.inspect\", inspected[\"magic\"] == \"NMF1\" and\n162:    inspected[\"version\"] == 1 and len(inspected[\"checksum\"]) == 64)\n163: loaded = Model.load(model_path, {\"maxBytes\": 8192})",
