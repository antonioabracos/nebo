# inference — MaxPool2D.new(kernel, stride)

Bounded CPU model, synthetic local weights only; no downloaded model or GPU claim; original observation: MaxPool2D.new(kernel, stride); SDK bounds: {"MAX_BATCH":16,"MAX_DIMENSION":64,"MAX_ELEMENTS":4096,"MAX_LAYERS":64,"MAX_MODEL_BYTES":1048576,"MAX_WARMUP":16,"MAX_WORKSPACE_BYTES":1048576}

```text
Identity: owner:G020:G020-S04-03 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
MaxPool2D.new(kernel: int | Sequence[int], stride: int | Sequence[int]) -> 'MaxPool2D'
```
