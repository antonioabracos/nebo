# inference — session.profile()

Bounded CPU model, synthetic local weights only; no downloaded model or GPU claim; original observation: session.profile(); SDK bounds: {"MAX_BATCH":16,"MAX_DIMENSION":64,"MAX_ELEMENTS":4096,"MAX_LAYERS":64,"MAX_MODEL_BYTES":1048576,"MAX_WARMUP":16,"MAX_WORKSPACE_BYTES":1048576}

```text
Identity: owner:G020:G020-S06-07 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
InferenceSession.profile(self) -> dict[str, Any]
```
...
[
  ...
  {
    "excerpt": "190:    memory.workspace_bytes <= 4096 and memory.peak_bytes > memory.workspace_bytes)\n191: profile = session.profile()\n192: ok(\"positive\", \"session.profile\", profile[\"runs\"] == 4 and\n193:    profile[\"operations\"] == (\"Linear\", \"BatchNorm\") and \"elapsedNs\" in profile)\n194: ",
