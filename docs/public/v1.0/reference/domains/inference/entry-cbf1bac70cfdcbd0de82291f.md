# inference — quantizedModel.requireCpuFeatures()

Bounded CPU model, synthetic local weights only; no downloaded model or GPU claim; original observation: quantizedModel.requireCpuFeatures(); SDK bounds: {"MAX_BATCH":16,"MAX_DIMENSION":64,"MAX_ELEMENTS":4096,"MAX_LAYERS":64,"MAX_MODEL_BYTES":1048576,"MAX_WARMUP":16,"MAX_WORKSPACE_BYTES":1048576}

```text
Identity: owner:G020:G020-S07-07 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
QuantizedModel.requireCpuFeatures(self) -> tuple[str, ...]
```
...
[
  ...
  {
    "excerpt": "210:    0.0 <= accuracy[\"maxAbsoluteError\"] < 0.1)\n211: ok(\"positive\", \"quantizedModel.requireCpuFeatures\",\n212:    quantized_model.requireCpuFeatures() == (\"cpu\", \"scalar-int8\", \"f64-fallback\"))\n213: \n214: # Boundaries include stable activation extremes, minimum tensors, and maximum batching.",
