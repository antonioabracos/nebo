# inference — Linear.new(inputSize, outputSize, bias)

Bounded CPU model, synthetic local weights only; no downloaded model or GPU claim; original observation: Linear.new(inputSize, outputSize, bias); SDK bounds: {"MAX_BATCH":16,"MAX_DIMENSION":64,"MAX_ELEMENTS":4096,"MAX_LAYERS":64,"MAX_MODEL_BYTES":1048576,"MAX_WARMUP":16,"MAX_WORKSPACE_BYTES":1048576}

```text
Identity: owner:G020:G020-S02-01 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Linear.new(inputSize: int, outputSize: int, bias: bool) -> 'Linear'
```
