# knowledge — result.scoreBreakdown()

Bounded local graph and retrieval; no external knowledge source or semantic truth guarantee; original observation: each ranking component and combined score are exposed; SDK bounds: {"MAX_CONTEXT_BYTES":16384,"MAX_DERIVED":4096,"MAX_ENTITIES":512,"MAX_FACTS":4096,"MAX_INDEX_ITEMS":2048,"MAX_PATH_DEPTH":16,"MAX_PROPERTIES":256,"MAX_QUERY_RESULTS":1024,"MAX_ROUNDS":64,"MAX_RULES":256,"MAX_TYPES":128,"MAX_VECTOR_DIMENSION":64}

```text
Identity: owner:G034:G034-S05-07 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
SearchResult.scoreBreakdown(self) -> Mapping[str, float]
```
...
[
  ...
  {
    "excerpt": "323:     surface(\"G034-S05-06\", results[0].identity == \"paris\" and all(result.identity != \"alice\" for result in results), results)\n324:     breakdown = results[0].scoreBreakdown()\n325:     surface(\"G034-S05-07\", set(breakdown) == {\"keyword\", \"semantic\", \"graph\", \"combined\"} and breakdown[\"combined\"] == results[0].score, breakdown)\n326:     result_provenance = results[0].provenance()\n327:     surface(\"G034-S05-08\", result_provenance[\"claim\"] == \"ranking-not-proof\" and result_provenance[\"item\"][\"claim\"] == \"similarity-only\", result_provenance)",
