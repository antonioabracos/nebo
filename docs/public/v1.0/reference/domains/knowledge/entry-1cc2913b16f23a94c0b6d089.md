# knowledge — query.collect(limit)

Bounded local graph and retrieval; no external knowledge source or semantic truth guarantee; original observation: materialization returns stable order only up to explicit limit; SDK bounds: {"MAX_CONTEXT_BYTES":16384,"MAX_DERIVED":4096,"MAX_ENTITIES":512,"MAX_FACTS":4096,"MAX_INDEX_ITEMS":2048,"MAX_PATH_DEPTH":16,"MAX_PROPERTIES":256,"MAX_QUERY_RESULTS":1024,"MAX_ROUNDS":64,"MAX_RULES":256,"MAX_TYPES":128,"MAX_VECTOR_DIMENSION":64}

```text
Identity: owner:G034:G034-S04-08 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
KnowledgeQuery.collect(self, limit: int) -> tuple[Mapping[str, Any], ...]
```
...
[
  ...
  {
    "excerpt": "274:     surface(\"G034-S04-07\", len(streamed) == 1 and stream.request(1) == (), streamed)\n275:     collected = projected.collect(1)\n276:     surface(\"G034-S04-08\", collected == streamed and collected[0][\"$region\"] == \"europe\", collected)\n277: \n278:     changed = matched.where({\"binding\": \"$person\", \"operator\": \"eq\", \"value\": \"bob\"}).collect(4)",
