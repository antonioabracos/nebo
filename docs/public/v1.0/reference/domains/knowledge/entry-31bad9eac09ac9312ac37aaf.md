# knowledge — search.query(text,filters,limit)

Bounded local graph and retrieval; no external knowledge source or semantic truth guarantee; original observation: keywords explicit vector and graph type constraints govern ranking; SDK bounds: {"MAX_CONTEXT_BYTES":16384,"MAX_DERIVED":4096,"MAX_ENTITIES":512,"MAX_FACTS":4096,"MAX_INDEX_ITEMS":2048,"MAX_PATH_DEPTH":16,"MAX_PROPERTIES":256,"MAX_QUERY_RESULTS":1024,"MAX_ROUNDS":64,"MAX_RULES":256,"MAX_TYPES":128,"MAX_VECTOR_DIMENSION":64}

```text
Identity: owner:G034:G034-S05-06 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
HybridSearch.query(self, text: str, filters: Mapping[str, Any], limit: int) -> tuple[SearchResult, ...]
```
...
[
  ...
  {
    "excerpt": "278:     changed = matched.where({\"binding\": \"$person\", \"operator\": \"eq\", \"value\": \"bob\"}).collect(4)\n279:     claim(\"metamorphic\", changed == (), \"filter input governs query result\")\n280:     claim(\"composition\", collected[0][\"$person\"] == \"alice\" and collected[0][\"$place\"] == \"paris\", \"match optional path and projection compose\")\n281:     rows_snapshot = collected\n282:     projected.collect(1)",
