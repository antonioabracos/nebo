# knowledge — graph.factsAbout(entity,limit)

Bounded local graph and retrieval; no external knowledge source or semantic truth guarantee; original observation: incident facts are sorted and explicitly bounded; SDK bounds: {"MAX_CONTEXT_BYTES":16384,"MAX_DERIVED":4096,"MAX_ENTITIES":512,"MAX_FACTS":4096,"MAX_INDEX_ITEMS":2048,"MAX_PATH_DEPTH":16,"MAX_PROPERTIES":256,"MAX_QUERY_RESULTS":1024,"MAX_ROUNDS":64,"MAX_RULES":256,"MAX_TYPES":128,"MAX_VECTOR_DIMENSION":64}

```text
Identity: owner:G034:G034-S01-06 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
KnowledgeGraph.factsAbout(self, entity: Entity, limit: int) -> tuple[Fact, ...]
```
...
[
  ...
  {
    "excerpt": "139:     surface(\"G034-S01-05\", graph.fact(lives.factId) is lives, graph.fact(lives.factId))\n140:     about = graph.factsAbout(alice, 8)\n141:     surface(\"G034-S01-06\", tuple(fact.factId for fact in about) == tuple(sorted((email.factId, lives.factId))), about)\n142:     snapshot = graph.snapshot()\n143:     surface(\"G034-S01-07\", snapshot.version == graph.version and len(snapshot.facts) == 2 and len(snapshot.retractions) == 1, snapshot)",
