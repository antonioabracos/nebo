# knowledge — KnowledgeGraph.new(schema,options)

Bounded local graph and retrieval; no external knowledge source or semantic truth guarantee; original observation: bounded graph receives stable schema-derived identity; SDK bounds: {"MAX_CONTEXT_BYTES":16384,"MAX_DERIVED":4096,"MAX_ENTITIES":512,"MAX_FACTS":4096,"MAX_INDEX_ITEMS":2048,"MAX_PATH_DEPTH":16,"MAX_PROPERTIES":256,"MAX_QUERY_RESULTS":1024,"MAX_ROUNDS":64,"MAX_RULES":256,"MAX_TYPES":128,"MAX_VECTOR_DIMENSION":64}

```text
Identity: owner:G034:G034-S01-01 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
KnowledgeGraph.new(cls, schema: Ontology, options: Mapping[str, Any] | None=None) -> 'KnowledgeGraph'
```
