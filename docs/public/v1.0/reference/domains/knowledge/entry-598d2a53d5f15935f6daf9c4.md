# knowledge — AiPolicy.requireGrounding(policy)

Bounded local graph and retrieval; no external knowledge source or semantic truth guarantee; original observation: immutable category policy enforces minimum citation count; SDK bounds: {"MAX_CONTEXT_BYTES":16384,"MAX_DERIVED":4096,"MAX_ENTITIES":512,"MAX_FACTS":4096,"MAX_INDEX_ITEMS":2048,"MAX_PATH_DEPTH":16,"MAX_PROPERTIES":256,"MAX_QUERY_RESULTS":1024,"MAX_ROUNDS":64,"MAX_RULES":256,"MAX_TYPES":128,"MAX_VECTOR_DIMENSION":64}

```text
Identity: owner:G034:G034-S06-06 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
AiPolicy.requireGrounding(policy: Mapping[str, Any]) -> GroundingRequirement
```
...
[
  ...
  {
    "excerpt": "361:     surface(\"G034-S06-03\", len(contradictions) == 1 and contradictions[0][\"code\"] == \"FUNCTIONAL_CONFLICT\", contradictions)\n362:     policy = AiPolicy.requireGrounding({\"categories\": (\"factual\", \"medical\"), \"minimumCitations\": 1})\n363:     grounded = Generation(\"Paris is connected to the cited fact.\", \"factual\", policy).ground(context)\n364:     surface(\"G034-S06-04\", grounded.text.startswith(\"Paris\") and grounded.grounding.decision == \"grounded\", grounded)\n365:     surface(\"G034-S06-05\", grounded.citations() == context.citations, grounded.citations())",
