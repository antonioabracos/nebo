# knowledge — query.path(predicate,bounds)

Bounded local graph and retrieval; no external knowledge source or semantic truth guarantee; original observation: path search obeys mandatory min and max depth; SDK bounds: {"MAX_CONTEXT_BYTES":16384,"MAX_DERIVED":4096,"MAX_ENTITIES":512,"MAX_FACTS":4096,"MAX_INDEX_ITEMS":2048,"MAX_PATH_DEPTH":16,"MAX_PROPERTIES":256,"MAX_QUERY_RESULTS":1024,"MAX_ROUNDS":64,"MAX_RULES":256,"MAX_TYPES":128,"MAX_VECTOR_DIMENSION":64}

```text
Identity: owner:G034:G034-S04-04 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
KnowledgeQuery.path(self, predicate: str, bounds: Mapping[str, Any]) -> 'KnowledgeQuery'
```
...
[
  ...
  {
    "excerpt": "265:     surface(\"G034-S04-03\", len(optional.optional_patterns) == 1, optional.optional_patterns)\n266:     pathed = optional.path(\"locatedIn\", {\"from\": \"$place\", \"to\": \"$region\", \"minDepth\": 1, \"maxDepth\": 2})\n267:     surface(\"G034-S04-04\", pathed.path_spec[\"maxDepth\"] == 2, pathed.path_spec)\n268:     projected = pathed.project((\"$person\", \"$place\", \"$region\", \"$fact\"))\n269:     surface(\"G034-S04-05\", projected.projection == (\"$person\", \"$place\", \"$region\", \"$fact\"), projected.projection)",
