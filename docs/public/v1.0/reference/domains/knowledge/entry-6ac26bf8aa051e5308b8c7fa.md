# knowledge — SemanticIndex.build(items,representation,options)

Bounded local graph and retrieval; no external knowledge source or semantic truth guarantee; original observation: versioned local index validates every explicit vector; SDK bounds: {"MAX_CONTEXT_BYTES":16384,"MAX_DERIVED":4096,"MAX_ENTITIES":512,"MAX_FACTS":4096,"MAX_INDEX_ITEMS":2048,"MAX_PATH_DEPTH":16,"MAX_PROPERTIES":256,"MAX_QUERY_RESULTS":1024,"MAX_ROUNDS":64,"MAX_RULES":256,"MAX_TYPES":128,"MAX_VECTOR_DIMENSION":64}

```text
Identity: owner:G034:G034-S05-01 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
SemanticIndex.build(cls, items: Sequence[Mapping[str, Any]], representation: Mapping[str, Sequence[float]], options: Mapping[str, Any]) -> 'SemanticIndex'
```
...
[
  ...
  {
    "excerpt": "302:     vectors = {\"alice\": (1.0, 0.0, 0.0), \"paris\": (0.0, 1.0, 0.0)}\n303:     index = SemanticIndex.build(items, vectors, {\"version\": 1, \"dimension\": 3, \"maxItems\": 8, \"metric\": \"cosine\", \"queryRepresentations\": {\"city\": (0.0, 1.0, 0.0), \"person\": (1.0, 0.0, 0.0)}})\n304:     return items, index, TextIndex(items, 1)\n305: \n306: ",
