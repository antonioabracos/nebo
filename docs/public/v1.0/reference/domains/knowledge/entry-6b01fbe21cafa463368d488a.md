# knowledge — ontology.validate(graph)

Bounded local graph and retrieval; no external knowledge source or semantic truth guarantee; original observation: validation reports violations without mutating or repairing graph; SDK bounds: {"MAX_CONTEXT_BYTES":16384,"MAX_DERIVED":4096,"MAX_ENTITIES":512,"MAX_FACTS":4096,"MAX_INDEX_ITEMS":2048,"MAX_PATH_DEPTH":16,"MAX_PROPERTIES":256,"MAX_QUERY_RESULTS":1024,"MAX_ROUNDS":64,"MAX_RULES":256,"MAX_TYPES":128,"MAX_VECTOR_DIMENSION":64}

```text
Identity: owner:G034:G034-S02-07 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Ontology.validate(self, graph: 'KnowledgeGraph') -> tuple[Mapping[str, Any], ...]
```
...
[
  ...
  {
    "excerpt": "185:     getattr(graph, \"assert\")(person, \"email\", \"p@example.invalid\", provenance(\"p-email\"))\n186:     violations = ontology.validate(graph)\n187:     surface(\"G034-S02-07\", violations == (), violations)\n188:     previous = Ontology.new(\"transport\", 1).defineType(\"Place\").defineType(\"City\").defineType(\"Person\")\n189:     previous.subtype(\"City\", \"Place\").property(\"livesIn\", \"Person\", \"Place\").property(\"hosts\", \"Place\", \"Person\").property(\"email\", \"Person\", \"Text\").inverse(\"livesIn\", \"hosts\")",
