# knowledge — reasoner.infer(graph)

Bounded local graph and retrieval; no external knowledge source or semantic truth guarantee; original observation: bounded fixpoint derives input-dependent facts with proof links; SDK bounds: {"MAX_CONTEXT_BYTES":16384,"MAX_DERIVED":4096,"MAX_ENTITIES":512,"MAX_FACTS":4096,"MAX_INDEX_ITEMS":2048,"MAX_PATH_DEPTH":16,"MAX_PROPERTIES":256,"MAX_QUERY_RESULTS":1024,"MAX_ROUNDS":64,"MAX_RULES":256,"MAX_TYPES":128,"MAX_VECTOR_DIMENSION":64}

```text
Identity: owner:G034:G034-S03-06 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Reasoner.infer(self, graph: KnowledgeGraph) -> InferenceResult
```
...
[
  ...
  {
    "excerpt": "228:     surface(\"G034-S03-05\", reasoner.rules[0].identity() == rule.identity(), reasoner.rules[0].identity())\n229:     inference = reasoner.infer(graph)\n230:     alice_europe = next(fact for fact in inference.facts if fact.subject == entities[\"alice\"] and fact.object == entities[\"europe\"])\n231:     surface(\"G034-S03-06\", len(inference.facts) == 1 and alice_europe.derived, inference)\n232:     bob_paris = getattr(graph, \"assert\")(entities[\"bob\"], \"livesIn\", entities[\"paris\"], provenance(\"bob-residence\"))",
