# knowledge — agent.knowledgeScope(capability)

Bounded local graph and retrieval; no external knowledge source or semantic truth guarantee; original observation: deny-default graph and index allowlists govern authorization; SDK bounds: {"MAX_CONTEXT_BYTES":16384,"MAX_DERIVED":4096,"MAX_ENTITIES":512,"MAX_FACTS":4096,"MAX_INDEX_ITEMS":2048,"MAX_PATH_DEPTH":16,"MAX_PROPERTIES":256,"MAX_QUERY_RESULTS":1024,"MAX_ROUNDS":64,"MAX_RULES":256,"MAX_TYPES":128,"MAX_VECTOR_DIMENSION":64}

```text
Identity: owner:G034:G034-S06-07 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
KnowledgeAgent.knowledgeScope(self, capability: Mapping[str, Any]) -> 'KnowledgeAgent'
```
...
[
  ...
  {
    "excerpt": "366:     surface(\"G034-S06-06\", policy.categories == frozenset({\"factual\", \"medical\"}) and policy.minimumCitations == 1, policy)\n367:     agent = KnowledgeAgent(\"research\").knowledgeScope({\"graphs\": (graph.graph_identity,), \"indexes\": (\"places-v1\",)})\n368:     agent.authorize(graph.graph_identity, \"places-v1\")\n369:     surface(\"G034-S06-07\", agent.scope.graphs == frozenset({graph.graph_identity}) and agent.scope.indexes == frozenset({\"places-v1\"}), agent.scope)\n370:     audit = grounded.grounding.audit()",
