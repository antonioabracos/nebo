# knowledge — query.where(condition)

Bounded local graph and retrieval; no external knowledge source or semantic truth guarantee; original observation: closed operators filter actual binding values; SDK bounds: {"MAX_CONTEXT_BYTES":16384,"MAX_DERIVED":4096,"MAX_ENTITIES":512,"MAX_FACTS":4096,"MAX_INDEX_ITEMS":2048,"MAX_PATH_DEPTH":16,"MAX_PROPERTIES":256,"MAX_QUERY_RESULTS":1024,"MAX_ROUNDS":64,"MAX_RULES":256,"MAX_TYPES":128,"MAX_VECTOR_DIMENSION":64}

```text
Identity: owner:G034:G034-S04-02 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
KnowledgeQuery.where(self, condition: Mapping[str, Any]) -> 'KnowledgeQuery'
```
...
[
  ...
  {
    "excerpt": "261:     surface(\"G034-S04-01\", matched.pattern[\"predicate\"] == \"livesIn\", matched.pattern)\n262:     filtered = matched.where({\"binding\": \"$person\", \"operator\": \"eq\", \"value\": \"alice\"})\n263:     surface(\"G034-S04-02\", len(filtered.conditions) == 1 and matched.conditions == (), filtered.conditions)\n264:     optional = filtered.optional({\"subject\": \"$place\", \"predicate\": \"locatedIn\", \"object\": \"$region\"})\n265:     surface(\"G034-S04-03\", len(optional.optional_patterns) == 1, optional.optional_patterns)",
