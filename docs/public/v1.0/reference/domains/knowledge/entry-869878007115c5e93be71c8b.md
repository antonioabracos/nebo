...
Bounded local graph and retrieval; no external knowledge source or semantic truth guarantee; original observation: audit exposes hashes versions and decision while omitting raw context; SDK bounds: {"MAX_CONTEXT_BYTES":16384,"MAX_DERIVED":4096,"MAX_ENTITIES":512,"MAX_FACTS":4096,"MAX_INDEX_ITEMS":2048,"MAX_PATH_DEPTH":16,"MAX_PROPERTIES":256,"MAX_QUERY_RESULTS":1024,"MAX_ROUNDS":64,"MAX_RULES":256,"MAX_TYPES":128,"MAX_VECTOR_DIMENSION":64}

```text
Identity: owner:G034:G034-S06-08 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
GroundingAudit.audit(self) -> Mapping[str, Any]
```
...
[
  ...
  {
    "excerpt": "369:     surface(\"G034-S06-07\", agent.scope.graphs == frozenset({graph.graph_identity}) and agent.scope.indexes == frozenset({\"places-v1\"}), agent.scope)\n370:     audit = grounded.grounding.audit()\n371:     surface(\"G034-S06-08\", audit[\"sensitiveContent\"] == \"omitted\" and \"Paris\" not in repr(audit) and len(audit[\"citationDigests\"][0]) == 64, audit)\n372: \n373:     smaller = knowledge.context(query, {\"maxResults\": 4, \"maxBytes\": 1})",
