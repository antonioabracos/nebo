# knowledge — rule.priority(value)

Bounded local graph and retrieval; no external knowledge source or semantic truth guarantee; original observation: explicit bounded priority governs deterministic rule order; SDK bounds: {"MAX_CONTEXT_BYTES":16384,"MAX_DERIVED":4096,"MAX_ENTITIES":512,"MAX_FACTS":4096,"MAX_INDEX_ITEMS":2048,"MAX_PATH_DEPTH":16,"MAX_PROPERTIES":256,"MAX_QUERY_RESULTS":1024,"MAX_ROUNDS":64,"MAX_RULES":256,"MAX_TYPES":128,"MAX_VECTOR_DIMENSION":64}

```text
Identity: owner:G034:G034-S03-04 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Rule.priority(self, value: int) -> 'Rule'
```
...
[
  ...
  {
    "excerpt": "224:     surface(\"G034-S03-03\", concluded.conclusion[\"predicate\"] == \"reachable\", concluded.conclusion)\n225:     rule = concluded.priority(7)\n226:     surface(\"G034-S03-04\", rule.priority_value == 7 and concluded.priority_value == 0, rule.priority_value)\n227:     reasoner = Reasoner.new(ontology, (rule,), {\"strategy\": \"monotonic\", \"maxRounds\": 8, \"maxDerived\": 16})\n228:     surface(\"G034-S03-05\", reasoner.rules[0].identity() == rule.identity(), reasoner.rules[0].identity())",
