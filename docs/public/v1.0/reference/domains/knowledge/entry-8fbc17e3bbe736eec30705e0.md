...
Bounded local graph and retrieval; no external knowledge source or semantic truth guarantee; original observation: result stream advances only under positive bounded demand; SDK bounds: {"MAX_CONTEXT_BYTES":16384,"MAX_DERIVED":4096,"MAX_ENTITIES":512,"MAX_FACTS":4096,"MAX_INDEX_ITEMS":2048,"MAX_PATH_DEPTH":16,"MAX_PROPERTIES":256,"MAX_QUERY_RESULTS":1024,"MAX_ROUNDS":64,"MAX_RULES":256,"MAX_TYPES":128,"MAX_VECTOR_DIMENSION":64}

```text
Identity: owner:G034:G034-S04-07 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
KnowledgeQuery.stream(self) -> 'ResultStream'
```
...
[
  ...
  {
    "excerpt": "271:     surface(\"G034-S04-06\", plan[\"index\"] == \"predicate+subject\" and plan[\"pathBound\"] == 2, plan)\n272:     stream = projected.stream()\n273:     streamed = stream.request(1)\n274:     surface(\"G034-S04-07\", len(streamed) == 1 and stream.request(1) == (), streamed)\n275:     collected = projected.collect(1)",
