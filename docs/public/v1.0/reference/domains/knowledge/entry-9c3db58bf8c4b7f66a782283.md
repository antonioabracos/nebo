# knowledge — rule.and(condition)

Bounded local graph and retrieval; no external knowledge source or semantic truth guarantee; original observation: additional premise returns a new rule value; SDK bounds: {"MAX_CONTEXT_BYTES":16384,"MAX_DERIVED":4096,"MAX_ENTITIES":512,"MAX_FACTS":4096,"MAX_INDEX_ITEMS":2048,"MAX_PATH_DEPTH":16,"MAX_PROPERTIES":256,"MAX_QUERY_RESULTS":1024,"MAX_ROUNDS":64,"MAX_RULES":256,"MAX_TYPES":128,"MAX_VECTOR_DIMENSION":64}

```text
Identity: owner:G034:G034-S03-02 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Rule.andCondition(self, condition: Mapping[str, Any]) -> 'Rule'
```
...
[
  ...
  {
    "excerpt": "220:     surface(\"G034-S03-01\", len(initial.patterns) == 1, initial.patterns)\n221:     combined = initial.andCondition({\"subject\": \"$place\", \"predicate\": \"locatedIn\", \"object\": \"$region\"})\n222:     surface(\"G034-S03-02\", len(combined.patterns) == 2 and len(initial.patterns) == 1, combined.patterns)\n223:     concluded = combined.then({\"subject\": \"$person\", \"predicate\": \"reachable\", \"object\": \"$region\"})\n224:     surface(\"G034-S03-03\", concluded.conclusion[\"predicate\"] == \"reachable\", concluded.conclusion)",
