# knowledge — graph.verify()

Bounded local graph and retrieval; no external knowledge source or semantic truth guarantee; original observation: schema constraints provenance and both indexes verify together; SDK bounds: {"MAX_CONTEXT_BYTES":16384,"MAX_DERIVED":4096,"MAX_ENTITIES":512,"MAX_FACTS":4096,"MAX_INDEX_ITEMS":2048,"MAX_PATH_DEPTH":16,"MAX_PROPERTIES":256,"MAX_QUERY_RESULTS":1024,"MAX_ROUNDS":64,"MAX_RULES":256,"MAX_TYPES":128,"MAX_VECTOR_DIMENSION":64}

```text
Identity: owner:G034:G034-S01-08 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
KnowledgeGraph.verify(self) -> VerificationResult
```
...
[
  ...
  {
    "excerpt": "143:     surface(\"G034-S01-07\", snapshot.version == graph.version and len(snapshot.facts) == 2 and len(snapshot.retractions) == 1, snapshot)\n144:     verification = graph.verify()\n145:     surface(\"G034-S01-08\", verification.valid and verification.checkedFacts == 2, verification)\n146: \n147:     changed = getattr(graph, \"assert\")(paris, \"label\", \"Paris\", provenance(\"label\"))",
