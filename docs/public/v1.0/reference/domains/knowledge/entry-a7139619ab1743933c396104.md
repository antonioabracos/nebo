# knowledge — reasoner.incremental(changes)

Bounded local graph and retrieval; no external knowledge source or semantic truth guarantee; original observation: addition-safe affected mode remains equal to full recomputation; SDK bounds: {"MAX_CONTEXT_BYTES":16384,"MAX_DERIVED":4096,"MAX_ENTITIES":512,"MAX_FACTS":4096,"MAX_INDEX_ITEMS":2048,"MAX_PATH_DEPTH":16,"MAX_PROPERTIES":256,"MAX_QUERY_RESULTS":1024,"MAX_ROUNDS":64,"MAX_RULES":256,"MAX_TYPES":128,"MAX_VECTOR_DIMENSION":64}

```text
Identity: owner:G034:G034-S03-07 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Reasoner.incremental(self, changes: Mapping[str, Any]) -> InferenceResult
```
...
[
  ...
  {
    "excerpt": "232:     bob_paris = getattr(graph, \"assert\")(entities[\"bob\"], \"livesIn\", entities[\"paris\"], provenance(\"bob-residence\"))\n233:     incremental = reasoner.incremental({\"graph\": graph, \"added\": (bob_paris.factId,), \"retracted\": ()})\n234:     bob_derived = next(fact for fact in incremental.facts if fact.subject == entities[\"bob\"])\n235:     surface(\"G034-S03-07\", incremental.mode == \"affected\" and len(incremental.facts) == 2 and incremental.recomputed == (bob_derived.factId,), incremental)\n236:     explanation = reasoner.explain(next(fact for fact in incremental.facts if fact.subject == entities[\"bob\"]))",
