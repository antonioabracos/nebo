# knowledge — index.search(query,limit)

Bounded local graph and retrieval; no external knowledge source or semantic truth guarantee; original observation: cosine ranking is deterministic bounded and score-visible; SDK bounds: {"MAX_CONTEXT_BYTES":16384,"MAX_DERIVED":4096,"MAX_ENTITIES":512,"MAX_FACTS":4096,"MAX_INDEX_ITEMS":2048,"MAX_PATH_DEPTH":16,"MAX_PROPERTIES":256,"MAX_QUERY_RESULTS":1024,"MAX_ROUNDS":64,"MAX_RULES":256,"MAX_TYPES":128,"MAX_VECTOR_DIMENSION":64}

```text
Identity: owner:G034:G034-S05-04 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
SemanticIndex.search(self, query: Sequence[float], limit: int) -> tuple[SearchResult, ...]
```
...
[
  ...
  {
    "excerpt": "316:     surface(\"G034-S05-03\", \"temp\" not in index._items and removal[\"identity\"] == \"temp\", removal)\n317:     semantic = index.search((0.0, 1.0, 0.0), 2)\n318:     surface(\"G034-S05-04\", tuple(result.identity for result in semantic) == (\"paris\", \"london\") and semantic[0].score == 1.0, semantic)\n319:     _, graph, _, _ = make_graph()\n320:     combined = HybridSearch.new(graph, TextIndex(items + [london], 1), index)",
