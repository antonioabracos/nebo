# knowledge — query.project(bindings)

Bounded local graph and retrieval; no external knowledge source or semantic truth guarantee; original observation: projection produces immutable named rows; SDK bounds: {"MAX_CONTEXT_BYTES":16384,"MAX_DERIVED":4096,"MAX_ENTITIES":512,"MAX_FACTS":4096,"MAX_INDEX_ITEMS":2048,"MAX_PATH_DEPTH":16,"MAX_PROPERTIES":256,"MAX_QUERY_RESULTS":1024,"MAX_ROUNDS":64,"MAX_RULES":256,"MAX_TYPES":128,"MAX_VECTOR_DIMENSION":64}

```text
Identity: owner:G034:G034-S04-05 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
KnowledgeQuery.project(self, bindings: Sequence[str]) -> 'KnowledgeQuery'
```
...
[
  ...
  {
    "excerpt": "267:     surface(\"G034-S04-04\", pathed.path_spec[\"maxDepth\"] == 2, pathed.path_spec)\n268:     projected = pathed.project((\"$person\", \"$place\", \"$region\", \"$fact\"))\n269:     surface(\"G034-S04-05\", projected.projection == (\"$person\", \"$place\", \"$region\", \"$fact\"), projected.projection)\n270:     plan = projected.explain()\n271:     surface(\"G034-S04-06\", plan[\"index\"] == \"predicate+subject\" and plan[\"pathBound\"] == 2, plan)",
