...
Bounded native media owner; legacy whole-file source probe is not a public typed API; original observation: image.flipHorizontal reverses each row

```text
Identity: owner:G019:G019-S03-F003 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
G019-S03-F003
Owner: runtime/image/image_transform.asm::nebo_image_flip_horizontal_into
```
