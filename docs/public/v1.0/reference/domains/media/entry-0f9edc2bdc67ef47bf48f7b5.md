...
Bounded native media owner; legacy whole-file source probe is not a public typed API; original observation: image.rotate90 maps one to three exact quarter turns

```text
Identity: owner:G019:G019-S03-F005 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
G019-S03-F005
Owner: runtime/image/image_transform.asm::nebo_image_rotate90_into
```
