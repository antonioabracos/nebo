...
Bounded native media owner; legacy whole-file source probe is not a public typed API; original observation: image.height observes the validated height

```text
Identity: owner:G019:G019-S02-F004 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
G019-S02-F004
Owner: runtime/image/image.asm::nebo_image_height
```
