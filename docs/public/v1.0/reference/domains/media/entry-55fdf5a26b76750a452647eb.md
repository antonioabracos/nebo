...
Bounded native media owner; legacy whole-file source probe is not a public typed API; original observation: device.write either queues complete frames or changes no state

```text
Identity: owner:G019:G019-S05-F008 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
G019-S05-F008
Owner: runtime/media/media_contract.asm::nebo_media_audio_device_write
```
