...
Bounded native media owner; legacy whole-file source probe is not a public typed API; original observation: Image.new validates dimensions then zeros caller-owned storage

```text
Identity: owner:G019:G019-S02-F001 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
G019-S02-F001
Owner: runtime/image/image.asm::nebo_image_init
```
