...
Bounded native media owner; legacy whole-file source probe is not a public typed API; original observation: image.width observes the validated width

```text
Identity: owner:G019:G019-S02-F003 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
G019-S02-F003
Owner: runtime/image/image.asm::nebo_image_width
```
