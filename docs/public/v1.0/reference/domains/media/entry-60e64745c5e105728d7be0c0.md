...
Bounded native media owner; legacy whole-file source probe is not a public typed API; original observation: frame.timestamp preserves deterministic microseconds

```text
Identity: owner:G019:G019-S06-F003 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
G019-S06-F003
Owner: runtime/media/media_contract.asm::nebo_video_frame_timestamp
```
