...
Bounded native media owner; legacy whole-file source probe is not a public typed API; original observation: image.convert maps RGBA8 and GRAY8 with documented weights

```text
Identity: owner:G019:G019-S03-F006 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
G019-S03-F006
Owner: runtime/image/image_transform.asm::nebo_image_convert_into
```
