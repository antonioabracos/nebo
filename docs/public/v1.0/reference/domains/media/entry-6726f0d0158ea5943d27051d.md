...
Bounded native media owner; legacy whole-file source probe is not a public typed API; original observation: Bmp.decode validates the full BMP before publishing pixels

```text
Identity: owner:G019:G019-S04-F001 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
G019-S04-F001
Owner: runtime/image/bmp.asm::nebo_bmp_decode
```
