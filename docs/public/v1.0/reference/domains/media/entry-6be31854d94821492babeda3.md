...
Bounded native media owner; legacy whole-file source probe is not a public typed API; original observation: Png.encode emits a real 73-byte bounded RGBA8 PNG

```text
Identity: owner:G019:G019-S04-F004 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
G019-S04-F004
Owner: runtime/media/media_contract.asm::nebo_png_encode
```
