...
Bounded native media owner; legacy whole-file source probe is not a public typed API; original observation: pipeline.encode requires transform and commits destination plus codec

```text
Identity: owner:G019:G019-S06-F007 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
G019-S06-F007
Owner: runtime/media/media_contract.asm::nebo_media_pipeline_encode
```
