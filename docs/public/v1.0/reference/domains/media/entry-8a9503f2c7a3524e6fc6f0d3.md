...
Bounded native media owner; legacy whole-file source probe is not a public typed API; original observation: audio.frameCount observes the canonical frame count

```text
Identity: owner:G019:G019-S05-F003 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
G019-S05-F003
Owner: runtime/media/media_contract.asm::nebo_audio_frame_count
```
