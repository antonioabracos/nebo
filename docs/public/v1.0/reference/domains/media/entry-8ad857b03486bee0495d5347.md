...
Bounded native media owner; legacy whole-file source probe is not a public typed API; original observation: image.region creates a generation-checked borrowed view

```text
Identity: owner:G019:G019-S02-F008 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
G019-S02-F008
Owner: runtime/image/image.asm::nebo_image_region
```
