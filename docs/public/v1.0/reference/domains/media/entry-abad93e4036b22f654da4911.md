...
Bounded native media owner; legacy whole-file source probe is not a public typed API; original observation: image.stride exposes row bytes including explicit padding

```text
Identity: owner:G019:G019-S02-F005 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
G019-S02-F005
Owner: runtime/image/image.asm::nebo_image_stride
```
