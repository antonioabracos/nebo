...
Bounded native media owner; legacy whole-file source probe is not a public typed API; original observation: AudioDevice.open provides an explicit offline sink while host audio stays gated

```text
Identity: owner:G019:G019-S05-F007 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
G019-S05-F007
Owner: runtime/media/media_contract.asm::nebo_media_audio_device_open
```
