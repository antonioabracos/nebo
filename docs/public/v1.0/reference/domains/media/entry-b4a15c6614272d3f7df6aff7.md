...
Bounded native media owner; legacy whole-file source probe is not a public typed API; original observation: image.pixel returns the requested in-bounds packed pixel

```text
Identity: owner:G019:G019-S02-F006 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
G019-S02-F006
Owner: runtime/image/image.asm::nebo_image_pixel
```
