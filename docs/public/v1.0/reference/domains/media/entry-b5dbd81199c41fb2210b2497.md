...
Bounded native media owner; legacy whole-file source probe is not a public typed API; original observation: PixelFormat.gray8 returns the versioned one-channel layout

```text
Identity: owner:G019:G019-S01-F006 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
G019-S01-F006
Owner: runtime/media/color.asm::nebo_pixel_format_gray8
```
