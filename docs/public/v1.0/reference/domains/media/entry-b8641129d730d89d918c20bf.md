...
Bounded native media owner; legacy whole-file source probe is not a public typed API; original observation: color.toSrgb selects the nearest canonical sRGB byte

```text
Identity: owner:G019:G019-S01-F004 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
G019-S01-F004
Owner: runtime/media/color.asm::nebo_color_to_srgb_q16
```
