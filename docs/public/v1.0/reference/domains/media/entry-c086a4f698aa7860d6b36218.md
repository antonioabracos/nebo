...
Bounded native media owner; legacy whole-file source probe is not a public typed API; original observation: audio.duration returns floor microseconds from frames and rate

```text
Identity: owner:G019:G019-S05-F004 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
G019-S05-F004
Owner: runtime/audio/pcm.asm::nebo_pcm_duration_us
```
