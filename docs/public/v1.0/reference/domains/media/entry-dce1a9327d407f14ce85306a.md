...
Bounded native media owner; legacy whole-file source probe is not a public typed API; original observation: image.save completes encoding before exact File capability writes

```text
Identity: owner:G019:G019-S04-F006 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
G019-S04-F006
Owner: runtime/image/image_io.asm::nebo_image_save
```
