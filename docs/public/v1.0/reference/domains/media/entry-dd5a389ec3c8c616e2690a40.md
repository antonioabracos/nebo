...
Bounded native media owner; legacy whole-file source probe is not a public typed API; original observation: image.setPixel changes exactly one in-bounds pixel

```text
Identity: owner:G019:G019-S02-F007 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
G019-S02-F007
Owner: runtime/image/image.asm::nebo_image_set_pixel
```
