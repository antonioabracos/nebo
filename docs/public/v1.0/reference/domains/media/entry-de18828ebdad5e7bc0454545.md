...
Bounded native media owner; legacy whole-file source probe is not a public typed API; original observation: pipeline.transform requires decoded state and publishes 60 percent

```text
Identity: owner:G019:G019-S06-F006 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
G019-S06-F006
Owner: runtime/media/media_contract.asm::nebo_media_pipeline_transform
```
