...
Bounded native media owner; legacy whole-file source probe is not a public typed API; original observation: AudioBuffer.new validates S16LE channels frames and rate

```text
Identity: owner:G019:G019-S05-F001 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
G019-S05-F001
Owner: runtime/audio/pcm.asm::nebo_pcm_init
```
