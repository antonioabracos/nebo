...
Bounded native media owner; legacy whole-file source probe is not a public typed API; original observation: image.resize applies explicit nearest or bilinear sampling

```text
Identity: owner:G019:G019-S03-F001 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
G019-S03-F001
Owner: runtime/image/image_transform.asm::nebo_image_resize_into
```
