...
Bounded native media owner; legacy whole-file source probe is not a public typed API; original observation: VideoStream.frames observes finite queue occupancy

```text
Identity: owner:G019:G019-S06-F004 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
G019-S06-F004
Owner: runtime/media/media_contract.asm::nebo_video_stream_frames
```
