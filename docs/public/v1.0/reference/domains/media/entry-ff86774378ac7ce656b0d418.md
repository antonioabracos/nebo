...
Bounded native media owner; legacy whole-file source probe is not a public typed API; original observation: frame.image returns the authenticated image identity

```text
Identity: owner:G019:G019-S06-F002 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
G019-S06-F002
Owner: runtime/media/media_contract.asm::nebo_video_frame_image
```
