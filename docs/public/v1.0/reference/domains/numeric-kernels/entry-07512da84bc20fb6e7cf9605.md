# numeric-kernels — kernel.simd.matmulMicroKernel()

CPU scalar reference; native SIMD dispatch is a separate owner; no timing guarantee; original observation: kernel.simd.matmulMicroKernel(); SDK bounds: {"MAX_BENCH_SAMPLES":64,"MAX_BENCH_WARMUP":32,"MAX_DIMENSION":64,"MAX_ELEMENTS":4096,"MAX_WORKERS":8,"MAX_WORKSPACE_BYTES":65536}

```text
Identity: owner:G017:G017-S03-03 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
SimdKernels.matmulMicroKernel(self, left: Sequence[Sequence[int | float]], right: Sequence[Sequence[int | float]], isa: str='sse2') -> list[list[float]]
```
...
[
  ...
  {
    "excerpt": "71: ok(\"positive\", \"kernel.simd.dot\", kernel.simd.dot(left, right, \"avx2\") == reference_dot)\n72: ok(\"positive\", \"kernel.simd.matmulMicroKernel\",\n73:    kernel.simd.matmulMicroKernel([[2, 3]], [[4], [5]], \"sse2\") == [[23.0]])\n74: ok(\"positive\", \"kernel.simd.reduceSum\", kernel.simd.reduceSum(left, \"avx2\") == 15.25)\n75: packed = kernel.simd.pack([[1, 2, 3], [4, 5, 6]], 2)",
