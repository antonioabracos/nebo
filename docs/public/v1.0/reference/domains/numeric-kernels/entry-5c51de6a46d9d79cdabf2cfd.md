# numeric-kernels — pool.submit(job)

CPU scalar reference; native SIMD dispatch is a separate owner; no timing guarantee; original observation: pool.submit(job); SDK bounds: {"MAX_BENCH_SAMPLES":64,"MAX_BENCH_WARMUP":32,"MAX_DIMENSION":64,"MAX_ELEMENTS":4096,"MAX_WORKERS":8,"MAX_WORKSPACE_BYTES":65536}

```text
Identity: owner:G017:G017-S05-04 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
ThreadPool.submit(self, job: Callable[[], Any]) -> Future[Any]
```
...
[
  ...
  {
    "excerpt": "104: ok(\"positive\", \"ThreadPool.new\", pool.size == 2)\n105: future_a = pool.submit(lambda: 19)\n106: future_b = pool.submit(lambda: 23)\n107: ok(\"positive\", \"pool.submit\", future_a.result() == 19)\n108: ok(\"positive\", \"pool.waitIdle\", pool.waitIdle() == (19, 23))",
