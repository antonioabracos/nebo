# numeric-kernels — benchmark.exportJson()

CPU scalar reference; native SIMD dispatch is a separate owner; no timing guarantee; original observation: benchmark.exportJson(); SDK bounds: {"MAX_BENCH_SAMPLES":64,"MAX_BENCH_WARMUP":32,"MAX_DIMENSION":64,"MAX_ELEMENTS":4096,"MAX_WORKERS":8,"MAX_WORKSPACE_BYTES":65536}

```text
Identity: owner:G017:G017-S06-06 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
BenchmarkResult.exportJson(self) -> str
```
