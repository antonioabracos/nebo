# numeric-kernels — kernel.simd.unalignedFallback()

CPU scalar reference; native SIMD dispatch is a separate owner; no timing guarantee; original observation: kernel.simd.unalignedFallback(); SDK bounds: {"MAX_BENCH_SAMPLES":64,"MAX_BENCH_WARMUP":32,"MAX_DIMENSION":64,"MAX_ELEMENTS":4096,"MAX_WORKERS":8,"MAX_WORKSPACE_BYTES":65536}

```text
Identity: owner:G017:G017-S03-06 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
SimdKernels.unalignedFallback(self, operation: str, *arguments: Any) -> KernelExecution
```
...
[
  ...
  {
    "excerpt": "78:    ((1.0, 2.0, 4.0, 5.0, 3.0, 6.0), 2, 3, 2))\n79: fallback_execution = kernel.simd.unalignedFallback(\"add\", [3, 4, 5], [7, 8, 9])\n80: ok(\"positive\", \"kernel.simd.unalignedFallback\",\n81:    fallback_execution.tier == \"scalar\" and fallback_execution.value == [10.0, 12.0, 14.0])\n82: ",
