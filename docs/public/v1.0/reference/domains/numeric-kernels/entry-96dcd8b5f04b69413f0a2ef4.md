# numeric-kernels — planner.workspaceBytes()

CPU scalar reference; native SIMD dispatch is a separate owner; no timing guarantee; original observation: planner.workspaceBytes(); SDK bounds: {"MAX_BENCH_SAMPLES":64,"MAX_BENCH_WARMUP":32,"MAX_DIMENSION":64,"MAX_ELEMENTS":4096,"MAX_WORKERS":8,"MAX_WORKSPACE_BYTES":65536}

```text
Identity: owner:G017:G017-S04-05 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
KernelPlan.workspaceBytes(self) -> int
KernelPlanner.workspaceBytes(self) -> int
```
...
[
  ...
  {
    "excerpt": "93: dispatch.select(\"matmul\", KernelContext(128, \"f64\", avx_features, alignment=32))\n94: ok(\"positive\", \"planner.workspaceBytes\", dispatch.workspaceBytes() == 4096)\n95: dispatch.deterministicMode(True)\n96: ok(\"positive\", \"planner.deterministicMode\",\n97:    dispatch.select(\"dot\", KernelContext(32, \"f64\", avx_features, alignment=32)).descriptor.tier == \"scalar\")",
