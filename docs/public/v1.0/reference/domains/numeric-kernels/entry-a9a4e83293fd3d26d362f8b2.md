# numeric-kernels — benchmark.throughput(units)

CPU scalar reference; native SIMD dispatch is a separate owner; no timing guarantee; original observation: benchmark.throughput(units); SDK bounds: {"MAX_BENCH_SAMPLES":64,"MAX_BENCH_WARMUP":32,"MAX_DIMENSION":64,"MAX_ELEMENTS":4096,"MAX_WORKERS":8,"MAX_WORKSPACE_BYTES":65536}

```text
Identity: owner:G017:G017-S06-03 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
BenchmarkResult.throughput(self, units: float) -> float
```
...
[
  ...
  {
    "excerpt": "118:    len(benchmark.samples_ns) == 5 and benchmark.result_digest)\n119: ok(\"positive\", \"benchmark.throughput\", benchmark.throughput(len(left)) > 0.0)\n120: ok(\"positive\", \"benchmark.allocations\", benchmark.allocations()[\"bytes\"] >= 0)\n121: ok(\"positive\", \"benchmark.compare\", benchmark.compare(benchmark)[\"regression\"] is False)\n122: exported = json.loads(benchmark.exportJson())",
