# numeric-kernels — planner.fallback()

CPU scalar reference; native SIMD dispatch is a separate owner; no timing guarantee; original observation: planner.fallback(); SDK bounds: {"MAX_BENCH_SAMPLES":64,"MAX_BENCH_WARMUP":32,"MAX_DIMENSION":64,"MAX_ELEMENTS":4096,"MAX_WORKERS":8,"MAX_WORKSPACE_BYTES":65536}

```text
Identity: owner:G017:G017-S04-04 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
KernelPlanner.fallback(self, operation: str, context: KernelContext | None=None) -> KernelPlan
```
...
[
  ...
  {
    "excerpt": "90: ok(\"positive\", \"planner.explain\", dispatch.explain()[\"kernel\"] == \"avx2.add\")\n91: ok(\"positive\", \"planner.fallback\",\n92:    dispatch.fallback(\"add\", KernelContext(16, \"f64\", scalar_features)).descriptor.tier == \"scalar\")\n93: dispatch.select(\"matmul\", KernelContext(128, \"f64\", avx_features, alignment=32))\n94: ok(\"positive\", \"planner.workspaceBytes\", dispatch.workspaceBytes() == 4096)",
