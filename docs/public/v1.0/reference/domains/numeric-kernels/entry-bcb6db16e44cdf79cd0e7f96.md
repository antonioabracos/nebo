# numeric-kernels — kernel.scalar.reduceSum()

CPU scalar reference; native SIMD dispatch is a separate owner; no timing guarantee; original observation: kernel.scalar.reduceSum(); SDK bounds: {"MAX_BENCH_SAMPLES":64,"MAX_BENCH_WARMUP":32,"MAX_DIMENSION":64,"MAX_ELEMENTS":4096,"MAX_WORKERS":8,"MAX_WORKSPACE_BYTES":65536}

```text
Identity: owner:G017:G017-S02-04 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
ScalarKernels.reduceSum(self, values: Iterable[int | float]) -> float
SimdKernels.reduceSum(self, values: Iterable[int | float], isa: str='sse2') -> float
```
...
[
  ...
  {
    "excerpt": "62:    [[58.0, 64.0], [139.0, 154.0]])\n63: ok(\"positive\", \"kernel.scalar.reduceSum\", kernel.scalar.reduceSum(left) == 15.25)\n64: ok(\"positive\", \"kernel.scalar.activation\",\n65:    kernel.scalar.activation([-3, 0, 2.5], \"relu\") == [0.0, 0.0, 2.5])\n66: ok(\"positive\", \"kernel.scalar.convert\",",
