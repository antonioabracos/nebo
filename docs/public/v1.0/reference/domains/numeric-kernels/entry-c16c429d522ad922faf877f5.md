# numeric-kernels — kernel.scalar.convert()

CPU scalar reference; native SIMD dispatch is a separate owner; no timing guarantee; original observation: kernel.scalar.convert(); SDK bounds: {"MAX_BENCH_SAMPLES":64,"MAX_BENCH_WARMUP":32,"MAX_DIMENSION":64,"MAX_ELEMENTS":4096,"MAX_WORKERS":8,"MAX_WORKSPACE_BYTES":65536}

```text
Identity: owner:G017:G017-S02-06 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
ScalarKernels.convert(self, values: Iterable[int | float], dtype: str='f64', policy: str='error') -> list[int | float]
```
...
[
  ...
  {
    "excerpt": "65:    kernel.scalar.activation([-3, 0, 2.5], \"relu\") == [0.0, 0.0, 2.5])\n66: ok(\"positive\", \"kernel.scalar.convert\",\n67:    kernel.scalar.convert([-2.9, 9.8, 300], \"u8\", \"saturate\") == [0, 9, 255])\n68: \n69: # S03 SIMD values are checked against the independent scalar oracle, including a tail.",
