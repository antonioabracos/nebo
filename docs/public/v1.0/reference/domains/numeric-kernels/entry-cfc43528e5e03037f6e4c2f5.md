# numeric-kernels — planner.select(operation, context)

CPU scalar reference; native SIMD dispatch is a separate owner; no timing guarantee; original observation: planner.select(operation, context); SDK bounds: {"MAX_BENCH_SAMPLES":64,"MAX_BENCH_WARMUP":32,"MAX_DIMENSION":64,"MAX_ELEMENTS":4096,"MAX_WORKERS":8,"MAX_WORKSPACE_BYTES":65536}

```text
Identity: owner:G017:G017-S04-02 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
KernelPlanner.select(self, operation: str, context: KernelContext) -> KernelPlan
```
...
[
  ...
  {
    "excerpt": "87: dispatch = KernelPlanner(registry)\n88: avx_plan = dispatch.select(\"add\", KernelContext(16, \"f64\", avx_features, alignment=32))\n89: ok(\"positive\", \"planner.select\", avx_plan.descriptor.tier == \"avx2\")\n90: ok(\"positive\", \"planner.explain\", dispatch.explain()[\"kernel\"] == \"avx2.add\")\n91: ok(\"positive\", \"planner.fallback\",",
