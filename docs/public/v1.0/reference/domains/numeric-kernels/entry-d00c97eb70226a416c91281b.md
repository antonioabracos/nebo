# numeric-kernels — planner.deterministicMode(enabled)

CPU scalar reference; native SIMD dispatch is a separate owner; no timing guarantee; original observation: planner.deterministicMode(enabled); SDK bounds: {"MAX_BENCH_SAMPLES":64,"MAX_BENCH_WARMUP":32,"MAX_DIMENSION":64,"MAX_ELEMENTS":4096,"MAX_WORKERS":8,"MAX_WORKSPACE_BYTES":65536}

```text
Identity: owner:G017:G017-S04-06 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
KernelPlanner.deterministicMode(self, enabled: bool) -> 'KernelPlanner'
```
