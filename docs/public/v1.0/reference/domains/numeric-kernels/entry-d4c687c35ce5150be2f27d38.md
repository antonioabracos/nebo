# numeric-kernels — benchmark.compare(baseline)

CPU scalar reference; native SIMD dispatch is a separate owner; no timing guarantee; original observation: benchmark.compare(baseline); SDK bounds: {"MAX_BENCH_SAMPLES":64,"MAX_BENCH_WARMUP":32,"MAX_DIMENSION":64,"MAX_ELEMENTS":4096,"MAX_WORKERS":8,"MAX_WORKSPACE_BYTES":65536}

```text
Identity: owner:G017:G017-S06-05 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
BenchmarkResult.compare(self, baseline: 'BenchmarkResult', tolerance: float=0.1) -> dict[str, Any]
```
...
[
  ...
  {
    "excerpt": "120: ok(\"positive\", \"benchmark.allocations\", benchmark.allocations()[\"bytes\"] >= 0)\n121: ok(\"positive\", \"benchmark.compare\", benchmark.compare(benchmark)[\"regression\"] is False)\n122: exported = json.loads(benchmark.exportJson())\n123: ok(\"positive\", \"benchmark.exportJson\",\n124:    exported[\"schema\"] == 1 and exported[\"metadata\"][\"superiorityClaim\"] is False)",
