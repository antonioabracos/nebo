# numeric-kernels — planner.parallelThreshold()

CPU scalar reference; native SIMD dispatch is a separate owner; no timing guarantee; original observation: planner.parallelThreshold(); SDK bounds: {"MAX_BENCH_SAMPLES":64,"MAX_BENCH_WARMUP":32,"MAX_DIMENSION":64,"MAX_ELEMENTS":4096,"MAX_WORKERS":8,"MAX_WORKSPACE_BYTES":65536}

```text
Identity: owner:G017:G017-S05-06 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
KernelPlanner.parallelThreshold(self, elements: int | None=None) -> int
```
...
[
  ...
  {
    "excerpt": "109: pool.close()\n110: ok(\"positive\", \"planner.parallelThreshold\", dispatch.parallelThreshold(384) == 384)\n111: \n112: # S06 observes live timings but only makes descriptive, bounded claims.\n113: benchmark = Benchmark.measure(lambda: kernel.scalar.dot(left, right),",
