# numeric-kernels — Benchmark.measure(callable, options)

CPU scalar reference; native SIMD dispatch is a separate owner; no timing guarantee; original observation: Benchmark.measure(callable, options); SDK bounds: {"MAX_BENCH_SAMPLES":64,"MAX_BENCH_WARMUP":32,"MAX_DIMENSION":64,"MAX_ELEMENTS":4096,"MAX_WORKERS":8,"MAX_WORKSPACE_BYTES":65536}

```text
Identity: owner:G017:G017-S06-02 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Benchmark.measure(callable_: Callable[[], Any], options: BenchmarkOptions | dict[str, Any] | None=None) -> BenchmarkResult
```
...
[
  ...
  {
    "excerpt": "112: # S06 observes live timings but only makes descriptive, bounded claims.\n113: benchmark = Benchmark.measure(lambda: kernel.scalar.dot(left, right),\n114:                               BenchmarkOptions(warmup=1, samples=5, units=len(left),\n115:                                                track_allocations=True))\n116: ok(\"positive\", \"neboc bench\", benchmark.metadata[\"descriptiveOnly\"] is True)",
