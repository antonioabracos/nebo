# numeric-kernels — Parallel.forRange(range, grain, callable)

CPU scalar reference; native SIMD dispatch is a separate owner; no timing guarantee; original observation: Parallel.forRange(range, grain, callable); SDK bounds: {"MAX_BENCH_SAMPLES":64,"MAX_BENCH_WARMUP":32,"MAX_DIMENSION":64,"MAX_ELEMENTS":4096,"MAX_WORKERS":8,"MAX_WORKSPACE_BYTES":65536}

```text
Identity: owner:G017:G017-S05-01 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Parallel.forRange(selected: range, grain: int, callable_: Callable[[range], Any], workers: int | None=None) -> tuple[Any, ...]
```
...
[
  ...
  {
    "excerpt": "99: # S05 uses real bounded worker threads but observes results in submission order.\n100: chunks = Parallel.forRange(range(0, 11), 4, lambda selected: sum(selected), workers=3)\n101: ok(\"positive\", \"Parallel.forRange\", chunks == (6, 22, 27))\n102: ok(\"positive\", \"Parallel.reduce\", Parallel.reduce(range(1, 8), 0, lambda a, b: a + b, 3, 2) == 28)\n103: pool = ThreadPool.new(2)",
...
- runtime/kernel/numeric_parallel.asm — SHA-256 849b1b30fc3130e5d388365ab764e72ff1b2981d9c788fe6b14e91f76c44f341
