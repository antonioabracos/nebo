# offline-distributed — crdt.deltaSince(version)

Local event/convergence model; no network or distributed availability guarantee; original observation: vector-bounded delta returns the two unseen operations; SDK bounds: {"MAX_BATCH":256,"MAX_BYTES":1048576,"MAX_CHANGES":4096,"MAX_EVENTS":4096,"MAX_QUEUE":256,"MAX_REPLICAS":16}

```text
Identity: owner:G033:G033-S03-08 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
_OpCrdt.deltaSince(self, version: VersionVector) -> tuple[Mapping[str, Any], ...]
```
...
[
  ...
  {
    "excerpt": "138: ok(\"positive\", \"crdt.merge\", counter_a.value() == 34)\n139: delta_counter = counter_a.deltaSince(VersionVector({\"counter-A\": 1}))\n140: ok(\"positive\", \"crdt.deltaSince\", tuple(item[\"id\"] for item in delta_counter) == (\"counter-B:1\", \"counter-A:2\"))\n141: ok(\"ownership\", \"crdt-delta-snapshot\", delta_counter[1][\"data\"][\"amount\"] == -2)\n142: transcript.append((counter_a.value(), sorted(set_a.value()), thaw(map_a.value()),",
