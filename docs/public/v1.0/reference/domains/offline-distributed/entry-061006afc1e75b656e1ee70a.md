# offline-distributed — session.exchangeManifest()

Local event/convergence model; no network or distributed availability guarantee; original observation: manifest binds protocol endpoint checkpoints limits and local transport; SDK bounds: {"MAX_BATCH":256,"MAX_BYTES":1048576,"MAX_CHANGES":4096,"MAX_EVENTS":4096,"MAX_QUEUE":256,"MAX_REPLICAS":16}

```text
Identity: owner:G033:G033-S05-02 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
SyncSession.exchangeManifest(self) -> Mapping[str, Any]
```
...
[
  ...
  {
    "excerpt": "172: ok(\"positive\", \"SyncSession.open\", session.local is sync_left and session.remote is sync_right)\n173: manifest = session.exchangeManifest()\n174: ok(\"positive\", \"session.exchangeManifest\", manifest[\"protocol\"] == \"NEBO-SYNC-V1\"\n175:    and manifest[\"transport\"] == \"in_process\")\n176: pulled = session.pull({\"changes\": 1, \"bytes\": 2048})",
