# offline-distributed — store.read(streamId,range)

Local event/convergence model; no network or distributed availability guarantee; original observation: explicit range returns the two original immutable events; SDK bounds: {"MAX_BATCH":256,"MAX_BYTES":1048576,"MAX_CHANGES":4096,"MAX_EVENTS":4096,"MAX_QUEUE":256,"MAX_REPLICAS":16}

```text
Identity: owner:G033:G033-S01-03 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
EventStore.read(self, streamId: str, range: Mapping[str, int]) -> tuple[DomainEvent, ...]
```
