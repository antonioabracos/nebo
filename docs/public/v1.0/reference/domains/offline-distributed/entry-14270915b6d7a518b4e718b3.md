# offline-distributed — causal.ready(change,version)

Local event/convergence model; no network or distributed availability guarantee; original observation: first upstream change is ready against empty vector; SDK bounds: {"MAX_BATCH":256,"MAX_BYTES":1048576,"MAX_CHANGES":4096,"MAX_EVENTS":4096,"MAX_QUEUE":256,"MAX_REPLICAS":16}

```text
Identity: owner:G033:G033-S04-07 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
_Causal.ready(self, change: Change, version: VersionVector) -> bool
```
