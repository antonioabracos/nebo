# offline-distributed — replica.status()

Local event/convergence model; no network or distributed availability guarantee; original observation: status reports two applied changes and zero pending gaps; SDK bounds: {"MAX_BATCH":256,"MAX_BYTES":1048576,"MAX_CHANGES":4096,"MAX_EVENTS":4096,"MAX_QUEUE":256,"MAX_REPLICAS":16}

```text
Identity: owner:G033:G033-S02-07 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Replica.status(self) -> Mapping[str, Any]
```
...
[
  ...
  {
    "excerpt": "113: ok(\"positive\", \"replica.peers\", replica_a.peers()[\"replica-B\"][\"replica-A\"] == 1)\n114: status = gap.status()\n115: ok(\"positive\", \"replica.status\", status[\"changes\"] == 2 and status[\"pending\"] == 0)\n116: checkpoint = gap.checkpoint()\n117: ok(\"positive\", \"replica.checkpoint\", len(checkpoint[\"hash\"]) == 64 and checkpoint[\"state\"][\"x\"] == 2)",
