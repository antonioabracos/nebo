# offline-distributed — replica.localVersion()

Local event/convergence model; no network or distributed availability guarantee; original observation: copy of local vector observes replica-A counter one; SDK bounds: {"MAX_BATCH":256,"MAX_BYTES":1048576,"MAX_CHANGES":4096,"MAX_EVENTS":4096,"MAX_QUEUE":256,"MAX_REPLICAS":16}

```text
Identity: owner:G033:G033-S02-02 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Replica.localVersion(self) -> VersionVector
```
...
[
  ...
  {
    "excerpt": "96: change_a = replica_a.emit({\"key\": \"score\", \"value\": 33})\n97: local_version = replica_a.localVersion()\n98: ok(\"positive\", \"replica.localVersion\", local_version[\"replica-A\"] == 1)\n99: delta = replica_a.changesSince(VersionVector.new(), {\"changes\": 8, \"bytes\": 4096})\n100: ok(\"positive\", \"replica.changesSince\", delta == (change_a,))",
