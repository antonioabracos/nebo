# offline-distributed — replica.validateInvariants(rules)

Local event/convergence model; no network or distributed availability guarantee; original observation: negative quota violates independently supplied invariant; SDK bounds: {"MAX_BATCH":256,"MAX_BYTES":1048576,"MAX_CHANGES":4096,"MAX_EVENTS":4096,"MAX_QUEUE":256,"MAX_REPLICAS":16}

```text
Identity: owner:G033:G033-S06-04 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Replica.validateInvariants(self, rules: Iterable[Callable[[Mapping[str, Any]], bool]]) -> Mapping[str, Any]
```
...
[
  ...
  {
    "excerpt": "205: repair_replica.emit({\"key\": \"quota\", \"value\": -4})\n206: invariants = repair_replica.validateInvariants((lambda state: state[\"quota\"] >= 0,))\n207: ok(\"positive\", \"replica.validateInvariants\", not invariants[\"valid\"])\n208: repair_change = repair_replica.repair({\"payload\": {\"key\": \"quota\", \"value\": 4},\n209:                                       \"reason\": \"quota must be nonnegative\"})",
