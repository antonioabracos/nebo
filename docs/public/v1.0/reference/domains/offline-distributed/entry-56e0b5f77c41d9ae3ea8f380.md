# offline-distributed — store.snapshot(streamId,state)

Local event/convergence model; no network or distributed availability guarantee; original observation: snapshot version two binds the exact event prefix hash; SDK bounds: {"MAX_BATCH":256,"MAX_BYTES":1048576,"MAX_CHANGES":4096,"MAX_EVENTS":4096,"MAX_QUEUE":256,"MAX_REPLICAS":16}

```text
Identity: owner:G033:G033-S01-05 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
EventStore.snapshot(self, streamId: str, state: Any) -> Snapshot
```
