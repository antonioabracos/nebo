# offline-distributed — replica.checkpoint()

Local event/convergence model; no network or distributed availability guarantee; original observation: canonical checkpoint hash binds vector state and pending set; SDK bounds: {"MAX_BATCH":256,"MAX_BYTES":1048576,"MAX_CHANGES":4096,"MAX_EVENTS":4096,"MAX_QUEUE":256,"MAX_REPLICAS":16}

```text
Identity: owner:G033:G033-S02-08 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Replica.checkpoint(self) -> Mapping[str, Any]
```
...
[
  ...
  {
    "excerpt": "115: ok(\"positive\", \"replica.status\", status[\"changes\"] == 2 and status[\"pending\"] == 0)\n116: checkpoint = gap.checkpoint()\n117: ok(\"positive\", \"replica.checkpoint\", len(checkpoint[\"hash\"]) == 64 and checkpoint[\"state\"][\"x\"] == 2)\n118: ok(\"composition\", \"causal-gap-drain\", gap.localVersion()[\"upstream\"] == 2)\n119: transcript.append((thaw(local_version.items()), thaw(application), thaw(status), thaw(checkpoint)))",
