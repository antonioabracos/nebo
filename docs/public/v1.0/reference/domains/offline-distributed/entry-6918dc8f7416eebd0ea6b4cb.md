# offline-distributed — store.append(streamId,event,expectedVersion)

Local event/convergence model; no network or distributed availability guarantee; original observation: two immutable events form an exact-version hash chain; SDK bounds: {"MAX_BATCH":256,"MAX_BYTES":1048576,"MAX_CHANGES":4096,"MAX_EVENTS":4096,"MAX_QUEUE":256,"MAX_REPLICAS":16}

```text
Identity: owner:G033:G033-S01-02 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
EventStore.append(self, streamId: str, event: Mapping[str, Any], expectedVersion: int) -> DomainEvent
```
