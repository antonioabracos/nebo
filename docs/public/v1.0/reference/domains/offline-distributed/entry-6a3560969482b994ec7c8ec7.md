# offline-distributed — crdt.merge(other)

Local event/convergence model; no network or distributed availability guarantee; original observation: op-set join yields thirty-four independent of merge order; SDK bounds: {"MAX_BATCH":256,"MAX_BYTES":1048576,"MAX_CHANGES":4096,"MAX_EVENTS":4096,"MAX_QUEUE":256,"MAX_REPLICAS":16}

```text
Identity: owner:G033:G033-S03-07 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
_OpCrdt.merge(self, other: '_OpCrdt') -> '_OpCrdt'
```
...
[
  ...
  {
    "excerpt": "136: counter_b = CrdtCounter.new(\"counter-B\").increment(23)\n137: counter_a.merge(counter_b)\n138: ok(\"positive\", \"crdt.merge\", counter_a.value() == 34)\n139: delta_counter = counter_a.deltaSince(VersionVector({\"counter-A\": 1}))\n140: ok(\"positive\", \"crdt.deltaSince\", tuple(item[\"id\"] for item in delta_counter) == (\"counter-B:1\", \"counter-A:2\"))",
