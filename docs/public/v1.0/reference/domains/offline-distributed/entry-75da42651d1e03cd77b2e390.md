# offline-distributed — replica.apply(changes)

Local event/convergence model; no network or distributed availability guarantee; original observation: atomic application publishes one change and state score thirty-three; SDK bounds: {"MAX_BATCH":256,"MAX_BYTES":1048576,"MAX_CHANGES":4096,"MAX_EVENTS":4096,"MAX_QUEUE":256,"MAX_REPLICAS":16}

```text
Identity: owner:G033:G033-S02-04 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Replica.apply(self, changes: Iterable[Change]) -> Mapping[str, Any]
```
...
[
  ...
  {
    "excerpt": "78: replayed = store.replay(\"account-33\", {\"initial\": 3,\n79:                                         \"apply\": lambda state, item: state + item.payload[\"amount\"]})\n80: ok(\"positive\", \"store.replay\", replayed == 21)\n81: snapshot = store.snapshot(\"account-33\", {\"balance\": replayed})\n82: ok(\"positive\", \"store.snapshot\", snapshot.version == 2 and snapshot.prefix_hash == second.content_hash)",
