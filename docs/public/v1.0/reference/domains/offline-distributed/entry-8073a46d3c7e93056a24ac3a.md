# offline-distributed — session.pull(limit)

Local event/convergence model; no network or distributed availability guarantee; original observation: one remote change enters local state within explicit bounds; SDK bounds: {"MAX_BATCH":256,"MAX_BYTES":1048576,"MAX_CHANGES":4096,"MAX_EVENTS":4096,"MAX_QUEUE":256,"MAX_REPLICAS":16}

```text
Identity: owner:G033:G033-S05-03 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
SyncSession.pull(self, limit: Mapping[str, int] | None=None) -> Mapping[str, Any]
```
...
[
  ...
  {
    "excerpt": "175:    and manifest[\"transport\"] == \"in_process\")\n176: pulled = session.pull({\"changes\": 1, \"bytes\": 2048})\n177: ok(\"positive\", \"session.pull\", pulled[\"changes\"] == 1 and sync_left.materialized()[\"right\"] == 59)\n178: sync_left.emit({\"key\": \"second-left\", \"value\": 61})\n179: pushed = session.push({\"changes\": 2, \"bytes\": 4096})",
