# offline-distributed — CrdtRegister.new(value,policy)

Local event/convergence model; no network or distributed availability guarantee; original observation: multi-value register starts with visible north value; SDK bounds: {"MAX_BATCH":256,"MAX_BYTES":1048576,"MAX_CHANGES":4096,"MAX_EVENTS":4096,"MAX_QUEUE":256,"MAX_REPLICAS":16}

```text
Identity: owner:G033:G033-S03-05 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
CrdtRegister.new(value: Any, policy: str | Mapping[str, Any]) -> 'CrdtRegister'
```
