# offline-distributed — replica.changesSince(version,limits)

Local event/convergence model; no network or distributed availability guarantee; original observation: bounded empty-vector delta contains exactly change replica-A one; SDK bounds: {"MAX_BATCH":256,"MAX_BYTES":1048576,"MAX_CHANGES":4096,"MAX_EVENTS":4096,"MAX_QUEUE":256,"MAX_REPLICAS":16}

```text
Identity: owner:G033:G033-S02-03 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Replica.changesSince(self, version: VersionVector, limits: Mapping[str, int]) -> tuple[Change, ...]
```
...
[
  ...
  {
    "excerpt": "98: ok(\"positive\", \"replica.localVersion\", local_version[\"replica-A\"] == 1)\n99: delta = replica_a.changesSince(VersionVector.new(), {\"changes\": 8, \"bytes\": 4096})\n100: ok(\"positive\", \"replica.changesSince\", delta == (change_a,))\n101: replica_b = Replica.open(\"replica-B\", None, policy)\n102: application = replica_b.apply(delta)",
...
- runtime/replication/sync.asm — SHA-256 4c4326b9daf30116dda80eb54084d3874efc51ed273bdeac42aa5d1d606ef5b1
