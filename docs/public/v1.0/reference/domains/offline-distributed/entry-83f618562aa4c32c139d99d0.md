# offline-distributed — HybridLogicalClock.new(clockCapability)

Local event/convergence model; no network or distributed availability guarantee; original observation: explicit deterministic clock emits physical seven logical zero; SDK bounds: {"MAX_BATCH":256,"MAX_BYTES":1048576,"MAX_CHANGES":4096,"MAX_EVENTS":4096,"MAX_QUEUE":256,"MAX_REPLICAS":16}

```text
Identity: owner:G033:G033-S04-04 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
HybridLogicalClock.new(clockCapability: LogicalClockCapability) -> 'HybridLogicalClock'
```
