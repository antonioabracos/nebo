# offline-distributed — event.metadata()

Local event/convergence model; no network or distributed availability guarantee; original observation: identity schema causality hash and provenance remain immutable; SDK bounds: {"MAX_BATCH":256,"MAX_BYTES":1048576,"MAX_CHANGES":4096,"MAX_EVENTS":4096,"MAX_QUEUE":256,"MAX_REPLICAS":16}

```text
Identity: owner:G033:G033-S01-07 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
DomainEvent.metadata(self) -> Mapping[str, Any]
```
...
[
  ...
  {
    "excerpt": "82: ok(\"positive\", \"store.snapshot\", snapshot.version == 2 and snapshot.prefix_hash == second.content_hash)\n83: metadata = first.metadata()\n84: ok(\"positive\", \"event.metadata\", metadata[\"id\"] == \"evt-A\" and metadata[\"provenance\"][\"device\"] == \"device-A\")\n85: verification = store.verify()\n86: ok(\"positive\", \"store.verify\", verification[\"verified\"] and verification[\"events\"] == 2)",
