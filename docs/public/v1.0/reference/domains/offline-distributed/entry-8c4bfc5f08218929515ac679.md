# offline-distributed — Conflict.values()

Local event/convergence model; no network or distributed availability guarantee; original observation: two concurrent candidate values remain explicitly visible; SDK bounds: {"MAX_BATCH":256,"MAX_BYTES":1048576,"MAX_CHANGES":4096,"MAX_EVENTS":4096,"MAX_QUEUE":256,"MAX_REPLICAS":16}

```text
Identity: owner:G033:G033-S06-01 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Conflict.values(self) -> Mapping[str, Any]
```
...
[
  ...
  {
    "excerpt": "196:                     {\"replica-A:1\": {\"actor\": \"A\"}, \"replica-B:1\": {\"actor\": \"B\"}})\n197: values = conflict.values()\n198: ok(\"positive\", \"Conflict.values\", tuple(values.values()) == (71, 73))\n199: resolution = conflict.resolve(\"choose:replica-B:1\")\n200: ok(\"positive\", \"conflict.resolve\", resolution[\"value\"] == 73 and len(resolution[\"parents\"]) == 2)",
