# offline-distributed — replica.verifyConvergence(peers)

Local event/convergence model; no network or distributed availability guarantee; original observation: canonical state hashes agree after sync and compaction; SDK bounds: {"MAX_BATCH":256,"MAX_BYTES":1048576,"MAX_CHANGES":4096,"MAX_EVENTS":4096,"MAX_QUEUE":256,"MAX_REPLICAS":16}

```text
Identity: owner:G033:G033-S06-07 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Replica.verifyConvergence(self, peers: Iterable['Replica']) -> Mapping[str, Any]
```
