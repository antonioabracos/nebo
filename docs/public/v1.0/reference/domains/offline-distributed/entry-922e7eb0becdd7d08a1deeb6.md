# offline-distributed — session.resume()

Local event/convergence model; no network or distributed availability guarantee; original observation: valid checkpoint resumes the same endpoint pair once; SDK bounds: {"MAX_BATCH":256,"MAX_BYTES":1048576,"MAX_CHANGES":4096,"MAX_EVENTS":4096,"MAX_QUEUE":256,"MAX_REPLICAS":16}

```text
Identity: owner:G033:G033-S05-07 (DOMAIN_CONTRACT_ID)
Edition: 1
...
[
  ...
  {
    "excerpt": "184: ok(\"positive\", \"session.pause\", len(pause_token) == 64 and session.report()[\"paused\"])\n185: ok(\"positive\", \"session.resume\", session.resume() and not session.report()[\"paused\"])\n186: report = session.report()\n187: ok(\"positive\", \"session.report\", report[\"transport\"] == \"in_process\"\n188:    and report[\"externalNetwork\"] is False and report[\"converged\"])",
