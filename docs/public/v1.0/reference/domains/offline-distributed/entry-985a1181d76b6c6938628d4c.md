# offline-distributed — causal.explainOrder(a,b)

Local event/convergence model; no network or distributed availability guarantee; original observation: direct parent relationship explains before order; SDK bounds: {"MAX_BATCH":256,"MAX_BYTES":1048576,"MAX_CHANGES":4096,"MAX_EVENTS":4096,"MAX_QUEUE":256,"MAX_REPLICAS":16}

```text
Identity: owner:G033:G033-S04-08 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
_Causal.explainOrder(self, a: Change, b: Change) -> Mapping[str, Any]
```
...
[
  ...
  {
    "excerpt": "158: ok(\"positive\", \"causal.ready\", causal.ready(parent, VersionVector.new()))\n159: order = causal.explainOrder(parent, child)\n160: ok(\"positive\", \"causal.explainOrder\", order[\"relation\"] == \"before\" and order[\"reason\"] == \"causal-parent\")\n161: ok(\"boundary\", \"clock-logical-tie\", new_clock.tick(7) == (7, 1, \"causal-clock\"))\n162: transcript.append((observed_vector.items(), other_vector.items(), stable.items(), thaw(order)))",
