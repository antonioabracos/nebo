# offline-distributed — SyncSession.open(local,remote,policy)

Local event/convergence model; no network or distributed availability guarantee; original observation: two distinct replicas open one bounded local session; SDK bounds: {"MAX_BATCH":256,"MAX_BYTES":1048576,"MAX_CHANGES":4096,"MAX_EVENTS":4096,"MAX_QUEUE":256,"MAX_REPLICAS":16}

```text
Identity: owner:G033:G033-S05-01 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
SyncSession.open(local: Replica, remote: Replica, policy: Mapping[str, Any]) -> 'SyncSession'
```
