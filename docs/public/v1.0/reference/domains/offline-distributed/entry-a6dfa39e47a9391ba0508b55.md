# offline-distributed — session.push(limit)

Local event/convergence model; no network or distributed availability guarantee; original observation: two local changes enter remote state atomically; SDK bounds: {"MAX_BATCH":256,"MAX_BYTES":1048576,"MAX_CHANGES":4096,"MAX_EVENTS":4096,"MAX_QUEUE":256,"MAX_REPLICAS":16}

```text
Identity: owner:G033:G033-S05-04 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
SyncSession.push(self, limit: Mapping[str, int] | None=None) -> Mapping[str, Any]
```
...
[
  ...
  {
    "excerpt": "111: peer_session = SyncSession.open(replica_a, replica_b, {\"maxChanges\": 4, \"maxBytes\": 4096})\n112: peer_session.push()\n113: ok(\"positive\", \"replica.peers\", replica_a.peers()[\"replica-B\"][\"replica-A\"] == 1)\n114: status = gap.status()\n115: ok(\"positive\", \"replica.status\", status[\"changes\"] == 2 and status[\"pending\"] == 0)",
