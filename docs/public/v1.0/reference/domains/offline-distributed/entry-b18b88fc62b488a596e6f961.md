# offline-distributed — version.observe(replicaId,counter)

Local event/convergence model; no network or distributed availability guarantee; original observation: monotonic observation records replica-A counter one; SDK bounds: {"MAX_BATCH":256,"MAX_BYTES":1048576,"MAX_CHANGES":4096,"MAX_EVENTS":4096,"MAX_QUEUE":256,"MAX_REPLICAS":16}

```text
Identity: owner:G033:G033-S04-02 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
VersionVector.observe(self, replicaId: str, counter: int) -> 'VersionVector'
```
...
[
  ...
  {
    "excerpt": "148: ok(\"positive\", \"VersionVector.new\", empty_vector.items() == ())\n149: observed_vector = empty_vector.observe(\"replica-A\", 1).observe(\"replica-B\", 0)\n150: ok(\"positive\", \"version.observe\", observed_vector[\"replica-A\"] == 1)\n151: other_vector = VersionVector({\"replica-A\": 0, \"replica-B\": 1})\n152: ok(\"positive\", \"version.compare\", observed_vector.compare(other_vector) == \"concurrent\")",
