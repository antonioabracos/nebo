# offline-distributed — session.syncBoth()

Local event/convergence model; no network or distributed availability guarantee; original observation: bounded bidirectional rounds reach identical materialized state; SDK bounds: {"MAX_BATCH":256,"MAX_BYTES":1048576,"MAX_CHANGES":4096,"MAX_EVENTS":4096,"MAX_QUEUE":256,"MAX_REPLICAS":16}

```text
Identity: owner:G033:G033-S05-05 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
SyncSession.syncBoth(self) -> Mapping[str, Any]
```
...
[
  ...
  {
    "excerpt": "180: ok(\"positive\", \"session.push\", pushed[\"changes\"] == 2 and sync_right.materialized()[\"left\"] == 41)\n181: sync_report = session.syncBoth()\n182: ok(\"positive\", \"session.syncBoth\", sync_report[\"converged\"] and sync_report[\"rounds\"] >= 1)\n183: pause_token = session.pause()\n184: ok(\"positive\", \"session.pause\", len(pause_token) == 64 and session.report()[\"paused\"])",
