# offline-distributed — replica.pending()

Local event/convergence model; no network or distributed availability guarantee; original observation: out-of-order child remains visible until its parent arrives; SDK bounds: {"MAX_BATCH":256,"MAX_BYTES":1048576,"MAX_CHANGES":4096,"MAX_EVENTS":4096,"MAX_QUEUE":256,"MAX_REPLICAS":16}

```text
Identity: owner:G033:G033-S02-05 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Replica.pending(self) -> tuple[Change, ...]
```
...
[
  ...
  {
    "excerpt": "108: gap.apply((child,))\n109: ok(\"positive\", \"replica.pending\", gap.pending() == (child,))\n110: gap.apply((parent,))\n111: peer_session = SyncSession.open(replica_a, replica_b, {\"maxChanges\": 4, \"maxBytes\": 4096})\n112: peer_session.push()",
