# offline-distributed — replica.stableVersion()

Local event/convergence model; no network or distributed availability guarantee; original observation: peer minimum proves replica-A counter one stable; SDK bounds: {"MAX_BATCH":256,"MAX_BYTES":1048576,"MAX_CHANGES":4096,"MAX_EVENTS":4096,"MAX_QUEUE":256,"MAX_REPLICAS":16}

```text
Identity: owner:G033:G033-S04-06 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Replica.stableVersion(self) -> VersionVector
```
...
[
  ...
  {
    "excerpt": "155: ok(\"positive\", \"change.causalParents\", child.causalParents() == (\"upstream:1\",))\n156: stable = replica_a.stableVersion()\n157: ok(\"positive\", \"replica.stableVersion\", stable[\"replica-A\"] == 1)\n158: ok(\"positive\", \"causal.ready\", causal.ready(parent, VersionVector.new()))\n159: order = causal.explainOrder(parent, child)",
...
- runtime/crdt/crdt_list.asm — SHA-256 866def7cb71ee2b0d25b021c479116d5f960de2212d106a1d6812af49c76942a
