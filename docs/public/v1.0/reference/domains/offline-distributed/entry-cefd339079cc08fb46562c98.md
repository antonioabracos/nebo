# offline-distributed — Replica.open(identity,storage,policy)

Local event/convergence model; no network or distributed availability guarantee; original observation: replica retains identity optional storage schemas and limits; SDK bounds: {"MAX_BATCH":256,"MAX_BYTES":1048576,"MAX_CHANGES":4096,"MAX_EVENTS":4096,"MAX_QUEUE":256,"MAX_REPLICAS":16}

```text
Identity: owner:G033:G033-S02-01 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Replica.open(identity: str, storage: EventStore | None, policy: Mapping[str, Any]) -> 'Replica'
```
