# offline-distributed — store.replay(streamId,reducer)

Local event/convergence model; no network or distributed availability guarantee; original observation: pure reducer independently derives state twenty-one; SDK bounds: {"MAX_BATCH":256,"MAX_BYTES":1048576,"MAX_CHANGES":4096,"MAX_EVENTS":4096,"MAX_QUEUE":256,"MAX_REPLICAS":16}

```text
Identity: owner:G033:G033-S01-04 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
EventStore.replay(self, streamId: str, reducer: Mapping[str, Any]) -> Any
```
