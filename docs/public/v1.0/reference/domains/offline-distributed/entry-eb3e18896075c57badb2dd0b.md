# offline-distributed — store.subscribe(filter)

Local event/convergence model; no network or distributed availability guarantee; original observation: bounded subscriber observes both matching events once; SDK bounds: {"MAX_BATCH":256,"MAX_BYTES":1048576,"MAX_CHANGES":4096,"MAX_EVENTS":4096,"MAX_QUEUE":256,"MAX_REPLICAS":16}

```text
Identity: owner:G033:G033-S01-06 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
EventStore.subscribe(self, filter: Mapping[str, Any]) -> Subscription
```
...
[
  ...
  {
    "excerpt": "56: store_cap = Capability.issue(\"event-store\", (\"open\", \"append\", \"read\", \"snapshot\",\n57:                                                \"subscribe\", \"verify\"), \"oracle\")\n58: \n59: \n60: def event(identity: str, actor: str, value: int, physical: int,",
