# offline-distributed — counter.increment(amount)

Local event/convergence model; no network or distributed availability guarantee; original observation: positive thirteen and negative two produce value eleven; SDK bounds: {"MAX_BATCH":256,"MAX_BYTES":1048576,"MAX_CHANGES":4096,"MAX_EVENTS":4096,"MAX_QUEUE":256,"MAX_REPLICAS":16}

```text
Identity: owner:G033:G033-S03-02 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
CrdtCounter.increment(self, amount: int) -> 'CrdtCounter'
```
...
[
  ...
  {
    "excerpt": "124: ok(\"positive\", \"CrdtCounter.new\", counter_a.replica == \"counter-A\" and counter_a.value() == 0)\n125: counter_a.increment(13).increment(-2)\n126: ok(\"positive\", \"counter.increment\", counter_a.value() == 11)\n127: set_a = CrdtSet.new({\"mode\": \"add_wins\", \"replicaId\": \"set-A\"}).add(\"nebula\")\n128: ok(\"positive\", \"CrdtSet.new\", set_a.mode == \"add_wins\" and set_a.value() == frozenset((\"nebula\",)))",
