# offline-distributed — replica.audit()

Local event/convergence model; no network or distributed availability guarantee; original observation: immutable timeline reports one repair zero gaps and no external network; SDK bounds: {"MAX_BATCH":256,"MAX_BYTES":1048576,"MAX_CHANGES":4096,"MAX_EVENTS":4096,"MAX_QUEUE":256,"MAX_REPLICAS":16}

```text
Identity: owner:G033:G033-S06-08 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Replica.audit(self) -> Mapping[str, Any]
```
...
[
  ...
  {
    "excerpt": "221: ok(\"positive\", \"replica.verifyConvergence\", convergence[\"converged\"])\n222: audit = repair_replica.audit()\n223: ok(\"positive\", \"replica.audit\", audit[\"repairs\"] == 1 and audit[\"externalNetwork\"] is False)\n224: ok(\"failure_atomicity\", \"stable-compaction-preserves-state\",\n225:    thaw(repair_replica.materialized()) == before_compaction)",
