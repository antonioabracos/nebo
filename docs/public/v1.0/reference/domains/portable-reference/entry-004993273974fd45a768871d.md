# portable-reference — edge.state(storeCapability)

Bytecode/component/eBPF reference only; no native non-x86 target backend certification; original observation: state requires exact delegated storage capability and positive budget; SDK bounds: {"MAX_BPF_INSTRUCTIONS":4096,"MAX_BPF_MAP_ENTRIES":65536,"MAX_CALLS":4096,"MAX_COMPONENTS":32,"MAX_EDGE_STATE_BYTES":1048576,"MAX_EXPORTS":128,"MAX_IMPORTS":128,"MAX_INPUT_BYTES":1048576,"MAX_INTERFACE_FUNCTIONS":128,"MAX_MEMORY_PAGES":4096,"MAX_SOURCE_BYTES":1048576,"MAX_TABLE_ITEMS":65536}

```text
Identity: owner:G039:G039-S05-04 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
EdgeFunction.state(self, storeCapability: Capability) -> 'EdgeFunction'
```
...
[
  ...
  {
    "excerpt": "124: passed(\"edge.budget(options)\", b'\"calls\":4' in edge.snapshot())\n125: edge.state(Capability.issue(\"edge.state\", \"store:local-memory\"))\n126: passed(\"edge.state(storeCapability)\", b\"store:local-memory\" in edge.snapshot())\n127: handled = edge.handle({\"value\": 13})\n128: passed(\"edge.handle(event)\", handled == {\"observed\": 20})",
