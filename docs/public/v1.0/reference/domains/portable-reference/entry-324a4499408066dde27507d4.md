# portable-reference — host.run(component,input)

Bytecode/component/eBPF reference only; no native non-x86 target backend certification; original observation: input size imports and delegated capabilities govern execution; SDK bounds: {"MAX_BPF_INSTRUCTIONS":4096,"MAX_BPF_MAP_ENTRIES":65536,"MAX_CALLS":4096,"MAX_COMPONENTS":32,"MAX_EDGE_STATE_BYTES":1048576,"MAX_EXPORTS":128,"MAX_IMPORTS":128,"MAX_INPUT_BYTES":1048576,"MAX_INTERFACE_FUNCTIONS":128,"MAX_MEMORY_PAGES":4096,"MAX_SOURCE_BYTES":1048576,"MAX_TABLE_ITEMS":65536}

```text
Identity: owner:G039:G039-S03-06 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
WasmHost.run(self, component: Component, input_: Any) -> ComponentInstance
```
...
[
  ...
  {
    "excerpt": "94: passed(\"WasmHost.new(bindings, budgets)\", host.metrics()[\"calls\"] == 0)\n95: host_instance = host.run(component, {\"reading\": 39})\n96: passed(\"host.run(component, input)\", host.metrics()[\"calls\"] == 1 and not host_instance.cancelled)\n97: host.cancel(host_instance)\n98: passed(\"host.cancel(instance)\", host_instance.cancelled)",
