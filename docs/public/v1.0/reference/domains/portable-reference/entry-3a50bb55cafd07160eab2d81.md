# portable-reference — edge.runLocal(event)

Bytecode/component/eBPF reference only; no native non-x86 target backend certification; original observation: local execution reports input-dependent result and no network; SDK bounds: {"MAX_BPF_INSTRUCTIONS":4096,"MAX_BPF_MAP_ENTRIES":65536,"MAX_CALLS":4096,"MAX_COMPONENTS":32,"MAX_EDGE_STATE_BYTES":1048576,"MAX_EXPORTS":128,"MAX_IMPORTS":128,"MAX_INPUT_BYTES":1048576,"MAX_INTERFACE_FUNCTIONS":128,"MAX_MEMORY_PAGES":4096,"MAX_SOURCE_BYTES":1048576,"MAX_TABLE_ITEMS":65536}

```text
Identity: owner:G039:G039-S05-06 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
EdgeFunction.runLocal(self, event: Any) -> Mapping[str, Any]
```
...
[
  ...
  {
    "excerpt": "129: passed(\"edge.snapshot()\", edge.snapshot() == edge.snapshot() and len(edge.snapshot()) > 64)\n130: local = edge.runLocal({\"value\": 23})\n131: passed(\"edge.runLocal(event)\", local[\"result\"] == {\"observed\": 30} and local[\"network\"] is False)\n132: passed(\"edge.metrics()\", edge.metrics()[\"calls\"] == 2 and edge.metrics()[\"inputBytes\"] == 24)\n133: policy = edge.verifyPolicy({\"capabilities\": [\"edge.state\"], \"network\": False, \"maxCalls\": 4})",
