# portable-reference — program.load(capability)

Bytecode/component/eBPF reference only; no native non-x86 target backend certification; original observation: verified capability-gated load reports simulated local only; SDK bounds: {"MAX_BPF_INSTRUCTIONS":4096,"MAX_BPF_MAP_ENTRIES":65536,"MAX_CALLS":4096,"MAX_COMPONENTS":32,"MAX_EDGE_STATE_BYTES":1048576,"MAX_EXPORTS":128,"MAX_IMPORTS":128,"MAX_INPUT_BYTES":1048576,"MAX_INTERFACE_FUNCTIONS":128,"MAX_MEMORY_PAGES":4096,"MAX_SOURCE_BYTES":1048576,"MAX_TABLE_ITEMS":65536}

```text
Identity: owner:G039:G039-S04-06 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
BpfProgram.load(self, capability: Capability) -> Mapping[str, Any]
```
...
[
  ...
  {
    "excerpt": "112: passed(\"program.verify()\", verification[\"forwardOnly\"] is True and verification[\"kernelLoad\"] == \"NOT_EXECUTED\")\n113: load = program.load(Capability.issue(\"bpf.load\", \"loader:simulated\"))\n114: passed(\"program.load(capability)\", load[\"loaded\"] == \"SIMULATED_LOCAL_ONLY\")\n115: passed(\"program.events()\", len(program.events()) == 3)\n116: program.detach()",
