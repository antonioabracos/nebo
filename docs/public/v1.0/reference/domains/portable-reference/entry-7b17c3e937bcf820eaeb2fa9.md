# portable-reference — SandboxedRandom.fromCapability(capability)

Bytecode/component/eBPF reference only; no native non-x86 target backend certification; original observation: deterministic random adapter requires exact seeded capability; SDK bounds: {"MAX_BPF_INSTRUCTIONS":4096,"MAX_BPF_MAP_ENTRIES":65536,"MAX_CALLS":4096,"MAX_COMPONENTS":32,"MAX_EDGE_STATE_BYTES":1048576,"MAX_EXPORTS":128,"MAX_IMPORTS":128,"MAX_INPUT_BYTES":1048576,"MAX_INTERFACE_FUNCTIONS":128,"MAX_MEMORY_PAGES":4096,"MAX_SOURCE_BYTES":1048576,"MAX_TABLE_ITEMS":65536}

```text
Identity: owner:G039:G039-S03-03 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
_CapabilityAdapter.fromCapability(cls, capability: Capability) -> '_CapabilityAdapter'
```
...
[
  ...
  {
    "excerpt": "83: # S03: typed capability adapters, bounded local host execution and metrics.\n84: fs = SandboxedFs.fromCapability(Capability.issue(\"fs.read\", \"fixture:readonly\"))\n85: passed(\"SandboxedFs.fromCapability(capability)\", fs.capability.kind == \"fs.read\")\n86: clock = SandboxedClock.fromCapability(Capability.issue(\"clock.monotonic\", \"clock:logical\"))\n87: passed(\"SandboxedClock.fromCapability(capability)\", clock.capability.kind == \"clock.monotonic\")",
