# portable-reference — artifact.reproducibilityReport()

Bytecode/component/eBPF reference only; no native non-x86 target backend certification; original observation: canonical ordered components yield stable bundle and manifest digests; SDK bounds: {"MAX_BPF_INSTRUCTIONS":4096,"MAX_BPF_MAP_ENTRIES":65536,"MAX_CALLS":4096,"MAX_COMPONENTS":32,"MAX_EDGE_STATE_BYTES":1048576,"MAX_EXPORTS":128,"MAX_IMPORTS":128,"MAX_INPUT_BYTES":1048576,"MAX_INTERFACE_FUNCTIONS":128,"MAX_MEMORY_PAGES":4096,"MAX_SOURCE_BYTES":1048576,"MAX_TABLE_ITEMS":65536}

```text
Identity: owner:G039:G039-S06-03 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
PortableArtifact.reproducibilityReport(self) -> Mapping[str, Any]
```
...
[
  ...
  {
    "excerpt": "140: passed(\"artifact.verify()\", artifact.verify()[\"valid\"] is True)\n141: reproducibility = artifact.reproducibilityReport()\n142: passed(\"artifact.reproducibilityReport()\", reproducibility[\"reproducible\"] is True and len(reproducibility[\"bundleSha256\"]) == 64)\n143: capability_report = artifact.capabilityReport()\n144: passed(\"artifact.capabilityReport()\", capability_report[\"required\"] == (\"console.write\",) and capability_report[\"ambientAuthority\"] is False)",
