# portable-reference — program.attach(hook,capability)

Bytecode/component/eBPF reference only; no native non-x86 target backend certification; original observation: attach consumes exact capability and records the chosen hook; SDK bounds: {"MAX_BPF_INSTRUCTIONS":4096,"MAX_BPF_MAP_ENTRIES":65536,"MAX_CALLS":4096,"MAX_COMPONENTS":32,"MAX_EDGE_STATE_BYTES":1048576,"MAX_EXPORTS":128,"MAX_IMPORTS":128,"MAX_INPUT_BYTES":1048576,"MAX_INTERFACE_FUNCTIONS":128,"MAX_MEMORY_PAGES":4096,"MAX_SOURCE_BYTES":1048576,"MAX_TABLE_ITEMS":65536}

```text
Identity: owner:G039:G039-S04-03 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
BpfProgram.attach(self, hook: str, capability: Capability) -> 'BpfProgram'
```
...
[
  ...
  {
    "excerpt": "107:                    {\"op\": \"map_lookup\", \"map\": 0}, {\"op\": \"exit\"}], [bpf_map])\n108: attach_cap = Capability.issue(\"bpf.attach\", \"tracepoint:synthetic\")\n109: program.attach(\"tracepoint/nebo_g039\", attach_cap)\n110: passed(\"program.attach(hook, capability)\", program.events()[-1][\"event\"] == \"attached\")\n111: verification = program.verify()",
