# portable-reference — EdgeFunction.from(function,interface)

Bytecode/component/eBPF reference only; no native non-x86 target backend certification; original observation: callable bytecode signature and interface determine edge identity; SDK bounds: {"MAX_BPF_INSTRUCTIONS":4096,"MAX_BPF_MAP_ENTRIES":65536,"MAX_CALLS":4096,"MAX_COMPONENTS":32,"MAX_EDGE_STATE_BYTES":1048576,"MAX_EXPORTS":128,"MAX_IMPORTS":128,"MAX_INPUT_BYTES":1048576,"MAX_INTERFACE_FUNCTIONS":128,"MAX_MEMORY_PAGES":4096,"MAX_SOURCE_BYTES":1048576,"MAX_TABLE_ITEMS":65536}

```text
Identity: owner:G039:G039-S05-01 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
EdgeFunction.fromFunction(function: Callable[[Any], Any], interface: Interface) -> 'EdgeFunction'
```
...
[
  ...
  {
    "excerpt": "120: edge_interface = Interface.define(\"edge.events\", \"3.1.4\", {\"handle\": {\"params\": [\"event\"], \"result\": \"response\"}})\n121: edge = EdgeFunction.fromFunction(edge_handler, edge_interface)\n122: passed(\"EdgeFunction.from(function, interface)\", len(edge.function_id) == 64 and callable(getattr(EdgeFunction, \"from\")))\n123: edge.budget({\"calls\": 4, \"inputBytes\": 512, \"stateBytes\": 4096})\n124: passed(\"edge.budget(options)\", b'\"calls\":4' in edge.snapshot())",
