# probabilistic — Mcmc.nuts(options)

Bounded seeded CPU probability model; no statistical certification or unlimited sampling; original observation: bounded depth-three no-u-turn kernel; SDK bounds: {"MAX_ACTIONS":1024,"MAX_CATEGORIES":4096,"MAX_CHAINS":16,"MAX_DRAWS":100000,"MAX_EXACT_STATES":1000000,"MAX_GRAPH_NODES":1024,"MAX_LATENTS":256,"MAX_OUTCOMES":4096,"MAX_PARTICLES":100000,"MAX_PLATE_SIZE":4096}

```text
Identity: owner:G037:G037-S04-03 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Mcmc.nuts(cls, options: Mapping[str, Any] | None=None) -> McmcKernel
```
...
[
  ...
  {
    "excerpt": "116: ok(\"positive\", \"Mcmc.hamiltonian\", hamiltonian.kind == \"hamiltonian\" and hamiltonian.leapfrog_steps == 4)\n117: nuts = Mcmc.nuts({\"draws\": 32, \"warmup\": 8, \"seed\": 33, \"stepSize\": 0.08, \"maxTreeDepth\": 3})\n118: ok(\"positive\", \"Mcmc.nuts\", nuts.kind == \"nuts\" and nuts.leapfrog_steps == 8)\n119: continuous_model = ProbModel.new(\"location\").latent(\"theta\", Distribution.normal(2, 1))\n120: samples = continuous_model.inferMcmc(metropolis, 4)",
...
- runtime/probabilistic/mcmc_smc.asm — SHA-256 fe9e367b4b398ac84785e6c377c7d2d3083799f4631e3401333ed6879adf12dc
