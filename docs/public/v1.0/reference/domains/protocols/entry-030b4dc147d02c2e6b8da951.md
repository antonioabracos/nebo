# protocols — rpc.metrics()

In-process protocol model only; no HTTP2/QUIC network transport or service deployment; original observation: three calls and one deduplicated stored result are observed; SDK bounds: {"MAX_DEDUPLICATION":256,"MAX_FIELDS":64,"MAX_MESSAGE_BYTES":65536,"MAX_METADATA_BYTES":4096,"MAX_REPEATED":4096,"MAX_RETRIES":8,"MAX_STREAM_CAPACITY":64}

```text
Identity: owner:G040:G040-S06-07 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
RpcRuntime.metrics(self) -> Mapping[str, int]
```
...
[
  ...
  {
    "excerpt": "109:                                   \"idempotencyKey\": \"echo-73\"})\n110: ok(\"positive\", \"client.call\", call.result is value and client_endpoint.runtime.metrics()[\"calls\"] == 1)\n111: metadata = call.metadata()\n112: ok(\"positive\", \"call.metadata\", metadata[\"transport\"] == \"in_memory\"\n113:    and metadata[\"requestId\"] == \"rpc-00000001\" and metadata[\"attempts\"] == 1)",
