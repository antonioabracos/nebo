# protocols — InMemoryTransport.pair(options)

In-process protocol model only; no HTTP2/QUIC network transport or service deployment; original observation: two peer endpoints share capacity six and logical runtime; SDK bounds: {"MAX_DEDUPLICATION":256,"MAX_FIELDS":64,"MAX_MESSAGE_BYTES":65536,"MAX_METADATA_BYTES":4096,"MAX_REPEATED":4096,"MAX_RETRIES":8,"MAX_STREAM_CAPACITY":64}

```text
Identity: owner:G040:G040-S07-04 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
InMemoryTransport.pair(options: Mapping[str, Any]) -> tuple[InMemoryEndpoint, InMemoryEndpoint]
```
...
[
  ...
  {
    "excerpt": "102: ok(\"positive\", \"service.expose\", server.service is service and server.policy[\"mode\"] == \"in_process\")\n103: client_endpoint, server_endpoint = InMemoryTransport.pair({\"capacity\": 4})\n104: server.bind(server_endpoint)\n105: capability = Capability.issue(\"LocalRpc\", (\"rpc\",))\n106: client = RpcClient.connect(client_endpoint, protocol, capability)",
