# protocols — decoder.finish()

In-process protocol model only; no HTTP2/QUIC network transport or service deployment; original observation: complete empty decoder tail closes exactly once; SDK bounds: {"MAX_DEDUPLICATION":256,"MAX_FIELDS":64,"MAX_MESSAGE_BYTES":65536,"MAX_METADATA_BYTES":4096,"MAX_REPEATED":4096,"MAX_RETRIES":8,"MAX_STREAM_CAPACITY":64}

```text
Identity: owner:G040:G040-S02-07 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
FrameDecoder.finish(self) -> tuple[Frame, ...]
```
...
[
  ...
  {
    "excerpt": "84: ok(\"positive\", \"decoder.push\", len(observed) == 1 and observed[0].payload == wire)\n85: ok(\"positive\", \"decoder.finish\", decoder.finish() == ())\n86: compressed = frame.compress(\"zlib\")\n87: ok(\"positive\", \"frame.compress\", compressed.codec == \"zlib\" and len(compressed.to_bytes()) < len(encoded_frame))\n88: transcript.append((wire.hex(), encoded_frame.hex(), checksum, compressed.to_bytes().hex()))",
