# protocols — client.openStream(method,options)

In-process protocol model only; no HTTP2/QUIC network transport or service deployment; original observation: stream owns capacity two under endpoint capacity; SDK bounds: {"MAX_DEDUPLICATION":256,"MAX_FIELDS":64,"MAX_MESSAGE_BYTES":65536,"MAX_METADATA_BYTES":4096,"MAX_REPEATED":4096,"MAX_RETRIES":8,"MAX_STREAM_CAPACITY":64}

```text
Identity: owner:G040:G040-S04-04 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
RpcClient.openStream(self, method: RpcMethod, options: Mapping[str, Any]) -> 'RpcStream'
```
...
[
  ...
  {
    "excerpt": "120: ok(\"positive\", \"RpcMethod.bidirectional\", bidi.shape == \"bidirectional\")\n121: stream = client.openStream(bidi, {\"capacity\": 2})\n122: ok(\"positive\", \"client.openStream\", stream.capacity == 2 and stream.method is bidi)\n123: ok(\"positive\", \"stream.send\", stream.send(value) == 1)\n124: received = stream.receive()",
