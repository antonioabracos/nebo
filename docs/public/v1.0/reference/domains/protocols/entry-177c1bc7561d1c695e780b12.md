# protocols — gateway.translate(from,to)

In-process protocol model only; no HTTP2/QUIC network transport or service deployment; original observation: translation proceeds only after backward compatibility; SDK bounds: {"MAX_DEDUPLICATION":256,"MAX_FIELDS":64,"MAX_MESSAGE_BYTES":65536,"MAX_METADATA_BYTES":4096,"MAX_REPEATED":4096,"MAX_RETRIES":8,"MAX_STREAM_CAPACITY":64}

```text
Identity: owner:G040:G040-S07-06 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Gateway.translate(self, from_: Protocol, to: Protocol) -> Mapping[str, Any]
```
...
[
  ...
  {
    "excerpt": "202: ok(\"positive\", \"Gateway.map\", gateway.protocol is previous and gateway.route == \"/v1/echo\")\n203: translation = gateway.translate(previous, protocol)\n204: ok(\"positive\", \"gateway.translate\", translation[\"compatible\"]\n205:    and translation[\"from\"] == 1 and translation[\"to\"] == 2)\n206: capabilities = transport_capabilities()",
