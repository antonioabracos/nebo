# protocols — Frame.wrap(payload,metadata)

In-process protocol model only; no HTTP2/QUIC network transport or service deployment; original observation: payload and explicit request metadata enter one frame; SDK bounds: {"MAX_DEDUPLICATION":256,"MAX_FIELDS":64,"MAX_MESSAGE_BYTES":65536,"MAX_METADATA_BYTES":4096,"MAX_REPEATED":4096,"MAX_RETRIES":8,"MAX_STREAM_CAPACITY":64}

```text
Identity: owner:G040:G040-S02-03 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Frame.wrap(payload: bytes, metadata: Mapping[str, Any]) -> 'Frame'
```
...
[
  ...
  {
    "excerpt": "74: ok(\"positive\", \"protocol.decode\", decoded.schema is echo and dict(decoded.values) == dict(value.values))\n75: frame = Frame.wrap(wire, {\"request\": 104, \"contentType\": \"application/nebo\"})\n76: ok(\"positive\", \"Frame.wrap\", frame.payload == wire and frame.metadata[\"request\"] == 104)\n77: checksum = frame.checksum()\n78: ok(\"positive\", \"frame.checksum\", checksum == hashlib.sha256(frame._encoded_content()).hexdigest())",
