# protocols — protocol.compatibility(previous)

In-process protocol model only; no HTTP2/QUIC network transport or service deployment; original observation: optional additions remain backward compatible; SDK bounds: {"MAX_DEDUPLICATION":256,"MAX_FIELDS":64,"MAX_MESSAGE_BYTES":65536,"MAX_METADATA_BYTES":4096,"MAX_REPEATED":4096,"MAX_RETRIES":8,"MAX_STREAM_CAPACITY":64}

```text
Identity: owner:G040:G040-S01-07 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Protocol.compatibility(self, previous: 'Protocol') -> Mapping[str, Any]
```
...
[
  ...
  {
    "excerpt": "62: previous = Protocol.define(\"Echo\", 1).message(previous_message)\n63: compatibility = protocol.compatibility(previous)\n64: ok(\"positive\", \"protocol.compatibility\", compatibility[\"compatible\"]\n65:    and compatibility[\"classification\"] == \"backward\")\n66: reserved_protocol = Protocol.define(\"Reserved\", 1).reserveField(21)",
