# protocols — Message.define(name,fields)

In-process protocol model only; no HTTP2/QUIC network transport or service deployment; original observation: fields are ordered by stable numeric identity; SDK bounds: {"MAX_DEDUPLICATION":256,"MAX_FIELDS":64,"MAX_MESSAGE_BYTES":65536,"MAX_METADATA_BYTES":4096,"MAX_REPEATED":4096,"MAX_RETRIES":8,"MAX_STREAM_CAPACITY":64}

```text
Identity: owner:G040:G040-S01-02 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Message.define(name: str, fields: Iterable[Field]) -> 'Message'
```
...
[
  ...
  {
    "excerpt": "46: # S01 — schemas retain stable field identities and explicit compatibility.\n47: protocol = Protocol.define(\"Echo\", 2)\n48: ok(\"positive\", \"Protocol.define\", protocol.name == \"Echo\" and protocol.version == 2)\n49: required = Field.required(\"value\", \"text\", 1)\n50: ok(\"positive\", \"Field.required\", required.mode == \"required\" and required.id == 1)",
...
- runtime/protocol/schema.asm — SHA-256 ae9353ca899526fc41d1febd78e9235d8b54279d0b252f582abd2ddb89a9786f
