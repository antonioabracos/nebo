# protocols — Api.fromProtocol(protocol)

In-process protocol model only; no HTTP2/QUIC network transport or service deployment; original observation: API model owns one canonical schema SHA-256; SDK bounds: {"MAX_DEDUPLICATION":256,"MAX_FIELDS":64,"MAX_MESSAGE_BYTES":65536,"MAX_METADATA_BYTES":4096,"MAX_REPEATED":4096,"MAX_RETRIES":8,"MAX_STREAM_CAPACITY":64}

```text
Identity: owner:G040:G040-S05-01 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Api.fromProtocol(protocol: Protocol) -> 'Api'
```
...
[
  ...
  {
    "excerpt": "134: # S05 — generation emits hashed artifacts whose Nebo sources use current syntax.\n135: api = Api.fromProtocol(protocol)\n136: ok(\"positive\", \"Api.fromProtocol\", api.protocol is protocol and len(api.schema_hash) == 64)\n137: generated_client = api.generateClient(\"x86_64-nebo\")\n138: ok(\"positive\", \"api.generateClient\", generated_client.kind == \"client\"",
