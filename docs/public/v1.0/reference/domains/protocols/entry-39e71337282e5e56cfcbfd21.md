# protocols — service.expose(implementation,policy)

In-process protocol model only; no HTTP2/QUIC network transport or service deployment; original observation: all declared methods have in-process implementations; SDK bounds: {"MAX_DEDUPLICATION":256,"MAX_FIELDS":64,"MAX_MESSAGE_BYTES":65536,"MAX_METADATA_BYTES":4096,"MAX_REPEATED":4096,"MAX_RETRIES":8,"MAX_STREAM_CAPACITY":64}

```text
Identity: owner:G040:G040-S03-04 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Service.expose(self, implementation: Mapping[str, Callable[[ValidatedMessage], Any]], policy: Mapping[str, Any]) -> 'RpcServer'
```
...
[
  ...
  {
    "excerpt": "100: implementation = {method.name: (lambda request: request) for method in service.methods}\n101: server = service.expose(implementation, {\"mode\": \"in_process\"})\n102: ok(\"positive\", \"service.expose\", server.service is service and server.policy[\"mode\"] == \"in_process\")\n103: client_endpoint, server_endpoint = InMemoryTransport.pair({\"capacity\": 4})\n104: server.bind(server_endpoint)",
