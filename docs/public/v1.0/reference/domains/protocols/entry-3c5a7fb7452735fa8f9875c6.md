# protocols — decoder.push(bytes)

In-process protocol model only; no HTTP2/QUIC network transport or service deployment; original observation: three arbitrary chunks emit exactly one verified frame; SDK bounds: {"MAX_DEDUPLICATION":256,"MAX_FIELDS":64,"MAX_MESSAGE_BYTES":65536,"MAX_METADATA_BYTES":4096,"MAX_REPEATED":4096,"MAX_RETRIES":8,"MAX_STREAM_CAPACITY":64}

```text
Identity: owner:G040:G040-S02-06 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
FrameDecoder.push(self, bytes: bytes) -> tuple[Frame, ...]
```
...
[
  ...
  {
    "excerpt": "82: encoded_frame = frame.to_bytes()\n83: observed = decoder.push(encoded_frame[:17]) + decoder.push(encoded_frame[17:53]) + decoder.push(encoded_frame[53:])\n84: ok(\"positive\", \"decoder.push\", len(observed) == 1 and observed[0].payload == wire)\n85: ok(\"positive\", \"decoder.finish\", decoder.finish() == ())\n86: compressed = frame.compress(\"zlib\")",
