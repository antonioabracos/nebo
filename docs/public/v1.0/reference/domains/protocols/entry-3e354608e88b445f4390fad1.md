# protocols — stream.halfClose()

In-process protocol model only; no HTTP2/QUIC network transport or service deployment; original observation: send side closes monotonically without losing cleanup authority; SDK bounds: {"MAX_DEDUPLICATION":256,"MAX_FIELDS":64,"MAX_MESSAGE_BYTES":65536,"MAX_METADATA_BYTES":4096,"MAX_REPEATED":4096,"MAX_RETRIES":8,"MAX_STREAM_CAPACITY":64}

```text
Identity: owner:G040:G040-S04-07 (DOMAIN_CONTRACT_ID)
Edition: 1
...
[
  ...
  {
    "excerpt": "125: ok(\"positive\", \"stream.receive\", received is value and stream.receive() is None)\n126: ok(\"positive\", \"stream.halfClose\", stream.halfClose() and stream.half_closed)\n127: cancel_stream = client.openStream(server_stream, {\"capacity\": 2})\n128: cancel_stream.send(value)\n129: ok(\"positive\", \"stream.cancel\", cancel_stream.cancel() == 1 and cancel_stream.cancelled",
