# protocols — api.generateServer(target)

In-process protocol model only; no HTTP2/QUIC network transport or service deployment; original observation: current Nebo syntax emits distinct hashed server source; SDK bounds: {"MAX_DEDUPLICATION":256,"MAX_FIELDS":64,"MAX_MESSAGE_BYTES":65536,"MAX_METADATA_BYTES":4096,"MAX_REPEATED":4096,"MAX_RETRIES":8,"MAX_STREAM_CAPACITY":64}

```text
Identity: owner:G040:G040-S05-03 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Api.generateServer(self, target: str) -> GeneratedArtifact
```
...
[
  ...
  {
    "excerpt": "139:    and b\"40.return\" in generated_client.content)\n140: generated_server = api.generateServer(\"x86_64-nebo\")\n141: ok(\"positive\", \"api.generateServer\", generated_server.kind == \"server\"\n142:    and generated_server.content != generated_client.content)\n143: generated_mock = api.generateMock({\"target\": \"x86_64-nebo\", \"seed\": 4005})",
