# protocols — RpcPolicy.retry(policy)

In-process protocol model only; no HTTP2/QUIC network transport or service deployment; original observation: three logical attempts are idempotent-only and bounded at eight; SDK bounds: {"MAX_DEDUPLICATION":256,"MAX_FIELDS":64,"MAX_MESSAGE_BYTES":65536,"MAX_METADATA_BYTES":4096,"MAX_REPEATED":4096,"MAX_RETRIES":8,"MAX_STREAM_CAPACITY":64}

```text
Identity: owner:G040:G040-S06-01 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
RpcPolicy.retry(policy: Mapping[str, Any]) -> Mapping[str, Any]
```
...
[
  ...
  {
    "excerpt": "157: # S06 — bounded resilience forbids unsafe retry and publishes local redacted telemetry.\n158: retry = RpcPolicy.retry({\"attempts\": 3})\n159: ok(\"positive\", \"RpcPolicy.retry\", retry[\"attempts\"] == 3 and retry[\"idempotentOnly\"])\n160: timeout = RpcPolicy.timeout(50)\n161: ok(\"positive\", \"RpcPolicy.timeout\", timeout[\"duration\"] == 50 and timeout[\"clock\"] == \"logical\")",
