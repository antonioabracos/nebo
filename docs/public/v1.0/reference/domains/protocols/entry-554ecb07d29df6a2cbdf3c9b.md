# protocols — RpcMethod.bidirectional(name,input,output)

In-process protocol model only; no HTTP2/QUIC network transport or service deployment; original observation: bidirectional shape retains typed input and output; SDK bounds: {"MAX_DEDUPLICATION":256,"MAX_FIELDS":64,"MAX_MESSAGE_BYTES":65536,"MAX_METADATA_BYTES":4096,"MAX_REPEATED":4096,"MAX_RETRIES":8,"MAX_STREAM_CAPACITY":64}

```text
Identity: owner:G040:G040-S04-03 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
RpcMethod.bidirectional(name: str, input: Message, output: Message) -> 'RpcMethod'
```
...
[
  ...
  {
    "excerpt": "96: client_stream = RpcMethod.clientStream(\"Upload\", echo, echo)\n97: bidi = RpcMethod.bidirectional(\"Converse\", echo, echo)\n98: service = Service.define(\"EchoService\", 2, (unary, server_stream, client_stream, bidi))\n99: ok(\"positive\", \"Service.define\", len(service.methods) == 4 and service.version == 2)\n100: implementation = {method.name: (lambda request: request) for method in service.methods}",
...
- runtime/protocol/stream.asm — SHA-256 8e24cebacbc824ce71430bc66d6494001356d03f9d963f2f0fa6744bab2eb9c7
