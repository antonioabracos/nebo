# protocols — RpcPolicy.bulkhead(limit)

In-process protocol model only; no HTTP2/QUIC network transport or service deployment; original observation: concurrency limit seven is retained; SDK bounds: {"MAX_DEDUPLICATION":256,"MAX_FIELDS":64,"MAX_MESSAGE_BYTES":65536,"MAX_METADATA_BYTES":4096,"MAX_REPEATED":4096,"MAX_RETRIES":8,"MAX_STREAM_CAPACITY":64}

```text
Identity: owner:G040:G040-S06-04 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
RpcPolicy.bulkhead(limit: int) -> Mapping[str, Any]
```
...
[
  ...
  {
    "excerpt": "163: ok(\"positive\", \"RpcPolicy.circuitBreaker\", circuit[\"failures\"] == 4 and circuit[\"cooldown\"] == 12)\n164: bulkhead = RpcPolicy.bulkhead(7)\n165: ok(\"positive\", \"RpcPolicy.bulkhead\", bulkhead[\"limit\"] == 7)\n166: request = Request(value).idempotencyKey(\"stable-echo-104\")\n167: ok(\"positive\", \"request.idempotencyKey\", request.idempotency_key == \"stable-echo-104\")",
