# protocols — stream.send(message)

In-process protocol model only; no HTTP2/QUIC network transport or service deployment; original observation: one typed item enters bounded queue; SDK bounds: {"MAX_DEDUPLICATION":256,"MAX_FIELDS":64,"MAX_MESSAGE_BYTES":65536,"MAX_METADATA_BYTES":4096,"MAX_REPEATED":4096,"MAX_RETRIES":8,"MAX_STREAM_CAPACITY":64}

```text
Identity: owner:G040:G040-S04-05 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
RpcStream.send(self, message: ValidatedMessage) -> int
```
...
[
  ...
  {
    "excerpt": "122: ok(\"positive\", \"client.openStream\", stream.capacity == 2 and stream.method is bidi)\n123: ok(\"positive\", \"stream.send\", stream.send(value) == 1)\n124: received = stream.receive()\n125: ok(\"positive\", \"stream.receive\", received is value and stream.receive() is None)\n126: ok(\"positive\", \"stream.halfClose\", stream.halfClose() and stream.half_closed)",
