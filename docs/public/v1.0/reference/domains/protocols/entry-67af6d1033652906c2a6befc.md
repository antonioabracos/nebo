# protocols — protocol.reserveField(id)

In-process protocol model only; no HTTP2/QUIC network transport or service deployment; original observation: unused field ID twenty-one is reserved once; SDK bounds: {"MAX_DEDUPLICATION":256,"MAX_FIELDS":64,"MAX_MESSAGE_BYTES":65536,"MAX_METADATA_BYTES":4096,"MAX_REPEATED":4096,"MAX_RETRIES":8,"MAX_STREAM_CAPACITY":64}

```text
Identity: owner:G040:G040-S01-08 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Protocol.reserveField(self, id: int) -> 'Protocol'
```
...
[
  ...
  {
    "excerpt": "65:    and compatibility[\"classification\"] == \"backward\")\n66: reserved_protocol = Protocol.define(\"Reserved\", 1).reserveField(21)\n67: ok(\"positive\", \"protocol.reserveField\", reserved_protocol._reserved == {21})\n68: transcript.append((protocol.name, protocol.version, dict(compatibility), tuple(sorted(reserved_protocol._reserved))))\n69: ",
