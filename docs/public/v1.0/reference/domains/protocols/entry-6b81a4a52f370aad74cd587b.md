# protocols — UnixTransport.open(path,capability)

In-process protocol model only; no HTTP2/QUIC network transport or service deployment; original observation: Unix path is capability-bound and environment-limited; SDK bounds: {"MAX_DEDUPLICATION":256,"MAX_FIELDS":64,"MAX_MESSAGE_BYTES":65536,"MAX_METADATA_BYTES":4096,"MAX_REPEATED":4096,"MAX_RETRIES":8,"MAX_STREAM_CAPACITY":64}

```text
Identity: owner:G040:G040-S07-03 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
UnixTransport.open(path: str, capability: Capability) -> Mapping[str, Any]
```
...
[
  ...
  {
    "excerpt": "187: # S07 — backend maturity is factual; only in-memory transport is active.\n188: http2 = Http2Transport.open({\"tls\": True})\n189: ok(\"positive\", \"Http2Transport.open\", not http2[\"available\"]\n190:    and http2[\"maturity\"] == \"UNAVAILABLE\" and http2[\"tls\"] == \"NOT_IMPLEMENTED\")\n191: quic = QuicTransport.open({\"tls\": True})",
...
- runtime/protocol/transport.asm — SHA-256 cadc6a7fe361e80033ce92f19728e9f92ff381bd5e085012c86df0cf1a97f885
