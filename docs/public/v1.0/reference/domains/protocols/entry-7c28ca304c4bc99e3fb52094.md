# protocols — message.validate(value)

In-process protocol model only; no HTTP2/QUIC network transport or service deployment; original observation: typed values become an immutable validated message; SDK bounds: {"MAX_DEDUPLICATION":256,"MAX_FIELDS":64,"MAX_MESSAGE_BYTES":65536,"MAX_METADATA_BYTES":4096,"MAX_REPEATED":4096,"MAX_RETRIES":8,"MAX_STREAM_CAPACITY":64}

```text
Identity: owner:G040:G040-S01-06 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Message.validate(self, value: Mapping[str, Any]) -> ValidatedMessage
```
...
[
  ...
  {
    "excerpt": "56: ok(\"positive\", \"Message.define\", tuple(item.id for item in echo.fields) == (1, 2, 3))\n57: value = echo.validate({\"value\": \"nebula\", \"requestId\": 73, \"tags\": [5, 8, 13]})\n58: ok(\"positive\", \"message.validate\", value.values[\"value\"] == \"nebula\"\n59:    and value.values[\"tags\"] == (5, 8, 13))\n60: protocol.message(echo)",
