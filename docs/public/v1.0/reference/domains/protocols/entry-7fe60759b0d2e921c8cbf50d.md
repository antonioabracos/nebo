# protocols — server.deduplicate(store,policy)

In-process protocol model only; no HTTP2/QUIC network transport or service deployment; original observation: ordered store is bounded to three responses; SDK bounds: {"MAX_DEDUPLICATION":256,"MAX_FIELDS":64,"MAX_MESSAGE_BYTES":65536,"MAX_METADATA_BYTES":4096,"MAX_REPEATED":4096,"MAX_RETRIES":8,"MAX_STREAM_CAPACITY":64}

```text
Identity: owner:G040:G040-S06-06 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
RpcServer.deduplicate(self, store: OrderedDict[str, Any], policy: Mapping[str, Any]) -> 'RpcServer'
```
...
[
  ...
  {
    "excerpt": "168: dedup_store: OrderedDict[str, object] = OrderedDict()\n169: ok(\"positive\", \"server.deduplicate\",\n170:    server.deduplicate(dedup_store, {\"limit\": 3}) is server and server._dedup_limit == 3)\n171: first = client.call(unary, value, {\"deadline\": 80, \"idempotent\": True,\n172:                                    \"idempotencyKey\": request.idempotency_key,",
