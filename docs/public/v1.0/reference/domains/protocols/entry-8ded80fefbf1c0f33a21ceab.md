# protocols — protocol.encode(message)

In-process protocol model only; no HTTP2/QUIC network transport or service deployment; original observation: canonical NBP1 bytes retain protocol and message identity; SDK bounds: {"MAX_DEDUPLICATION":256,"MAX_FIELDS":64,"MAX_MESSAGE_BYTES":65536,"MAX_METADATA_BYTES":4096,"MAX_REPEATED":4096,"MAX_RETRIES":8,"MAX_STREAM_CAPACITY":64}

```text
Identity: owner:G040:G040-S02-01 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Protocol.encode(self, message: ValidatedMessage) -> bytes
```
...
[
  ...
  {
    "excerpt": "70: # S02 — canonical messages pass through checksummed frames and chunk boundaries.\n71: wire = protocol.encode(value)\n72: ok(\"positive\", \"protocol.encode\", wire[:4] == b\"NBP1\" and len(wire) < 65536)\n73: decoded = protocol.decode(wire, {\"maxBytes\": 4096})\n74: ok(\"positive\", \"protocol.decode\", decoded.schema is echo and dict(decoded.values) == dict(value.values))",
