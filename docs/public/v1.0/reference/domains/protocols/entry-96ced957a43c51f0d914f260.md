# protocols — client.call(method,request,options)

In-process protocol model only; no HTTP2/QUIC network transport or service deployment; original observation: typed request returns independently observed typed result; SDK bounds: {"MAX_DEDUPLICATION":256,"MAX_FIELDS":64,"MAX_MESSAGE_BYTES":65536,"MAX_METADATA_BYTES":4096,"MAX_REPEATED":4096,"MAX_RETRIES":8,"MAX_STREAM_CAPACITY":64}

```text
Identity: owner:G040:G040-S03-06 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
RpcClient.call(self, method: RpcMethod, request: ValidatedMessage, options: Mapping[str, Any]) -> RpcCall
```
