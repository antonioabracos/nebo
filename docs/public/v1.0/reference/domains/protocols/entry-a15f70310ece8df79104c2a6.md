# protocols — FrameDecoder.incremental(limits)

In-process protocol model only; no HTTP2/QUIC network transport or service deployment; original observation: decoder owns explicit payload metadata and buffer limits; SDK bounds: {"MAX_DEDUPLICATION":256,"MAX_FIELDS":64,"MAX_MESSAGE_BYTES":65536,"MAX_METADATA_BYTES":4096,"MAX_REPEATED":4096,"MAX_RETRIES":8,"MAX_STREAM_CAPACITY":64}

```text
Identity: owner:G040:G040-S02-05 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
FrameDecoder.incremental(limits: Mapping[str, int]) -> 'FrameDecoder'
```
...
[
  ...
  {
    "excerpt": "78: ok(\"positive\", \"frame.checksum\", checksum == hashlib.sha256(frame._encoded_content()).hexdigest())\n79: decoder = FrameDecoder.incremental({\"maxPayload\": 65536, \"maxMetadata\": 4096,\n80:                                     \"maxBuffered\": 140000})\n81: ok(\"positive\", \"FrameDecoder.incremental\", decoder.max_payload == 65536)\n82: encoded_frame = frame.to_bytes()",
