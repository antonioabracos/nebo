# protocols — api.generateConformanceSuite()

In-process protocol model only; no HTTP2/QUIC network transport or service deployment; original observation: suite distinguishes roundtrip acceptance and truncation rejection; SDK bounds: {"MAX_DEDUPLICATION":256,"MAX_FIELDS":64,"MAX_MESSAGE_BYTES":65536,"MAX_METADATA_BYTES":4096,"MAX_REPEATED":4096,"MAX_RETRIES":8,"MAX_STREAM_CAPACITY":64}

```text
Identity: owner:G040:G040-S05-06 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Api.generateConformanceSuite(self) -> GeneratedArtifact
```
...
[
  ...
  {
    "excerpt": "148:    and b\"EchoMessage\" in documentation.content)\n149: conformance = api.generateConformanceSuite()\n150: ok(\"positive\", \"api.generateConformanceSuite\", b\"truncated\\tREJECT\" in conformance.content)\n151: verification = generated_client.verify()\n152: ok(\"positive\", \"generated.verify\", verification[\"verified\"]",
