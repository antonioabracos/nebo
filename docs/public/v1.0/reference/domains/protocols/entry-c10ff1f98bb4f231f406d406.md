# protocols — Field.repeated(name,type,id,limit)

In-process protocol model only; no HTTP2/QUIC network transport or service deployment; original observation: repeated u64 field is bounded at eight; SDK bounds: {"MAX_DEDUPLICATION":256,"MAX_FIELDS":64,"MAX_MESSAGE_BYTES":65536,"MAX_METADATA_BYTES":4096,"MAX_REPEATED":4096,"MAX_RETRIES":8,"MAX_STREAM_CAPACITY":64}

```text
Identity: owner:G040:G040-S01-05 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Field.repeated(name: str, type: str, id: int, limit: int) -> 'Field'
```
...
[
  ...
  {
    "excerpt": "52: ok(\"positive\", \"Field.optional\", optional.mode == \"optional\" and optional.id == 2)\n53: repeated = Field.repeated(\"tags\", \"u64\", 3, 8)\n54: ok(\"positive\", \"Field.repeated\", repeated.mode == \"repeated\" and repeated.limit == 8)\n55: echo = Message.define(\"EchoMessage\", (repeated, required, optional))\n56: ok(\"positive\", \"Message.define\", tuple(item.id for item in echo.fields) == (1, 2, 3))",
