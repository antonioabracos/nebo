# protocols — Gateway.map(protocol,route)

In-process protocol model only; no HTTP2/QUIC network transport or service deployment; original observation: version-one protocol maps to explicit local route; SDK bounds: {"MAX_DEDUPLICATION":256,"MAX_FIELDS":64,"MAX_MESSAGE_BYTES":65536,"MAX_METADATA_BYTES":4096,"MAX_REPEATED":4096,"MAX_RETRIES":8,"MAX_STREAM_CAPACITY":64}

```text
Identity: owner:G040:G040-S07-05 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Gateway.map(protocol: Protocol, route: str) -> 'Gateway'
```
...
[
  ...
  {
    "excerpt": "200:    and pair[1].peer is pair[0] and pair[0].capacity == 6)\n201: gateway = Gateway.map(previous, \"/v1/echo\")\n202: ok(\"positive\", \"Gateway.map\", gateway.protocol is previous and gateway.route == \"/v1/echo\")\n203: translation = gateway.translate(previous, protocol)\n204: ok(\"positive\", \"gateway.translate\", translation[\"compatible\"]",
