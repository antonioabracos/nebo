# protocols — Field.optional(name,type,id)

In-process protocol model only; no HTTP2/QUIC network transport or service deployment; original observation: optional u64 field has stable ID two; SDK bounds: {"MAX_DEDUPLICATION":256,"MAX_FIELDS":64,"MAX_MESSAGE_BYTES":65536,"MAX_METADATA_BYTES":4096,"MAX_REPEATED":4096,"MAX_RETRIES":8,"MAX_STREAM_CAPACITY":64}

```text
Identity: owner:G040:G040-S01-04 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Field.optional(name: str, type: str, id: int) -> 'Field'
```
...
[
  ...
  {
    "excerpt": "50: ok(\"positive\", \"Field.required\", required.mode == \"required\" and required.id == 1)\n51: optional = Field.optional(\"requestId\", \"u64\", 2)\n52: ok(\"positive\", \"Field.optional\", optional.mode == \"optional\" and optional.id == 2)\n53: repeated = Field.repeated(\"tags\", \"u64\", 3, 8)\n54: ok(\"positive\", \"Field.repeated\", repeated.mode == \"repeated\" and repeated.limit == 8)",
