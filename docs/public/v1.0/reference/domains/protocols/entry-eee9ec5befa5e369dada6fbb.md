# protocols — RpcMethod.unary(name,requestType,responseType)

In-process protocol model only; no HTTP2/QUIC network transport or service deployment; original observation: Reflect binds EchoMessage request and response; SDK bounds: {"MAX_DEDUPLICATION":256,"MAX_FIELDS":64,"MAX_MESSAGE_BYTES":65536,"MAX_METADATA_BYTES":4096,"MAX_REPEATED":4096,"MAX_RETRIES":8,"MAX_STREAM_CAPACITY":64}

```text
Identity: owner:G040:G040-S03-02 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
RpcMethod.unary(name: str, requestType: Message, responseType: Message) -> 'RpcMethod'
```
...
[
  ...
  {
    "excerpt": "90: # S03 — one capability-bound unary call produces independently observed metadata.\n91: unary = RpcMethod.unary(\"Reflect\", echo, echo)\n92: ok(\"positive\", \"RpcMethod.unary\", unary.shape == \"unary\")\n93: unary = unary.effects((\"read\", \"network\", \"cancel\"))\n94: ok(\"positive\", \"method.effects\", unary.effect_set == frozenset((\"read\", \"network\", \"cancel\")))",
