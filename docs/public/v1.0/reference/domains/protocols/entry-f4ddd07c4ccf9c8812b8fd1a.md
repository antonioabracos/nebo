# protocols — api.generateClient(target)

In-process protocol model only; no HTTP2/QUIC network transport or service deployment; original observation: current Nebo syntax emits hashed typed client source; SDK bounds: {"MAX_DEDUPLICATION":256,"MAX_FIELDS":64,"MAX_MESSAGE_BYTES":65536,"MAX_METADATA_BYTES":4096,"MAX_REPEATED":4096,"MAX_RETRIES":8,"MAX_STREAM_CAPACITY":64}

```text
Identity: owner:G040:G040-S05-02 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Api.generateClient(self, target: str) -> GeneratedArtifact
```
...
[
  ...
  {
    "excerpt": "136: ok(\"positive\", \"Api.fromProtocol\", api.protocol is protocol and len(api.schema_hash) == 64)\n137: generated_client = api.generateClient(\"x86_64-nebo\")\n138: ok(\"positive\", \"api.generateClient\", generated_client.kind == \"client\"\n139:    and b\"40.return\" in generated_client.content)\n140: generated_server = api.generateServer(\"x86_64-nebo\")",
