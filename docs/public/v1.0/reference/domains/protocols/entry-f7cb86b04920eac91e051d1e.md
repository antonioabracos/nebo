# protocols — RpcPolicy.timeout(duration)

In-process protocol model only; no HTTP2/QUIC network transport or service deployment; original observation: duration fifty is evaluated against logical time; SDK bounds: {"MAX_DEDUPLICATION":256,"MAX_FIELDS":64,"MAX_MESSAGE_BYTES":65536,"MAX_METADATA_BYTES":4096,"MAX_REPEATED":4096,"MAX_RETRIES":8,"MAX_STREAM_CAPACITY":64}

```text
Identity: owner:G040:G040-S06-02 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
RpcPolicy.timeout(duration: int) -> Mapping[str, Any]
```
...
[
  ...
  {
    "excerpt": "159: ok(\"positive\", \"RpcPolicy.retry\", retry[\"attempts\"] == 3 and retry[\"idempotentOnly\"])\n160: timeout = RpcPolicy.timeout(50)\n161: ok(\"positive\", \"RpcPolicy.timeout\", timeout[\"duration\"] == 50 and timeout[\"clock\"] == \"logical\")\n162: circuit = RpcPolicy.circuitBreaker({\"failures\": 4, \"cooldown\": 12})\n163: ok(\"positive\", \"RpcPolicy.circuitBreaker\", circuit[\"failures\"] == 4 and circuit[\"cooldown\"] == 12)",
