# protocols — transport.capabilities()

In-process protocol model only; no HTTP2/QUIC network transport or service deployment; original observation: exactly in-memory is active and all maturity is factual; SDK bounds: {"MAX_DEDUPLICATION":256,"MAX_FIELDS":64,"MAX_MESSAGE_BYTES":65536,"MAX_METADATA_BYTES":4096,"MAX_REPEATED":4096,"MAX_RETRIES":8,"MAX_STREAM_CAPACITY":64}

```text
Identity: owner:G040:G040-S07-07 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
InMemoryEndpoint.capabilities(self) -> Mapping[str, Any]
```
...
[
  ...
  {
    "excerpt": "205:    and translation[\"from\"] == 1 and translation[\"to\"] == 2)\n206: capabilities = transport_capabilities()\n207: ok(\"positive\", \"transport.capabilities\", capabilities[\"in_memory\"][\"available\"]\n208:    and sum(1 for item in capabilities.values() if item[\"available\"]) == 1)\n209: transcript.append((dict(http2), dict(quic), dict(unix), dict(translation),",
