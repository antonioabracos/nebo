# public-foundation — FormatPlan.clone/drop

Current corrected G059 identifies typed IR/ABI nodes, evaluation policy and internal named lowering; these are reached by real format/interpolation source cases but are not standalone public constructors. Native ABI probes remain internal evidence. Ownership e cleanup definidos, sem alias mutável.

```text
Identity: public:g059-format:FormatPlan.clone/drop (DOMAIN_CONTRACT_ID)
Edition: 1
