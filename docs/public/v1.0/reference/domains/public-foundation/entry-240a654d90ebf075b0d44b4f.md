# public-foundation — FormatPlan

Current corrected G059 identifies typed IR/ABI nodes, evaluation policy and internal named lowering; these are reached by real format/interpolation source cases but are not standalone public constructors. Native ABI probes remain internal evidence. IR tipado comum a \`.format(...)\`, \`${...}\`, diagnostics, RenderPlan e tagged templates.

```text
Identity: public:g059-format:FormatPlan (DOMAIN_CONTRACT_ID)
Edition: 1
