# public-foundation — FormatNode.value(ValueRef, FormatProfile)

Current corrected G059 identifies typed IR/ABI nodes, evaluation policy and internal named lowering; these are reached by real format/interpolation source cases but are not standalone public constructors. Native ABI probes remain internal evidence. Valor avaliado uma vez com profile tipado.

```text
Identity: public:g059-format:FormatNode.value(ValueRef, FormatProfile) (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
FormatNode.value(ValueRef, FormatProfile)
Current corrected G059 identifies typed IR/ABI nodes, evaluation policy and internal named lowering; these are reached by real format/interpolation source cases but are not standalone public constructors. Native ABI probes remain internal evidence. Valor avaliado uma vez com profile tipado.
```
...
### format:fixed-17.5; expected 23

```nebo
start(){"%.2f".format(17.5).console();23.return;}
```
