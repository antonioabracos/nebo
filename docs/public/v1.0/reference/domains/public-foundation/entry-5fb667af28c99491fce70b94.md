# public-foundation — FormatNode.position(Int, ValueRef, FormatProfile)

Current corrected G059 identifies typed IR/ABI nodes, evaluation policy and internal named lowering; these are reached by real format/interpolation source cases but are not standalone public constructors. Native ABI probes remain internal evidence. Argumento posicional para i18n.

```text
Identity: public:g059-format:FormatNode.position(Int, ValueRef, FormatProfile) (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
FormatNode.position(Int, ValueRef, FormatProfile)
Current corrected G059 identifies typed IR/ABI nodes, evaluation policy and internal named lowering; these are reached by real format/interpolation source cases but are not standalone public constructors. Native ABI probes remain internal evidence. Argumento posicional para i18n.
```
...
### format:position-repeat; expected 23

```nebo
start(){"%1$d/%1$d".format(29).console();23.return;}
```

Oracle: {"capabilities": {"console": "RETAINED_DOCUMENT", "filesystem": "EXPLICIT_SCRATCH_EFFECT_ORACLE", "network": "DENIED_BY_SECCOMP"}, "console_text_utf8": "29/29", "filesystem_effects": {}, "independent_builds": 2, "kinds": \[2\], "process_exit": 23, "publications": 1, "runtime_determinism": "BYTE_IDENTICAL", "runtime_sha256": "54518bbca1a0fc4f42433592ce288ab2d238b2fca29013fe0a280c696d94e67e", "text": {"bytes_hex": "32392f3239"}}
