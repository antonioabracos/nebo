# public-foundation — FormatNode.named(SymbolId, ValueRef, FormatProfile)

Current corrected G059 identifies typed IR/ABI nodes, evaluation policy and internal named lowering; these are reached by real format/interpolation source cases but are not standalone public constructors. Native ABI probes remain internal evidence. Argumento nomeado.

```text
Identity: public:g059-format:FormatNode.named(SymbolId, ValueRef, FormatProfile) (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
FormatNode.named(SymbolId, ValueRef, FormatProfile)
Current corrected G059 identifies typed IR/ABI nodes, evaluation policy and internal named lowering; these are reached by real format/interpolation source cases but are not standalone public constructors. Native ABI probes remain internal evidence. Argumento nomeado.
```
