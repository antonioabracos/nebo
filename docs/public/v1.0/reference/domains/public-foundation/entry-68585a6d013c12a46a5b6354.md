# public-foundation — FormatPlan.write()

Current corrected G059 identifies typed IR/ABI nodes, evaluation policy and internal named lowering; these are reached by real format/interpolation source cases but are not standalone public constructors. Native ABI probes remain internal evidence. Escreve uma vez após allocation/reserve bem-sucedida.

```text
Identity: public:g059-format:FormatPlan.write() (DOMAIN_CONTRACT_ID)
Edition: 1
...
### format:checked-nested; expected 23

```nebo
start(){"[%s]".formatWith(0,"%d".formatWith(0,83).unwrapOr("bad")).unwrapOr("bad").console();23.return;}
```

Oracle: {"capabilities": {"console": "RETAINED_DOCUMENT", "filesystem": "EXPLICIT_SCRATCH_EFFECT_ORACLE", "network": "DENIED_BY_SECCOMP"}, "console_text_utf8": "\[83\]", "filesystem_effects": {}, "independent_builds": 2, "kinds": \[2\], "process_exit": 23, "publications": 1, "runtime_determinism": "BYTE_IDENTICAL", "runtime_sha256": "5e98ab4f14c96a040e8a980559c4e9de8ac985a008135c2027bd2c06c0aef89e", "text": {"bytes_hex": "5b38335d"}}
