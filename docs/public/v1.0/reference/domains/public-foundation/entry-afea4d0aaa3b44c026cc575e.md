# public-foundation — FormatNode.literal(Text)

Current corrected G059 identifies typed IR/ABI nodes, evaluation policy and internal named lowering; these are reached by real format/interpolation source cases but are not standalone public constructors. Native ABI probes remain internal evidence. Segmento literal.

```text
Identity: public:g059-format:FormatNode.literal(Text) (DOMAIN_CONTRACT_ID)
Edition: 1
...
### format:empty-template; expected 23

```nebo
start(){"".format().console();23.return;}
```
