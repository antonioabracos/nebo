# public-foundation — EvaluationCount.EXACTLY_ONCE

Current corrected G059 identifies typed IR/ABI nodes, evaluation policy and internal named lowering; these are reached by real format/interpolation source cases but are not standalone public constructors. Native ABI probes remain internal evidence. Sizing e rendering reutilizam temporários, nunca reavaliam expressões.

```text
Identity: public:g059-format:EvaluationCount.EXACTLY_ONCE (DOMAIN_CONTRACT_ID)
Edition: 1
