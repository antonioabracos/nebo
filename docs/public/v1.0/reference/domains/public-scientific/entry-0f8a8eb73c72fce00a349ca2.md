...
```text
Identity: public:claim:a0ba8f4fcae691e3c03a109f (DOMAIN_CONTRACT_ID)
Edition: 1
...
### matrix:Int-product-return-255; expected 255

```nebo
import "std.scientific" { Matrix; }.scientific;
start(){Vector<Int,2>.filled(17).v;v.outer(v).m;m.matvec(v).out;out.at(0).x;x.console();255.return;}
```

Oracle: {"capabilities": {"console": "RETAINED_DOCUMENT", "filesystem": "EXPLICIT_SCRATCH_EFFECT_ORACLE", "network": "DENIED_BY_SECCOMP"}, "console_text_utf8": "9826", "filesystem_effects": {}, "independent_builds": 2, "kinds": \[4\], "process_exit": 255, "runtime_determinism": "BYTE_IDENTICAL", "runtime_sha256": "2adbf0d2f74659fd42a4ac5f981c71495fa0e7a7acfe8af7e2e60e6b90546c58", "text": {"bytes_hex": "39383236"}}

## Rejected examples

### matrix:matvec-argument-dtype; expected NEBO_TYPE_MISMATCH

```nebo
import "std.scientific" { Matrix; }.scientific;
start(){Matrix<Float>.zeros(2,2).m;m.matvec(Vector<Int,2>.filled(17));23.return;}
```
