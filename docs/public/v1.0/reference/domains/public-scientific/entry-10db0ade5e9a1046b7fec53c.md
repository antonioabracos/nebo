...
```text
Identity: public:claim:83e9dc5a668458ad9f7c7ad2 (DOMAIN_CONTRACT_ID)
Edition: 1
...
### matrix:Int-get-none-2-0; expected 23

```nebo
import "std.scientific" { Matrix; }.scientific;
start(){Matrix<Int>.filled(2,3,17).m;m.get(2,0).o;o.isNone().console();23.return;}
```
...
### matrix:arity-get-0; expected NEBO_TYPE_MISMATCH

```nebo
import "std.scientific" { Matrix; }.scientific;
start(){Matrix<Int>.zeros(2,3).m;m.get(0);23.return;}
```
