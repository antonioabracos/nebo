...
```text
Identity: public:claim:acdc4fddba7db7198a72c471 (DOMAIN_CONTRACT_ID)
Edition: 1
...
### matrix:Int-filled-0-3-rows; expected 23

```nebo
import "std.scientific" { Matrix; }.scientific;
start(){Matrix<Int>.filled(0,3,17).m;m.rows().console();23.return;}
```
...
### matrix:rows-arity; expected NEBO_TYPE_MISMATCH

```nebo
import "std.scientific" { Matrix; }.scientific;
start(){Matrix<Int>.zeros(2,3).m;m.rows(1);23.return;}
```
