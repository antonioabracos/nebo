...
```text
Identity: public:claim:01c8466cc14b4a8ab40b7544 (DOMAIN_CONTRACT_ID)
Edition: 1
...
### matrix:Float-slice-empty; expected 23

```nebo
import "std.scientific" { Matrix; }.scientific;
start(){Matrix<Float>.zeros(2,3).m;m.slice(2,0,3,0).v;v.rows().console();23.return;}
```
...
### matrix:slice-non-range; expected NEBO_TYPE_MISMATCH

```nebo
import "std.scientific" { Matrix; }.scientific;
start(){Matrix<Int>.zeros(2,3).m;m.slice(0,1);23.return;}
```
