...
```text
Identity: public:claim:128a9fa1bc34f7feff448ad9 (DOMAIN_CONTRACT_ID)
Edition: 1
...
### tensor:matmul-0-\[3\]-\[3\]; expected 23

```nebo
import "std.scientific" { Tensor; }.scientific;
start(){Vector<Float> [0.25, 0.5, 0.75].va;Tensor<Float>.fromBuffer(va,Tuple.of(3)).a;Vector<Float> [-1.0, 1.5, 2.0].vb;Tensor<Float>.fromBuffer(vb,Tuple.of(3)).b;a.matmul(b).t;t.rank().console();t.elementCount().console();"${t.at(Tuple.of()):fixed(6)}".console();23.return;}
```

Oracle: {"capabilities": {"console": "RETAINED_DOCUMENT", "filesystem": "EXPLICIT_SCRATCH_EFFECT_ORACLE", "network": "DENIED_BY_SECCOMP"}, "console_text_utf8": "012.000000", "filesystem_effects": {}, "independent_builds": 2, "kinds": \[4, 4, 2\], "process_exit": 23, "runtime_determinism": "BYTE_IDENTICAL", "runtime_sha256": "2615eea6f203dabbaa2a6c8f2697983584c93491f2083180fc79ddb4063e9318", "text": {"bytes_hex": "3031322e303030303030"}}
