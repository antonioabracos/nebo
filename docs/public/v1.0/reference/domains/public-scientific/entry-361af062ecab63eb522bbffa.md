...
```text
Identity: public:claim:bfcf950a97aeec5198bb857d (DOMAIN_CONTRACT_ID)
Edition: 1
...
### tensor:map-pure-interpolation; expected 23

```nebo
import "std.scientific" { Tensor; }.scientific;
(Float.x)kernel(){(x*2.0).return;}start(){Tensor<Float>.filled(Tuple.of(2),1.5).a;"${a.map(kernel).at(Tuple.of(1)):fixed(2)}".console();23.return;}
```

Oracle: {"capabilities": {"console": "RETAINED_DOCUMENT", "filesystem": "EXPLICIT_SCRATCH_EFFECT_ORACLE", "network": "DENIED_BY_SECCOMP"}, "console_text_utf8": "3.00", "filesystem_effects": {}, "independent_builds": 2, "kinds": \[2\], "process_exit": 23, "runtime_determinism": "BYTE_IDENTICAL", "runtime_sha256": "9ce58e3a380985d0a471a160da9c4cdd3b7f73e748574df35f5cbd1ddcee6b53", "text": {"bytes_hex": "332e3030"}}

## Rejected examples

### tensor:map-kernel-value; expected NEBO_TYPE_MISMATCH

```nebo
import "std.scientific" { Tensor; }.scientific;
(Float.x)wrong(){17.return;}start(){Tensor<Float>.zeros(Tuple.of(2)).a;a.map(17);23.return;}
```
