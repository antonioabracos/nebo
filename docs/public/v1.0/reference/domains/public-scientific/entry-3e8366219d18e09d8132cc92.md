...
```text
Identity: public:claim:b5565503717f69d120550102 (DOMAIN_CONTRACT_ID)
Edition: 1
...
### matrix:Int-filled-2-3-layout; expected 23

```nebo
import "std.scientific" { Matrix; }.scientific;
start(){Matrix<Int>.filled(2,3,17).m;m.layout().console();23.return;}
```
...
### matrix:layout-arity; expected NEBO_TYPE_MISMATCH

```nebo
import "std.scientific" { Matrix; }.scientific;
start(){Matrix<Int>.zeros(2,3).m;m.layout(1);23.return;}
```
