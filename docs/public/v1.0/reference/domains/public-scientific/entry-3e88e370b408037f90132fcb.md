...
```text
Identity: public:claim:35879b9ed477bfa66506f0aa (DOMAIN_CONTRACT_ID)
Edition: 1
...
### matrix:Int-axis-max-0-False-0-0; expected 23

```nebo
import "std.scientific" { Matrix; }.scientific;
start(){Matrix<Int>.fromRows([[1, 3, 7], [11, 17, 23]]).m;m.a;0.axis;a.max(axis).v;v.at(0).x;x.console();23.return;}
```

Oracle: {"capabilities": {"console": "RETAINED_DOCUMENT", "filesystem": "EXPLICIT_SCRATCH_EFFECT_ORACLE", "network": "DENIED_BY_SECCOMP"}, "console_text_utf8": "11", "filesystem_effects": {}, "independent_builds": 2, "kinds": \[4\], "process_exit": 23, "runtime_determinism": "BYTE_IDENTICAL", "runtime_sha256": "3aaac50690036dea45d201163c3085735b9a3e0564cc96599924653717d7c0c4", "text": {"bytes_hex": "3131"}}

## Rejected examples

### matrix:axis-max-type; expected NEBO_TYPE_MISMATCH

```nebo
import "std.scientific" { Matrix; }.scientific;
start(){Matrix<Float>.zeros(2,3).m;m.max(true);23.return;}
```
