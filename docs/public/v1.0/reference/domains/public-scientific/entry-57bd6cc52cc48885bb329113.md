...
```text
Identity: public:claim:8b8385699a2cdf43411b8cfe (DOMAIN_CONTRACT_ID)
Edition: 1
...
### matrix:Int-zeros-0-0-rows; expected 23

```nebo
import "std.scientific" { Matrix; }.scientific;
start(){Matrix<Int>.zeros(0,0).m;m.rows().console();23.return;}
```
...
### matrix:zeros-arity; expected NEBO_TYPE_MISMATCH

```nebo
import "std.scientific" { Matrix; }.scientific;
start(){Matrix<Int>.zeros(2).m;23.return;}
```
