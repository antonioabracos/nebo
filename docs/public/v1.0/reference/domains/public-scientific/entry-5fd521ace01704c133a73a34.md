...
```text
Identity: public:claim:3610c3c4724f161aa6c933d1 (DOMAIN_CONTRACT_ID)
Edition: 1
...
### matrix:operation-preserves-input; expected 23

```nebo
import "std.scientific" { Matrix; }.scientific;
start(){Matrix<Int>.fromRows([[17,29],[43,71]]).a;a.scale(3).b;a.at(1,1).console();23.return;}
```
...
### matrix:type-scale-1.0; expected NEBO_TYPE_MISMATCH

```nebo
import "std.scientific" { Matrix; }.scientific;
start(){Matrix<Int>.zeros(2,3).m;m.scale(1.0);23.return;}
```
