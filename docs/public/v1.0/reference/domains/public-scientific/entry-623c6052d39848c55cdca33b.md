...
```text
Identity: public:claim:edcd2d3b21e0c8e9efaef92d (DOMAIN_CONTRACT_ID)
Edition: 1
...
### tensor:function-Int-17-23; expected 23

```nebo
import "std.scientific" { Tensor; }.scientific;
(Int.x)make(){Tensor<Int>.filled(Tuple.of(2,3),x).return;}start(){(17).make().t;t.at(Tuple.of(1,2)).v;v.console();23.return;}
```
...
### tensor:status-type; expected NEBO_ENTRYPOINT_INVALID_SIGNATURE

```nebo
import "std.scientific" { Tensor; }.scientific;
start(){Tensor<Int>.zeros(Tuple.of(2)).return;}
```
