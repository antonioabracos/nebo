...
```text
Identity: public:claim:b77dcb3a1673a00fe5bbc4e9 (DOMAIN_CONTRACT_ID)
Edition: 1
...
### matrix:Int-into-shape-3-0-2; expected 23

```nebo
import "std.scientific" { Matrix; }.scientific;
start(){Matrix<Int>.zeros(3,0).a;Matrix<Int>.zeros(0,2).b;Matrix<Int>.zeros(3,2).out;a.matmulInto(b,out);out.rows().console();23.return;}
```

Oracle: {"capabilities": {"console": "RETAINED_DOCUMENT", "filesystem": "EXPLICIT_SCRATCH_EFFECT_ORACLE", "network": "DENIED_BY_SECCOMP"}, "console_text_utf8": "3", "filesystem_effects": {}, "independent_builds": 2, "kinds": \[4\], "process_exit": 23, "runtime_determinism": "BYTE_IDENTICAL", "runtime_sha256": "31d898c40aa78dc7169f4fabac579cf3c04c2d6a0db4aad3d7360a0b56e9bc3e", "text": {"bytes_hex": "33"}}

## Rejected examples

### matrix:into-output-type; expected NEBO_TYPE_MISMATCH

```nebo
import "std.scientific" { Matrix; }.scientific;
start(){Matrix<Int>.zeros(2,2).a;Matrix<Int>.zeros(2,2).b;Matrix<Int>.zeros(2,2).out;a.matmulInto(b,17);23.return;}
```
