...
```text
Identity: public:claim:fe3e9e19a3b6697bf30c3745 (DOMAIN_CONTRACT_ID)
Edition: 1
...
### matrix:Int-axis-min-0-False-0-0; expected 23

```nebo
import "std.scientific" { Matrix; }.scientific;
start(){Matrix<Int>.fromRows([[1, 3, 7], [11, 17, 23]]).m;m.a;0.axis;a.min(axis).v;v.at(0).x;x.console();23.return;}
```
...
### matrix:axis-min-type; expected NEBO_TYPE_MISMATCH

```nebo
import "std.scientific" { Matrix; }.scientific;
start(){Matrix<Float>.zeros(2,3).m;m.min(true);23.return;}
```
