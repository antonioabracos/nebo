...
```text
Identity: public:claim:70e6cace1143cc733604c11d (DOMAIN_CONTRACT_ID)
Edition: 1
...
### tensor:transpose-Bool; expected 23

```nebo
import "std.scientific" { Tensor; }.scientific;
start(){Tensor<Bool>.filled(Tuple.of(2,3),true).a;a.permute(Tuple.of(1,0)).v;(v.storageId()==a.storageId()).console();v.rank().console();v.elementCount().console();v.at(Tuple.of(2,1)).console();v.contiguous().c;(c.storageId()!=v.storageId()).console();c.isContiguous().console();23.return;}
```
