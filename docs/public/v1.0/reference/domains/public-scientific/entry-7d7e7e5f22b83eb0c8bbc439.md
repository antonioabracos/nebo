...
```text
Identity: public:claim:196f6628e44b25ab3e4eb406 (DOMAIN_CONTRACT_ID)
Edition: 1
...
### matrix:Int-axis-empty-sum-0; expected 23

```nebo
import "std.scientific" { Matrix; }.scientific;
start(){Matrix<Int>.zeros(0,3).m;m.sum(0).v;v.at(2).x;x.console();23.return;}
```
...
### matrix:axis-sum-type; expected NEBO_TYPE_MISMATCH

```nebo
import "std.scientific" { Matrix; }.scientific;
start(){Matrix<Float>.zeros(2,3).m;m.sum(true);23.return;}
```
