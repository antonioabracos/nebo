...
```text
Identity: public:claim:7be67c7bdff47d9033e1e889 (DOMAIN_CONTRACT_ID)
Edition: 1
...
### matrix:Int-function-return-7-23; expected 23

```nebo
import "std.scientific" { Matrix; }.scientific;
(Int.x)make(){Matrix<Int>.filled(2,3,x).return;}start(){(7).make().a;(8).make().b;(a.sum()+b.sum()).v;v.console();23.return;}
```

Oracle: {"capabilities": {"console": "RETAINED_DOCUMENT", "filesystem": "EXPLICIT_SCRATCH_EFFECT_ORACLE", "network": "DENIED_BY_SECCOMP"}, "console_text_utf8": "90", "filesystem_effects": {}, "independent_builds": 2, "kinds": \[4\], "process_exit": 23, "runtime_determinism": "BYTE_IDENTICAL", "runtime_sha256": "51bc737d22c6d496881d04756326ded860890880febad0c726e4f051047b1dfc", "text": {"bytes_hex": "3930"}}

## Rejected examples

### matrix:Int-function-readonly-direct; expected NEBO_BORROW_CONFLICT

```nebo
import "std.scientific" { Matrix; }.scientific;
(Matrix<Int>.x)write(Matrix<Int>.y){x.matmulInto(y,x);23.return;}start(){Matrix<Int>.zeros(2,2).m;m.write(m);23.return;}
```
