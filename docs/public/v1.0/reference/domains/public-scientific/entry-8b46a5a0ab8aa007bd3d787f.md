...
```text
Identity: public:claim:59ccdea5ea58f7a8dd068f63 (DOMAIN_CONTRACT_ID)
Edition: 1
...
### matrix:Int-filled-2-3-at; expected 23

```nebo
import "std.scientific" { Matrix; }.scientific;
start(){Matrix<Int>.filled(2,3,17).m;m.at(1,2).v;v.console();23.return;}
```
...
### matrix:filled-wrong-int; expected NEBO_TYPE_MISMATCH

```nebo
import "std.scientific" { Matrix; }.scientific;
start(){Matrix<Int>.filled(2,3,1.5).m;23.return;}
```
