...
```text
Identity: public:claim:33db5857b437f41d4535fef3 (DOMAIN_CONTRACT_ID)
Edition: 1
...
### matrix:Float-empty-transpose-3-0-True; expected 23

```nebo
import "std.scientific" { Matrix; }.scientific;
start(){Matrix<Float>.zeros(3,0).m;m.transposeView().contiguous().v;v.rows().r;v.columns().c;"${r}/${c}".console();23.return;}
```

Oracle: {"capabilities": {"console": "RETAINED_DOCUMENT", "filesystem": "EXPLICIT_SCRATCH_EFFECT_ORACLE", "network": "DENIED_BY_SECCOMP"}, "console_text_utf8": "0/3", "filesystem_effects": {}, "independent_builds": 2, "kinds": \[2\], "process_exit": 23, "runtime_determinism": "BYTE_IDENTICAL", "runtime_sha256": "fc9c237906e9b46f19f5611a3ac359e8e6c80fca0afb9ee2ec2fff3e924819a1", "text": {"bytes_hex": "302f33"}}

## Rejected examples

### matrix:contiguous-arity; expected NEBO_TYPE_MISMATCH

```nebo
import "std.scientific" { Matrix; }.scientific;
start(){Matrix<Int>.zeros(2,3).m;m.contiguous(1);23.return;}
```
