...
```text
Identity: public:claim:596c8f79dd98c9f8afc299ff (DOMAIN_CONTRACT_ID)
Edition: 1
...
### statistics:nan-propagation-mean; expected 23

```nebo
start(){Vector<Float> [1.0,(∞-∞)].a;a.mean().isNaN().console();23.return;}
```
...
### statistics:float-status; expected NEBO_ENTRYPOINT_INVALID_SIGNATURE

```nebo
start(){Vector<Float> [1.0,2.0].a;a.mean().return;}
```
