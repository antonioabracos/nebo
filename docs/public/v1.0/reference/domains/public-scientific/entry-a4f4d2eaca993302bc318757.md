...
```text
Identity: public:claim:e3a8de44c8db1736ddaf18ce (DOMAIN_CONTRACT_ID)
Edition: 1
...
### matrix:Int-filled-2-3-isSquare; expected 23

```nebo
import "std.scientific" { Matrix; }.scientific;
start(){Matrix<Int>.filled(2,3,17).m;m.isSquare().console();23.return;}
```
...
### matrix:square-arity; expected NEBO_TYPE_MISMATCH

```nebo
import "std.scientific" { Matrix; }.scientific;
start(){Matrix<Int>.zeros(2,3).m;m.isSquare(1);23.return;}
```
