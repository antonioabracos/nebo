...
```text
Identity: public:claim:0165b7b80d4fe488475d0e91 (DOMAIN_CONTRACT_ID)
Edition: 1
...
### tensor:zeros-Bool-\[0\]; expected 23

```nebo
import "std.scientific" { Tensor; }.scientific;
start(){Tensor<Bool>.zeros(Tuple.of(0)).t;t.elementCount().console();23.return;}
```
...
### tensor:zeros-arity; expected NEBO_TYPE_MISMATCH

```nebo
import "std.scientific" { Tensor; }.scientific;
start(){Tensor<Int>.zeros(Tuple.of(2),0);23.return;}
```
