...
```text
Identity: public:claim:aadede83eaf8200b2dce8bd1 (DOMAIN_CONTRACT_ID)
Edition: 1
...
### tensor:binary-Float-2-maximum; expected 23

```nebo
import "std.scientific" { Tensor; }.scientific;
start(){Vector<Float> [0.75, 1.5].va;Tensor<Float>.fromBuffer(va,Tuple.of(2,1)).a;Vector<Float> [2.5, 3.75, 5.0].vb;Tensor<Float>.fromBuffer(vb,Tuple.of(1,3)).b;a.maximum(b).c;c.rank().console();c.elementCount().console();"${c.at(Tuple.of(0,0)):fixed(6)}".console();"${c.at(Tuple.of(0,1)):fixed(6)}".console();"${c.at(Tuple.of(0,2)):fixed(6)}".console();"${c.at(Tuple.of(1,0)):fixed(6)}".console();"${c.at(Tuple.of(1,1)):fixed(6)}".console();"${c.at(Tuple.of(1,2)):fixed(6)}".console();23.return;}
```

Oracle: {"capabilities": {"console": "RETAINED_DOCUMENT", "filesystem": "EXPLICIT_SCRATCH_EFFECT_ORACLE", "network": "DENIED_BY_SECCOMP"}, "console_text_utf8": "262.5000003.7500005.0000002.5000003.7500005.000000", "filesystem_effects": {}, "independent_builds": 2, "kinds": \[4, 4, 2, 2, 2, 2, 2, 2\], "process_exit": 23, "runtime_determinism": "BYTE_IDENTICAL", "runtime_sha256": "492ffbbecc9d22176c8591e20aa5cbc3fa4fdf697f7db6727de9f04c4e67360e", "text": {"bytes_hex": "3236322e353030303030332e373530303030352e303030303030322e353030303030332e373530303030352e303030303030"}}

## Rejected examples

### tensor:unknown; expected NEBO_TYPE_MISMATCH

```nebo
import "std.scientific" { Tensor; }.scientific;
start(){Tensor<Int>.zeros(Tuple.of(2)).a;a.missing();23.return;}
```
