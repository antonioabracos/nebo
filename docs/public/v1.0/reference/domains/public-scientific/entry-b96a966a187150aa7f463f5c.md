...
```text
Identity: public:claim:3dc7f795239e6002434501e3 (DOMAIN_CONTRACT_ID)
Edition: 1
...
### tensor:bool-reduce-\[1_1_0_0\]-0-any; expected 23

```nebo
import "std.scientific" { Tensor; }.scientific;
start(){Bytes.fromValues(1,1,0,0).b;Tensor<Bool>.fromBuffer(b,Tuple.of(2,2)).a;a.any(0).t;t.at(Tuple.of(0)).console();t.at(Tuple.of(1)).console();23.return;}
```
...
### tensor:any-arity; expected NEBO_TYPE_MISMATCH

```nebo
import "std.scientific" { Tensor; }.scientific;
start(){Tensor<Bool>.zeros(Tuple.of(2)).a;a.any();23.return;}
```
