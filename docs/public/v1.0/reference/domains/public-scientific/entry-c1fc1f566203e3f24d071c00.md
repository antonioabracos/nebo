...
```text
Identity: public:claim:1f712c919eafba30e5481135 (DOMAIN_CONTRACT_ID)
Edition: 1
...
### matrix:Int-filled-2-3-columns; expected 23

```nebo
import "std.scientific" { Matrix; }.scientific;
start(){Matrix<Int>.filled(2,3,17).m;m.columns().console();23.return;}
```
...
### matrix:columns-arity; expected NEBO_TYPE_MISMATCH

```nebo
import "std.scientific" { Matrix; }.scientific;
start(){Matrix<Int>.zeros(2,3).m;m.columns(1);23.return;}
```
