...
```text
Identity: public:claim:1e2ceb2abb0cfcdb3c0f6970 (DOMAIN_CONTRACT_ID)
Edition: 1
...
### tensor:reshape-Bool; expected 23

```nebo
import "std.scientific" { Tensor; }.scientific;
start(){Tensor<Bool>.filled(Tuple.of(2,3),true).a;a.reshape(Tuple.of(3,2)).v;(v.storageId()==a.storageId()).console();v.rank().console();v.elementCount().console();v.at(Tuple.of(2,1)).console();v.contiguous().c;(c.storageId()!=v.storageId()).console();c.isContiguous().console();23.return;}
```

Oracle: {"capabilities": {"console": "RETAINED_DOCUMENT", "filesystem": "EXPLICIT_SCRATCH_EFFECT_ORACLE", "network": "DENIED_BY_SECCOMP"}, "console_text_utf8": "true26truetruetrue", "filesystem_effects": {}, "independent_builds": 2, "kinds": \[5, 4, 4, 5, 5, 5\], "process_exit": 23, "runtime_determinism": "BYTE_IDENTICAL", "runtime_sha256": "080641037ca28768f77671733612d2851472d7a7c89e179f97711a949b451b95", "text": {"bytes_hex": "747275653236747275657472756574727565"}}

## Rejected examples

### tensor:shape-element-type; expected NEBO_TYPE_MISMATCH

```nebo
import "std.scientific" { Tensor; }.scientific;
start(){Tensor<Float>.zeros(Vector<Float> [1.0,2.0]);23.return;}
```
