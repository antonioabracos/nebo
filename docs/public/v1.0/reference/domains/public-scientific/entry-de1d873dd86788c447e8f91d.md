...
```text
Identity: public:claim:246b4088ae9157f21a5429bb (DOMAIN_CONTRACT_ID)
Edition: 1
...
### matrix:Int-map-empty-effects; expected 23

```nebo
import "std.scientific" { Matrix; }.scientific;
(Int.x)observe(){x.console();x.return;}start(){Matrix<Int>.zeros(0,3).m;m.map(observe).out;out.rows().console();23.return;}
```
...
### matrix:Int-map-invalid-receiver; expected NEBO_TYPE_MISMATCH

```nebo
import "std.scientific" { Matrix; }.scientific;
(Float.x)f(){x.return;}start(){17.outside;Matrix<Int>.zeros(2,2).m;m.map(f);23.return;}
```
