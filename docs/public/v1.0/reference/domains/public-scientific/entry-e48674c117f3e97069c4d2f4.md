...
```text
Identity: public:claim:0acf91593123cad6cc387a4c (DOMAIN_CONTRACT_ID)
Edition: 1
...
### tensor:slice-Bool; expected 23

```nebo
import "std.scientific" { Tensor; }.scientific;
start(){Tensor<Bool>.filled(Tuple.of(2,3),true).a;a.slice(Tuple.of(1,0,2,2)).v;(v.storageId()==a.storageId()).console();v.rank().console();v.elementCount().console();v.at(Tuple.of(1,1)).console();v.contiguous().c;(c.storageId()!=v.storageId()).console();c.isContiguous().console();23.return;}
```

Oracle: {"capabilities": {"console": "RETAINED_DOCUMENT", "filesystem": "EXPLICIT_SCRATCH_EFFECT_ORACLE", "network": "DENIED_BY_SECCOMP"}, "console_text_utf8": "true24truetruetrue", "filesystem_effects": {}, "independent_builds": 2, "kinds": \[5, 4, 4, 5, 5, 5\], "process_exit": 23, "runtime_determinism": "BYTE_IDENTICAL", "runtime_sha256": "ea924f98bc7367d5c2dd2a0cbed91ff10562cb5f3b7af03c8d46bfd435ab6004", "text": {"bytes_hex": "747275653234747275657472756574727565"}}
