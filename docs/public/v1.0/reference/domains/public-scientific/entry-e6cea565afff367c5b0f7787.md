...
```text
Identity: public:claim:6b2570ce2d55e0ff653836c1 (DOMAIN_CONTRACT_ID)
Edition: 1
...
### tensor:binary-Float-2-divide; expected 23

```nebo
import "std.scientific" { Tensor; }.scientific;
start(){Vector<Float> [0.75, 1.5].va;Tensor<Float>.fromBuffer(va,Tuple.of(2,1)).a;Vector<Float> [2.5, 3.75, 5.0].vb;Tensor<Float>.fromBuffer(vb,Tuple.of(1,3)).b;a.divide(b).c;c.rank().console();c.elementCount().console();"${c.at(Tuple.of(0,0)):fixed(6)}".console();"${c.at(Tuple.of(0,1)):fixed(6)}".console();"${c.at(Tuple.of(0,2)):fixed(6)}".console();"${c.at(Tuple.of(1,0)):fixed(6)}".console();"${c.at(Tuple.of(1,1)):fixed(6)}".console();"${c.at(Tuple.of(1,2)):fixed(6)}".console();23.return;}
```

Oracle: {"capabilities": {"console": "RETAINED_DOCUMENT", "filesystem": "EXPLICIT_SCRATCH_EFFECT_ORACLE", "network": "DENIED_BY_SECCOMP"}, "console_text_utf8": "260.3000000.2000000.1500000.6000000.4000000.300000", "filesystem_effects": {}, "independent_builds": 2, "kinds": \[4, 4, 2, 2, 2, 2, 2, 2\], "process_exit": 23, "runtime_determinism": "BYTE_IDENTICAL", "runtime_sha256": "58c5a13d825a65255a8233ee4b9531bdb0cf3ba9ca5a589e14416e14808c0209", "text": {"bytes_hex": "3236302e333030303030302e323030303030302e313530303030302e363030303030302e343030303030302e333030303030"}}
