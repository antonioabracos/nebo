# public-visual — ScanPlan.onCancel
...
```text
Identity: public:g170-scan:onCancel (DOMAIN_CONTRACT_ID)
Edition: 1
...
### scan:cancel-fallback-17; expected 23

```nebo
start(){"Input".scan(.int(),.cancelOn("17"),.onCancel(83),.mock("17")).console();23.return;}
```
...
### scan:cancel-fallback-type; expected NEBO_TYPE_MISMATCH

```nebo
start(){"Input".scan(.text(),.onCancel(17));23.return;}
```
