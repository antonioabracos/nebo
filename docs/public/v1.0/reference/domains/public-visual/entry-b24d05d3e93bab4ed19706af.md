...
```text
Identity: public:g170-console:Color.rgb (DOMAIN_CONTRACT_ID)
Edition: 1
...
### console:color-implicit-status; expected 0

```nebo
start(){Color.rgb(17,29,53).red().console();}
```

Oracle: {"capabilities": {"console": "RETAINED_DOCUMENT", "filesystem": "EXPLICIT_SCRATCH_EFFECT_ORACLE", "network": "DENIED_BY_SECCOMP"}, "console_state_trace": true, "console_text_utf8": "17", "filesystem_effects": {}, "independent_builds": 2, "kinds": \[4\], "observation": {"native_oracle": "console_public_test.compare"}, "process_exit": 0, "publications": 1, "runtime_determinism": "DOMAIN_INVARIANTS", "runtime_sha256": "ef63802d6adb274672ff60210fb3e604b8b8a8cb20f791d1a031c8b9da9add38", "text": {"bytes_hex": "3137"}}

## Rejected examples

### console:rgb-arity; expected NEBO_TYPE_MISMATCH

```nebo
start(){Color.rgb(17,29);23.return;}
```
