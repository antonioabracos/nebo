...
```text
Identity: public:g170-scan:enum (DOMAIN_CONTRACT_ID)
Edition: 1
...
### scan:choice-variant-enum-red; expected 23

```nebo
start(){"Choice".scan(.console(),.enum(["red","blue"]),.mock("red")).console();23.return;}
```

Oracle: {"capabilities": {"console": "RETAINED_DOCUMENT", "filesystem": "EXPLICIT_SCRATCH_EFFECT_ORACLE", "network": "DENIED_BY_SECCOMP"}, "console_text_utf8": "red", "filesystem_effects": {}, "independent_builds": 2, "kinds": \[2\], "observation": {"native_oracle": "scan_public_test.observe"}, "process_exit": 23, "publications": 1, "runtime_determinism": "DOMAIN_INVARIANTS", "runtime_sha256": "c692129cbe59342e9e6de26bae943237eb4ddcd064ec70c9f31dfcea82d37fbb", "scan_trace": true, "text": {"bytes_hex": "726564"}}

## Rejected examples

### scan:choice-type-enum; expected NEBO_TYPE_MISMATCH

```nebo
start(){"Choice".scan(.console(),.enum([17,29]));23.return;}
```
