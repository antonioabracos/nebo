...
```text
Identity: public:g170-scan:result (DOMAIN_CONTRACT_ID)
Edition: 1
...
### scan:result-value-int-True; expected 23

```nebo
start(){"Input".scan(.int(),.result(),.mock("17")).unwrapOr(71).console();23.return;}
```
...
### scan:option-arity-result; expected NEBO_TYPE_MISMATCH

```nebo
start(){"Input".scan(.result(17));23.return;}
```
