# robotics — trajectory.sample(time)

Deterministic DSP/PID/replay model; no physical actuator, sensor or hard real-time proof; original observation: midpoint publishes position velocity and acceleration; SDK bounds: {"MAX_LOOP_TICKS":512,"MAX_MATRIX_DIMENSION":16,"MAX_SAMPLES":4096,"MAX_TRANSFORM_POINTS":256}

```text
Identity: owner:G041:G041-S05-07 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Trajectory.sample(self, time: float) -> TrajectorySample
```
...
[
  ...
  {
    "excerpt": "252:     surface(\"G041-S05-06\", trajectory.start != trajectory.goal and trajectory.duration == 3.0, trajectory.duration)\n253:     midpoint = trajectory.sample(1.5)\n254:     surface(\"G041-S05-07\", midpoint.position == (0.5, -0.25) and midpoint.velocity[0] > 0.0, midpoint)\n255:     validation = trajectory.validate(limits)\n256:     surface(\"G041-S05-08\", validation[\"valid\"] is True and validation[\"peakVelocity\"] <= 1.0, dict(validation))",
