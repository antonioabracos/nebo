# robotics — Filter.highPass(cutoff,sampleRate,order)

Deterministic DSP/PID/replay model; no physical actuator, sensor or hard real-time proof; original observation: high-pass retains distinct cutoff and rate; SDK bounds: {"MAX_LOOP_TICKS":512,"MAX_MATRIX_DIMENSION":16,"MAX_SAMPLES":4096,"MAX_TRANSFORM_POINTS":256}

```text
Identity: owner:G041:G041-S02-02 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Filter.highPass(cls, cutoff: float, sampleRate: float, order: int) -> 'Filter'
```
...
[
  ...
  {
    "excerpt": "116:     negative(\"UNSUPPORTED_METHOD\", \"signal.interpolate\", lambda: signal.interpolate(0.5, \"cubic\"))\n117:     negative(\"ANTI_ALIAS_FILTER_REQUIRED\", \"signal.decimate\", lambda: signal.decimate(2, Filter.highPass(1.0, 4.0, 1)))\n118:     negative(\"UNSUPPORTED_WINDOW\", \"signal.spectrum\", lambda: signal.spectrum({\"window\": \"blackman\"}))\n119: \n120: ",
