# robotics — model.simulate(input,initialState)

Deterministic DSP/PID/replay model; no physical actuator, sensor or hard real-time proof; original observation: three bounded inputs produce ordered state/output steps; SDK bounds: {"MAX_LOOP_TICKS":512,"MAX_MATRIX_DIMENSION":16,"MAX_SAMPLES":4096,"MAX_TRANSFORM_POINTS":256}

```text
Identity: owner:G041:G041-S04-02 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
StateSpace.simulate(self, input: Sequence[Sequence[float]], initialState: Sequence[float]) -> tuple[StateStep, ...]
```
...
[
  ...
  {
    "excerpt": "201:     surface(\"G041-S04-01\", model.state_count == 2 and model.input_count == 1 and model.sample_time == 0.1, model.sample_time)\n202:     simulation = model.simulate(((1.0,), (1.0,), (0.0,)), (0.0, 0.0))\n203:     surface(\"G041-S04-02\", len(simulation) == 3 and simulation[-1].state[0] > 0.0, simulation)\n204:     point = model.step((1.0, 2.0), (3.0,))\n205:     surface(\"G041-S04-03\", point.state == (1.2, 2.3) and point.output == (1.0,), point)",
