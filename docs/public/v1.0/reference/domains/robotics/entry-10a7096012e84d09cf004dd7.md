# robotics — robot.forwardKinematics(joints)

Deterministic DSP/PID/replay model; no physical actuator, sensor or hard real-time proof; original observation: joint angles produce the analytical planar pose; SDK bounds: {"MAX_LOOP_TICKS":512,"MAX_MATRIX_DIMENSION":16,"MAX_SAMPLES":4096,"MAX_TRANSFORM_POINTS":256}

```text
Identity: owner:G041:G041-S05-04 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
RobotModel.forwardKinematics(self, joints: Sequence[float]) -> Pose
```
