# robotics — loop.replay(log)

Deterministic DSP/PID/replay model; no physical actuator, sensor or hard real-time proof; original observation: canonical simulation log yields input-dependent deterministic digest; SDK bounds: {"MAX_LOOP_TICKS":512,"MAX_MATRIX_DIMENSION":16,"MAX_SAMPLES":4096,"MAX_TRANSFORM_POINTS":256}

```text
Identity: owner:G041:G041-S07-09 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
RobotLoop.replay(self, log: Sequence[Mapping[str, Any]]) -> ReplayReport
```
