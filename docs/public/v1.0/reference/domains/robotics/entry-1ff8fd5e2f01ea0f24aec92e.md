# robotics — loop.run(cancellation)

Deterministic DSP/PID/replay model; no physical actuator, sensor or hard real-time proof; original observation: deadline miss stops writes and enters safe state exactly once; SDK bounds: {"MAX_LOOP_TICKS":512,"MAX_MATRIX_DIMENSION":16,"MAX_SAMPLES":4096,"MAX_TRANSFORM_POINTS":256}

```text
Identity: owner:G041:G041-S07-07 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
RobotLoop.run(self, cancellation: Mapping[str, Any]) -> LoopRunResult
```
...
[
  ...
  {
    "excerpt": "337:     surface(\"G041-S07-06\", loop.watchdog(0.015)._watchdog == 0.015, loop._watchdog)\n338:     result = loop.run({\"maxTicks\": 3, \"phaseDurations\": (0.004, 0.012, 0.004)})\n339:     surface(\"G041-S07-07\", result.status == \"SAFE\" and result.fault == \"DEADLINE_MISS\" and result.safeStateCalls == 1, result)\n340:     timing = loop.timingReport()\n341:     surface(\"G041-S07-08\", timing[\"ticks\"] == 2 and timing[\"misses\"] == 1, dict(timing))",
