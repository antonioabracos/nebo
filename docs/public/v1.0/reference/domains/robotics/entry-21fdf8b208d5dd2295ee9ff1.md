# robotics — RobotLoop.new(period,deadline,controller)

Deterministic DSP/PID/replay model; no physical actuator, sensor or hard real-time proof; original observation: period deadline and controller define a simulation loop; SDK bounds: {"MAX_LOOP_TICKS":512,"MAX_MATRIX_DIMENSION":16,"MAX_SAMPLES":4096,"MAX_TRANSFORM_POINTS":256}

```text
Identity: owner:G041:G041-S07-01 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
RobotLoop.new(cls, period: float, deadline: float, controller: Any) -> 'RobotLoop'
```
