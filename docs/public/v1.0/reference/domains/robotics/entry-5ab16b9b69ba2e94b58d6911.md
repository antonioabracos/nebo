# robotics — Trajectory.plan(start,goal,limits,obstacles)

Deterministic DSP/PID/replay model; no physical actuator, sensor or hard real-time proof; original observation: collision-free cubic plan respects explicit feasibility limits; SDK bounds: {"MAX_LOOP_TICKS":512,"MAX_MATRIX_DIMENSION":16,"MAX_SAMPLES":4096,"MAX_TRANSFORM_POINTS":256}

```text
Identity: owner:G041:G041-S05-06 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Trajectory.plan(cls, start: Sequence[float], goal: Sequence[float], limits: Mapping[str, Any], obstacles: Sequence[Mapping[str, Any]]) -> 'Trajectory'
```
...
[
  ...
  {
    "excerpt": "250:     limits = {\"maximumVelocity\": 1.0, \"maximumAcceleration\": 2.0, \"duration\": 3.0}\n251:     trajectory = Trajectory.plan((0.0, 0.0), (1.0, -0.5), limits, ({\"center\": (5.0, 5.0), \"radius\": 0.5},))\n252:     surface(\"G041-S05-06\", trajectory.start != trajectory.goal and trajectory.duration == 3.0, trajectory.duration)\n253:     midpoint = trajectory.sample(1.5)\n254:     surface(\"G041-S05-07\", midpoint.position == (0.5, -0.25) and midpoint.velocity[0] > 0.0, midpoint)",
