# robotics — signal.statistics()

Deterministic DSP/PID/replay model; no physical actuator, sensor or hard real-time proof; original observation: count mean extrema RMS rate and unit are observed; SDK bounds: {"MAX_LOOP_TICKS":512,"MAX_MATRIX_DIMENSION":16,"MAX_SAMPLES":4096,"MAX_TRANSFORM_POINTS":256}

```text
Identity: owner:G041:G041-S01-08 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
SignalBuffer.statistics(self) -> Mapping[str, Any]
```
...
[
  ...
  {
    "excerpt": "94:     surface(\"G041-S01-07\", len(spectrum) == 3 and close(spectrum[0].frequency, 0.0), spectrum)\n95:     statistics = signal.statistics()\n96:     surface(\"G041-S01-08\", statistics[\"count\"] == 4 and close(statistics[\"mean\"], 0.0), dict(statistics))\n97: \n98:     altered = SignalBuffer.new(8, 4.0, \"m/s\")",
