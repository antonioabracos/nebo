# robotics — signal.spectrum(options)

Deterministic DSP/PID/replay model; no physical actuator, sensor or hard real-time proof; original observation: bounded DFT exposes frequency magnitude and phase; SDK bounds: {"MAX_LOOP_TICKS":512,"MAX_MATRIX_DIMENSION":16,"MAX_SAMPLES":4096,"MAX_TRANSFORM_POINTS":256}

```text
Identity: owner:G041:G041-S01-07 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
SignalBuffer.spectrum(self, options: Mapping[str, Any] | None) -> tuple[SpectrumBin, ...]
```
...
[
  ...
  {
    "excerpt": "92:     surface(\"G041-S01-06\", close(interpolated, 0.5), interpolated)\n93:     spectrum = signal.spectrum({\"window\": \"rectangular\"})\n94:     surface(\"G041-S01-07\", len(spectrum) == 3 and close(spectrum[0].frequency, 0.0), spectrum)\n95:     statistics = signal.statistics()\n96:     surface(\"G041-S01-08\", statistics[\"count\"] == 4 and close(statistics[\"mean\"], 0.0), dict(statistics))",
