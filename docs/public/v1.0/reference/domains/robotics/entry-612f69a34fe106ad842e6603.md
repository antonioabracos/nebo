# robotics — fusion.state()

Deterministic DSP/PID/replay model; no physical actuator, sensor or hard real-time proof; original observation: immutable fused position and velocity reflect the update; SDK bounds: {"MAX_LOOP_TICKS":512,"MAX_MATRIX_DIMENSION":16,"MAX_SAMPLES":4096,"MAX_TRANSFORM_POINTS":256}

```text
Identity: owner:G041:G041-S06-05 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
SensorFusion.state(self) -> tuple[float, float]
```
...
[
  ...
  {
    "excerpt": "144:     low.step(8.0)\n145:     claim(\"ownership\", low.frequencyResponse(9) == reset_response, \"response snapshot does not expose filter state\")\n146:     initial = kalman.step(1.0)\n147:     negative(\"NON_FINITE\", \"filter.step\", lambda: kalman.step(float(\"inf\")))\n148:     claim(\"failure_atomicity\", close(kalman.step(1.0), initial, 0.5), \"rejected Kalman measurement does not poison state\")",
