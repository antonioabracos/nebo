# robotics — Filter.lowPass(cutoff,sampleRate,order)

Deterministic DSP/PID/replay model; no physical actuator, sensor or hard real-time proof; original observation: low-pass retains cutoff rate and bounded order; SDK bounds: {"MAX_LOOP_TICKS":512,"MAX_MATRIX_DIMENSION":16,"MAX_SAMPLES":4096,"MAX_TRANSFORM_POINTS":256}

```text
Identity: owner:G041:G041-S02-01 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Filter.lowPass(cls, cutoff: float, sampleRate: float, order: int) -> 'Filter'
```
...
[
  ...
  {
    "excerpt": "88:     surface(\"G041-S01-04\", len(resampled) == 7 and close(resampled[1].value or 0.0, 0.5), resampled[1])\n89:     decimated = signal.decimate(2, Filter.lowPass(1.0, 4.0, 1))\n90:     surface(\"G041-S01-05\", len(decimated) == 2 and decimated[0].timestamp == 0.0, decimated)\n91:     interpolated = signal.interpolate(0.375, \"linear\")\n92:     surface(\"G041-S01-06\", close(interpolated, 0.5), interpolated)",
