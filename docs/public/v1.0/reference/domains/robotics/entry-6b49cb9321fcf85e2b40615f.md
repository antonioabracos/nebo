# robotics — robot.inverseKinematics(target,seed,options)

Deterministic DSP/PID/replay model; no physical actuator, sensor or hard real-time proof; original observation: reachable target converges with bounded residual and singularity report; SDK bounds: {"MAX_LOOP_TICKS":512,"MAX_MATRIX_DIMENSION":16,"MAX_SAMPLES":4096,"MAX_TRANSFORM_POINTS":256}

```text
Identity: owner:G041:G041-S05-05 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
RobotModel.inverseKinematics(self, target: Pose, seed: Sequence[float], options: Mapping[str, Any] | None) -> IkResult
```
...
[
  ...
  {
    "excerpt": "247:     surface(\"G041-S05-04\", close(forward.position[0], 1.0) and close(forward.position[1], 1.0), forward)\n248:     inverse = robot.inverseKinematics(target_pose, (0.1, 1.0), {\"elbow\": \"nearest\"})\n249:     surface(\"G041-S05-05\", inverse.converged and inverse.residual <= 1.0e-8, inverse)\n250:     limits = {\"maximumVelocity\": 1.0, \"maximumAcceleration\": 2.0, \"duration\": 3.0}\n251:     trajectory = Trajectory.plan((0.0, 0.0), (1.0, -0.5), limits, ({\"center\": (5.0, 5.0), \"radius\": 0.5},))",
