# robotics — fusion.addSensor(name,model,frame)

Deterministic DSP/PID/replay model; no physical actuator, sensor or hard real-time proof; original observation: unique sensor binds state index noise and matching frame; SDK bounds: {"MAX_LOOP_TICKS":512,"MAX_MATRIX_DIMENSION":16,"MAX_SAMPLES":4096,"MAX_TRANSFORM_POINTS":256}

```text
Identity: owner:G041:G041-S06-02 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
SensorFusion.addSensor(self, name: str, model: Mapping[str, Any], frame: str) -> 'SensorFusion'
```
...
[
  ...
  {
    "excerpt": "279:     surface(\"G041-S06-01\", fusion.frame == \"base\" and fusion.process_noise == 0.02, fusion.state())\n280:     fusion.addSensor(\"position\", {\"stateIndex\": 0, \"measurementNoise\": 0.25}, \"base\")\n281:     surface(\"G041-S06-02\", \"position\" in fusion._sensors and fusion._sensors[\"position\"][\"measurementNoise\"] == 0.25, fusion._sensors)\n282:     predicted = fusion.predict(2.0, 0.5)\n283:     surface(\"G041-S06-03\", predicted == (0.75, 2.0), predicted)",
