# robotics — loop.watchdog(timeout)

Deterministic DSP/PID/replay model; no physical actuator, sensor or hard real-time proof; original observation: watchdog timeout is positive and within the loop period; SDK bounds: {"MAX_LOOP_TICKS":512,"MAX_MATRIX_DIMENSION":16,"MAX_SAMPLES":4096,"MAX_TRANSFORM_POINTS":256}

```text
Identity: owner:G041:G041-S07-06 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
RobotLoop.watchdog(self, timeout: float) -> 'RobotLoop'
```
...
[
  ...
  {
    "excerpt": "322:     loop.safeState(lambda reason: safe_events.append(reason))\n323:     loop.watchdog(0.015)\n324:     return loop\n325: \n326: ",
