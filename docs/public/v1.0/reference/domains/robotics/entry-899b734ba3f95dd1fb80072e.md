# robotics — Filter.kalman(model,processNoise,measurementNoise)

Deterministic DSP/PID/replay model; no physical actuator, sensor or hard real-time proof; original observation: model process uncertainty and measurement noise remain distinct; SDK bounds: {"MAX_LOOP_TICKS":512,"MAX_MATRIX_DIMENSION":16,"MAX_SAMPLES":4096,"MAX_TRANSFORM_POINTS":256}

```text
Identity: owner:G041:G041-S02-05 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Filter.kalman(cls, model: Mapping[str, Any], processNoise: float, measurementNoise: float) -> 'Filter'
```
...
[
  ...
  {
    "excerpt": "129:     surface(\"G041-S02-04\", moving.kind == \"moving-average\" and moving.parameters[\"window\"] == 3, moving.parameters)\n130:     kalman = Filter.kalman({\"transition\": 1.0, \"measurement\": 1.0, \"initialState\": 0.0}, 0.1, 0.4)\n131:     surface(\"G041-S02-05\", kalman.kind == \"kalman\" and kalman.parameters[\"processNoise\"] != kalman.parameters[\"measurementNoise\"], kalman.parameters)\n132:     filtered = tuple(moving.step(value) for value in (3.0, 6.0, 9.0, 12.0))\n133:     surface(\"G041-S02-06\", filtered == (3.0, 4.5, 6.0, 9.0), filtered)",
