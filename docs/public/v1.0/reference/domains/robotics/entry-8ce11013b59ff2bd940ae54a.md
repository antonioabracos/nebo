# robotics — loop.readSensors(function)

Deterministic DSP/PID/replay model; no physical actuator, sensor or hard real-time proof; original observation: validated sensor callback is installed; SDK bounds: {"MAX_LOOP_TICKS":512,"MAX_MATRIX_DIMENSION":16,"MAX_SAMPLES":4096,"MAX_TRANSFORM_POINTS":256}

```text
Identity: owner:G041:G041-S07-02 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
RobotLoop.readSensors(self, function: Callable[..., Any]) -> 'RobotLoop'
```
...
[
  ...
  {
    "excerpt": "318:     loop = RobotLoop.new(0.02, 0.01, controller)\n319:     loop.readSensors(lambda tick: float(tick) / 10.0)\n320:     loop.compute(lambda sensed, control, period: control.step(sensed, period).output)\n321:     loop.writeActuators(lambda command: writes.append(command))\n322:     loop.safeState(lambda reason: safe_events.append(reason))",
