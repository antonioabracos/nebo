# robotics — fusion.predict(control,dt)

Deterministic DSP/PID/replay model; no physical actuator, sensor or hard real-time proof; original observation: constant-acceleration prediction updates state and covariance; SDK bounds: {"MAX_LOOP_TICKS":512,"MAX_MATRIX_DIMENSION":16,"MAX_SAMPLES":4096,"MAX_TRANSFORM_POINTS":256}

```text
Identity: owner:G041:G041-S06-03 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
SensorFusion.predict(self, control: float, dt: float) -> tuple[float, float]
```
...
[
  ...
  {
    "excerpt": "281:     surface(\"G041-S06-02\", \"position\" in fusion._sensors and fusion._sensors[\"position\"][\"measurementNoise\"] == 0.25, fusion._sensors)\n282:     predicted = fusion.predict(2.0, 0.5)\n283:     surface(\"G041-S06-03\", predicted == (0.75, 2.0), predicted)\n284:     accepted = fusion.update(\"position\", {\"value\": 0.8, \"frame\": \"base\"}, 1.0)\n285:     surface(\"G041-S06-04\", accepted.accepted and accepted.sensor == \"position\", accepted)",
