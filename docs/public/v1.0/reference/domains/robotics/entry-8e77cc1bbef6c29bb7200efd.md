# robotics — controller.step(measurement,dt)

Deterministic DSP/PID/replay model; no physical actuator, sensor or hard real-time proof; original observation: measurement and dt produce saturated term-level evidence; SDK bounds: {"MAX_LOOP_TICKS":512,"MAX_MATRIX_DIMENSION":16,"MAX_SAMPLES":4096,"MAX_TRANSFORM_POINTS":256}

```text
Identity: owner:G041:G041-S03-03 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Filter.step(self, value: float | None) -> float
PidController.step(self, measurement: float, dt: float) -> PidStep
StateSpace.step(self, state: Sequence[float], input: Sequence[float]) -> StateStep
```
...
[
  ...
  {
    "excerpt": "131:     surface(\"G041-S02-05\", kalman.kind == \"kalman\" and kalman.parameters[\"processNoise\"] != kalman.parameters[\"measurementNoise\"], kalman.parameters)\n132:     filtered = tuple(moving.step(value) for value in (3.0, 6.0, 9.0, 12.0))\n133:     surface(\"G041-S02-06\", filtered == (3.0, 4.5, 6.0, 9.0), filtered)\n134:     moving.reset(5.0)\n135:     surface(\"G041-S02-07\", close(moving.step(8.0), 6.5), 6.5)",
