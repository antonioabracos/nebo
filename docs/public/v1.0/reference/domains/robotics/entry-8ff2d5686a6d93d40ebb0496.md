# robotics — fusion.covariance()

Deterministic DSP/PID/replay model; no physical actuator, sensor or hard real-time proof; original observation: measurement update reduces observed-state uncertainty; SDK bounds: {"MAX_LOOP_TICKS":512,"MAX_MATRIX_DIMENSION":16,"MAX_SAMPLES":4096,"MAX_TRANSFORM_POINTS":256}

```text
Identity: owner:G041:G041-S06-06 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
SensorFusion.covariance(self) -> tuple[tuple[float, float], tuple[float, float]]
```
...
[
  ...
  {
    "excerpt": "287:     surface(\"G041-S06-05\", len(state) == 2 and state[0] > 0.75, state)\n288:     covariance = fusion.covariance()\n289:     surface(\"G041-S06-06\", len(covariance) == 2 and covariance[0][0] < 1.0, covariance)\n290:     fusion.rejectOutliers({\"method\": \"normalized-innovation\", \"threshold\": 2.0})\n291:     surface(\"G041-S06-07\", fusion._policy[\"threshold\"] == 2.0, fusion._policy)",
