# robotics — filter.frequencyResponse(points)

Deterministic DSP/PID/replay model; no physical actuator, sensor or hard real-time proof; original observation: low-pass response attenuates the Nyquist endpoint; SDK bounds: {"MAX_LOOP_TICKS":512,"MAX_MATRIX_DIMENSION":16,"MAX_SAMPLES":4096,"MAX_TRANSFORM_POINTS":256}

```text
Identity: owner:G041:G041-S02-08 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Filter.frequencyResponse(self, points: int) -> tuple[FrequencyResponsePoint, ...]
```
