# robotics — signal.resample(rate,method)

Deterministic DSP/PID/replay model; no physical actuator, sensor or hard real-time proof; original observation: linear resampling produces seven input-dependent samples; SDK bounds: {"MAX_LOOP_TICKS":512,"MAX_MATRIX_DIMENSION":16,"MAX_SAMPLES":4096,"MAX_TRANSFORM_POINTS":256}

```text
Identity: owner:G041:G041-S01-04 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
SignalBuffer.resample(self, rate: float, method: str) -> tuple[SignalSample, ...]
```
...
[
  ...
  {
    "excerpt": "86:     surface(\"G041-S01-03\", tuple(point.value for point in window) == (1.0, 0.0, -1.0), window)\n87:     resampled = signal.resample(8.0, \"linear\")\n88:     surface(\"G041-S01-04\", len(resampled) == 7 and close(resampled[1].value or 0.0, 0.5), resampled[1])\n89:     decimated = signal.decimate(2, Filter.lowPass(1.0, 4.0, 1))\n90:     surface(\"G041-S01-05\", len(decimated) == 2 and decimated[0].timestamp == 0.0, decimated)",
