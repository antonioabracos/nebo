# robotics — signal.decimate(factor,filter)

Deterministic DSP/PID/replay model; no physical actuator, sensor or hard real-time proof; original observation: explicit smoothing filter precedes bounded decimation; SDK bounds: {"MAX_LOOP_TICKS":512,"MAX_MATRIX_DIMENSION":16,"MAX_SAMPLES":4096,"MAX_TRANSFORM_POINTS":256}

```text
Identity: owner:G041:G041-S01-05 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
SignalBuffer.decimate(self, factor: int, filter: 'Filter') -> tuple[SignalSample, ...]
```
