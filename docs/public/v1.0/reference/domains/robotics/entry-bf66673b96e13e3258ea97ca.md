# robotics — SignalBuffer.new(capacity,sampleRate,unit)

Deterministic DSP/PID/replay model; no physical actuator, sensor or hard real-time proof; original observation: capacity rate and engineering unit remain explicit; SDK bounds: {"MAX_LOOP_TICKS":512,"MAX_MATRIX_DIMENSION":16,"MAX_SAMPLES":4096,"MAX_TRANSFORM_POINTS":256}

```text
Identity: owner:G041:G041-S01-01 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
SignalBuffer.new(cls, capacity: int, sampleRate: float, unit: str) -> 'SignalBuffer'
```
