# robotics — model.discretize(period,method)

Deterministic DSP/PID/replay model; no physical actuator, sensor or hard real-time proof; original observation: explicit Euler period governs discrete A and B matrices; SDK bounds: {"MAX_LOOP_TICKS":512,"MAX_MATRIX_DIMENSION":16,"MAX_SAMPLES":4096,"MAX_TRANSFORM_POINTS":256}

```text
Identity: owner:G041:G041-S04-06 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
StateSpace.discretize(self, period: float, method: str) -> 'StateSpace'
```
