# robotics — PidController.new(kp,ki,kd,options)

Deterministic DSP/PID/replay model; no physical actuator, sensor or hard real-time proof; original observation: gains anti-windup and derivative filtering are explicit; SDK bounds: {"MAX_LOOP_TICKS":512,"MAX_MATRIX_DIMENSION":16,"MAX_SAMPLES":4096,"MAX_TRANSFORM_POINTS":256}

```text
Identity: owner:G041:G041-S03-01 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
PidController.new(cls, kp: float, ki: float, kd: float, options: Mapping[str, Any] | None) -> 'PidController'
```
...
[
  ...
  {
    "excerpt": "79: def subgroup_s01() -> None:\n80:     signal = SignalBuffer.new(8, 4.0, \"m/s\")\n81:     surface(\"G041-S01-01\", signal.capacity == 8 and signal.sample_rate == 4.0 and signal.unit == \"m/s\", signal.capacity)\n82:     for value, timestamp in ((0.0, 0.0), (1.0, 0.25), (0.0, 0.5), (-1.0, 0.75)):\n83:         signal.push(value, timestamp)",
