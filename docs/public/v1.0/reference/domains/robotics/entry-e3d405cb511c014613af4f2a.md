# robotics — RobotModel.load(description,capability)

Deterministic DSP/PID/replay model; no physical actuator, sensor or hard real-time proof; original observation: versioned two-link model accepts simulation capability only; SDK bounds: {"MAX_LOOP_TICKS":512,"MAX_MATRIX_DIMENSION":16,"MAX_SAMPLES":4096,"MAX_TRANSFORM_POINTS":256}

```text
Identity: owner:G041:G041-S05-03 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
RobotModel.load(cls, description: Mapping[str, Any], capability: str) -> 'RobotModel'
```
...
[
  ...
  {
    "excerpt": "243:     description = {\"version\": 1, \"linkLengths\": (1.0, 1.0), \"jointLimits\": ((-math.pi, math.pi), (-math.pi, math.pi)), \"frame\": \"base\"}\n244:     robot = RobotModel.load(description, \"simulation\")\n245:     surface(\"G041-S05-03\", robot.capability == \"simulation\" and robot.link_lengths == (1.0, 1.0), robot.link_lengths)\n246:     forward = robot.forwardKinematics((0.0, math.pi / 2.0))\n247:     surface(\"G041-S05-04\", close(forward.position[0], 1.0) and close(forward.position[1], 1.0), forward)",
