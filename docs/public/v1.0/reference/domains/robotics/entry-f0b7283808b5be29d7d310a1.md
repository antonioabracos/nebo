# robotics — controller.setLimits(min,max)

Deterministic DSP/PID/replay model; no physical actuator, sensor or hard real-time proof; original observation: ordered finite output limits govern saturation; SDK bounds: {"MAX_LOOP_TICKS":512,"MAX_MATRIX_DIMENSION":16,"MAX_SAMPLES":4096,"MAX_TRANSFORM_POINTS":256}

```text
Identity: owner:G041:G041-S03-04 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
PidController.setLimits(self, min: float, max: float) -> 'PidController'
```
...
[
  ...
  {
    "excerpt": "162:     surface(\"G041-S03-02\", controller.setTarget(10.0).target == 10.0, controller.target)\n163:     controller.setLimits(-3.0, 3.0)\n164:     step = controller.step(0.0, 0.1)\n165:     surface(\"G041-S03-03\", step.output == 3.0 and step.saturated, step)\n166:     surface(\"G041-S03-04\", controller.minimum == -3.0 and controller.maximum == 3.0, (controller.minimum, controller.maximum))",
