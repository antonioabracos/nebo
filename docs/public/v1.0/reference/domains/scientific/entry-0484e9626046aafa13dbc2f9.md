# scientific — optimizer.constraints(functions)

Bounded CPU sparse/FFT/solver reference; no arbitrary precision or convergence outside documented limits; original observation: optimizer.constraints(functions); SDK bounds: {"MAX_DENSE_CELLS":65536,"MAX_DIMENSION":4096,"MAX_FFT_LENGTH":64,"MAX_GRID_CELLS":65536,"MAX_ITERATIONS":100000,"MAX_NONZERO":65536,"MAX_TRACE":4096}

```text
Identity: owner:G036:G036-S06-05 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
OptimizerConfig.constraints(self, functions: Sequence[Callable[[Sequence[float]], Any]]) -> 'Optimizer'
Optimizer.constraints(self, functions: Sequence[Callable[[Sequence[float]], Any]]) -> 'Optimizer'
```
