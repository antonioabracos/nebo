# scientific — optimizer.minimize(function, initial)

Bounded CPU sparse/FFT/solver reference; no arbitrary precision or convergence outside documented limits; original observation: optimizer.minimize(function, initial); SDK bounds: {"MAX_DENSE_CELLS":65536,"MAX_DIMENSION":4096,"MAX_FFT_LENGTH":64,"MAX_GRID_CELLS":65536,"MAX_ITERATIONS":100000,"MAX_NONZERO":65536,"MAX_TRACE":4096}

```text
Identity: owner:G036:G036-S06-06 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
OptimizerConfig.minimize(self, function: Callable[[Sequence[float]], Any], initial: Sequence[Any]) -> 'Solution'
Optimizer.minimize(self, function: Callable[[Sequence[float]], Any], initial: Sequence[Any]) -> 'Solution'
```
...
[
  ...
  {
    "excerpt": "151: ok(\"positive\", \"optimizer.constraints\", len(constrained._constraints) == 1)\n152: solution = constrained.minimize(objective, (0, 0))\n153: ok(\"positive\", \"optimizer.minimize\", solution.status == \"converged\" and close(solution.point[0], 3, 1e-6))\n154: ok(\"positive\", \"solution.gradientNorm\", solution.gradientNorm() < 1e-7)\n155: optimality = solution.optimalityReport()",
...
- runtime/optimization/optimization.asm — SHA-256 a002a416a1a977e1213fdd089dfa73b415c95dede56ab5aac5301c58720332df
