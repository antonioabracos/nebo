# scientific — trajectory.errorReport()

Bounded CPU sparse/FFT/solver reference; no arbitrary precision or convergence outside documented limits; original observation: trajectory.errorReport(); SDK bounds: {"MAX_DENSE_CELLS":65536,"MAX_DIMENSION":4096,"MAX_FFT_LENGTH":64,"MAX_GRID_CELLS":65536,"MAX_ITERATIONS":100000,"MAX_NONZERO":65536,"MAX_TRACE":4096}

```text
Identity: owner:G036:G036-S04-08 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Trajectory.errorReport(self) -> Mapping[str, Any]
```
...
[
  ...
  {
    "excerpt": "115: ok(\"positive\", \"trajectory.steps\", trajectory.steps() == 20)\n116: ode_report = trajectory.errorReport()\n117: ok(\"positive\", \"trajectory.errorReport\", ode_report[\"acceptedSteps\"] == 20 and len(ode_report[\"events\"]) == 1)\n118: transcript.append((trajectory.sample(0.5), trajectory.sample(1), dict(ode_report)))\n119: ",
