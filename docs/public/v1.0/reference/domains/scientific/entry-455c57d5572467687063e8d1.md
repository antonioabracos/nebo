# scientific — signal.correlate(other, mode)

Bounded CPU sparse/FFT/solver reference; no arbitrary precision or convergence outside documented limits; original observation: signal.correlate(other, mode); SDK bounds: {"MAX_DENSE_CELLS":65536,"MAX_DIMENSION":4096,"MAX_FFT_LENGTH":64,"MAX_GRID_CELLS":65536,"MAX_ITERATIONS":100000,"MAX_NONZERO":65536,"MAX_TRACE":4096}

```text
Identity: owner:G036:G036-S02-05 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Signal.correlate(self, other: Sequence[Any], mode: str='full') -> tuple[float, ...]
```
...
[
  ...
  {
    "excerpt": "73: ok(\"positive\", \"signal.convolve\", signal.convolve((2, 1), \"full\") == (2.0, 5.0, 8.0, 3.0))\n74: ok(\"positive\", \"signal.correlate\", signal.correlate((2, 1), \"full\") == (1.0, 4.0, 7.0, 6.0))\n75: ok(\"positive\", \"signal.autocorrelation\", signal.autocorrelation() == (3.0, 8.0, 14.0, 8.0, 3.0))\n76: magnitudes = spectrum.magnitude()\n77: ok(\"positive\", \"spectrum.magnitude\", close(magnitudes[0], 10) and close(magnitudes[2], 2))",
