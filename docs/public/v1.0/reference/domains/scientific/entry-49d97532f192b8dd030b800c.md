# scientific — OdeMethod.adaptiveRk(options)

Bounded CPU sparse/FFT/solver reference; no arbitrary precision or convergence outside documented limits; original observation: OdeMethod.adaptiveRk(options); SDK bounds: {"MAX_DENSE_CELLS":65536,"MAX_DIMENSION":4096,"MAX_FFT_LENGTH":64,"MAX_GRID_CELLS":65536,"MAX_ITERATIONS":100000,"MAX_NONZERO":65536,"MAX_TRACE":4096}

```text
Identity: owner:G036:G036-S04-04 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
OdeMethod.adaptiveRk(options: Mapping[str, Any]) -> OdeMethodConfig
```
...
[
  ...
  {
    "excerpt": "108: ok(\"positive\", \"OdeMethod.rk4\", fixed_method.kind == \"rk4\" and fixed_method.step == 0.05)\n109: adaptive_method = OdeMethod.adaptiveRk({\"initialStep\": 0.2, \"maximumStep\": 0.25})\n110: ok(\"positive\", \"OdeMethod.adaptiveRk\", adaptive_method.kind == \"adaptive-rk4\")\n111: ok(\"positive\", \"ode.event\", len(problem._events) == 1)\n112: trajectory = problem.solve(fixed_method, {\"maxSteps\": 100})",
