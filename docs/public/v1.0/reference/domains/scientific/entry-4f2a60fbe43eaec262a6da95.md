# scientific — ContinuousOptimizer.lbfgs(options)

Bounded CPU sparse/FFT/solver reference; no arbitrary precision or convergence outside documented limits; original observation: ContinuousOptimizer.lbfgs(options); SDK bounds: {"MAX_DENSE_CELLS":65536,"MAX_DIMENSION":4096,"MAX_FFT_LENGTH":64,"MAX_GRID_CELLS":65536,"MAX_ITERATIONS":100000,"MAX_NONZERO":65536,"MAX_TRACE":4096}

```text
Identity: owner:G036:G036-S06-02 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
ContinuousOptimizer.lbfgs(options: Mapping[str, Any] | None=None) -> OptimizerConfig
```
...
[
  ...
  {
    "excerpt": "143: ok(\"positive\", \"ContinuousOptimizer.gradientDescent\", gradient_config.kind == \"gradientDescent\")\n144: lbfgs_config = ContinuousOptimizer.lbfgs({\"learningRate\": 0.5})\n145: ok(\"positive\", \"ContinuousOptimizer.lbfgs\", lbfgs_config.kind == \"lbfgs\")\n146: adam_config = ContinuousOptimizer.adam({\"learningRate\": 0.2})\n147: ok(\"positive\", \"ContinuousOptimizer.adam\", adam_config.kind == \"adam\")",
