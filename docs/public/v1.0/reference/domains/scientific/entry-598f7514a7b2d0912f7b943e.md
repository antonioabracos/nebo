# scientific — Boundary.dirichlet(region, value)

Bounded CPU sparse/FFT/solver reference; no arbitrary precision or convergence outside documented limits; original observation: Boundary.dirichlet(region, value); SDK bounds: {"MAX_DENSE_CELLS":65536,"MAX_DIMENSION":4096,"MAX_FFT_LENGTH":64,"MAX_GRID_CELLS":65536,"MAX_ITERATIONS":100000,"MAX_NONZERO":65536,"MAX_TRACE":4096}

```text
Identity: owner:G036:G036-S05-03 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Boundary.dirichlet(region: str, value: Any) -> 'Boundary'
```
...
[
  ...
  {
    "excerpt": "124: ok(\"positive\", \"Grid.coordinates\", len(coordinates) == 17 and coordinates[8] == 0.5)\n125: lower = Boundary.dirichlet(\"lower\", 0)\n126: ok(\"positive\", \"Boundary.dirichlet\", lower.kind == \"dirichlet\" and lower.value == 0)\n127: flux = Boundary.neumann(\"upper\", 0)\n128: ok(\"positive\", \"Boundary.neumann\", flux.kind == \"neumann\" and flux.value == 0)",
