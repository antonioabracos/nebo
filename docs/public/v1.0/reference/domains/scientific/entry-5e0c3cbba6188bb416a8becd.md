# scientific — ode.event(condition, direction)

Bounded CPU sparse/FFT/solver reference; no arbitrary precision or convergence outside documented limits; original observation: ode.event(condition, direction); SDK bounds: {"MAX_DENSE_CELLS":65536,"MAX_DIMENSION":4096,"MAX_FFT_LENGTH":64,"MAX_GRID_CELLS":65536,"MAX_ITERATIONS":100000,"MAX_NONZERO":65536,"MAX_TRACE":4096}

```text
Identity: owner:G036:G036-S04-05 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
OdeProblem.event(self, condition: Callable[..., Any], direction: int=0) -> 'OdeProblem'
```
...
[
  ...
  {
    "excerpt": "105: ok(\"positive\", \"OdeProblem.define\", problem.initial == (1.0,) and problem.interval == (0.0, 1.0))\n106: problem.event(lambda _time, state: state[0] - 2.0, 1)\n107: fixed_method = OdeMethod.rk4(0.05)\n108: ok(\"positive\", \"OdeMethod.rk4\", fixed_method.kind == \"rk4\" and fixed_method.step == 0.05)\n109: adaptive_method = OdeMethod.adaptiveRk({\"initialStep\": 0.2, \"maximumStep\": 0.25})",
