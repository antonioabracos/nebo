# scientific — Integrate.adaptive(function, interval, tolerance)

Bounded CPU sparse/FFT/solver reference; no arbitrary precision or convergence outside documented limits; original observation: Integrate.adaptive(function, interval, tolerance); SDK bounds: {"MAX_DENSE_CELLS":65536,"MAX_DIMENSION":4096,"MAX_FFT_LENGTH":64,"MAX_GRID_CELLS":65536,"MAX_ITERATIONS":100000,"MAX_NONZERO":65536,"MAX_TRACE":4096}

```text
Identity: owner:G036:G036-S03-06 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Integrate.adaptive(function: Callable[[float], Any], interval: Sequence[Any], tolerance: Any) -> NumericResult
```
...
[
  ...
  {
    "excerpt": "94: ok(\"positive\", \"Integrate.simpson\", close(simpson.value, 1/3, 1e-12))\n95: adaptive = Integrate.adaptive(math.sin, (0, math.pi), 1e-9)\n96: ok(\"positive\", \"Integrate.adaptive\", close(adaptive.value, 2.0, 1e-9))\n97: multi = Integrate.multiDimensional(lambda point: point[0] + point[1], ((0, 1), (0, 1)), {\"steps\": 8})\n98: ok(\"positive\", \"Integrate.multiDimensional\", close(multi.value, 1.0, 1e-12))",
