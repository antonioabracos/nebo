# scientific — WindowFunction.hann(length)

Bounded CPU sparse/FFT/solver reference; no arbitrary precision or convergence outside documented limits; original observation: WindowFunction.hann(length); SDK bounds: {"MAX_DENSE_CELLS":65536,"MAX_DIMENSION":4096,"MAX_FFT_LENGTH":64,"MAX_GRID_CELLS":65536,"MAX_ITERATIONS":100000,"MAX_NONZERO":65536,"MAX_TRACE":4096}

```text
Identity: owner:G036:G036-S02-09 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
WindowFunction.hann(length: int) -> tuple[float, ...]
```
...
[
  ...
  {
    "excerpt": "79: ok(\"positive\", \"spectrum.power\", close(powers[0], 100) and close(powers[2], 4))\n80: hann = WindowFunction.hann(5)\n81: ok(\"positive\", \"WindowFunction.hann\", hann[0] == hann[-1] == 0.0 and close(hann[2], 1))\n82: transcript.append(([round(value.real, 12) for value in spectrum], powers, hann))\n83: ",
