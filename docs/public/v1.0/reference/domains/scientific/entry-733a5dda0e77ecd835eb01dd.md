# scientific — ode.solve(method, tolerances)

Bounded CPU sparse/FFT/solver reference; no arbitrary precision or convergence outside documented limits; original observation: ode.solve(method, tolerances); SDK bounds: {"MAX_DENSE_CELLS":65536,"MAX_DIMENSION":4096,"MAX_FFT_LENGTH":64,"MAX_GRID_CELLS":65536,"MAX_ITERATIONS":100000,"MAX_NONZERO":65536,"MAX_TRACE":4096}

```text
Identity: owner:G036:G036-S04-02 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
OdeProblem.solve(self, method: OdeMethodConfig, tolerances: Mapping[str, Any] | None=None) -> 'Trajectory'
PdeProblem.solve(self, method: str='jacobi', options: Mapping[str, Any] | None=None) -> 'Field'
```
...
[
  ...
  {
    "excerpt": "111: ok(\"positive\", \"ode.event\", len(problem._events) == 1)\n112: trajectory = problem.solve(fixed_method, {\"maxSteps\": 100})\n113: ok(\"positive\", \"ode.solve\", close(trajectory.sample(1)[0], math.e, 2e-6))\n114: ok(\"positive\", \"trajectory.sample\", close(trajectory.sample(0.5)[0], math.exp(0.5), 1e-6))\n115: ok(\"positive\", \"trajectory.steps\", trajectory.steps() == 20)",
...
- runtime/ode/ode.asm — SHA-256 98310dc903c552251f593ea931890a2a175891e94ff8198d4fb26c5f16727f9f
