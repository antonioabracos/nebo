# scientific — Grid.uniform(bounds, shape)

Bounded CPU sparse/FFT/solver reference; no arbitrary precision or convergence outside documented limits; original observation: Grid.uniform(bounds, shape); SDK bounds: {"MAX_DENSE_CELLS":65536,"MAX_DIMENSION":4096,"MAX_FFT_LENGTH":64,"MAX_GRID_CELLS":65536,"MAX_ITERATIONS":100000,"MAX_NONZERO":65536,"MAX_TRACE":4096}

```text
Identity: owner:G036:G036-S05-01 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Grid.uniform(bounds: Sequence[Any], shape: Sequence[int]) -> 'Grid'
```
...
[
  ...
  {
    "excerpt": "120: # S05 — uniform coordinates, both boundary kinds, Jacobi solve and derivatives.\n121: grid = Grid.uniform((0, 1), (17,))\n122: ok(\"positive\", \"Grid.uniform\", grid.shape == (17,))\n123: coordinates = grid.coordinates()\n124: ok(\"positive\", \"Grid.coordinates\", len(coordinates) == 17 and coordinates[8] == 0.5)",
