# scientific — ScientificContext.new(options)

Bounded CPU sparse/FFT/solver reference; no arbitrary precision or convergence outside documented limits; original observation: ScientificContext.new(options); SDK bounds: {"MAX_DENSE_CELLS":65536,"MAX_DIMENSION":4096,"MAX_FFT_LENGTH":64,"MAX_GRID_CELLS":65536,"MAX_ITERATIONS":100000,"MAX_NONZERO":65536,"MAX_TRACE":4096}

```text
Identity: owner:G036:G036-S07-01 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
ScientificContext.new(options: Mapping[str, Any] | None=None) -> 'ScientificContext'
```
...
[
  ...
  {
    "excerpt": "159: # S07 — versioned context, generic result evidence and the two CLI algorithms.\n160: context = ScientificContext.new({\"absoluteTolerance\": 1e-10})\n161: ok(\"positive\", \"ScientificContext.new\", context.manifest()[\"threads\"] == 1)\n162: manifest = solution.reproducibilityManifest()\n163: ok(\"positive\", \"result.reproducibilityManifest\", manifest[\"schema\"] == \"nebo-scientific-result-v1\" and len(manifest[\"payloadSha256\"]) == 64)",
...
- runtime/scientific/reproducibility.asm — SHA-256 1850cc707be98d299a46b44e53fb33c4971ad2e53d76d6524c5f95edcaee89b1
