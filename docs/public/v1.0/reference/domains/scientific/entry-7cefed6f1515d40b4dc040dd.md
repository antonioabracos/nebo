# scientific — solver.checkpoint()

Bounded CPU sparse/FFT/solver reference; no arbitrary precision or convergence outside documented limits; original observation: solver.checkpoint(); SDK bounds: {"MAX_DENSE_CELLS":65536,"MAX_DIMENSION":4096,"MAX_FFT_LENGTH":64,"MAX_GRID_CELLS":65536,"MAX_ITERATIONS":100000,"MAX_NONZERO":65536,"MAX_TRACE":4096}

```text
Identity: owner:G036:G036-S07-05 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
_EvidenceResult.checkpoint(self) -> bytes
```
...
[
  ...
  {
    "excerpt": "167: ok(\"positive\", \"solver.convergenceTrace\", len(trace) > 1 and trace[-1][\"gradientNorm\"] < trace[0][\"gradientNorm\"])\n168: checkpoint = solution.checkpoint()\n169: ok(\"positive\", \"solver.checkpoint\", json.loads(checkpoint)[\"schema\"] == \"nebo-scientific-checkpoint-v1\")\n170: benchmark = scientific_bench(\"core\")\n171: ok(\"positive\", \"neboc scientific-bench\", benchmark[\"schema\"] == \"nebo-scientific-bench-v1\" and len(benchmark[\"sha256\"]) == 64)",
