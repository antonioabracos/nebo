# scientific — result.compare(reference, tolerances)

Bounded CPU sparse/FFT/solver reference; no arbitrary precision or convergence outside documented limits; original observation: result.compare(reference, tolerances); SDK bounds: {"MAX_DENSE_CELLS":65536,"MAX_DIMENSION":4096,"MAX_FFT_LENGTH":64,"MAX_GRID_CELLS":65536,"MAX_ITERATIONS":100000,"MAX_NONZERO":65536,"MAX_TRACE":4096}

```text
Identity: owner:G036:G036-S07-03 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
_EvidenceResult.compare(self, reference: Any, tolerances: Mapping[str, Any] | None=None) -> Mapping[str, Any]
```
...
[
  ...
  {
    "excerpt": "163: ok(\"positive\", \"result.reproducibilityManifest\", manifest[\"schema\"] == \"nebo-scientific-result-v1\" and len(manifest[\"payloadSha256\"]) == 64)\n164: comparison = solution.compare(solution, {\"absolute\": 0, \"relative\": 0})\n165: ok(\"positive\", \"result.compare\", comparison[\"passed\"] and comparison[\"maximumError\"] == 0)\n166: trace = solution.convergenceTrace()\n167: ok(\"positive\", \"solver.convergenceTrace\", len(trace) > 1 and trace[-1][\"gradientNorm\"] < trace[0][\"gradientNorm\"])",
