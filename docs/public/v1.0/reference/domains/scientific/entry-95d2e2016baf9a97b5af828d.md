# scientific — Integrate.trapezoid(function, interval, steps)

Bounded CPU sparse/FFT/solver reference; no arbitrary precision or convergence outside documented limits; original observation: Integrate.trapezoid(function, interval, steps); SDK bounds: {"MAX_DENSE_CELLS":65536,"MAX_DIMENSION":4096,"MAX_FFT_LENGTH":64,"MAX_GRID_CELLS":65536,"MAX_ITERATIONS":100000,"MAX_NONZERO":65536,"MAX_TRACE":4096}

```text
Identity: owner:G036:G036-S03-04 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Integrate.trapezoid(function: Callable[[float], Any], interval: Sequence[Any], steps: int) -> NumericResult
```
...
[
  ...
  {
    "excerpt": "90: ok(\"positive\", \"Root.newton\", close(newton.value, math.sqrt(2), 1e-10))\n91: trapezoid = Integrate.trapezoid(lambda x: x*x, (0, 1), 100)\n92: ok(\"positive\", \"Integrate.trapezoid\", close(trapezoid.value, 1/3, 2e-5))\n93: simpson = Integrate.simpson(lambda x: x*x, (0, 1), 10)\n94: ok(\"positive\", \"Integrate.simpson\", close(simpson.value, 1/3, 1e-12))",
