# scientific — signal.convolve(kernel, mode)

Bounded CPU sparse/FFT/solver reference; no arbitrary precision or convergence outside documented limits; original observation: signal.convolve(kernel, mode); SDK bounds: {"MAX_DENSE_CELLS":65536,"MAX_DIMENSION":4096,"MAX_FFT_LENGTH":64,"MAX_GRID_CELLS":65536,"MAX_ITERATIONS":100000,"MAX_NONZERO":65536,"MAX_TRACE":4096}

```text
Identity: owner:G036:G036-S02-04 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Signal.convolve(self, kernel: Sequence[Any], mode: str='full') -> tuple[float, ...]
```
