# scientific — Fft.plan(length, direction, options)

Bounded CPU sparse/FFT/solver reference; no arbitrary precision or convergence outside documented limits; original observation: Fft.plan(length, direction, options); SDK bounds: {"MAX_DENSE_CELLS":65536,"MAX_DIMENSION":4096,"MAX_FFT_LENGTH":64,"MAX_GRID_CELLS":65536,"MAX_ITERATIONS":100000,"MAX_NONZERO":65536,"MAX_TRACE":4096}

```text
Identity: owner:G036:G036-S02-01 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Fft.plan(length: int, direction: str='both', options: Mapping[str, Any] | None=None) -> FftPlan
```
...
[
  ...
  {
    "excerpt": "64: # S02 — scalar DFT, inverse identity, signal operations and Hann endpoints.\n65: plan = Fft.plan(4, \"both\", {\"normalization\": \"inverse\"})\n66: ok(\"positive\", \"Fft.plan\", plan.length == 4 and plan.direction == \"both\")\n67: spectrum = plan.forward((1, 2, 3, 4))\n68: ok(\"positive\", \"fft.forward\", close(spectrum[0].real, 10) and close(spectrum[1].imag, 2))",
