# scientific — spectrum.power()

Bounded CPU sparse/FFT/solver reference; no arbitrary precision or convergence outside documented limits; original observation: spectrum.power(); SDK bounds: {"MAX_DENSE_CELLS":65536,"MAX_DIMENSION":4096,"MAX_FFT_LENGTH":64,"MAX_GRID_CELLS":65536,"MAX_ITERATIONS":100000,"MAX_NONZERO":65536,"MAX_TRACE":4096}

```text
Identity: owner:G036:G036-S02-08 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Spectrum.power(self) -> tuple[float, ...]
```
...
[
  ...
  {
    "excerpt": "77: ok(\"positive\", \"spectrum.magnitude\", close(magnitudes[0], 10) and close(magnitudes[2], 2))\n78: powers = spectrum.power()\n79: ok(\"positive\", \"spectrum.power\", close(powers[0], 100) and close(powers[2], 4))\n80: hann = WindowFunction.hann(5)\n81: ok(\"positive\", \"WindowFunction.hann\", hann[0] == hann[-1] == 0.0 and close(hann[2], 1))",
