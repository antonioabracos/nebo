# scientific — solution.gradientNorm()

Bounded CPU sparse/FFT/solver reference; no arbitrary precision or convergence outside documented limits; original observation: solution.gradientNorm(); SDK bounds: {"MAX_DENSE_CELLS":65536,"MAX_DIMENSION":4096,"MAX_FFT_LENGTH":64,"MAX_GRID_CELLS":65536,"MAX_ITERATIONS":100000,"MAX_NONZERO":65536,"MAX_TRACE":4096}

```text
Identity: owner:G036:G036-S06-07 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Solution.gradientNorm(self) -> float
```
...
[
  ...
  {
    "excerpt": "153: ok(\"positive\", \"optimizer.minimize\", solution.status == \"converged\" and close(solution.point[0], 3, 1e-6))\n154: ok(\"positive\", \"solution.gradientNorm\", solution.gradientNorm() < 1e-7)\n155: optimality = solution.optimalityReport()\n156: ok(\"positive\", \"solution.optimalityReport\", optimality[\"status\"] == \"converged\" and optimality[\"method\"] == \"gradientDescent\")\n157: transcript.append((solution.point, dict(optimality)))",
