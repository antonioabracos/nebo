# scientific — Integrate.multiDimensional(function, domain, options)

Bounded CPU sparse/FFT/solver reference; no arbitrary precision or convergence outside documented limits; original observation: Integrate.multiDimensional(function, domain, options); SDK bounds: {"MAX_DENSE_CELLS":65536,"MAX_DIMENSION":4096,"MAX_FFT_LENGTH":64,"MAX_GRID_CELLS":65536,"MAX_ITERATIONS":100000,"MAX_NONZERO":65536,"MAX_TRACE":4096}

```text
Identity: owner:G036:G036-S03-07 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Integrate.multiDimensional(function: Callable[[Sequence[float]], Any], domain: Sequence[Sequence[Any]], options: Mapping[str, Any] | None=None) -> NumericResult
```
...
[
  ...
  {
    "excerpt": "96: ok(\"positive\", \"Integrate.adaptive\", close(adaptive.value, 2.0, 1e-9))\n97: multi = Integrate.multiDimensional(lambda point: point[0] + point[1], ((0, 1), (0, 1)), {\"steps\": 8})\n98: ok(\"positive\", \"Integrate.multiDimensional\", close(multi.value, 1.0, 1e-12))\n99: ok(\"positive\", \"numericResult.errorEstimate\", 0 <= adaptive.errorEstimate() <= 1e-9)\n100: transcript.append((bisection.value, brent.value, newton.value,",
