# scientific — field.gradient()

Bounded CPU sparse/FFT/solver reference; no arbitrary precision or convergence outside documented limits; original observation: field.gradient(); SDK bounds: {"MAX_DENSE_CELLS":65536,"MAX_DIMENSION":4096,"MAX_FFT_LENGTH":64,"MAX_GRID_CELLS":65536,"MAX_ITERATIONS":100000,"MAX_NONZERO":65536,"MAX_TRACE":4096}

```text
Identity: owner:G036:G036-S05-07 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Field.gradient(self) -> tuple[float, ...]
```
...
[
  ...
  {
    "excerpt": "132: ok(\"positive\", \"pde.solve\", field.status == \"converged\" and close(field.values[8], 0.25, 1e-7))\n133: gradient = field.gradient()\n134: ok(\"positive\", \"field.gradient\", close(gradient[8], 0.0, 1e-7))\n135: laplacian = field.laplacian()\n136: ok(\"positive\", \"field.laplacian\", close(laplacian[8], -2.0, 1e-6))",
