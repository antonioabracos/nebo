# scientific — optimizer.bounds(lower, upper)

Bounded CPU sparse/FFT/solver reference; no arbitrary precision or convergence outside documented limits; original observation: optimizer.bounds(lower, upper); SDK bounds: {"MAX_DENSE_CELLS":65536,"MAX_DIMENSION":4096,"MAX_FFT_LENGTH":64,"MAX_GRID_CELLS":65536,"MAX_ITERATIONS":100000,"MAX_NONZERO":65536,"MAX_TRACE":4096}

```text
Identity: owner:G036:G036-S06-04 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
OptimizerConfig.bounds(self, lower: Sequence[Any], upper: Sequence[Any]) -> 'Optimizer'
Optimizer.bounds(self, lower: Sequence[Any], upper: Sequence[Any]) -> 'Optimizer'
```
...
[
  ...
  {
    "excerpt": "147: ok(\"positive\", \"ContinuousOptimizer.adam\", adam_config.kind == \"adam\")\n148: bounded = gradient_config.bounds((-5, -5), (5, 5))\n149: ok(\"positive\", \"optimizer.bounds\", bounded.lower == (-5.0, -5.0) and bounded.upper == (5.0, 5.0))\n150: constrained = bounded.constraints((lambda point: (point[0] - 4, (1, 0)),))\n151: ok(\"positive\", \"optimizer.constraints\", len(constrained._constraints) == 1)",
