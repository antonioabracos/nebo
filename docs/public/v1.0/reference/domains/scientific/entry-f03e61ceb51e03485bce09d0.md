# scientific — Boundary.neumann(region, flux)

Bounded CPU sparse/FFT/solver reference; no arbitrary precision or convergence outside documented limits; original observation: Boundary.neumann(region, flux); SDK bounds: {"MAX_DENSE_CELLS":65536,"MAX_DIMENSION":4096,"MAX_FFT_LENGTH":64,"MAX_GRID_CELLS":65536,"MAX_ITERATIONS":100000,"MAX_NONZERO":65536,"MAX_TRACE":4096}

```text
Identity: owner:G036:G036-S05-04 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Boundary.neumann(region: str, flux: Any) -> 'Boundary'
```
...
[
  ...
  {
    "excerpt": "126: ok(\"positive\", \"Boundary.dirichlet\", lower.kind == \"dirichlet\" and lower.value == 0)\n127: flux = Boundary.neumann(\"upper\", 0)\n128: ok(\"positive\", \"Boundary.neumann\", flux.kind == \"neumann\" and flux.value == 0)\n129: pde = PdeProblem.define(lambda _x: 2.0, grid, (lower, Boundary.dirichlet(\"upper\", 0)))\n130: ok(\"positive\", \"PdeProblem.define\", pde.grid is grid and len(pde.boundaries) == 2)",
