# scientific — Root.brent(function, bracket, tolerance)

Bounded CPU sparse/FFT/solver reference; no arbitrary precision or convergence outside documented limits; original observation: Root.brent(function, bracket, tolerance); SDK bounds: {"MAX_DENSE_CELLS":65536,"MAX_DIMENSION":4096,"MAX_FFT_LENGTH":64,"MAX_GRID_CELLS":65536,"MAX_ITERATIONS":100000,"MAX_NONZERO":65536,"MAX_TRACE":4096}

```text
Identity: owner:G036:G036-S03-02 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Root.brent(function: Callable[[float], Any], bracket: Sequence[Any], tolerance: Any) -> NumericResult
```
...
[
  ...
  {
    "excerpt": "86: ok(\"positive\", \"Root.bisection\", close(bisection.value, math.sqrt(2), 1e-9))\n87: brent = Root.brent(lambda x: x*x - 2, (0, 2), 1e-10)\n88: ok(\"positive\", \"Root.brent\", close(brent.value, math.sqrt(2), 1e-9))\n89: newton = Root.newton(lambda x: x*x - 2, lambda x: 2*x, 1.0)\n90: ok(\"positive\", \"Root.newton\", close(newton.value, math.sqrt(2), 1e-10))",
