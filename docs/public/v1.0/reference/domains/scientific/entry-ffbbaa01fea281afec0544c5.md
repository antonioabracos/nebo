# scientific — fft.forward(signal)

Bounded CPU sparse/FFT/solver reference; no arbitrary precision or convergence outside documented limits; original observation: fft.forward(signal); SDK bounds: {"MAX_DENSE_CELLS":65536,"MAX_DIMENSION":4096,"MAX_FFT_LENGTH":64,"MAX_GRID_CELLS":65536,"MAX_ITERATIONS":100000,"MAX_NONZERO":65536,"MAX_TRACE":4096}

```text
Identity: owner:G036:G036-S02-02 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
FftPlan.forward(self, signal: Sequence[Any]) -> Spectrum
```
...
[
  ...
  {
    "excerpt": "66: ok(\"positive\", \"Fft.plan\", plan.length == 4 and plan.direction == \"both\")\n67: spectrum = plan.forward((1, 2, 3, 4))\n68: ok(\"positive\", \"fft.forward\", close(spectrum[0].real, 10) and close(spectrum[1].imag, 2))\n69: restored = plan.inverse(spectrum)\n70: ok(\"positive\", \"fft.inverse\", all(close(value.real, expected) and close(value.imag, 0)",
...
- runtime/fft/fft.asm — SHA-256 5d4a326f9020dc079d7eead4bebf523f5dcdd97a57b15238877a8bcb2ce12507
