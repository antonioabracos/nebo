# simulation — simulation.diff(a,b)

Seeded bounded ECS/physics model; no physical fidelity or safety certification; original observation: diff identifies changed simulation state; SDK bounds: {"MAX_ADVANCE_TICKS":256,"MAX_BODIES":1024,"MAX_CONSTRAINTS":4096,"MAX_CONTACTS":4096,"MAX_ENTITIES":4096,"MAX_PARTICLES":65536,"MAX_RESOURCES":1024,"MAX_ROLLBACK_TICKS":1024,"MAX_SYSTEMS":128,"MAX_TRACE_TICKS":1024}

```text
Identity: owner:G043:G043-S05-06 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Simulation.diff(a: Snapshot, b: Snapshot) -> Mapping[str, Any]
```
...
[
  ...
  {
    "excerpt": "187: final_snapshot = rollback_sim.snapshot()\n188: snapshot_diff = Simulation.diff(initial_snapshot, final_snapshot)\n189: ok(\"positive\", \"simulation.diff(a,b)\", not snapshot_diff[\"equal\"] and\n190:    \"simulationState\" in snapshot_diff[\"changed\"])\n191: rollback_sim.rollback.setWindow(2)",
