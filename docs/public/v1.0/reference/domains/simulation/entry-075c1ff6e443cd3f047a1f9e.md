# simulation — schedule.explain()

Seeded bounded ECS/physics model; no physical fidelity or safety certification; original observation: explanation exposes ordering barriers groups and one-thread policy; SDK bounds: {"MAX_ADVANCE_TICKS":256,"MAX_BODIES":1024,"MAX_CONSTRAINTS":4096,"MAX_CONTACTS":4096,"MAX_ENTITIES":4096,"MAX_PARTICLES":65536,"MAX_RESOURCES":1024,"MAX_ROLLBACK_TICKS":1024,"MAX_SYSTEMS":128,"MAX_TRACE_TICKS":1024}

```text
Identity: owner:G043:G043-S02-09 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Schedule.explain(self) -> Mapping[str, Any]
```
...
[
  ...
  {
    "excerpt": "101:    events == [\"prepare\", \"move\", \"observe\"])\n102: explanation = schedule.explain()\n103: ok(\"positive\", \"schedule.explain()\", explanation[\"ordering\"] == executed and\n104:    explanation[\"threads\"] == 1 and explanation[\"barriers\"] == 2 and\n105:    (\"move\", \"prepare\") in explanation[\"conflicts\"] and",
