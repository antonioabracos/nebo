...
Seeded bounded ECS/physics model; no physical fidelity or safety certification; original observation: profile exposes systems physics snapshots allocations and ticks; SDK bounds: {"MAX_ADVANCE_TICKS":256,"MAX_BODIES":1024,"MAX_CONSTRAINTS":4096,"MAX_CONTACTS":4096,"MAX_ENTITIES":4096,"MAX_PARTICLES":65536,"MAX_RESOURCES":1024,"MAX_ROLLBACK_TICKS":1024,"MAX_SYSTEMS":128,"MAX_TRACE_TICKS":1024}

```text
Identity: owner:G043:G043-S07-06 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Simulation.profile(self) -> Mapping[str, Any]
```
...
[
  ...
  {
    "excerpt": "248: ok(\"positive\", \"particles.applyField(field)\", particles.snapshot()[0][\"velocity\"] == (1.0, 2.0, 0.0))\n249: profile = simulation.profile()\n250: ok(\"positive\", \"simulation.profile()\", profile[\"ticks\"] == 3 and profile[\"snapshots\"] >= 1)\n251: trace = simulation.trace(2)\n252: ok(\"positive\", \"simulation.trace(ticks)\", tuple(item[\"tick\"] for item in trace) == (2, 3))",
