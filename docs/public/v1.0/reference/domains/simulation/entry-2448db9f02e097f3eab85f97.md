# simulation — scene.unload(region)

Seeded bounded ECS/physics model; no physical fidelity or safety certification; original observation: intersecting loaded resource is released; SDK bounds: {"MAX_ADVANCE_TICKS":256,"MAX_BODIES":1024,"MAX_CONSTRAINTS":4096,"MAX_CONTACTS":4096,"MAX_ENTITIES":4096,"MAX_PARTICLES":65536,"MAX_RESOURCES":1024,"MAX_ROLLBACK_TICKS":1024,"MAX_SYSTEMS":128,"MAX_TRACE_TICKS":1024}

```text
Identity: owner:G043:G043-S06-06 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Scene.unload(self, region: Sequence[Any]) -> tuple[str, ...]
```
...
[
  ...
  {
    "excerpt": "216: ok(\"positive\", \"scene.stream(region,budget)\", streamed == (\"terrain-a\",))\n217: unloaded = scene.unload((0, 0, 5, 5))\n218: ok(\"positive\", \"scene.unload(region)\", unloaded == (\"terrain-a\",))\n219: dependencies = scene.dependencies()\n220: ok(\"positive\", \"scene.dependencies()\", tuple((item[\"id\"], item[\"version\"]) for item in dependencies) ==",
