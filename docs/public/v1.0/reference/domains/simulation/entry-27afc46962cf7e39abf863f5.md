# simulation — Field.vector(function,bounds)

Seeded bounded ECS/physics model; no physical fidelity or safety certification; original observation: bounded vector field samples zero two zero; SDK bounds: {"MAX_ADVANCE_TICKS":256,"MAX_BODIES":1024,"MAX_CONSTRAINTS":4096,"MAX_CONTACTS":4096,"MAX_ENTITIES":4096,"MAX_PARTICLES":65536,"MAX_RESOURCES":1024,"MAX_ROLLBACK_TICKS":1024,"MAX_SYSTEMS":128,"MAX_TRACE_TICKS":1024}

```text
Identity: owner:G043:G043-S07-04 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Field.vector(function: Callable[[tuple[float, float, float]], Sequence[Any]], bounds: Sequence[Any]) -> 'Field'
```
...
[
  ...
  {
    "excerpt": "244: ok(\"positive\", \"particles.step(dt)\", particles.snapshot()[0][\"position\"] == (1.5, 0.0, 0.0))\n245: field = Field.vector(lambda _position: (0, 2, 0), (-10, -10, -10, 10, 10, 10))\n246: ok(\"positive\", \"Field.vector(function,bounds)\", field.sample((0, 0, 0)) == (0.0, 2.0, 0.0))\n247: particles.applyField(field)\n248: ok(\"positive\", \"particles.applyField(field)\", particles.snapshot()[0][\"velocity\"] == (1.0, 2.0, 0.0))",
