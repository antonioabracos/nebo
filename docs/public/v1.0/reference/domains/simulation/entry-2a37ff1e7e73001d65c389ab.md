# simulation — scene.load(resource,capability)

Seeded bounded ECS/physics model; no physical fidelity or safety certification; original observation: local-read capability admits bounded versioned resource; SDK bounds: {"MAX_ADVANCE_TICKS":256,"MAX_BODIES":1024,"MAX_CONSTRAINTS":4096,"MAX_CONTACTS":4096,"MAX_ENTITIES":4096,"MAX_PARTICLES":65536,"MAX_RESOURCES":1024,"MAX_ROLLBACK_TICKS":1024,"MAX_SYSTEMS":128,"MAX_TRACE_TICKS":1024}

```text
Identity: owner:G043:G043-S06-04 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Scene.load(self, resource: Mapping[str, Any], capability: Mapping[str, Any]) -> str
```
...
[
  ...
  {
    "excerpt": "211: resource_b = {\"id\": \"terrain-b\", \"version\": \"v2\", \"bytes\": 200, \"bounds\": (6, 0, 10, 5)}\n212: loaded_id = scene.load(resource_a, {\"kind\": \"local-read\", \"allowed\": True})\n213: scene.load(resource_b, {\"kind\": \"local-read\", \"allowed\": True})\n214: ok(\"positive\", \"scene.load(resource,capability)\", loaded_id == \"terrain-a\")\n215: streamed = scene.stream((0, 0, 10, 5), 250)",
