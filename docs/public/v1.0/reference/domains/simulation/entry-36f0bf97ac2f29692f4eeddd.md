# simulation — SceneTemplate.fromWorld(world,selection)

Seeded bounded ECS/physics model; no physical fidelity or safety certification; original observation: selected entity becomes versioned detached template; SDK bounds: {"MAX_ADVANCE_TICKS":256,"MAX_BODIES":1024,"MAX_CONSTRAINTS":4096,"MAX_CONTACTS":4096,"MAX_ENTITIES":4096,"MAX_PARTICLES":65536,"MAX_RESOURCES":1024,"MAX_ROLLBACK_TICKS":1024,"MAX_SYSTEMS":128,"MAX_TRACE_TICKS":1024}

```text
Identity: owner:G043:G043-S06-02 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
SceneTemplate.fromWorld(world: World, selection: Sequence[EntityRef]) -> 'SceneTemplate'
```
...
[
  ...
  {
    "excerpt": "204: ok(\"positive\", \"Scene.new(name)\", scene.name == \"arena\")\n205: template = SceneTemplate.fromWorld(template_world, (template_entity,))\n206: ok(\"positive\", \"SceneTemplate.fromWorld(world,selection)\", template.version == 1 and len(template.entities) == 1)\n207: instances = scene.instantiate(template, {\"translation\": (10, 0, -2)})\n208: ok(\"positive\", \"scene.instantiate(template,transform)\",",
