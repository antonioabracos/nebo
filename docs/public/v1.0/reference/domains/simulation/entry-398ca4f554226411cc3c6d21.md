# simulation — world.destroy(entity)

Seeded bounded ECS/physics model; no physical fidelity or safety certification; original observation: reused slot increments generation and stale ID remains invalid; SDK bounds: {"MAX_ADVANCE_TICKS":256,"MAX_BODIES":1024,"MAX_CONSTRAINTS":4096,"MAX_CONTACTS":4096,"MAX_ENTITIES":4096,"MAX_PARTICLES":65536,"MAX_RESOURCES":1024,"MAX_ROLLBACK_TICKS":1024,"MAX_SYSTEMS":128,"MAX_TRACE_TICKS":1024}

```text
Identity: owner:G043:G043-S01-03 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
World.destroy(self, entity: EntityId | EntityRef) -> None
```
...
[
  ...
  {
    "excerpt": "61: old_id = entity.id\n62: world.destroy(entity)\n63: replacement = world.spawn()\n64: ok(\"positive\", \"world.destroy(entity)\", replacement.id.slot == old_id.slot and\n65:    replacement.id.generation == old_id.generation+1)",
