# simulation — rollback.setWindow(ticks)

Seeded bounded ECS/physics model; no physical fidelity or safety certification; original observation: history retains only the bounded two-tick window; SDK bounds: {"MAX_ADVANCE_TICKS":256,"MAX_BODIES":1024,"MAX_CONSTRAINTS":4096,"MAX_CONTACTS":4096,"MAX_ENTITIES":4096,"MAX_PARTICLES":65536,"MAX_RESOURCES":1024,"MAX_ROLLBACK_TICKS":1024,"MAX_SYSTEMS":128,"MAX_TRACE_TICKS":1024}

```text
Identity: owner:G043:G043-S05-07 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
RollbackController.setWindow(self, ticks: int) -> None
```
...
[
  ...
  {
    "excerpt": "190:    \"simulationState\" in snapshot_diff[\"changed\"])\n191: rollback_sim.rollback.setWindow(2)\n192: ok(\"positive\", \"rollback.setWindow(ticks)\", rollback_sim._rollback_window == 2 and\n193:    min(rollback_sim._snapshots) >= 1)\n194: rollback_sim.rollback.confirm(2)",
