# simulation — world.query&lt;Components&gt;()

Seeded bounded ECS/physics model; no physical fidelity or safety certification; original observation: query returns only matching live entity; SDK bounds: {"MAX_ADVANCE_TICKS":256,"MAX_BODIES":1024,"MAX_CONSTRAINTS":4096,"MAX_CONTACTS":4096,"MAX_ENTITIES":4096,"MAX_PARTICLES":65536,"MAX_RESOURCES":1024,"MAX_ROLLBACK_TICKS":1024,"MAX_SYSTEMS":128,"MAX_TRACE_TICKS":1024}

```text
Identity: owner:G043:G043-S01-07 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
World.query(self, components: Sequence[str]) -> tuple[EntityRef, ...]
```
...
[
  ...
  {
    "excerpt": "54: entity.add(Component.of(\"Velocity\", {\"x\": 1, \"y\": 0, \"z\": 0}))\n55: queried = world.query((\"Position\", \"Velocity\"))\n56: ok(\"positive\", \"world.query<Components>()\", tuple(item.id for item in queried) == (entity.id,))\n57: removed_velocity = entity.remove(\"Velocity\")\n58: ok(\"positive\", \"entity.remove<Component>()\", removed_velocity.type == \"Velocity\")",
