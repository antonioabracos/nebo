# simulation — simulation.divergenceReport(reference)

Seeded bounded ECS/physics model; no physical fidelity or safety certification; original observation: equal runs match and changed input diverges at tick one; SDK bounds: {"MAX_ADVANCE_TICKS":256,"MAX_BODIES":1024,"MAX_CONSTRAINTS":4096,"MAX_CONTACTS":4096,"MAX_ENTITIES":4096,"MAX_PARTICLES":65536,"MAX_RESOURCES":1024,"MAX_ROLLBACK_TICKS":1024,"MAX_SYSTEMS":128,"MAX_TRACE_TICKS":1024}

```text
Identity: owner:G043:G043-S04-08 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Simulation.divergenceReport(self, reference: 'Simulation | Sequence[str]') -> Mapping[str, Any]
```
...
[
  ...
  {
    "excerpt": "159: different.tick({\"delta\": 3})\n160: ok(\"positive\", \"simulation.divergenceReport(reference)\",\n161:    simulation.divergenceReport(same)[\"matches\"] and\n162:    different.divergenceReport((first_hash,))[\"firstTick\"] == 1)\n163: transcript.append((first_hash, simulation.stateHash(), simulation.tickNumber()))",
