# simulation — physics.body(shape,mass,pose)

Seeded bounded ECS/physics model; no physical fidelity or safety certification; original observation: dynamic and static sphere bodies receive stable IDs; SDK bounds: {"MAX_ADVANCE_TICKS":256,"MAX_BODIES":1024,"MAX_CONSTRAINTS":4096,"MAX_CONTACTS":4096,"MAX_ENTITIES":4096,"MAX_PARTICLES":65536,"MAX_RESOURCES":1024,"MAX_ROLLBACK_TICKS":1024,"MAX_SYSTEMS":128,"MAX_TRACE_TICKS":1024}

```text
Identity: owner:G043:G043-S03-02 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
PhysicsWorld.body(self, shape: Shape, mass: Any, pose: Sequence[Any]) -> RigidBody
```
...
[
  ...
  {
    "excerpt": "113: ok(\"positive\", \"PhysicsWorld.new(options)\", physics.integrator == \"semi-implicit-euler-v1\")\n114: body_a = physics.body(Shape.sphere(1), 2, (5, 0, 0))\n115: body_b = physics.body(Shape.sphere(1), 0, (6.5, 0, 0))\n116: ok(\"positive\", \"physics.body(shape,mass,pose)\", body_a.id == 1 and body_b.id == 2)\n117: body_a.setVelocity((1, 0, 0), (0, 0, 0))",
