# simulation — simulation.rollbackTo(tick)

Seeded bounded ECS/physics model; no physical fidelity or safety certification; original observation: rollback restores tick zero then replays retained input to present; SDK bounds: {"MAX_ADVANCE_TICKS":256,"MAX_BODIES":1024,"MAX_CONSTRAINTS":4096,"MAX_CONTACTS":4096,"MAX_ENTITIES":4096,"MAX_PARTICLES":65536,"MAX_RESOURCES":1024,"MAX_ROLLBACK_TICKS":1024,"MAX_SYSTEMS":128,"MAX_TRACE_TICKS":1024}

```text
Identity: owner:G043:G043-S05-03 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Simulation.rollbackTo(self, tick: int) -> Mapping[str, Any]
```
...
[
  ...
  {
    "excerpt": "180:    rollback_sim._state[\"counter\"] == 2)\n181: rollback_result = rollback_sim.rollbackTo(0)\n182: ok(\"positive\", \"simulation.rollbackTo(tick)\", rollback_result ==\n183:    {\"restoredTick\": 0, \"replayedInputs\": 1} and rollback_sim._state[\"counter\"] == 2)\n184: replay_hash = rollback_sim.replay(({\"delta\": 2}, {\"delta\": 3}, {\"delta\": -1}))",
