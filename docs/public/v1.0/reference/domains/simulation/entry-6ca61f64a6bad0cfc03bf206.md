# simulation — simulation.restore(snapshot)

Seeded bounded ECS/physics model; no physical fidelity or safety certification; original observation: restore returns counter and tick to captured state; SDK bounds: {"MAX_ADVANCE_TICKS":256,"MAX_BODIES":1024,"MAX_CONSTRAINTS":4096,"MAX_CONTACTS":4096,"MAX_ENTITIES":4096,"MAX_PARTICLES":65536,"MAX_RESOURCES":1024,"MAX_ROLLBACK_TICKS":1024,"MAX_SYSTEMS":128,"MAX_TRACE_TICKS":1024}

```text
Identity: owner:G043:G043-S05-02 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Simulation.restore(self, snapshot: Snapshot) -> None
```
...
[
  ...
  {
    "excerpt": "177: rollback_sim.tick({\"delta\": 3})\n178: rollback_sim.restore(tick_one)\n179: ok(\"positive\", \"simulation.restore(snapshot)\", rollback_sim.tickNumber() == 1 and\n180:    rollback_sim._state[\"counter\"] == 2)\n181: rollback_result = rollback_sim.rollbackTo(0)",
