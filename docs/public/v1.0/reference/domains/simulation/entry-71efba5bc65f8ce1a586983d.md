# simulation — simulation.deterministicMode(enabled)

Seeded bounded ECS/physics model; no physical fidelity or safety certification; original observation: deterministic mode is explicit before first tick; SDK bounds: {"MAX_ADVANCE_TICKS":256,"MAX_BODIES":1024,"MAX_CONSTRAINTS":4096,"MAX_CONTACTS":4096,"MAX_ENTITIES":4096,"MAX_PARTICLES":65536,"MAX_RESOURCES":1024,"MAX_ROLLBACK_TICKS":1024,"MAX_SYSTEMS":128,"MAX_TRACE_TICKS":1024}

```text
Identity: owner:G043:G043-S04-07 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Simulation.deterministicMode(self, enabled: bool) -> None
```
...
[
  ...
  {
    "excerpt": "143: ok(\"positive\", \"simulation.fixedStep(duration)\", simulation._fixed_step == 0.25)\n144: simulation.deterministicMode(True)\n145: ok(\"positive\", \"simulation.deterministicMode(enabled)\", simulation._deterministic)\n146: first_hash = simulation.tick({\"delta\": 2})\n147: ok(\"positive\", \"simulation.tick(input)\", simulation._state[\"counter\"] == 12 and len(first_hash) == 64)",
