# simulation — scene.stream(region,budget)

Seeded bounded ECS/physics model; no physical fidelity or safety certification; original observation: budget 250 loads only the first 100-byte resource; SDK bounds: {"MAX_ADVANCE_TICKS":256,"MAX_BODIES":1024,"MAX_CONSTRAINTS":4096,"MAX_CONTACTS":4096,"MAX_ENTITIES":4096,"MAX_PARTICLES":65536,"MAX_RESOURCES":1024,"MAX_ROLLBACK_TICKS":1024,"MAX_SYSTEMS":128,"MAX_TRACE_TICKS":1024}

```text
Identity: owner:G043:G043-S06-05 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Scene.stream(self, region: Sequence[Any], budget: int) -> tuple[str, ...]
```
...
[
  ...
  {
    "excerpt": "214: ok(\"positive\", \"scene.load(resource,capability)\", loaded_id == \"terrain-a\")\n215: streamed = scene.stream((0, 0, 10, 5), 250)\n216: ok(\"positive\", \"scene.stream(region,budget)\", streamed == (\"terrain-a\",))\n217: unloaded = scene.unload((0, 0, 5, 5))\n218: ok(\"positive\", \"scene.unload(region)\", unloaded == (\"terrain-a\",))",
