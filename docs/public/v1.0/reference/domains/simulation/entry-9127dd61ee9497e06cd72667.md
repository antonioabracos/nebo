# simulation — world.runSystems(context)

Seeded bounded ECS/physics model; no physical fidelity or safety certification; original observation: three callbacks execute once in schedule order; SDK bounds: {"MAX_ADVANCE_TICKS":256,"MAX_BODIES":1024,"MAX_CONSTRAINTS":4096,"MAX_CONTACTS":4096,"MAX_ENTITIES":4096,"MAX_PARTICLES":65536,"MAX_RESOURCES":1024,"MAX_ROLLBACK_TICKS":1024,"MAX_SYSTEMS":128,"MAX_TRACE_TICKS":1024}

```text
Identity: owner:G043:G043-S02-08 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
World.runSystems(self, context: Mapping[str, Any]) -> tuple[str, ...]
```
...
[
  ...
  {
    "excerpt": "98:    ((\"prepare\",), (\"move\",), (\"observe\",)))\n99: executed = schedule_world.runSystems({\"events\": events})\n100: ok(\"positive\", \"world.runSystems(context)\", executed == (\"prepare\", \"move\", \"observe\") and\n101:    events == [\"prepare\", \"move\", \"observe\"])\n102: explanation = schedule.explain()",
