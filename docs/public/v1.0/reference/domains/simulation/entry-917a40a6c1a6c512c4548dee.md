# simulation — entity.remove&lt;Component&gt;()

Seeded bounded ECS/physics model; no physical fidelity or safety certification; original observation: removed velocity value is returned; SDK bounds: {"MAX_ADVANCE_TICKS":256,"MAX_BODIES":1024,"MAX_CONSTRAINTS":4096,"MAX_CONTACTS":4096,"MAX_ENTITIES":4096,"MAX_PARTICLES":65536,"MAX_RESOURCES":1024,"MAX_ROLLBACK_TICKS":1024,"MAX_SYSTEMS":128,"MAX_TRACE_TICKS":1024}

```text
Identity: owner:G043:G043-S01-05 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
EntityRef.remove(self, component_type: str) -> Component
```
...
[
  ...
  {
    "excerpt": "56: ok(\"positive\", \"world.query<Components>()\", tuple(item.id for item in queried) == (entity.id,))\n57: removed_velocity = entity.remove(\"Velocity\")\n58: ok(\"positive\", \"entity.remove<Component>()\", removed_velocity.type == \"Velocity\")\n59: ok(\"positive\", \"world.entityCount()\", world.entityCount() == 1)\n60: ok(\"positive\", \"world.validate()\", world.validate()[\"valid\"] and world.validate()[\"entities\"] == 1)",
