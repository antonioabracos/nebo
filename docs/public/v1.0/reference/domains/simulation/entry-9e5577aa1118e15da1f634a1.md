# simulation — system.writes(components)

Seeded bounded ECS/physics model; no physical fidelity or safety certification; original observation: write access set contains Position; SDK bounds: {"MAX_ADVANCE_TICKS":256,"MAX_BODIES":1024,"MAX_CONSTRAINTS":4096,"MAX_CONTACTS":4096,"MAX_ENTITIES":4096,"MAX_PARTICLES":65536,"MAX_RESOURCES":1024,"MAX_ROLLBACK_TICKS":1024,"MAX_SYSTEMS":128,"MAX_TRACE_TICKS":1024}

```text
Identity: owner:G043:G043-S02-03 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
System.writes(self, components: Iterable[str]) -> 'System'
```
...
[
  ...
  {
    "excerpt": "80: ok(\"positive\", \"System.define(name,query,function)\", prepare.name == \"prepare\")\n81: prepare = prepare.writes((\"Position\",))\n82: ok(\"positive\", \"system.writes(components)\", prepare._writes == {\"Position\"})\n83: move = System.define(\"move\", (\"Position\",), record(\"move\")).reads((\"Position\",))\n84: ok(\"positive\", \"system.reads(components)\", move._reads == {\"Position\"})",
