# simulation — simulation.trace(ticks)

Seeded bounded ECS/physics model; no physical fidelity or safety certification; original observation: two-entry bounded trace exposes ticks two and three; SDK bounds: {"MAX_ADVANCE_TICKS":256,"MAX_BODIES":1024,"MAX_CONSTRAINTS":4096,"MAX_CONTACTS":4096,"MAX_ENTITIES":4096,"MAX_PARTICLES":65536,"MAX_RESOURCES":1024,"MAX_ROLLBACK_TICKS":1024,"MAX_SYSTEMS":128,"MAX_TRACE_TICKS":1024}

```text
Identity: owner:G043:G043-S07-07 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Simulation.trace(self, ticks: int) -> tuple[Mapping[str, Any], ...]
```
...
[
  ...
  {
    "excerpt": "150: ok(\"positive\", \"simulation.tickNumber()\", simulation.tickNumber() == 3)\n151: ok(\"positive\", \"simulation.stateHash()\", simulation.stateHash() == simulation.trace(1)[0][\"stateHash\"])\n152: same = Simulation.new(World.new({\"entityBudget\": 4, \"seed\": 7}),\n153:                       {\"deterministic\": True, \"initialState\": {\"counter\": 10}, \"seed\": 7})\n154: same.fixedStep(0.25)",
