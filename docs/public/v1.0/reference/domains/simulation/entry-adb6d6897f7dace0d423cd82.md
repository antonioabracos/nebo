# simulation — scene.memoryReport()

Seeded bounded ECS/physics model; no physical fidelity or safety certification; original observation: entity storage and loaded bytes are reported; SDK bounds: {"MAX_ADVANCE_TICKS":256,"MAX_BODIES":1024,"MAX_CONSTRAINTS":4096,"MAX_CONTACTS":4096,"MAX_ENTITIES":4096,"MAX_PARTICLES":65536,"MAX_RESOURCES":1024,"MAX_ROLLBACK_TICKS":1024,"MAX_SYSTEMS":128,"MAX_TRACE_TICKS":1024}

```text
Identity: owner:G043:G043-S06-08 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Scene.memoryReport(self) -> Mapping[str, Any]
```
...
[
  ...
  {
    "excerpt": "221:    ((\"terrain-a\", \"v1\"), (\"terrain-b\", \"v2\")))\n222: memory = scene.memoryReport()\n223: ok(\"positive\", \"scene.memoryReport()\", memory[\"entityStorage\"] == 1 and\n224:    memory[\"componentStorage\"] == {\"Position\": 1} and memory[\"loadedBytes\"] == 0 and\n225:    memory[\"resourceStorage\"][\"terrain-a\"] == {\"bytes\": 100, \"loaded\": False})",
