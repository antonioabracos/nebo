# simulation — physics.step(dt)

Seeded bounded ECS/physics model; no physical fidelity or safety certification; original observation: one tenth step advances position by one tenth; SDK bounds: {"MAX_ADVANCE_TICKS":256,"MAX_BODIES":1024,"MAX_CONSTRAINTS":4096,"MAX_CONTACTS":4096,"MAX_ENTITIES":4096,"MAX_PARTICLES":65536,"MAX_RESOURCES":1024,"MAX_ROLLBACK_TICKS":1024,"MAX_SYSTEMS":128,"MAX_TRACE_TICKS":1024}

```text
Identity: owner:G043:G043-S03-05 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
PhysicsWorld.step(self, dt: Any) -> None
ParticleSystem.step(self, dt: Any) -> None
```
...
[
  ...
  {
    "excerpt": "120: ok(\"positive\", \"physics.constraint(a,b,rule)\", constraint_id == 1)\n121: physics.step(0.1)\n122: ok(\"positive\", \"physics.step(dt)\", close(body_a.pose[0], 5.1) and physics._steps == 1)\n123: ok(\"positive\", \"physics.contacts()\", len(physics.contacts()) == 1 and\n124:    physics.contacts()[0][\"penetration\"] > 0)",
