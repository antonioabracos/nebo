# simulation — world.compileSchedule()

Seeded bounded ECS/physics model; no physical fidelity or safety certification; original observation: dependency levels compile to deterministic DAG order; SDK bounds: {"MAX_ADVANCE_TICKS":256,"MAX_BODIES":1024,"MAX_CONSTRAINTS":4096,"MAX_CONTACTS":4096,"MAX_ENTITIES":4096,"MAX_PARTICLES":65536,"MAX_RESOURCES":1024,"MAX_ROLLBACK_TICKS":1024,"MAX_SYSTEMS":128,"MAX_TRACE_TICKS":1024}

```text
Identity: owner:G043:G043-S02-07 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
World.compileSchedule(self) -> 'Schedule'
```
...
[
  ...
  {
    "excerpt": "94: ok(\"positive\", \"world.addSystem(system)\", len(schedule_world._systems) == 3)\n95: schedule = schedule_world.compileSchedule()\n96: ok(\"positive\", \"world.compileSchedule()\", tuple(item.name for item in schedule.ordered) ==\n97:    (\"prepare\", \"move\", \"observe\") and schedule.parallelGroups ==\n98:    ((\"prepare\",), (\"move\",), (\"observe\",)))",
