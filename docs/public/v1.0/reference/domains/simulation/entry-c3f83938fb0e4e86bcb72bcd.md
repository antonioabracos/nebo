# simulation — PhysicsWorld.new(options)

Seeded bounded ECS/physics model; no physical fidelity or safety certification; original observation: semi-implicit integrator gravity and solver policy are explicit; SDK bounds: {"MAX_ADVANCE_TICKS":256,"MAX_BODIES":1024,"MAX_CONSTRAINTS":4096,"MAX_CONTACTS":4096,"MAX_ENTITIES":4096,"MAX_PARTICLES":65536,"MAX_RESOURCES":1024,"MAX_ROLLBACK_TICKS":1024,"MAX_SYSTEMS":128,"MAX_TRACE_TICKS":1024}

```text
Identity: owner:G043:G043-S03-01 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
PhysicsWorld.new(options: Mapping[str, Any]) -> 'PhysicsWorld'
```
...
[
  ...
  {
    "excerpt": "45: # S01 — generation-safe entities, archetype migration and invalidated views.\n46: world = World.new({\"entityBudget\": 8, \"seed\": 43})\n47: ok(\"positive\", \"World.new(options)\", world.entity_budget == 8 and world.seed == 43)\n48: entity = world.spawn()\n49: ok(\"positive\", \"world.spawn()\", entity.id == EntityId(0, 1))",
