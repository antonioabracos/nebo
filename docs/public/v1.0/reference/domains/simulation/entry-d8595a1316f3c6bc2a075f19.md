...
Seeded bounded ECS/physics model; no physical fidelity or safety certification; original observation: snapshot has schema profile tick state and authenticated hash; SDK bounds: {"MAX_ADVANCE_TICKS":256,"MAX_BODIES":1024,"MAX_CONSTRAINTS":4096,"MAX_CONTACTS":4096,"MAX_ENTITIES":4096,"MAX_PARTICLES":65536,"MAX_RESOURCES":1024,"MAX_ROLLBACK_TICKS":1024,"MAX_SYSTEMS":128,"MAX_TRACE_TICKS":1024}

```text
Identity: owner:G043:G043-S05-01 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Simulation.snapshot(self) -> Snapshot
ParticleSystem.snapshot(self) -> tuple[Mapping[str, Any], ...]
```
...
[
  ...
  {
    "excerpt": "169: rollback_sim.fixedStep(0.5)\n170: initial_snapshot = rollback_sim.snapshot()\n171: ok(\"positive\", \"simulation.snapshot()\", initial_snapshot.schemaVersion == 1 and\n172:    initial_snapshot.tick == 0 and initial_snapshot.resourcePolicy == \"local-references-v1\")\n173: rollback_sim.recordInput(0, {\"delta\": 2})",
