# simulation — body.setVelocity(linear,angular)

Seeded bounded ECS/physics model; no physical fidelity or safety certification; original observation: linear and angular velocity are retained; SDK bounds: {"MAX_ADVANCE_TICKS":256,"MAX_BODIES":1024,"MAX_CONSTRAINTS":4096,"MAX_CONTACTS":4096,"MAX_ENTITIES":4096,"MAX_PARTICLES":65536,"MAX_RESOURCES":1024,"MAX_ROLLBACK_TICKS":1024,"MAX_SYSTEMS":128,"MAX_TRACE_TICKS":1024}

```text
Identity: owner:G043:G043-S03-03 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
RigidBody.setVelocity(self, linear: Sequence[Any], angular: Sequence[Any]) -> None
```
...
[
  ...
  {
    "excerpt": "116: ok(\"positive\", \"physics.body(shape,mass,pose)\", body_a.id == 1 and body_b.id == 2)\n117: body_a.setVelocity((1, 0, 0), (0, 0, 0))\n118: ok(\"positive\", \"body.setVelocity(linear,angular)\", body_a.linearVelocity == (1.0, 0.0, 0.0))\n119: constraint_id = physics.constraint(body_a, body_b, {\"type\": \"distance\", \"restLength\": 1.5})\n120: ok(\"positive\", \"physics.constraint(a,b,rule)\", constraint_id == 1)",
