# simulation — particles.emit(count,state)

Seeded bounded ECS/physics model; no physical fidelity or safety certification; original observation: two deterministic particles are emitted atomically; SDK bounds: {"MAX_ADVANCE_TICKS":256,"MAX_BODIES":1024,"MAX_CONSTRAINTS":4096,"MAX_CONTACTS":4096,"MAX_ENTITIES":4096,"MAX_PARTICLES":65536,"MAX_RESOURCES":1024,"MAX_ROLLBACK_TICKS":1024,"MAX_SYSTEMS":128,"MAX_TRACE_TICKS":1024}

```text
Identity: owner:G043:G043-S07-02 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
ParticleSystem.emit(self, count: int, state: Mapping[str, Any]) -> int
```
...
[
  ...
  {
    "excerpt": "241: ok(\"positive\", \"ParticleSystem.new(capacity,emitter,updater)\", particles.capacity == 4)\n242: ok(\"positive\", \"particles.emit(count,state)\", particles.emit(2, {\"x\": 1}) == 2)\n243: particles.step(0.5)\n244: ok(\"positive\", \"particles.step(dt)\", particles.snapshot()[0][\"position\"] == (1.5, 0.0, 0.0))\n245: field = Field.vector(lambda _position: (0, 2, 0), (-10, -10, -10, 10, 10, 10))",
