# simulation — scene.instantiate(template,transform)

Seeded bounded ECS/physics model; no physical fidelity or safety certification; original observation: translated instance owns position eleven two one; SDK bounds: {"MAX_ADVANCE_TICKS":256,"MAX_BODIES":1024,"MAX_CONSTRAINTS":4096,"MAX_CONTACTS":4096,"MAX_ENTITIES":4096,"MAX_PARTICLES":65536,"MAX_RESOURCES":1024,"MAX_ROLLBACK_TICKS":1024,"MAX_SYSTEMS":128,"MAX_TRACE_TICKS":1024}

```text
Identity: owner:G043:G043-S06-03 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Scene.instantiate(self, template: SceneTemplate, transform: Mapping[str, Any]) -> tuple[EntityRef, ...]
```
...
[
  ...
  {
    "excerpt": "206: ok(\"positive\", \"SceneTemplate.fromWorld(world,selection)\", template.version == 1 and len(template.entities) == 1)\n207: instances = scene.instantiate(template, {\"translation\": (10, 0, -2)})\n208: ok(\"positive\", \"scene.instantiate(template,transform)\",\n209:    instances[0].get(\"Position\").value().data == {\"x\": 11.0, \"y\": 2.0, \"z\": 1.0})\n210: resource_a = {\"id\": \"terrain-a\", \"version\": \"v1\", \"bytes\": 100, \"bounds\": (0, 0, 5, 5)}",
