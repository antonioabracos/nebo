# simulation — physics.overlap(shape,pose,limit)

Seeded bounded ECS/physics model; no physical fidelity or safety certification; original observation: query sphere overlaps both bodies; SDK bounds: {"MAX_ADVANCE_TICKS":256,"MAX_BODIES":1024,"MAX_CONSTRAINTS":4096,"MAX_CONTACTS":4096,"MAX_ENTITIES":4096,"MAX_PARTICLES":65536,"MAX_RESOURCES":1024,"MAX_ROLLBACK_TICKS":1024,"MAX_SYSTEMS":128,"MAX_TRACE_TICKS":1024}

```text
Identity: owner:G043:G043-S03-08 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
PhysicsWorld.overlap(self, shape: Shape, pose: Sequence[Any], limit: int) -> tuple[int, ...]
```
...
[
  ...
  {
    "excerpt": "126: ok(\"positive\", \"physics.raycast(ray,limit)\", tuple(hit[\"body\"] for hit in ray_hits) == (1, 2))\n127: overlaps = physics.overlap(Shape.sphere(0.25), (5.75, 0, 0), 4)\n128: ok(\"positive\", \"physics.overlap(shape,pose,limit)\", overlaps == (1, 2))\n129: energy = physics.energyReport()\n130: ok(\"positive\", \"physics.energyReport()\", energy[\"integrator\"] == \"semi-implicit-euler-v1\" and",
