# simulation — simulation.advance(realDuration)

Seeded bounded ECS/physics model; no physical fidelity or safety certification; original observation: half duration produces exactly two bounded ticks; SDK bounds: {"MAX_ADVANCE_TICKS":256,"MAX_BODIES":1024,"MAX_CONSTRAINTS":4096,"MAX_CONTACTS":4096,"MAX_ENTITIES":4096,"MAX_PARTICLES":65536,"MAX_RESOURCES":1024,"MAX_ROLLBACK_TICKS":1024,"MAX_SYSTEMS":128,"MAX_TRACE_TICKS":1024}

```text
Identity: owner:G043:G043-S04-04 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Simulation.advance(self, realDuration: Any) -> int
```
...
[
  ...
  {
    "excerpt": "147: ok(\"positive\", \"simulation.tick(input)\", simulation._state[\"counter\"] == 12 and len(first_hash) == 64)\n148: advanced = simulation.advance(0.5)\n149: ok(\"positive\", \"simulation.advance(realDuration)\", advanced == 2)\n150: ok(\"positive\", \"simulation.tickNumber()\", simulation.tickNumber() == 3)\n151: ok(\"positive\", \"simulation.stateHash()\", simulation.stateHash() == simulation.trace(1)[0][\"stateHash\"])",
