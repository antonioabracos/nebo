# simulation — simulation.recordInput(tick,input)

Seeded bounded ECS/physics model; no physical fidelity or safety certification; original observation: canonical deterministic input is retained by tick; SDK bounds: {"MAX_ADVANCE_TICKS":256,"MAX_BODIES":1024,"MAX_CONSTRAINTS":4096,"MAX_CONTACTS":4096,"MAX_ENTITIES":4096,"MAX_PARTICLES":65536,"MAX_RESOURCES":1024,"MAX_ROLLBACK_TICKS":1024,"MAX_SYSTEMS":128,"MAX_TRACE_TICKS":1024}

```text
Identity: owner:G043:G043-S05-04 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Simulation.recordInput(self, tick: int, input_: Any) -> None
```
...
[
  ...
  {
    "excerpt": "172:    initial_snapshot.tick == 0 and initial_snapshot.resourcePolicy == \"local-references-v1\")\n173: rollback_sim.recordInput(0, {\"delta\": 2})\n174: ok(\"positive\", \"simulation.recordInput(tick,input)\", rollback_sim._inputs[0] == {\"delta\": 2})\n175: rollback_sim.tick({\"delta\": 2})\n176: tick_one = rollback_sim.snapshot()",
