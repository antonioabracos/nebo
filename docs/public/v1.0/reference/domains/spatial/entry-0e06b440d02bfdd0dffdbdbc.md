# spatial — mesh.validate()

Local bounded geometry/GIS; no external maps or global navigation assurance; original observation: topology report is indexed-surface; SDK bounds: {"MAX_CODEC_BYTES":65536,"MAX_FACES":8192,"MAX_INDEX_ITEMS":4096,"MAX_POINTS":4096,"MAX_ROUTE_EDGES":8192,"MAX_ROUTE_NODES":2048}

```text
Identity: owner:G042:G042-S03-08 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
_GeometryEvidence.validate(self) -> ValidationReport
Polygon.validate(self) -> ValidationReport
Mesh.validate(self) -> ValidationReport
```
...
[
  ...
  {
    "excerpt": "61:                       {\"boundary\": \"include\", \"precision\": \"exact\"})\n62: ok(\"positive\", \"Polygon.from(rings,policy)\", square.validate().valid and\n63:    donut.area() == 84 and not donut.contains(Point2.of(5, 5)))\n64: crossing = Segment.between(Point2.of(3, -2), Point2.of(3, 2))\n65: ok(\"positive\", \"geometry.intersects(a,b)\", geometry.intersects(segment, crossing))",
