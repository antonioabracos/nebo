# spatial — mesh.weld(tolerance)

Local bounded geometry/GIS; no external maps or global navigation assurance; original observation: zero tolerance preserves four unique vertices; SDK bounds: {"MAX_CODEC_BYTES":65536,"MAX_FACES":8192,"MAX_INDEX_ITEMS":4096,"MAX_POINTS":4096,"MAX_ROUTE_EDGES":8192,"MAX_ROUTE_NODES":2048}

```text
Identity: owner:G042:G042-S03-06 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Mesh.weld(self, tolerance: Any) -> 'Mesh'
```
...
[
  ...
  {
    "excerpt": "101: ok(\"positive\", \"mesh.triangulate()\", triangles.indices == ((0, 1, 2), (0, 2, 3)))\n102: welded = mesh.weld(0)\n103: ok(\"positive\", \"mesh.weld(tolerance)\", welded.vertexCount() == 4)\n104: simplified = triangles.simplify(1, 1)\n105: ok(\"positive\", \"mesh.simplify(targetCount,error)\", simplified.faceCount() == 1 and",
