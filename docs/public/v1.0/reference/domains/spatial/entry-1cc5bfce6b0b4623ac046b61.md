# spatial — Route.find(start,end,costModel)

Local bounded geometry/GIS; no external maps or global navigation assurance; original observation: deterministic shortest cost is nine; SDK bounds: {"MAX_CODEC_BYTES":65536,"MAX_FACES":8192,"MAX_INDEX_ITEMS":4096,"MAX_POINTS":4096,"MAX_ROUTE_EDGES":8192,"MAX_ROUTE_NODES":2048}

```text
Identity: owner:G042:G042-S06-02 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Route.find(start: str, end: str, costModel: Mapping[str, Any], graph: SpatialGraph | None=None) -> RouteResult
```
...
[
  ...
  {
    "excerpt": "159: ok(\"positive\", \"SpatialGraph.fromFeatures(features,policy)\", len(graph.nodes) == 4 and len(graph.edges) == 4)\n160: route = Route.find(\"A\", \"C\", {\"name\": \"distance\", \"units\": \"metre\"}, graph)\n161: ok(\"positive\", \"Route.find(start,end,costModel)\", route.costs()[\"total\"] == 9)\n162: alternatives = route.alternatives(2, 0.4)\n163: ok(\"positive\", \"route.alternatives(count,diversity)\", len(alternatives) == 1 and alternatives[0].costs()[\"total\"] == 13)",
