# spatial — match.confidence()

Local bounded geometry/GIS; no external maps or global navigation assurance; original observation: confidence equals two thirds; SDK bounds: {"MAX_CODEC_BYTES":65536,"MAX_FACES":8192,"MAX_INDEX_ITEMS":4096,"MAX_POINTS":4096,"MAX_ROUTE_EDGES":8192,"MAX_ROUTE_NODES":2048}

```text
Identity: owner:G042:G042-S06-08 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
MatchResult.confidence(self) -> float
```
...
[
  ...
  {
    "excerpt": "169: ok(\"positive\", \"MapMatcher.match(observations,graph,options)\", matched.nodeIds == (\"A\", \"C\"))\n170: ok(\"positive\", \"match.confidence()\", close(matched.confidence(), 2/3))\n171: replanned = route.replan({\"disabledEdges\": ((\"A\", \"B\"),)})\n172: ok(\"positive\", \"route.replan(changes)\", replanned.costs()[\"total\"] == 13)\n173: transcript.append((route.instructions(), alternatives[0].instructions(), matched.nodeIds, matched.confidence()))",
