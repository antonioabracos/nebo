# spatial — GeometryCodec.read(stream,format,limits)

Local bounded geometry/GIS; no external maps or global navigation assurance; original observation: bounded WKT line decodes to three points; SDK bounds: {"MAX_CODEC_BYTES":65536,"MAX_FACES":8192,"MAX_INDEX_ITEMS":4096,"MAX_POINTS":4096,"MAX_ROUTE_EDGES":8192,"MAX_ROUTE_NODES":2048}

```text
Identity: owner:G042:G042-S07-01 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
GeometryCodec.read(stream: Any, format: str, limits: Mapping[str, Any]) -> _GeometryEvidence
```
...
[
  ...
  {
    "excerpt": "175: # S07 — bounded canonical codec, validation/repair evidence and dataset indexing.\n176: decoded = GeometryCodec.read(io.StringIO(\"LINESTRING(0 0,3 4,6 4)\"), \"WKT_BOUNDED_V1\", {\"maximumBytes\": 64})\n177: ok(\"positive\", \"GeometryCodec.read(stream,format,limits)\", isinstance(decoded, Polyline) and len(decoded.points) == 3)\n178: encoded = GeometryCodec.write(decoded, \"WKT_BOUNDED_V1\", {\"canonical\": True})\n179: ok(\"positive\", \"GeometryCodec.write(geometry,format,options)\", encoded == \"LINESTRING(0 0,3 4,6 4)\")",
