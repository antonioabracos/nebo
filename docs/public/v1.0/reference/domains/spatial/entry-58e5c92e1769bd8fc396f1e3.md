# spatial — Projection.convert(point,from,to)

Local bounded geometry/GIS; no external maps or global navigation assurance; original observation: Paris converts to filtered Web Mercator; SDK bounds: {"MAX_CODEC_BYTES":65536,"MAX_FACES":8192,"MAX_INDEX_ITEMS":4096,"MAX_POINTS":4096,"MAX_ROUTE_EDGES":8192,"MAX_ROUTE_NODES":2048}

```text
Identity: owner:G042:G042-S05-03 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Projection.convert(point: Any, from_: Crs, to: Crs) -> Any
```
...
[
  ...
  {
    "excerpt": "137: ok(\"positive\", \"Crs.parse(identifier)\", wgs84.axisOrder == \"latitude-longitude\" and mercator.units == \"metre\")\n138: projected = Projection.convert(paris, wgs84, mercator)\n139: ok(\"positive\", \"Projection.convert(point,from,to)\", projected.crs == mercator and projected.precision.mode == \"filtered\")\n140: nearby = GeoCoordinate.of(48.8576, 2.3522, 35)\n141: geo_distance = paris.distance(nearby, \"WGS84_SPHERE_V1\")",
