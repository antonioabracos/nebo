# spatial — mesh.simplify(targetCount,error)

Local bounded geometry/GIS; no external maps or global navigation assurance; original observation: explicit target produces one face; SDK bounds: {"MAX_CODEC_BYTES":65536,"MAX_FACES":8192,"MAX_INDEX_ITEMS":4096,"MAX_POINTS":4096,"MAX_ROUTE_EDGES":8192,"MAX_ROUTE_NODES":2048}

```text
Identity: owner:G042:G042-S03-07 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Mesh.simplify(self, targetCount: int, error: Any) -> 'Mesh'
```
...
[
  ...
  {
    "excerpt": "103: ok(\"positive\", \"mesh.weld(tolerance)\", welded.vertexCount() == 4)\n104: simplified = triangles.simplify(1, 1)\n105: ok(\"positive\", \"mesh.simplify(targetCount,error)\", simplified.faceCount() == 1 and\n106:    simplified.simplificationError == 1)\n107: ok(\"positive\", \"mesh.validate()\", mesh.validate().topology == \"indexed-surface\")",
