# spatial — mesh.computeNormals(policy)

Local bounded geometry/GIS; no external maps or global navigation assurance; original observation: flat normal is positive z; SDK bounds: {"MAX_CODEC_BYTES":65536,"MAX_FACES":8192,"MAX_INDEX_ITEMS":4096,"MAX_POINTS":4096,"MAX_ROUTE_EDGES":8192,"MAX_ROUTE_NODES":2048}

```text
Identity: owner:G042:G042-S03-04 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Mesh.computeNormals(self, policy: Mapping[str, Any]) -> 'Mesh'
```
...
[
  ...
  {
    "excerpt": "97: ok(\"positive\", \"mesh.faceCount()\", mesh.faceCount() == 1)\n98: normal_mesh = mesh.computeNormals({\"mode\": \"flat\"})\n99: ok(\"positive\", \"mesh.computeNormals(policy)\", normal_mesh.normals == ((0.0, 0.0, 1.0),))\n100: triangles = mesh.triangulate()\n101: ok(\"positive\", \"mesh.triangulate()\", triangles.indices == ((0, 1, 2), (0, 2, 3)))",
