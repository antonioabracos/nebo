# spatial — GeometryCodec.write(geometry,format,options)

Local bounded geometry/GIS; no external maps or global navigation assurance; original observation: canonical WKT round-trip is byte stable; SDK bounds: {"MAX_CODEC_BYTES":65536,"MAX_FACES":8192,"MAX_INDEX_ITEMS":4096,"MAX_POINTS":4096,"MAX_ROUTE_EDGES":8192,"MAX_ROUTE_NODES":2048}

```text
Identity: owner:G042:G042-S07-02 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
GeometryCodec.write(geometry_: _GeometryEvidence, format: str, options: Mapping[str, Any]) -> str
```
...
[
  ...
  {
    "excerpt": "177: ok(\"positive\", \"GeometryCodec.read(stream,format,limits)\", isinstance(decoded, Polyline) and len(decoded.points) == 3)\n178: encoded = GeometryCodec.write(decoded, \"WKT_BOUNDED_V1\", {\"canonical\": True})\n179: ok(\"positive\", \"GeometryCodec.write(geometry,format,options)\", encoded == \"LINESTRING(0 0,3 4,6 4)\")\n180: ok(\"positive\", \"geometry.validate()\", square.validate() == square.validate() and square.validate().valid)\n181: ok(\"positive\", \"geometry.repair(plan)\", square.repair({\"action\": \"none\"}) is square)",
