# spatial — index.update(handle,bounds)

Local bounded geometry/GIS; no external maps or global navigation assurance; original observation: east bounds move atomically; SDK bounds: {"MAX_CODEC_BYTES":65536,"MAX_FACES":8192,"MAX_INDEX_ITEMS":4096,"MAX_POINTS":4096,"MAX_ROUTE_EDGES":8192,"MAX_ROUTE_NODES":2048}

```text
Identity: owner:G042:G042-S04-05 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
SpatialIndex.update(self, handle: int, bounds: Sequence[Any]) -> None
```
