# spatial — mesh.vertexCount()

Local bounded geometry/GIS; no external maps or global navigation assurance; original observation: quad exposes four vertices; SDK bounds: {"MAX_CODEC_BYTES":65536,"MAX_FACES":8192,"MAX_INDEX_ITEMS":4096,"MAX_POINTS":4096,"MAX_ROUTE_EDGES":8192,"MAX_ROUTE_NODES":2048}

```text
Identity: owner:G042:G042-S03-02 (DOMAIN_CONTRACT_ID)
Edition: 1
...
[
  ...
  {
    "excerpt": "95: ok(\"positive\", \"Mesh.from(vertices,indices)\", mesh.validate().valid)\n96: ok(\"positive\", \"mesh.vertexCount()\", mesh.vertexCount() == 4)\n97: ok(\"positive\", \"mesh.faceCount()\", mesh.faceCount() == 1)\n98: normal_mesh = mesh.computeNormals({\"mode\": \"flat\"})\n99: ok(\"positive\", \"mesh.computeNormals(policy)\", normal_mesh.normals == ((0.0, 0.0, 1.0),))",
