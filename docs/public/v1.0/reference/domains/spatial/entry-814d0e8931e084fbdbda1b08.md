# spatial — route.alternatives(count,diversity)

Local bounded geometry/GIS; no external maps or global navigation assurance; original observation: unique alternate has cost thirteen; SDK bounds: {"MAX_CODEC_BYTES":65536,"MAX_FACES":8192,"MAX_INDEX_ITEMS":4096,"MAX_POINTS":4096,"MAX_ROUTE_EDGES":8192,"MAX_ROUTE_NODES":2048}

```text
Identity: owner:G042:G042-S06-03 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
RouteResult.alternatives(self, count: int, diversity: Any) -> tuple['RouteResult', ...]
```
...
[
  ...
  {
    "excerpt": "161: ok(\"positive\", \"Route.find(start,end,costModel)\", route.costs()[\"total\"] == 9)\n162: alternatives = route.alternatives(2, 0.4)\n163: ok(\"positive\", \"route.alternatives(count,diversity)\", len(alternatives) == 1 and alternatives[0].costs()[\"total\"] == 13)\n164: ok(\"positive\", \"route.costs()\", route.costs() == {\"model\": \"distance\", \"total\": 9.0, \"units\": \"metre\"})\n165: ok(\"positive\", \"route.instructions()\", route.instructions() == (\"continue:A->B\", \"continue:B->C\"))",
