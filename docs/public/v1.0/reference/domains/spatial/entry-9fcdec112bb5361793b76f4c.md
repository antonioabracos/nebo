# spatial — GeoPath.length(model)

Local bounded geometry/GIS; no external maps or global navigation assurance; original observation: two geodesic legs compose; SDK bounds: {"MAX_CODEC_BYTES":65536,"MAX_FACES":8192,"MAX_INDEX_ITEMS":4096,"MAX_POINTS":4096,"MAX_ROUTE_EDGES":8192,"MAX_ROUTE_NODES":2048}

```text
Identity: owner:G042:G042-S05-07 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
GeoPath.length(self, model: str) -> float
```
