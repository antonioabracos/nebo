# spatial — index.verifyAgainst(items)

Local bounded geometry/GIS; no external maps or global navigation assurance; original observation: index exactly matches independent scan items; SDK bounds: {"MAX_CODEC_BYTES":65536,"MAX_FACES":8192,"MAX_INDEX_ITEMS":4096,"MAX_POINTS":4096,"MAX_ROUTE_EDGES":8192,"MAX_ROUTE_NODES":2048}

```text
Identity: owner:G042:G042-S04-09 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
SpatialIndex.verifyAgainst(self, items: Sequence[tuple[Sequence[Any], Any]]) -> Mapping[str, Any]
```
...
[
  ...
  {
    "excerpt": "124: ok(\"positive\", \"index.nearest(point,count)\", index.nearest(Point2.of(6, 6), 1) == (\"east\",))\n125: verification = index.verifyAgainst((((0, 0, 2, 2), \"west\"), ((4, 4, 7, 7), \"east\")))\n126: reverse_verification = index.verifyAgainst((((4, 4, 7, 7), \"east\"), ((0, 0, 2, 2), \"west\")))\n127: ok(\"positive\", \"index.verifyAgainst(items)\", verification[\"valid\"] and\n128:    reverse_verification[\"valid\"] and verification[\"scanCount\"] == 2)",
