# spatial — geometry.repair(plan)

Local bounded geometry/GIS; no external maps or global navigation assurance; original observation: explicit no-op plan preserves identity; SDK bounds: {"MAX_CODEC_BYTES":65536,"MAX_FACES":8192,"MAX_INDEX_ITEMS":4096,"MAX_POINTS":4096,"MAX_ROUTE_EDGES":8192,"MAX_ROUTE_NODES":2048}

```text
Identity: owner:G042:G042-S07-04 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
_GeometryEvidence.repair(self, plan: Mapping[str, Any]) -> '_GeometryEvidence'
```
...
[
  ...
  {
    "excerpt": "180: ok(\"positive\", \"geometry.validate()\", square.validate() == square.validate() and square.validate().valid)\n181: ok(\"positive\", \"geometry.repair(plan)\", square.repair({\"action\": \"none\"}) is square)\n182: precision_report = square.precisionReport()\n183: ok(\"positive\", \"geometry.precisionReport()\", precision_report[\"mode\"] == \"exact\" and precision_report[\"units\"] == \"millimetre\")\n184: dataset_index = SpatialDataset((Point2.of(1, 1), square)).index({\"capacity\": 4})",
