...
Local bounded geometry/GIS; no external maps or global navigation assurance; original observation: ordered A-B-C instructions returned; SDK bounds: {"MAX_CODEC_BYTES":65536,"MAX_FACES":8192,"MAX_INDEX_ITEMS":4096,"MAX_POINTS":4096,"MAX_ROUTE_EDGES":8192,"MAX_ROUTE_NODES":2048}

```text
Identity: owner:G042:G042-S06-05 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
RouteResult.instructions(self) -> tuple[str, ...]
```
...
[
  ...
  {
    "excerpt": "164: ok(\"positive\", \"route.costs()\", route.costs() == {\"model\": \"distance\", \"total\": 9.0, \"units\": \"metre\"})\n165: ok(\"positive\", \"route.instructions()\", route.instructions() == (\"continue:A->B\", \"continue:B->C\"))\n166: route_line = route.geometry()\n167: ok(\"positive\", \"route.geometry()\", route_line.points == (nodes[\"A\"], nodes[\"B\"], nodes[\"C\"]))\n168: matched = MapMatcher.match((Point2.of(1, 0), Point2.of(8, 0)), graph, {\"maximumDistance\": 3})",
