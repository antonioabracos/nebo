...
Local bounded geometry/GIS; no external maps or global navigation assurance; original observation: owned west value is returned and removed; SDK bounds: {"MAX_CODEC_BYTES":65536,"MAX_FACES":8192,"MAX_INDEX_ITEMS":4096,"MAX_POINTS":4096,"MAX_ROUTE_EDGES":8192,"MAX_ROUTE_NODES":2048}

```text
Identity: owner:G042:G042-S04-04 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
SpatialIndex.remove(self, handle: int) -> Any
```
...
[
  ...
  {
    "excerpt": "128:    reverse_verification[\"valid\"] and verification[\"scanCount\"] == 2)\n129: ok(\"positive\", \"index.remove(handle)\", index.remove(h1) == \"west\" and index.within((-1, -1, 8, 8), 8) == (\"east\",))\n130: transcript.append((h1, h2, dict(verification), kd.nearest(Point2.of(8, 8), 2)))\n131: \n132: # S05 — versioned CRS, projection error and geodesic observations.",
