# training — trainer.progress()

Bounded CPU reference gradients and local checkpoint model; no large-model training claim; original observation: trainer.progress(); SDK bounds: {"MAX_BATCH":64,"MAX_CHECKPOINT_BYTES":1048576,"MAX_ELEMENTS":4096,"MAX_EPOCHS":1024,"MAX_GRAPH_NODES":256,"MAX_PARAMETERS":4096,"MAX_PREFETCH":8,"MAX_ROTATIONS":8,"MAX_SAMPLES":4096}

```text
Identity: owner:G021:G021-S05-07 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Trainer.progress(self) -> dict[str, Any]
```
...
[
  ...
  {
    "excerpt": "194:    callback_epochs == list(range(1, 25)))\n195: progress = single_trainer.progress()\n196: ok(\"positive\", \"trainer.progress\", progress[\"epochs\"] == 24 and\n197:    progress[\"batches\"] == 25 and progress[\"latest\"][\"loss\"] < 0.02)\n198: ",
