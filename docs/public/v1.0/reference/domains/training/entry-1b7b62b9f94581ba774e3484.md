# training — augmentation.compose(stages)

Bounded CPU reference gradients and local checkpoint model; no large-model training claim; original observation: augmentation.compose(stages); SDK bounds: {"MAX_BATCH":64,"MAX_CHECKPOINT_BYTES":1048576,"MAX_ELEMENTS":4096,"MAX_EPOCHS":1024,"MAX_GRAPH_NODES":256,"MAX_PARAMETERS":4096,"MAX_PREFETCH":8,"MAX_ROTATIONS":8,"MAX_SAMPLES":4096}

```text
Identity: owner:G021:G021-S06-07 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Augmentation.compose(self, stages: Sequence[Callable[[Any], Any]]) -> Callable[[Any], Any]
```
...
[
  ...
  {
    "excerpt": "214:    sorted(parts[0].samples + parts[1].samples) == list(dataset.samples))\n215: pipeline = augmentation.compose([lambda value: value + 2, lambda value: value * 3])\n216: ok(\"positive\", \"augmentation.compose\", pipeline(4) == 18)\n217: \n218: # S07: canonical atomic checkpoint bytes and explicit reproducibility state.",
