# training — Metric.confusionMatrix(prediction, target)

Bounded CPU reference gradients and local checkpoint model; no large-model training claim; original observation: Metric.confusionMatrix(prediction, target); SDK bounds: {"MAX_BATCH":64,"MAX_CHECKPOINT_BYTES":1048576,"MAX_ELEMENTS":4096,"MAX_EPOCHS":1024,"MAX_GRAPH_NODES":256,"MAX_PARAMETERS":4096,"MAX_PREFETCH":8,"MAX_ROTATIONS":8,"MAX_SAMPLES":4096}

```text
Identity: owner:G021:G021-S03-07 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Metric.confusionMatrix(prediction: Tensor | Sequence[int], target: Sequence[int]) -> tuple[tuple[int, ...], ...]
```
...
[
  ...
  {
    "excerpt": "135:    {\"precision\": (0.5, 1.0), \"recall\": (1.0, 2 / 3)})\n136: matrix = Metric.confusionMatrix(metric_logits, metric_target)\n137: ok(\"positive\", \"Metric.confusionMatrix\", matrix == ((1, 0), (1, 2)))\n138: \n139: # S04: frozen one-step formulas and atomic state roundtrips.",
