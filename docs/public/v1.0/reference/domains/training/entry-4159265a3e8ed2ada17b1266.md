# training — training.seedAll(seed)

Bounded CPU reference gradients and local checkpoint model; no large-model training claim; original observation: training.seedAll(seed); SDK bounds: {"MAX_BATCH":64,"MAX_CHECKPOINT_BYTES":1048576,"MAX_ELEMENTS":4096,"MAX_EPOCHS":1024,"MAX_GRAPH_NODES":256,"MAX_PARAMETERS":4096,"MAX_PREFETCH":8,"MAX_ROTATIONS":8,"MAX_SAMPLES":4096}

```text
Identity: owner:G021:G021-S07-03 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Training.seedAll(self, seed: int) -> dict[str, int]
```
...
[
  ...
  {
    "excerpt": "163: # S05: tiny versioned corpus converges through live autodiff and callbacks.\n164: training.seedAll(2105)\n165: train_samples = MlDataset([\n166:     (Tensor([0.0], (1,)), Tensor([1.0], (1,))),\n167:     (Tensor([1.0], (1,)), Tensor([3.0], (1,))),",
