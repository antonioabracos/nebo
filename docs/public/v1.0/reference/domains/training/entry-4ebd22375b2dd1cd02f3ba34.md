# training — gradients.clipValue(min, max)

Bounded CPU reference gradients and local checkpoint model; no large-model training claim; original observation: gradients.clipValue(min, max); SDK bounds: {"MAX_BATCH":64,"MAX_CHECKPOINT_BYTES":1048576,"MAX_ELEMENTS":4096,"MAX_EPOCHS":1024,"MAX_GRAPH_NODES":256,"MAX_PARAMETERS":4096,"MAX_PREFETCH":8,"MAX_ROTATIONS":8,"MAX_SAMPLES":4096}

```text
Identity: owner:G021:G021-S02-05 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
GradientSet.clipValue(self, minimum: float, maximum: float) -> int
```
...
[
  ...
  {
    "excerpt": "100: vector._gradient = [-7.0, 0.5]\n101: changed = gradients.clipValue(-1.0, 1.0)\n102: ok(\"positive\", \"gradients.clipValue\", changed >= 1 and vector.gradient().values == (-1.0, 0.5))\n103: checkpointed = autograd.checkpoint(lambda value: value.multiply(value).sum())\n104: checkpoint_input = Tensor([2, 3], (2,), True)",
