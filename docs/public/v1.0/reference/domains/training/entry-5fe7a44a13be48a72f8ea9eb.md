# training — MlDataset.from(dataset)

Bounded CPU reference gradients and local checkpoint model; no large-model training claim; original observation: MlDataset.from(dataset); SDK bounds: {"MAX_BATCH":64,"MAX_CHECKPOINT_BYTES":1048576,"MAX_ELEMENTS":4096,"MAX_EPOCHS":1024,"MAX_GRAPH_NODES":256,"MAX_PARAMETERS":4096,"MAX_PREFETCH":8,"MAX_ROTATIONS":8,"MAX_SAMPLES":4096}

```text
Identity: owner:G021:G021-S06-01 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
MlDataset.fromDataset(dataset: Iterable[Any]) -> 'MlDataset'
```
...
[
  ...
  {
    "excerpt": "199: # S06: local immutable dataset transforms and deterministic seeded ordering.\n200: dataset = getattr(MlDataset, \"from\")([0, 1, 2, 3, 4, 5])\n201: ok(\"positive\", \"MlDataset.from\", dataset.samples == (0, 1, 2, 3, 4, 5))\n202: shuffled = dataset.shuffle(21)\n203: ok(\"positive\", \"dataset.shuffle\", shuffled.samples == (0, 5, 2, 4, 3, 1))",
