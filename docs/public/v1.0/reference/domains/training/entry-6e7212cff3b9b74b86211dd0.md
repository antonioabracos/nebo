# training — training.deterministic(enabled)

Bounded CPU reference gradients and local checkpoint model; no large-model training claim; original observation: training.deterministic(enabled); SDK bounds: {"MAX_BATCH":64,"MAX_CHECKPOINT_BYTES":1048576,"MAX_ELEMENTS":4096,"MAX_EPOCHS":1024,"MAX_GRAPH_NODES":256,"MAX_PARAMETERS":4096,"MAX_PREFETCH":8,"MAX_ROTATIONS":8,"MAX_SAMPLES":4096}

```text
Identity: owner:G021:G021-S07-04 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Training.deterministic(self, enabled: bool) -> bool
```
