# training — Checkpoint.save(path, model, optimizer, state)

Bounded CPU reference gradients and local checkpoint model; no large-model training claim; original observation: Checkpoint.save(path, model, optimizer, state); SDK bounds: {"MAX_BATCH":64,"MAX_CHECKPOINT_BYTES":1048576,"MAX_ELEMENTS":4096,"MAX_EPOCHS":1024,"MAX_GRAPH_NODES":256,"MAX_PARAMETERS":4096,"MAX_PREFETCH":8,"MAX_ROTATIONS":8,"MAX_SAMPLES":4096}

```text
Identity: owner:G021:G021-S07-01 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Checkpoint.save(path: str | os.PathLike[str], model: Any, optimizer: Optimizer, state: Mapping[str, Any]) -> CheckpointRecord
```
...
[
  ...
  {
    "excerpt": "225: checkpoint_path = scratch / \"tiny.ncp\"\n226: saved = Checkpoint.save(checkpoint_path, checkpoint_model, checkpoint_optimizer,\n227:                         {\"epoch\": 3, \"corpus\": \"tiny-linear-v1\"})\n228: ok(\"positive\", \"Checkpoint.save\", saved.path == checkpoint_path and\n229:    checkpoint_path.stat().st_size < 1_048_576)",
