# training — checkpoint.rotate(policy)

Bounded CPU reference gradients and local checkpoint model; no large-model training claim; original observation: checkpoint.rotate(policy); SDK bounds: {"MAX_BATCH":64,"MAX_CHECKPOINT_BYTES":1048576,"MAX_ELEMENTS":4096,"MAX_EPOCHS":1024,"MAX_GRAPH_NODES":256,"MAX_PARAMETERS":4096,"MAX_PREFETCH":8,"MAX_ROTATIONS":8,"MAX_SAMPLES":4096}

```text
Identity: owner:G021:G021-S07-06 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
CheckpointRecord.rotate(self, policy: int | Mapping[str, Any]) -> tuple[Path, ...]
```
...
[
  ...
  {
    "excerpt": "246:     (scratch / (checkpoint_path.name + suffix)).write_bytes(b\"rotation\")\n247: removed = loaded.rotate({\"keep\": 2})\n248: ok(\"positive\", \"checkpoint.rotate\", tuple(path.name for path in removed) == (\"tiny.ncp.1\",))\n249: second_path = scratch / \"second.ncp\"\n250: second = Checkpoint.save(second_path, checkpoint_model, checkpoint_optimizer,",
