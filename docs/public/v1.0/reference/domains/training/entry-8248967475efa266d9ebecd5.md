# training — trainer.evaluate(dataset)

Bounded CPU reference gradients and local checkpoint model; no large-model training claim; original observation: trainer.evaluate(dataset); SDK bounds: {"MAX_BATCH":64,"MAX_CHECKPOINT_BYTES":1048576,"MAX_ELEMENTS":4096,"MAX_EPOCHS":1024,"MAX_GRAPH_NODES":256,"MAX_PARAMETERS":4096,"MAX_PREFETCH":8,"MAX_ROTATIONS":8,"MAX_SAMPLES":4096}

```text
Identity: owner:G021:G021-S05-04 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Trainer.evaluate(self, dataset: MlDataset | Iterable[tuple[Tensor, Tensor]]) -> float
```
...
[
  ...
  {
    "excerpt": "178:    single_model.weight.values[0] > 0.0)\n179: evaluation = single_trainer.evaluate(train_samples)\n180: ok(\"positive\", \"trainer.evaluate\", 0.0 < evaluation < first_batch_loss)\n181: callback_epochs: list[int] = []\n182: single_trainer.onEpoch(lambda event: callback_epochs.append(event[\"epoch\"]))",
