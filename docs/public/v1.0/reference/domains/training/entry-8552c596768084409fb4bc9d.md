# training — Trainer.new(model, optimizer, loss, options)

Bounded CPU reference gradients and local checkpoint model; no large-model training claim; original observation: Trainer.new(model, optimizer, loss, options); SDK bounds: {"MAX_BATCH":64,"MAX_CHECKPOINT_BYTES":1048576,"MAX_ELEMENTS":4096,"MAX_EPOCHS":1024,"MAX_GRAPH_NODES":256,"MAX_PARAMETERS":4096,"MAX_PREFETCH":8,"MAX_ROTATIONS":8,"MAX_SAMPLES":4096}

```text
Identity: owner:G021:G021-S05-01 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Trainer.new(model: Any, optimizer: Optimizer, loss: Callable[[Tensor, Tensor], Tensor], options: Mapping[str, Any] | None=None) -> 'Trainer'
```
...
[
  ...
  {
    "excerpt": "141: sgd_parameter._gradient = [0.5, -2.0]\n142: sgd = Sgd.new([sgd_parameter], 0.1, {\"momentum\": 0.0})\n143: ok(\"positive\", \"Sgd.new\", sgd.learning_rate == 0.1 and sgd.momentum == 0.0)\n144: ok(\"positive\", \"optimizer.step\", sgd.step() == 1 and sgd_parameter.values == (1.95, -0.8))\n145: state = sgd.state()",
