# training — dataset.split(ratios, seed)

Bounded CPU reference gradients and local checkpoint model; no large-model training claim; original observation: dataset.split(ratios, seed); SDK bounds: {"MAX_BATCH":64,"MAX_CHECKPOINT_BYTES":1048576,"MAX_ELEMENTS":4096,"MAX_EPOCHS":1024,"MAX_GRAPH_NODES":256,"MAX_PARAMETERS":4096,"MAX_PREFETCH":8,"MAX_ROTATIONS":8,"MAX_SAMPLES":4096}

```text
Identity: owner:G021:G021-S06-06 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
MlDataset.split(self, ratios: Sequence[float], seed: int) -> tuple['MlDataset', ...]
```
...
[
  ...
  {
    "excerpt": "211: ok(\"positive\", \"dataset.map\", mapped.samples == (0, 1, 4, 9, 16, 25))\n212: parts = dataset.split([0.5, 0.5], 7)\n213: ok(\"positive\", \"dataset.split\", len(parts) == 2 and\n214:    sorted(parts[0].samples + parts[1].samples) == list(dataset.samples))\n215: pipeline = augmentation.compose([lambda value: value + 2, lambda value: value * 3])",
