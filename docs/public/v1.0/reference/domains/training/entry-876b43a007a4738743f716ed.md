# training — optimizer.loadState(state)

Bounded CPU reference gradients and local checkpoint model; no large-model training claim; original observation: optimizer.loadState(state); SDK bounds: {"MAX_BATCH":64,"MAX_CHECKPOINT_BYTES":1048576,"MAX_ELEMENTS":4096,"MAX_EPOCHS":1024,"MAX_GRAPH_NODES":256,"MAX_PARAMETERS":4096,"MAX_PREFETCH":8,"MAX_ROTATIONS":8,"MAX_SAMPLES":4096}

```text
Identity: owner:G021:G021-S04-06 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Optimizer.loadState(self, state: Mapping[str, Any]) -> None
Sgd.loadState(self, state: Mapping[str, Any]) -> None
Adam.loadState(self, state: Mapping[str, Any]) -> None
```
...
[
  ...
  {
    "excerpt": "151:                    {\"momentum\": 0.2, \"weightDecay\": 0.1})\n152: sgd_copy.loadState(state)\n153: ok(\"positive\", \"optimizer.loadState\", sgd_copy.state() == state)\n154: adam_parameter = Tensor([1.0], (1,), True)\n155: adam_parameter._gradient = [0.25]",
