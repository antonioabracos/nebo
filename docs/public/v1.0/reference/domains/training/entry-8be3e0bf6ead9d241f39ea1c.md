# training — training.environmentManifest()

Bounded CPU reference gradients and local checkpoint model; no large-model training claim; original observation: training.environmentManifest(); SDK bounds: {"MAX_BATCH":64,"MAX_CHECKPOINT_BYTES":1048576,"MAX_ELEMENTS":4096,"MAX_EPOCHS":1024,"MAX_GRAPH_NODES":256,"MAX_PARAMETERS":4096,"MAX_PREFETCH":8,"MAX_ROTATIONS":8,"MAX_SAMPLES":4096}

```text
Identity: owner:G021:G021-S07-05 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Training.environmentManifest(self) -> dict[str, Any]
```
...
[
  ...
  {
    "excerpt": "241: ok(\"positive\", \"training.deterministic\", training.deterministic(True))\n242: manifest = training.environmentManifest()\n243: ok(\"positive\", \"training.environmentManifest\", manifest[\"profile\"] == \"g021-bounded-cpu\"\n244:    and manifest[\"networkDownloads\"] == 0 and manifest[\"limits\"][\"graphNodes\"] == 256)\n245: for suffix in (\".1\", \".2\", \".3\"):",
