# training — Metric.precisionRecall(prediction, target)

Bounded CPU reference gradients and local checkpoint model; no large-model training claim; original observation: Metric.precisionRecall(prediction, target); SDK bounds: {"MAX_BATCH":64,"MAX_CHECKPOINT_BYTES":1048576,"MAX_ELEMENTS":4096,"MAX_EPOCHS":1024,"MAX_GRAPH_NODES":256,"MAX_PARAMETERS":4096,"MAX_PREFETCH":8,"MAX_ROTATIONS":8,"MAX_SAMPLES":4096}

```text
Identity: owner:G021:G021-S03-06 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Metric.precisionRecall(prediction: Tensor | Sequence[int], target: Sequence[int]) -> dict[str, tuple[float, ...]]
```
...
[
  ...
  {
    "excerpt": "132: ok(\"positive\", \"Metric.accuracy\", Metric.accuracy(metric_logits, metric_target) == 0.75)\n133: precision_recall = Metric.precisionRecall(metric_logits, metric_target)\n134: ok(\"positive\", \"Metric.precisionRecall\", precision_recall ==\n135:    {\"precision\": (0.5, 1.0), \"recall\": (1.0, 2 / 3)})\n136: matrix = Metric.confusionMatrix(metric_logits, metric_target)",
