# training — gradients.clipNorm(maxNorm)

Bounded CPU reference gradients and local checkpoint model; no large-model training claim; original observation: gradients.clipNorm(maxNorm); SDK bounds: {"MAX_BATCH":64,"MAX_CHECKPOINT_BYTES":1048576,"MAX_ELEMENTS":4096,"MAX_EPOCHS":1024,"MAX_GRAPH_NODES":256,"MAX_PARAMETERS":4096,"MAX_PREFETCH":8,"MAX_ROTATIONS":8,"MAX_SAMPLES":4096}

```text
Identity: owner:G021:G021-S02-04 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
GradientSet.clipNorm(self, maxNorm: float) -> float
```
...
[
  ...
  {
    "excerpt": "96: vector._gradient = [3.0, 4.0]\n97: observed_norm = gradients.clipNorm(2.5)\n98: ok(\"positive\", \"gradients.clipNorm\", observed_norm >= 5.0 and\n99:    close(math.sqrt(sum(value * value for value in vector.gradient().values)), 2.5))\n100: vector._gradient = [-7.0, 0.5]",
