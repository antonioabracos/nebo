# training — tensor.requiresGradient(enabled)

Bounded CPU reference gradients and local checkpoint model; no large-model training claim; original observation: tensor.requiresGradient(enabled); SDK bounds: {"MAX_BATCH":64,"MAX_CHECKPOINT_BYTES":1048576,"MAX_ELEMENTS":4096,"MAX_EPOCHS":1024,"MAX_GRAPH_NODES":256,"MAX_PARAMETERS":4096,"MAX_PREFETCH":8,"MAX_ROTATIONS":8,"MAX_SAMPLES":4096}

```text
Identity: owner:G021:G021-S01-02 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Tensor.requiresGradient(self, enabled: bool) -> 'Tensor'
```
...
[
  ...
  {
    "excerpt": "61: with autograd.recording(True) as scope:\n62:     x = Tensor([3.0], (1,)).requiresGradient(True)\n63:     square = x.multiply(x)\n64:     result = square.add(x)\n65: ok(\"positive\", \"Autograd.recording\", scope.enabled and autograd._enabled)",
