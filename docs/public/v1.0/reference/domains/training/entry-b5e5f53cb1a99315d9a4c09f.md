# training — trainer.fit(dataset, epochs)

Bounded CPU reference gradients and local checkpoint model; no large-model training claim; original observation: trainer.fit(dataset, epochs); SDK bounds: {"MAX_BATCH":64,"MAX_CHECKPOINT_BYTES":1048576,"MAX_ELEMENTS":4096,"MAX_EPOCHS":1024,"MAX_GRAPH_NODES":256,"MAX_PARAMETERS":4096,"MAX_PREFETCH":8,"MAX_ROTATIONS":8,"MAX_SAMPLES":4096}

```text
Identity: owner:G021:G021-S05-02 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Trainer.fit(self, dataset: MlDataset | Iterable[tuple[Tensor, Tensor]], epochs: int) -> tuple[dict[str, Any], ...]
```
...
[
  ...
  {
    "excerpt": "187:                             Loss.meanSquaredError).earlyStopping(\"loss\", 2)\n188: early_history = early_trainer.fit(MlDataset([\n189:     (Tensor([0.0], (1,)), Tensor([0.0], (1,)))]), 10)\n190: ok(\"positive\", \"trainer.earlyStopping\", single_trainer._early == (\"loss\", 4) and\n191:    len(early_history) == 3)",
