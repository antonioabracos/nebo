# training — trainer.trainBatch(batch)

Bounded CPU reference gradients and local checkpoint model; no large-model training claim; original observation: trainer.trainBatch(batch); SDK bounds: {"MAX_BATCH":64,"MAX_CHECKPOINT_BYTES":1048576,"MAX_ELEMENTS":4096,"MAX_EPOCHS":1024,"MAX_GRAPH_NODES":256,"MAX_PARAMETERS":4096,"MAX_PREFETCH":8,"MAX_ROTATIONS":8,"MAX_SAMPLES":4096}

```text
Identity: owner:G021:G021-S05-03 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Trainer.trainBatch(self, batch: MlDataset | Iterable[tuple[Tensor, Tensor]]) -> float
```
...
[
  ...
  {
    "excerpt": "175: ok(\"positive\", \"Trainer.new\", single_trainer.max_epochs == 64)\n176: first_batch_loss = single_trainer.trainBatch(train_samples)\n177: ok(\"positive\", \"trainer.trainBatch\", first_batch_loss == 21.0 and\n178:    single_model.weight.values[0] > 0.0)\n179: evaluation = single_trainer.evaluate(train_samples)",
