# training — dataset.prefetch(count)

Bounded CPU reference gradients and local checkpoint model; no large-model training claim; original observation: dataset.prefetch(count); SDK bounds: {"MAX_BATCH":64,"MAX_CHECKPOINT_BYTES":1048576,"MAX_ELEMENTS":4096,"MAX_EPOCHS":1024,"MAX_GRAPH_NODES":256,"MAX_PARAMETERS":4096,"MAX_PREFETCH":8,"MAX_ROTATIONS":8,"MAX_SAMPLES":4096}

```text
Identity: owner:G021:G021-S06-04 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
MlDataset.prefetch(self, count: int) -> 'MlDataset'
```
...
[
  ...
  {
    "excerpt": "206:    ((0, 1, 2, 3), (4, 5)))\n207: prefetched = dataset.prefetch(3)\n208: ok(\"positive\", \"dataset.prefetch\", prefetched.prefetch_count == 3 and\n209:    prefetched.samples == dataset.samples)\n210: mapped = dataset.map(lambda value: value * value)",
