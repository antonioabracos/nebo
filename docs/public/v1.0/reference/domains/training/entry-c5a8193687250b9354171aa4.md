# training — Loss.binaryCrossEntropy(input, target, options)

Bounded CPU reference gradients and local checkpoint model; no large-model training claim; original observation: Loss.binaryCrossEntropy(input, target, options); SDK bounds: {"MAX_BATCH":64,"MAX_CHECKPOINT_BYTES":1048576,"MAX_ELEMENTS":4096,"MAX_EPOCHS":1024,"MAX_GRAPH_NODES":256,"MAX_PARAMETERS":4096,"MAX_PREFETCH":8,"MAX_ROTATIONS":8,"MAX_SAMPLES":4096}

```text
Identity: owner:G021:G021-S03-03 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Loss.binaryCrossEntropy(input: Tensor, target: Tensor, options: Mapping[str, Any] | None=None) -> Tensor
```
...
[
  ...
  {
    "excerpt": "123: ok(\"positive\", \"Loss.crossEntropy\", close(cross_entropy.values[0], ce_expected))\n124: bce = Loss.binaryCrossEntropy(Tensor([0.8, 0.25], (2,), True),\n125:                               Tensor([1.0, 0.0], (2,)))\n126: ok(\"positive\", \"Loss.binaryCrossEntropy\",\n127:    close(bce.values[0], (-math.log(0.8) - math.log(0.75)) / 2))",
