# training — trainer.earlyStopping(metric, patience)

Bounded CPU reference gradients and local checkpoint model; no large-model training claim; original observation: trainer.earlyStopping(metric, patience); SDK bounds: {"MAX_BATCH":64,"MAX_CHECKPOINT_BYTES":1048576,"MAX_ELEMENTS":4096,"MAX_EPOCHS":1024,"MAX_GRAPH_NODES":256,"MAX_PARAMETERS":4096,"MAX_PREFETCH":8,"MAX_ROTATIONS":8,"MAX_SAMPLES":4096}

```text
Identity: owner:G021:G021-S05-06 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Trainer.earlyStopping(self, metric: str, patience: int) -> 'Trainer'
```
...
[
  ...
  {
    "excerpt": "183: ok(\"positive\", \"trainer.onEpoch\", len(single_trainer._callbacks) == 1)\n184: single_trainer.earlyStopping(\"loss\", 4)\n185: early_model = TinyLinearModel()\n186: early_trainer = Trainer.new(early_model, Sgd.new(early_model.parameters(), 0.01),\n187:                             Loss.meanSquaredError).earlyStopping(\"loss\", 2)",
...
- runtime/training/trainer.asm — SHA-256 4ecc814a6c13040625307c0f24a5d00fad95309d180d09b85b1cd797402984c1
