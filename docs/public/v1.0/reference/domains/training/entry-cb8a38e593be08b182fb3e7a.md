# training — scheduler.learningRate(step)

Bounded CPU reference gradients and local checkpoint model; no large-model training claim; original observation: scheduler.learningRate(step); SDK bounds: {"MAX_BATCH":64,"MAX_CHECKPOINT_BYTES":1048576,"MAX_ELEMENTS":4096,"MAX_EPOCHS":1024,"MAX_GRAPH_NODES":256,"MAX_PARAMETERS":4096,"MAX_PREFETCH":8,"MAX_ROTATIONS":8,"MAX_SAMPLES":4096}

```text
Identity: owner:G021:G021-S04-07 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
LearningRateScheduler.learningRate(self, step: int) -> float
```
...
[
  ...
  {
    "excerpt": "159: scheduler = LearningRateScheduler(0.2, 0.5, 0.025)\n160: ok(\"positive\", \"scheduler.learningRate\", scheduler.learningRate(3) == 0.025 and\n161:    scheduler.learningRate(9) == 0.025)\n162: \n163: # S05: tiny versioned corpus converges through live autodiff and callbacks.",
