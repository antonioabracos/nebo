# training — Loss.meanSquaredError(prediction, target, reduction)

Bounded CPU reference gradients and local checkpoint model; no large-model training claim; original observation: Loss.meanSquaredError(prediction, target, reduction); SDK bounds: {"MAX_BATCH":64,"MAX_CHECKPOINT_BYTES":1048576,"MAX_ELEMENTS":4096,"MAX_EPOCHS":1024,"MAX_GRAPH_NODES":256,"MAX_PARAMETERS":4096,"MAX_PREFETCH":8,"MAX_ROTATIONS":8,"MAX_SAMPLES":4096}

```text
Identity: owner:G021:G021-S03-01 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Loss.meanSquaredError(prediction: Tensor, target: Tensor, reduction: str='mean') -> Tensor
```
...
[
  ...
  {
    "excerpt": "87: p = Tensor([2.0], (1,), True)\n88: loss = Loss.meanSquaredError(p, Tensor([5.0], (1,)))\n89: loss.backward()\n90: ok(\"positive\", \"loss.backward\", p.gradient() is not None and p.gradient().values == (-6.0,))\n91: vector = Tensor([1.0, 2.0], (2,), True)",
