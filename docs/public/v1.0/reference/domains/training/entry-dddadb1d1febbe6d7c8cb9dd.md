# training — Loss.crossEntropy(logits, target, options)

Bounded CPU reference gradients and local checkpoint model; no large-model training claim; original observation: Loss.crossEntropy(logits, target, options); SDK bounds: {"MAX_BATCH":64,"MAX_CHECKPOINT_BYTES":1048576,"MAX_ELEMENTS":4096,"MAX_EPOCHS":1024,"MAX_GRAPH_NODES":256,"MAX_PARAMETERS":4096,"MAX_PREFETCH":8,"MAX_ROTATIONS":8,"MAX_SAMPLES":4096}

```text
Identity: owner:G021:G021-S03-02 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Loss.crossEntropy(logits: Tensor, target: Sequence[int], options: Mapping[str, Any] | None=None) -> Tensor
```
...
[
  ...
  {
    "excerpt": "120: logits = Tensor([2.0, 1.0, 0.0, 0.0, 2.0, 1.0], (2, 3), True)\n121: cross_entropy = Loss.crossEntropy(logits, [0, 1], {\"reduction\": \"mean\"})\n122: ce_expected = (-math.log(math.exp(2) / (math.exp(2) + math.exp(1) + 1)) * 2) / 2\n123: ok(\"positive\", \"Loss.crossEntropy\", close(cross_entropy.values[0], ce_expected))\n124: bce = Loss.binaryCrossEntropy(Tensor([0.8, 0.25], (2,), True),",
