# training — checkpoint.compare(other)

Bounded CPU reference gradients and local checkpoint model; no large-model training claim; original observation: checkpoint.compare(other); SDK bounds: {"MAX_BATCH":64,"MAX_CHECKPOINT_BYTES":1048576,"MAX_ELEMENTS":4096,"MAX_EPOCHS":1024,"MAX_GRAPH_NODES":256,"MAX_PARAMETERS":4096,"MAX_PREFETCH":8,"MAX_ROTATIONS":8,"MAX_SAMPLES":4096}

```text
Identity: owner:G021:G021-S07-07 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
CheckpointRecord.compare(self, other: 'CheckpointRecord') -> dict[str, Any]
```
...
[
  ...
  {
    "excerpt": "251:                          {\"epoch\": 4, \"corpus\": \"tiny-linear-v1\"})\n252: comparison = loaded.compare(second)\n253: ok(\"positive\", \"checkpoint.compare\", not comparison[\"equal\"] and\n254:    comparison[\"changedParameters\"] == ())\n255: ",
