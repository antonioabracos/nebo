# training — dataset.batch(size, dropLast)

Bounded CPU reference gradients and local checkpoint model; no large-model training claim; original observation: dataset.batch(size, dropLast); SDK bounds: {"MAX_BATCH":64,"MAX_CHECKPOINT_BYTES":1048576,"MAX_ELEMENTS":4096,"MAX_EPOCHS":1024,"MAX_GRAPH_NODES":256,"MAX_PARAMETERS":4096,"MAX_PREFETCH":8,"MAX_ROTATIONS":8,"MAX_SAMPLES":4096}

```text
Identity: owner:G021:G021-S06-03 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
MlDataset.batch(self, size: int, dropLast: bool=False) -> tuple['MlDataset', ...]
```
...
[
  ...
  {
    "excerpt": "203: ok(\"positive\", \"dataset.shuffle\", shuffled.samples == (0, 5, 2, 4, 3, 1))\n204: batches = dataset.batch(4, False)\n205: ok(\"positive\", \"dataset.batch\", tuple(part.samples for part in batches) ==\n206:    ((0, 1, 2, 3), (4, 5)))\n207: prefetched = dataset.prefetch(3)",
