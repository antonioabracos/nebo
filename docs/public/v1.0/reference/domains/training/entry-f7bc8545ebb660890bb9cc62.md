# training — autograd.gradCheck(function, inputs)

Bounded CPU reference gradients and local checkpoint model; no large-model training claim; original observation: autograd.gradCheck(function, inputs); SDK bounds: {"MAX_BATCH":64,"MAX_CHECKPOINT_BYTES":1048576,"MAX_ELEMENTS":4096,"MAX_EPOCHS":1024,"MAX_GRAPH_NODES":256,"MAX_PARAMETERS":4096,"MAX_PREFETCH":8,"MAX_ROTATIONS":8,"MAX_SAMPLES":4096}

```text
Identity: owner:G021:G021-S02-07 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Autograd.gradCheck(self, function: Callable[..., Tensor], inputs: Sequence[Tensor], step: float=1e-05, tolerance: float=0.0001) -> dict[str, Any]
```
...
[
  ...
  {
    "excerpt": "108:    checkpoint_input.gradient().values == (4.0, 6.0))\n109: gradcheck = autograd.gradCheck(lambda value: value.multiply(value).sum(),\n110:                               [Tensor([1.25, -2.0], (2,))])\n111: ok(\"positive\", \"autograd.gradCheck\", gradcheck[\"passed\"] and\n112:    gradcheck[\"elements\"] == 2 and gradcheck[\"maximumError\"] < 1e-8)",
...
- runtime/autograd/gradcheck.asm — SHA-256 c7abe4f1545fbff28f7f178f081a9781999cda3cc6e0bef1d7941f5d6d1630ca
