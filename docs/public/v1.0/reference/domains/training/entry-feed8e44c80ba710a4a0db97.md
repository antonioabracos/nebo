...
Bounded CPU reference gradients and local checkpoint model; no large-model training claim; original observation: loss.backward(); SDK bounds: {"MAX_BATCH":64,"MAX_CHECKPOINT_BYTES":1048576,"MAX_ELEMENTS":4096,"MAX_EPOCHS":1024,"MAX_GRAPH_NODES":256,"MAX_PARAMETERS":4096,"MAX_PREFETCH":8,"MAX_ROTATIONS":8,"MAX_SAMPLES":4096}

```text
Identity: owner:G021:G021-S02-01 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Tensor.backward(self, gradient: 'Tensor | Sequence[float] | float | None'=None) -> None
```
...
[
  ...
  {
    "excerpt": "68: ok(\"positive\", \"operation.backwardRule\", rule((2.0,)) == ((6.0,), (6.0,)))\n69: result.backward()\n70: ok(\"positive\", \"tensor.gradient\", x.gradient() is not None and x.gradient().values == (7.0,))\n71: detached = result.detach()\n72: ok(\"positive\", \"tensor.detach\", detached.values == (12.0,) and",
