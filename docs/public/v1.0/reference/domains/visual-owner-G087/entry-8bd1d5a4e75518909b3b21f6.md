...
RF116 internal visual/model receipt; not a public visual.* facade. Public receiver-first Console/Scan has separate public identities. No live hardware claim.; original observation: Serializers return exact byte length without hidden NUL

```text
Identity: owner:G087:G087-S08-03 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
length
Owner: runtime/color.asm::nebo_color_serialize_hex
```
