# visual-owner-G088 — no-fallback

RF116 internal visual/model receipt; not a public visual.* facade. Public receiver-first Console/Scan has separate public identities. No live hardware claim.; original observation: No accessible candidate fails without output mutation

```text
Identity: owner:G088:G088-S06-07 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
no-fallback
Owner: runtime/g088_source_probe.asm::nebo_g088_negative_probe
```
