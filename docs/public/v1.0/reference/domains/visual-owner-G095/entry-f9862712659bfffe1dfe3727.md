...
RF116 internal visual/model receipt; not a public visual.* facade. Public receiver-first Console/Scan has separate public identities. No live hardware claim.; original observation: exit 37; X data schema bound

```text
Identity: owner:G095:.x (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
.x
Owner: compiler/semantic/visual/g095_source_vertical.inc
```
