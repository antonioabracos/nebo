# visual-owner-G099 — .logPanel

RF116 internal visual/model receipt; not a public visual.* facade. Public receiver-first Console/Scan has separate public identities. No live hardware claim.; original observation: one bounded log widget is explicitly attached and its source is evaluated once

```text
Identity: owner:G099:.logPanel (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
.logPanel
Owner: runtime/g099_dashboard.asm
```
