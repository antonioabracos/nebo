# visual-owner-G102 — .path

RF116 internal visual/model receipt; not a public visual.* facade. Public receiver-first Console/Scan has separate public identities. No live hardware claim.; original observation: logical path id is nonzero bounded and capability checked

```text
Identity: owner:G102:.path (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
.path
Owner: nebo_g102_animation_export_model
```
