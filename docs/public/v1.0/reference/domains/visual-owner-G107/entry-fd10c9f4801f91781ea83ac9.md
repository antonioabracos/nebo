# visual-owner-G107 — fallback:

RF116 internal visual/model receipt; not a public visual.* facade. Public receiver-first Console/Scan has separate public identities. No live hardware claim.; original observation: an unavailable visual sink uses the explicit summary fallback while detect-target remains fail-closed

```text
Identity: owner:G107:fallback: (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
fallback:
Owner: compiler/semantic/visual/g107_source_vertical.inc;runtime/console_routing.asm
```
...
- runtime/console_routing.asm — SHA-256 e533eb82e0cb816ebe5c13f47d9c88bcdfc4d19509e111482500bc784d71d1e8
