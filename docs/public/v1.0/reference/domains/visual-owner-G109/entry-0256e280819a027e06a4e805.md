# visual-owner-G109 — .debugRendererInfo(...)

RF116 internal visual/model receipt; not a public visual.* facade. Public receiver-first Console/Scan has separate public identities. No live hardware claim.; original observation: deterministic renderer profiling publishes logical work and peak bytes without reading clocks, display state or secrets

```text
Identity: owner:G109:.debugRendererInfo(...) (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
.debugRendererInfo(...)
Owner: runtime/g109_source_probe.asm;runtime/g109_diagnostics.asm
```
