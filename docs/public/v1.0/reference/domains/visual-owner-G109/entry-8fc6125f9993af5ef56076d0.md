# visual-owner-G109 — .inspect(...)

RF116 internal visual/model receipt; not a public visual.* facade. Public receiver-first Console/Scan has separate public identities. No live hardware claim.; original observation: bounded structured inspection publishes code, renderer class, depth, safe flags and fingerprint with no pointer field

```text
Identity: owner:G109:.inspect(...) (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
.inspect(...)
Owner: compiler/semantic/visual/g109_source_vertical.inc;runtime/g109_diagnostics.asm
```
