# visual-owner-G110 — visual.open()

RF116 internal visual/model receipt; not a public visual.* facade. Public receiver-first Console/Scan has separate public identities. No live hardware claim.; original observation: historical public facade is rejected; receiver-first RenderPlan reaches internal open lifecycle

```text
Identity: owner:G110:visual.open() (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
visual.open()
Owner: compiler/semantic/visual/g110_source_vertical.inc;runtime/internal_visual_console.asm
```
