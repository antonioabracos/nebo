# workflows — rules.when(condition)

Bounded local workflow/agent state machine; no remote scheduler or external side effects; original observation: labeled pure condition is registered; SDK bounds: {"MAX_ATTEMPTS":8,"MAX_DOMAIN_CASES":256,"MAX_HISTORY":256,"MAX_ROWS":64,"MAX_RULES":64,"MAX_SAGA_STEPS":16,"MAX_STATES":128,"MAX_STEPS":64,"MAX_TRANSITIONS":1024}

```text
Identity: owner:G045:G045-S05-02 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
RuleSet.when(self, condition: Callable[[Mapping[str, Any]], bool], label: str | None=None) -> Rule
```
...
[
  ...
  {
    "excerpt": "147: ok(\"positive\", \"RuleSet.new\", rules.name == \"shipping-rules\" and rules.version == 3)\n148: priority_rule = rules.when(lambda ctx: ctx[\"total\"] >= 100, \"priority\")\n149: ok(\"positive\", \"rules.when\", priority_rule.label == \"priority\")\n150: priority_rule.then(lambda ctx: {\"lane\": \"express\"})\n151: ok(\"positive\", \"rule.then\", callable(priority_rule.action))",
