...
```text
Saga.new(cls, steps: Sequence[WorkflowStep]) -> 'Saga'
```
...
[
  ...
  {
    "excerpt": "54: # S01 — deterministic finite state machine declarations and observations.\n55: machine = StateMachine.new(\"order-machine\", \"pending\")\n56: ok(\"positive\", \"StateMachine.new\", machine.name == \"order-machine\" and machine.current == \"pending\")\n57: pending = machine.state(\"pending\")\n58: approved = machine.state(\"approved\")",
