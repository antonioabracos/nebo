# workflows — table.validateCoverage(domain)

Bounded local workflow/agent state machine; no remote scheduler or external side effects; original observation: two supplied domain cases have no gap; SDK bounds: {"MAX_ATTEMPTS":8,"MAX_DOMAIN_CASES":256,"MAX_HISTORY":256,"MAX_ROWS":64,"MAX_RULES":64,"MAX_SAGA_STEPS":16,"MAX_STATES":128,"MAX_STEPS":64,"MAX_TRANSITIONS":1024}

```text
Identity: owner:G045:G045-S06-04 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
DecisionTable.validateCoverage(self, domain: Sequence[Mapping[str, Any]]) -> dict[str, Any]
```
...
[
  ...
  {
    "excerpt": "174: ok(\"positive\", \"table.decide\", table_decision == {\"queue\": \"priority\"})\n175: coverage = table.validateCoverage(({\"tier\": 5, \"region\": \"EU\"}, {\"tier\": 75, \"region\": \"EU\"}))\n176: ok(\"positive\", \"table.validateCoverage\", coverage == {\"covered\": True, \"cases\": 2, \"gaps\": ()})\n177: conflicts = table.validateConflicts()\n178: ok(\"positive\", \"table.validateConflicts\", conflicts == {\"valid\": True, \"conflicts\": ()})",
