# workflows — table.explain(input)

Bounded local workflow/agent state machine; no remote scheduler or external side effects; original observation: explanation names row two and priority result; SDK bounds: {"MAX_ATTEMPTS":8,"MAX_DOMAIN_CASES":256,"MAX_HISTORY":256,"MAX_ROWS":64,"MAX_RULES":64,"MAX_SAGA_STEPS":16,"MAX_STATES":128,"MAX_STEPS":64,"MAX_TRANSITIONS":1024}

```text
Identity: owner:G045:G045-S06-07 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
RuleSet.explain(self, decision: Decision) -> dict[str, Any]
DecisionTable.explain(self, input: Mapping[str, Any]) -> dict[str, Any]
```
...
[
  ...
  {
    "excerpt": "158: ok(\"positive\", \"rules.fire\", fired.fired and fired.values == ({\"lane\": \"express\"}, {\"tax\": \"vat\"}))\n159: explanation = rules.explain(decision)\n160: ok(\"positive\", \"rules.explain\", explanation[\"facts\"][\"token\"] == \"<redacted>\"\n161:    and explanation[\"matched\"] == decision.matched)\n162: rules_valid = rules.validate()",
