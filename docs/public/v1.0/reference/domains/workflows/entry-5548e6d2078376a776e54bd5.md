# workflows — step.compensate(action)

Bounded local workflow/agent state machine; no remote scheduler or external side effects; original observation: charge owns explicit refund compensation; SDK bounds: {"MAX_ATTEMPTS":8,"MAX_DOMAIN_CASES":256,"MAX_HISTORY":256,"MAX_ROWS":64,"MAX_RULES":64,"MAX_SAGA_STEPS":16,"MAX_STATES":128,"MAX_STEPS":64,"MAX_TRANSITIONS":1024}

```text
Identity: owner:G045:G045-S04-04 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
WorkflowStep.compensate(self, action: Callable[[dict[str, Any]], Any]) -> 'WorkflowStep'
```
...
[
  ...
  {
    "excerpt": "130: ok(\"positive\", \"step.idempotencyKey\", saga_first.key_function({\"order\": 17}) == \"charge:17\")\n131: saga_first.compensate(lambda ctx: compensations.append(\"refund\"))\n132: ok(\"positive\", \"step.compensate\", callable(saga_first.compensation))\n133: saga_second = WorkflowStep(\"prepare\", lambda ctx: {\"prepared\": True})\n134: saga_third = WorkflowStep(\"ship\", lambda ctx: (_ for _ in ()).throw(RuntimeError(\"carrier\")))",
