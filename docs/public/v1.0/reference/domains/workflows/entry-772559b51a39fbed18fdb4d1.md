# workflows — state.child(name)

Bounded local workflow/agent state machine; no remote scheduler or external side effects; original observation: payment child retains working parent; SDK bounds: {"MAX_ATTEMPTS":8,"MAX_DOMAIN_CASES":256,"MAX_HISTORY":256,"MAX_ROWS":64,"MAX_RULES":64,"MAX_SAGA_STEPS":16,"MAX_STATES":128,"MAX_STEPS":64,"MAX_TRANSITIONS":1024}

```text
Identity: owner:G045:G045-S02-01 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
State.child(self, name: str) -> 'State'
```
...
[
  ...
  {
    "excerpt": "79: working = chart.state(\"working\")\n80: region_a = working.child(\"payment\")\n81: ok(\"positive\", \"state.child\", region_a.parent is working)\n82: working.child(\"inventory\")\n83: working.initial(region_a)",
