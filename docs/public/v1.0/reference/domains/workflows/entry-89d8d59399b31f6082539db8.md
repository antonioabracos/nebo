# workflows — workflow.migrate(instance,targetVersion,plan)

Bounded local workflow/agent state machine; no remote scheduler or external side effects; original observation: atomic plan advances instance one to version two; SDK bounds: {"MAX_ATTEMPTS":8,"MAX_DOMAIN_CASES":256,"MAX_HISTORY":256,"MAX_ROWS":64,"MAX_RULES":64,"MAX_SAGA_STEPS":16,"MAX_STATES":128,"MAX_STEPS":64,"MAX_TRANSITIONS":1024}

```text
Identity: owner:G045:G045-S07-01 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Workflow.migrate(self, instance: str | Mapping[str, Any], targetVersion: int, plan: Callable[[dict[str, Any]], Mapping[str, Any]]) -> dict[str, Any]
```
...
[
  ...
  {
    "excerpt": "201: ops_v2.step(\"notify\", lambda ctx: {\"notified\": True})\n202: migrated = ops_v2.migrate(ops_instance[\"id\"], 2, lambda ctx: {**ctx, \"schema\": \"v2\"})\n203: ok(\"positive\", \"workflow.migrate\", migrated[\"version\"] == 2 and migrated[\"context\"][\"schema\"] == \"v2\")\n204: compatibility = ops_v2.compatibility(ops_v1)\n205: ok(\"positive\", \"workflow.compatibility\", compatibility[\"classification\"] == \"additive\"",
