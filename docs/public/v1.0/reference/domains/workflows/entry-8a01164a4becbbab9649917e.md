# workflows — workflow.checkpoint()

Bounded local workflow/agent state machine; no remote scheduler or external side effects; original observation: checkpoint revision and SHA-256 seal advance; SDK bounds: {"MAX_ATTEMPTS":8,"MAX_DOMAIN_CASES":256,"MAX_HISTORY":256,"MAX_ROWS":64,"MAX_RULES":64,"MAX_SAGA_STEPS":16,"MAX_STATES":128,"MAX_STEPS":64,"MAX_TRANSITIONS":1024}

```text
Identity: owner:G045:G045-S03-06 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Workflow.checkpoint(self) -> dict[str, Any]
```
...
[
  ...
  {
    "excerpt": "112: ok(\"positive\", \"workflow.sleepUntil\", workflow.sleepUntil(135) == 135)\n113: checkpoint = workflow.checkpoint()\n114: ok(\"positive\", \"workflow.checkpoint\", checkpoint[\"checkpoint\"] == 2 and len(checkpoint[\"checksum\"]) == 64)\n115: resumed = workflow.resume(run_snapshot[\"id\"])\n116: ok(\"positive\", \"workflow.resume\", resumed[\"status\"] == \"running\" and resumed[\"version\"] == 1)",
