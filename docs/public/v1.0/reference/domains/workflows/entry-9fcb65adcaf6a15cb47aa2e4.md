# workflows — step.idempotencyKey(function)

Bounded local workflow/agent state machine; no remote scheduler or external side effects; original observation: stable order-derived key is required; SDK bounds: {"MAX_ATTEMPTS":8,"MAX_DOMAIN_CASES":256,"MAX_HISTORY":256,"MAX_ROWS":64,"MAX_RULES":64,"MAX_SAGA_STEPS":16,"MAX_STATES":128,"MAX_STEPS":64,"MAX_TRANSITIONS":1024}

```text
Identity: owner:G045:G045-S04-03 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
WorkflowStep.idempotencyKey(self, function: Callable[[Mapping[str, Any]], str]) -> 'WorkflowStep'
```
...
[
  ...
  {
    "excerpt": "128: ok(\"positive\", \"step.timeout\", saga_first.timeout_duration == 25)\n129: saga_first.idempotencyKey(lambda ctx: f\"charge:{ctx['order']}\")\n130: ok(\"positive\", \"step.idempotencyKey\", saga_first.key_function({\"order\": 17}) == \"charge:17\")\n131: saga_first.compensate(lambda ctx: compensations.append(\"refund\"))\n132: ok(\"positive\", \"step.compensate\", callable(saga_first.compensation))",
...
- runtime/workflow/saga.asm — SHA-256 e63614e5d1a269d898165066aa6ed2044214db609b240d97dd2095d95edfeec1
