# workflows — workflow.cancel(reason)

Bounded local workflow/agent state machine; no remote scheduler or external side effects; original observation: explicit reason produces audited cancelled status; SDK bounds: {"MAX_ATTEMPTS":8,"MAX_DOMAIN_CASES":256,"MAX_HISTORY":256,"MAX_ROWS":64,"MAX_RULES":64,"MAX_SAGA_STEPS":16,"MAX_STATES":128,"MAX_STEPS":64,"MAX_TRANSITIONS":1024}

```text
Identity: owner:G045:G045-S03-08 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Workflow.cancel(self, reason: str) -> dict[str, Any]
```
...
[
  ...
  {
    "excerpt": "116: ok(\"positive\", \"workflow.resume\", resumed[\"status\"] == \"running\" and resumed[\"version\"] == 1)\n117: cancelled = workflow.cancel(\"customer-request\")\n118: ok(\"positive\", \"workflow.cancel\", cancelled[\"status\"] == \"cancelled\" and cancelled[\"reason\"] == \"customer-request\")\n119: transcript.append((run_snapshot[\"id\"], wait, checkpoint[\"checksum\"], resumed[\"status\"], cancelled))\n120: ",
