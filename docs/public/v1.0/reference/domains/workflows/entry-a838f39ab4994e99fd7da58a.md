# workflows — saga.execute(context)

Bounded local workflow/agent state machine; no remote scheduler or external side effects; original observation: fault compensates charge and exposes stuck step; SDK bounds: {"MAX_ATTEMPTS":8,"MAX_DOMAIN_CASES":256,"MAX_HISTORY":256,"MAX_ROWS":64,"MAX_RULES":64,"MAX_SAGA_STEPS":16,"MAX_STATES":128,"MAX_STEPS":64,"MAX_TRANSITIONS":1024}

```text
Identity: owner:G045:G045-S04-06 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Saga.execute(self, context: Mapping[str, Any]) -> dict[str, Any]
```
...
[
  ...
  {
    "excerpt": "136: ok(\"positive\", \"Saga.new\", len(saga.steps) == 3)\n137: saga_result = saga.execute({\"order\": 17})\n138: ok(\"positive\", \"saga.execute\", saga_result[\"status\"] == \"stuck\" and compensations == [\"refund\"])\n139: ok(\"positive\", \"saga.status\", saga.status() == \"stuck\")\n140: manual = saga.manualIntervention(\"carrier-unavailable\")",
