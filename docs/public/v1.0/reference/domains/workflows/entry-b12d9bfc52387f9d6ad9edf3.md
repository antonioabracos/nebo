# workflows — machine.validate()

Bounded local workflow/agent state machine; no remote scheduler or external side effects; original observation: reachability targets and determinism validate; SDK bounds: {"MAX_ATTEMPTS":8,"MAX_DOMAIN_CASES":256,"MAX_HISTORY":256,"MAX_ROWS":64,"MAX_RULES":64,"MAX_SAGA_STEPS":16,"MAX_STATES":128,"MAX_STEPS":64,"MAX_TRANSITIONS":1024}

```text
Identity: owner:G045:G045-S01-08 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
StateMachine.validate(self) -> dict[str, Any]
RuleSet.validate(self) -> dict[str, Any]
```
...
[
  ...
  {
    "excerpt": "68: ok(\"positive\", \"machine.transitionHistory\", history[0][\"from\"] == \"pending\" and history[0][\"event\"] == \"approve\")\n69: validation = machine.validate()\n70: ok(\"positive\", \"machine.validate\", validation == {\"valid\": True, \"states\": 2, \"transitions\": 1,\n71:                                                      \"reachable\": (\"approved\", \"pending\")})\n72: transcript.append((transition, history, validation))",
