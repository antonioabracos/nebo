# workflows — state.onExit(action)

Bounded local workflow/agent state machine; no remote scheduler or external side effects; original observation: idle exit action runs first; SDK bounds: {"MAX_ATTEMPTS":8,"MAX_DOMAIN_CASES":256,"MAX_HISTORY":256,"MAX_ROWS":64,"MAX_RULES":64,"MAX_SAGA_STEPS":16,"MAX_STATES":128,"MAX_STEPS":64,"MAX_TRANSITIONS":1024}

```text
Identity: owner:G045:G045-S02-05 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
State.onExit(self, action: Callable[[dict[str, Any]], Any]) -> 'State'
```
...
[
  ...
  {
    "excerpt": "88: ok(\"positive\", \"state.onEnter\", callable(working.enter_action))\n89: idle.onExit(lambda ctx: lifecycle.append(\"exit-idle\"))\n90: ok(\"positive\", \"state.onExit\", callable(idle.exit_action))\n91: working.history(\"deep\")\n92: ok(\"positive\", \"state.history\", working.history_mode == \"deep\")",
