# workflows — workflow.waitFor(event,deadline)

Bounded local workflow/agent state machine; no remote scheduler or external side effects; original observation: wait persists event and logical deadline; SDK bounds: {"MAX_ATTEMPTS":8,"MAX_DOMAIN_CASES":256,"MAX_HISTORY":256,"MAX_ROWS":64,"MAX_RULES":64,"MAX_SAGA_STEPS":16,"MAX_STATES":128,"MAX_STEPS":64,"MAX_TRANSITIONS":1024}

```text
Identity: owner:G045:G045-S03-04 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Workflow.waitFor(self, event: str, deadline: int) -> dict[str, Any]
```
...
[
  ...
  {
    "excerpt": "109:    and run_snapshot[\"context\"][\"reserved\"] == 45 and len(run_snapshot[\"checksum\"]) == 64)\n110: wait = workflow.waitFor(\"warehouse-confirmed\", 120)\n111: ok(\"positive\", \"workflow.waitFor\", wait == {\"event\": \"warehouse-confirmed\", \"deadline\": 120})\n112: ok(\"positive\", \"workflow.sleepUntil\", workflow.sleepUntil(135) == 135)\n113: checkpoint = workflow.checkpoint()",
