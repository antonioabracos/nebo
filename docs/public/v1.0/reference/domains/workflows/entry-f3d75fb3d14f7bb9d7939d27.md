# workflows — rule.then(action)

Bounded local workflow/agent state machine; no remote scheduler or external side effects; original observation: matched rule owns explicit action; SDK bounds: {"MAX_ATTEMPTS":8,"MAX_DOMAIN_CASES":256,"MAX_HISTORY":256,"MAX_ROWS":64,"MAX_RULES":64,"MAX_SAGA_STEPS":16,"MAX_STATES":128,"MAX_STEPS":64,"MAX_TRANSITIONS":1024}

```text
Identity: owner:G045:G045-S05-03 (DOMAIN_CONTRACT_ID)
Edition: 1
...
```text
Rule.then(self, action: Callable[[dict[str, Any]], Any]) -> 'Rule'
```
...
[
  ...
  {
    "excerpt": "149: ok(\"positive\", \"rules.when\", priority_rule.label == \"priority\")\n150: priority_rule.then(lambda ctx: {\"lane\": \"express\"})\n151: ok(\"positive\", \"rule.then\", callable(priority_rule.action))\n152: priority_rule.priority(45)\n153: ok(\"positive\", \"rule.priority\", priority_rule.priority_value == 45)",
