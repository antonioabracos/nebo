...
```text
Identity: prelude:1:Range (SERIALIZED_SYMBOL_ID)
Edition: 1
Target: x86_64-systemv-elf-linux
Availability: EDITION_1_BOUNDED
Serialized SymbolId: 0xbbaa81a7272b4f17
```

## Syntax or signature

```text
Range.exclusive(start, end) | Range.inclusive(start, end)
```

## Origin and explicit import

Implicit origin: PRELUDE. Original module: std.core. Explicit alternative: import "std.core" { Range; }.explicit; --no-prelude removes implicit visibility. Import every referenced prelude name explicitly; imports grant no capability.

## Behavior, methods and protocols

length counts members; contains tests membership. The four Unicode endpoint forms retain their exact inclusive/exclusive meaning. Bind an iterable before using it in for.
...
Zero step, invalid direction, overflow or unsupported scalar types reject. for does not admit an arbitrary inline expression as its iterable.
...
### local:hidden-Range; expected NEBO-RF166-G163-002
