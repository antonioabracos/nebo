...
```text
Identity: claim:49faf7047d69177900e8fe96 (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### random:function-local-state; expected 23

```nebo
(Int.value)draw(){Random.seed(value).r;r.uniform(10,100).return;}start(){29.draw().console();23.return;}
```
