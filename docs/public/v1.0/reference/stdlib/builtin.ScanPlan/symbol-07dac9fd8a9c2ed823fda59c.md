# builtin.ScanPlan — ScanPlan.select
...
```text
Identity: g170-scan:select (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### scan:choice-variant-select-red; expected 23

```nebo
start(){"Choice".scan(.console(),.select(["red","blue"]),.mock("red")).console();23.return;}
```

Oracle: {"capabilities": {"console": "RETAINED_DOCUMENT", "filesystem": "EXPLICIT_SCRATCH_EFFECT_ORACLE", "network": "DENIED_BY_SECCOMP"}, "console_text_utf8": "red", "filesystem_effects": {}, "independent_builds": 2, "kinds": \[2\], "observation": {"native_oracle": "scan_public_test.observe"}, "process_exit": 23, "publications": 1, "runtime_determinism": "DOMAIN_INVARIANTS", "runtime_sha256": "d0f27b3d99ddc31ceaa14de62305013dd052236bdece8b72cb8b6ffbcba8b47f", "scan_trace": true, "text": {"bytes_hex": "726564"}}

## Rejected examples

### scan:choice-type-select; expected NEBO_TYPE_MISMATCH

```nebo
start(){"Choice".scan(.console(),.select([17,29]));23.return;}
```
