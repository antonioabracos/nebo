# builtin.ScanPlan — ScanPlan.cancelOn
...
```text
Identity: g170-scan:cancelOn (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### scan:cancel-result-cancel; expected 23

```nebo
start(){"Input".scan(.int(),.cancelOn("cancel"),.result(),.mock("cancel")).isErr().console();23.return;}
```
...
### scan:option-type-cancelOn; expected NEBO_TYPE_MISMATCH

```nebo
start(){"Input".scan(.cancelOn(17));23.return;}
```
