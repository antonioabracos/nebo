# builtin.ScanPlan — ScanPlan.maxLen
...
```text
Identity: g170-scan:maxLen (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### scan:maxlen-exact; expected 23

```nebo
start(){"Input".scan(.text(),.maxLen(2),.mock("ab")).console();23.return;}
```
...
### scan:option-type-maxLen; expected NEBO_TYPE_MISMATCH

```nebo
start(){"Input".scan(.maxLen(true));23.return;}
```
