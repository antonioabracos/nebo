# builtin.ScanPlan — ScanPlan.chomp
...
```text
Identity: g170-scan:chomp (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### scan:explicit-chomp; expected 23

```nebo
start(){"Input".scan(.text(),.chomp(),.mock("Nebo\r\n")).console();23.return;}
```
...
### scan:option-arity-chomp; expected NEBO_TYPE_MISMATCH

```nebo
start(){"Input".scan(.chomp(17));23.return;}
```
