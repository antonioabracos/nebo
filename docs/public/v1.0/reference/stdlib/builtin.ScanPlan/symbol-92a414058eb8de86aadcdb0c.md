# builtin.ScanPlan — ScanPlan.text
...
```text
Identity: g170-scan:text (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### scan:text-4e65626f; expected 23

```nebo
start(){"Name: ".scan(.text(),.mock("Nebo")).console();23.return;}
```
...
### scan:option-arity-text; expected NEBO_TYPE_MISMATCH

```nebo
start(){"Input".scan(.text(17));23.return;}
```
