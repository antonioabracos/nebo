# builtin.ScanPlan — ScanPlan.optional
...
```text
Identity: g170-scan:optional (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### scan:none-empty; expected 23

```nebo
start(){"Age".scan(.int(),.optional(),.mock("")).isNone().console();23.return;}
```
...
### scan:option-arity-optional; expected NEBO_TYPE_MISMATCH

```nebo
start(){"Input".scan(.optional(17));23.return;}
```
