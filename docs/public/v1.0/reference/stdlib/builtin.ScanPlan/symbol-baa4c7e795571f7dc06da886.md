# builtin.ScanPlan — ScanPlan.char
...
```text
Identity: g170-scan:char (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### scan:char-120; expected 23

```nebo
start(){"Key: ".scan(.char(),.mock("x")).codepoint().console();23.return;}
```

Oracle: {"capabilities": {"console": "RETAINED_DOCUMENT", "filesystem": "EXPLICIT_SCRATCH_EFFECT_ORACLE", "network": "DENIED_BY_SECCOMP"}, "console_text_utf8": "120", "filesystem_effects": {}, "independent_builds": 2, "kinds": \[4\], "process_exit": 23, "runtime_determinism": "BYTE_IDENTICAL", "runtime_sha256": "0ebb2adad72450de8faee4e18dc5eed8c019557ac47f18d314302a7f2fcd0c48", "text": {"bytes_hex": "313230"}}

## Rejected examples

### scan:option-arity-char; expected NEBO_TYPE_MISMATCH

```nebo
start(){"Input".scan(.char(17));23.return;}
```
