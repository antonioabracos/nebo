# builtin.ScanPlan — ScanPlan.allowEmpty
...
```text
Identity: g170-scan:allowEmpty (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### scan:capacity-0; expected 23

```nebo
start(){"Input".scan(.text(),.allowEmpty(),.mock("")).byteLength().console();23.return;}
```
...
### scan:option-arity-allowEmpty; expected NEBO_TYPE_MISMATCH

```nebo
start(){"Input".scan(.allowEmpty(17));23.return;}
```
