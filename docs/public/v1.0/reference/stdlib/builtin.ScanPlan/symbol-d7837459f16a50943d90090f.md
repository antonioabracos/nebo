# builtin.ScanPlan — ScanPlan.required
...
```text
Identity: g170-scan:required (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### scan:result-value-text-True; expected 23

```nebo
start(){"Input".scan(.text(),.required(),.result(),.mock("Nebo")).unwrapOr("fallback").console();23.return;}
```
...
### scan:option-arity-required; expected NEBO_TYPE_MISMATCH

```nebo
start(){"Input".scan(.required(17));23.return;}
```
