# builtin.Text — FormatString&lt;TArgs&gt;
...
```text
Identity: g059-format:FormatString<TArgs> (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### format:parsed-interpolation; expected 23

```nebo
start(){"${FormatString.parseChecked("%d").get().validate(Tuple.of(Int))}".console();23.return;}
```
...
### format:parsed-signature-type; expected NEBO-FORMAT-003

```nebo
start(){"%d".strict().validate(Tuple.of(17));23.return;}
```
