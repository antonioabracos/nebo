# builtin.Text — INTERPOLATION_EVALUATION=EXACTLY_ONCE
...
```text
Identity: g061-format:INTERPOLATION_EVALUATION=EXACTLY_ONCE (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
```text
INTERPOLATION_EVALUATION=EXACTLY_ONCE
Ordinary token/AST expression children; native pure-only effect policy with resolved callee bodies; static G059 nodes and G062 profiles; 64 segments, 16 nested quotes, 4096 source-content bytes; native named-product fields preserve declared types/offsets and source order (8 declarations/8 fields); native-token tooling preserves literals and exposes expressions; rename uses existing G162 receiver-method SymbolIds; privacy remains a separate obligation Checked format/interpolation profile; 64 segments; finite text/output budgets; pure interpolation
```
...
### interpolation:nested; expected 23

```nebo
start(){"outer=${"inner=${17+29}"}".console();23.return;}
```

Oracle: {"capabilities": {"console": "RETAINED_DOCUMENT", "filesystem": "EXPLICIT_SCRATCH_EFFECT_ORACLE", "network": "DENIED_BY_SECCOMP"}, "console_text_utf8": "outer=inner=46", "filesystem_effects": {}, "independent_builds": 2, "kinds": \[2\], "process_exit": 23, "publications": 1, "runtime_determinism": "BYTE_IDENTICAL", "runtime_sha256": "900b729ab3cca2a2d8547a71e1879b464c902dab1e47e36b7294f590f766fafd", "text": {"bytes_hex": "6f757465723d696e6e65723d3436"}}

## Rejected examples

### interpolation:effect-function; expected NEBO_INTERPOLATION_EFFECT_FORBIDDEN

```nebo
(Int.x)plus(){x.console();(x+29).return;}start(){"${17.plus()}";23.return;}
```
