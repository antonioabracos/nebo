...
```text
Identity: g060-format:%{idade:d} (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### format:named-repeat; expected 23

```nebo
start(){"%{value:d}/%{value:d}".format(value:29).console();23.return;}
```
...
### format:named-type; expected NEBO-FORMAT-003

```nebo
start(){"%{value:d}".format(value:"Nebo");23.return;}
```
