...
```text
Identity: g060-format:NEBO-FORMAT-004 (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### format:checked-error-width; expected 23

```nebo
start(){"%257d".template;template.formatWith(0 ,17).unwrapOr("fallback").console();23.return;}
```
...
### format:precision-limit; expected NEBO-FORMAT-004

```nebo
start(){"%.7f".format(1.5);23.return;}
```

Oracle: {"artifact_publication": "REJECTED", "compiler_exit": 1, "diagnostic": "NEBO-FORMAT-004"}
