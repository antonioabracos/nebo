...
```text
Identity: g060-format:%j (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### format:json-4e65626f; expected 23

```nebo
start(){"%j".format("Nebo").console();23.return;}
```
...
### format:wrong-json; expected NEBO-FORMAT-003

```nebo
start(){"%j".format(false);23.return;}
```
