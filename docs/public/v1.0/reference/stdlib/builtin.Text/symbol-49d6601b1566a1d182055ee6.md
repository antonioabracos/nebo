# builtin.Text — .missingArgs(policy)
...
```text
Identity: g060-format:.missingArgs(policy) (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### format:configured-missing; expected 23

```nebo
start(){"%s".missingArgs(1).format().unwrapOr("fallback").console();23.return;}
```
...
### format:configured-type-missingArgs; expected NEBO-FORMAT-003

```nebo
start(){"%d".missingArgs(true);23.return;}
```
