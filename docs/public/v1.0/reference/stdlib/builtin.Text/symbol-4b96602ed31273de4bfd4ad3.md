# builtin.Text — .unknownPlaceholder(policy)
...
```text
Identity: g060-format:.unknownPlaceholder(policy) (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### format:configured-unknown; expected 23

```nebo
start(){"A%z".unknownPlaceholder(1).format().unwrapOr("fallback").console();23.return;}
```

Oracle: {"capabilities": {"console": "RETAINED_DOCUMENT", "filesystem": "EXPLICIT_SCRATCH_EFFECT_ORACLE", "network": "DENIED_BY_SECCOMP"}, "console_text_utf8": "A%z", "filesystem_effects": {}, "independent_builds": 2, "kinds": \[2\], "process_exit": 23, "publications": 1, "runtime_determinism": "BYTE_IDENTICAL", "runtime_sha256": "f8f4431d9dc961220e120576111a30c70d641f1bdead720c6c993981131e7e8e", "text": {"bytes_hex": "41257a"}}

## Rejected examples

### format:configured-type-unknownPlaceholder; expected NEBO-FORMAT-003

```nebo
start(){"%d".unknownPlaceholder(true);23.return;}
```
