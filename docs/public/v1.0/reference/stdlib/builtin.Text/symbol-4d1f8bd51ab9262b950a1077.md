...
```text
Identity: g060-format:.loose() (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### format:configured-loose-missing; expected 23

```nebo
start(){"%s".loose().format().unwrapOr("fallback").console();23.return;}
```

Oracle: {"capabilities": {"console": "RETAINED_DOCUMENT", "filesystem": "EXPLICIT_SCRATCH_EFFECT_ORACLE", "network": "DENIED_BY_SECCOMP"}, "console_text_utf8": "&lt;missing&gt;", "filesystem_effects": {}, "independent_builds": 2, "kinds": \[2\], "process_exit": 23, "publications": 1, "runtime_determinism": "BYTE_IDENTICAL", "runtime_sha256": "098dc8889093ff1159e98a424cd0c8cd6f964c484df650f9ead141442f51df62", "text": {"bytes_hex": "3c6d697373696e673e"}}

## Rejected examples

### format:configured-arity-loose; expected NEBO-FORMAT-002

```nebo
start(){"%d".loose(17);23.return;}
```
