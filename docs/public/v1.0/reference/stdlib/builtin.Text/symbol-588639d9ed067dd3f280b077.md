...
```text
Identity: g060-format:.validateOnly() (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### format:configured-silent; expected 23

```nebo
start(){"%d".strict().format(17);23.return;}
```
...
### format:configured-arity-validateOnly; expected NEBO-FORMAT-002

```nebo
start(){"%d".validateOnly(17);23.return;}
```
