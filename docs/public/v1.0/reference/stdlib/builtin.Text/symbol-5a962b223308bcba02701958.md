...
```text
Identity: g060-format:.strict() (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### format:configured-strict-29; expected 23

```nebo
start(){"%d".strict().format(29).unwrapOr("fallback").console();23.return;}
```
...
### format:configured-arity-strict; expected NEBO-FORMAT-002

```nebo
start(){"%d".strict(17);23.return;}
```
