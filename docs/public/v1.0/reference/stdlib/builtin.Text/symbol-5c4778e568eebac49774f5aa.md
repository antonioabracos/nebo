...
```text
Identity: g060-format:NEBO-FORMAT-003 (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### format:checked-error-type; expected 23

```nebo
start(){"%d".template;template.formatWith(0 ,"Nebo").unwrapOr("fallback").console();23.return;}
```
