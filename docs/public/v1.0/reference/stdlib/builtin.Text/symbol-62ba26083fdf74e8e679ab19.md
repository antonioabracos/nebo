...
```text
Identity: g060-format:%x (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### format:hex-29; expected 23

```nebo
start(){"%x".format(29).console();23.return;}
```

Oracle: {"capabilities": {"console": "RETAINED_DOCUMENT", "filesystem": "EXPLICIT_SCRATCH_EFFECT_ORACLE", "network": "DENIED_BY_SECCOMP"}, "console_text_utf8": "1d", "filesystem_effects": {}, "independent_builds": 2, "kinds": \[2\], "process_exit": 23, "publications": 1, "runtime_determinism": "BYTE_IDENTICAL", "runtime_sha256": "4abc10c253391c870502a4207f9ca8dff2457ddf07b54f6b76353ed42c5253f4", "text": {"bytes_hex": "3164"}}

## Rejected examples

### format:wrong-hex; expected NEBO-FORMAT-003

```nebo
start(){"%x".format("Nebo");23.return;}
```
