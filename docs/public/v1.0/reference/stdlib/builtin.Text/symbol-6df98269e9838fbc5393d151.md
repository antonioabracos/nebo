# builtin.Text — .placeholderMode(mode)
...
```text
Identity: g060-format:.placeholderMode(mode) (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### format:configured-mode-named-error; expected 23

```nebo
start(){"%d".placeholderMode(1).format(29).unwrapOr("fallback").console();23.return;}
```
...
### format:configured-type-placeholderMode; expected NEBO-FORMAT-003

```nebo
start(){"%d".placeholderMode(true);23.return;}
```
