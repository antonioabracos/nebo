...
```text
Identity: g060-format:NEBO-FORMAT-006 (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### format:checked-error-position; expected 23

```nebo
start(){"%0$d".template;template.formatWith(0 ,17).unwrapOr("fallback").console();23.return;}
```
