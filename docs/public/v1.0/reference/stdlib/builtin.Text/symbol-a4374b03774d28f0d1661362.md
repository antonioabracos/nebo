# builtin.Text — INTERPOLATION_ORDER=LEFT_TO_RIGHT
...
```text
Identity: g061-format:INTERPOLATION_ORDER=LEFT_TO_RIGHT (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
```text
INTERPOLATION_ORDER=LEFT_TO_RIGHT
Ordinary token/AST expression children; native pure-only effect policy with resolved callee bodies; static G059 nodes and G062 profiles; 64 segments, 16 nested quotes, 4096 source-content bytes; native named-product fields preserve declared types/offsets and source order (8 declarations/8 fields); native-token tooling preserves literals and exposes expressions; rename uses existing G162 receiver-method SymbolIds; privacy remains a separate obligation Checked format/interpolation profile; 64 segments; finite text/output budgets; pure interpolation
```
...
### interpolation:arithmetic-17-29; expected 23

```nebo
start(){17.first;29.second;"${first+second}|${first-second}".console();23.return;}
```

Oracle: {"capabilities": {"console": "RETAINED_DOCUMENT", "filesystem": "EXPLICIT_SCRATCH_EFFECT_ORACLE", "network": "DENIED_BY_SECCOMP"}, "console_text_utf8": "46|-12", "filesystem_effects": {}, "independent_builds": 2, "kinds": \[2\], "process_exit": 23, "publications": 1, "runtime_determinism": "BYTE_IDENTICAL", "runtime_sha256": "692a827ad6f3fc456232a520e1ba56159246687d797e87f0c8d40f5022f6a426", "text": {"bytes_hex": "34367c2d3132"}}

## Rejected examples

### interpolation:effect-transitive; expected NEBO_INTERPOLATION_EFFECT_FORBIDDEN

```nebo
(Int.x)plus(){x.console();x.return;}(Int.x)twice(){x.plus().return;}start(){"${17.twice()}";23.return;}
```
