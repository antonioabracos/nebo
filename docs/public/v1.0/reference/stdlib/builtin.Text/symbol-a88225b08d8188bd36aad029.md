...
```text
Identity: g060-format:.noCoerce() (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### format:configured-no-coerce-29; expected 23

```nebo
start(){"%s".allowCoerce().noCoerce().format(29).unwrapOr("fallback").console();23.return;}
```
...
### format:configured-arity-noCoerce; expected NEBO-FORMAT-002

```nebo
start(){"%d".noCoerce(17);23.return;}
```
