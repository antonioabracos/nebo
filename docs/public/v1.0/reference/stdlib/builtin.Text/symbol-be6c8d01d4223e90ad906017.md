...
```text
Identity: g060-format:%b (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### format:bool-True; expected 23

```nebo
start(){"%b".format(true).console();23.return;}
```
...
### format:wrong-bool; expected NEBO-FORMAT-003

```nebo
start(){"%b".format(17);23.return;}
```
