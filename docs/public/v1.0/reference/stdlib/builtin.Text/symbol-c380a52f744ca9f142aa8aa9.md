...
```text
Identity: g060-format:.extraArgs(policy) (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### format:configured-extra; expected 23

```nebo
start(){"%d".extraArgs(1).format(17,29).unwrapOr("fallback").console();23.return;}
```
...
### format:configured-type-extraArgs; expected NEBO-FORMAT-003

```nebo
start(){"%d".extraArgs(true);23.return;}
```
