...
```text
Identity: g060-format:%8s (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### format:align-right-4e65626f; expected 23

```nebo
start(){"%8s".format("Nebo").console();23.return;}
```

Oracle: {"capabilities": {"console": "RETAINED_DOCUMENT", "filesystem": "EXPLICIT_SCRATCH_EFFECT_ORACLE", "network": "DENIED_BY_SECCOMP"}, "console_text_utf8": "    Nebo", "filesystem_effects": {}, "independent_builds": 2, "kinds": \[2\], "process_exit": 23, "publications": 1, "runtime_determinism": "BYTE_IDENTICAL", "runtime_sha256": "343070c0e777aa61c3666cc196d413dc1d933db266e5f6627757f465f5e876f8", "text": {"bytes_hex": "202020204e65626f"}}

## Rejected examples

### format:width-limit; expected NEBO-FORMAT-004

```nebo
start(){"%257s".format("Nebo");23.return;}
```
