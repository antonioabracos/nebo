...
```text
Identity: g060-format:NEBO-FORMAT-007 (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### format:checked-error-coercion; expected 23

```nebo
start(){"%s".template;template.formatWith(0 ,17).unwrapOr("fallback").console();23.return;}
```
...
### format:coercion-forbidden; expected NEBO-FORMAT-007

```nebo
start(){"%s".format(17);23.return;}
```

Oracle: {"artifact_publication": "REJECTED", "compiler_exit": 1, "diagnostic": "NEBO-FORMAT-007"}
