...
```text
Identity: g060-format:NEBO-FORMAT-005 (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### format:checked-error-named; expected 23

```nebo
start(){"%{absent:d}".template;template.formatWith(0 ,17).unwrapOr("fallback").console();23.return;}
```
