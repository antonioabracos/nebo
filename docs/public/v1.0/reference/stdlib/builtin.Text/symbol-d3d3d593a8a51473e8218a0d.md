# builtin.Text — FormatString.validate(types)
...
```text
Identity: g059-format:FormatString.validate(types) (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### format:parsed-validate-2564-Int; expected 23

```nebo
start(){FormatString.parseChecked("%d").get().t;t.validate(Tuple.of(Int)).console();23.return;}
```
...
### format:parsed-signature-unknown; expected NEBO-FORMAT-003

```nebo
start(){"%d".strict().validate(Tuple.of(Missing));23.return;}
```
