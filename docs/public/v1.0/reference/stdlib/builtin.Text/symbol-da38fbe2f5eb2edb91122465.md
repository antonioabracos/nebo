...
```text
Identity: g060-format:NEBO-FORMAT-001 (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### format:checked-error-unknown; expected 23

```nebo
start(){"%z".template;template.formatWith(0 ,17).unwrapOr("fallback").console();23.return;}
```
