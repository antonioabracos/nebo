...
```text
Identity: g060-format:.allowCoerce() (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### format:configured-coerce-29; expected 23

```nebo
start(){"%s".allowCoerce().format(29).unwrapOr("fallback").console();23.return;}
```
...
### format:configured-arity-allowCoerce; expected NEBO-FORMAT-002

```nebo
start(){"%d".allowCoerce(17);23.return;}
```
