# builtin.channels — CancellationToken

The CancellationToken receiver/type supports the individually listed operations under the same bounded ownership and target profile. Use the executed construction form below.

```text
Identity: material-type:CancellationToken (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
```text
CancellationToken — material receiver/type profile
```

## Bounded behavior

The CancellationToken receiver/type supports the individually listed operations under the same bounded ownership and target profile. Use the executed construction form below. Int64 FIFO capacity 1..64; unbuffered no-peer; closed/empty/full/wait error codes; independent token state
