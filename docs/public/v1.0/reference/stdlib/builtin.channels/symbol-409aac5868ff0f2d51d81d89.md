# builtin.channels — CancellationToken.cancel

Repeated cancellation preserves cancelled state.

```text
Identity: material:CancellationToken.cancel (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
Repeated cancellation preserves cancelled state. Int64 FIFO capacity 1..64; unbuffered no-peer; closed/empty/full/wait error codes; independent token state
