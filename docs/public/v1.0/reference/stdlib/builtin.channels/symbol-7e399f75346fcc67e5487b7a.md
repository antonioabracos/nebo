# builtin.channels — CancellationToken.new

Creates independent cancellation state.

```text
Identity: material:CancellationToken.new (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
Creates independent cancellation state. Int64 FIFO capacity 1..64; unbuffered no-peer; closed/empty/full/wait error codes; independent token state
