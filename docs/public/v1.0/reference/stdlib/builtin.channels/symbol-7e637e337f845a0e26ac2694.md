# builtin.channels — CancellationToken.throwIfCancelled

Returns the cancellation result; the example observes success zero versus the explicit fallback on cancellation.

```text
Identity: material:CancellationToken.throwIfCancelled (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
```text
CancellationToken.throwIfCancelled() -> Result<Int,Int>
```

## Bounded behavior

Returns the cancellation result; the example observes success zero versus the explicit fallback on cancellation. Int64 FIFO capacity 1..64; unbuffered no-peer; closed/empty/full/wait error codes; independent token state
