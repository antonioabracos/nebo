# builtin.channels — Channel.tryReceive

Dequeues FIFO payload; empty yields 11.

```text
Identity: material:Channel.tryReceive (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
```text
Channel<Int>.tryReceive() -> Result<Int,Int>
```

## Bounded behavior

Dequeues FIFO payload; empty yields 11. Int64 FIFO capacity 1..64; unbuffered no-peer; closed/empty/full/wait error codes; independent token state
