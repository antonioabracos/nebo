# builtin.channels — Channel.send

Bounded wait reports 8 when progress is unavailable; no unbounded blocking promise.

```text
Identity: material:Channel.send (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
```text
Channel<Int>.send(Int) -> Result<Int,Int>
```

## Bounded behavior

Bounded wait reports 8 when progress is unavailable; no unbounded blocking promise. Int64 FIFO capacity 1..64; unbuffered no-peer; closed/empty/full/wait error codes; independent token state
