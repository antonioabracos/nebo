# builtin.channels — Channel.receive

Bounded wait reports 8 when progress is unavailable.

```text
Identity: material:Channel.receive (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
```text
Channel<Int>.receive() -> Result<Int,Int>
```

## Bounded behavior

Bounded wait reports 8 when progress is unavailable. Int64 FIFO capacity 1..64; unbuffered no-peer; closed/empty/full/wait error codes; independent token state
