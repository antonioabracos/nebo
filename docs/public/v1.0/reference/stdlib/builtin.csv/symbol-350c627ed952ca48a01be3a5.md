# builtin.csv — Csv.write

Writes independently checkable canonical CSV bytes and returns row count; use a private filesystem directory.

```text
Identity: material:Csv.write (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
```text
Csv.write(Table, Path, Schema) -> Int
```

## Bounded behavior

Writes independently checkable canonical CSV bytes and returns row count; use a private filesystem directory. Typed nullable Int schema; 32 rows; exact independent output bytes; malformed width/cell/overflow failure before publication
