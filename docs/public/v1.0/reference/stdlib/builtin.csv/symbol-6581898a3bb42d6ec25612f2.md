# builtin.csv — Csv.read

Reads nullable Int cells according to an explicit schema, at most 32 rows; wrong width, malformed cells and overflow fail.

```text
Identity: material:Csv.read (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
```text
Csv.read(Path, Schema) -> Table
```

## Bounded behavior

Reads nullable Int cells according to an explicit schema, at most 32 rows; wrong width, malformed cells and overflow fail. Typed nullable Int schema; 32 rows; exact independent output bytes; malformed width/cell/overflow failure before publication
