# builtin.json — Json.toJson

Produces canonical sorted-key UTF-8 bytes. The keyword option cannot be replaced by a Bool or shadowed binding.

```text
Identity: material:Json.toJson (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
```text
Json.toJson(canonicalKeys) -> Text
```

## Bounded behavior

Produces canonical sorted-key UTF-8 bytes. The keyword option cannot be replaced by a Bool or shadowed binding. Canonical key order; duplicate/trailing/malformed/depth rejection; 4096-byte input; private atomic sink
