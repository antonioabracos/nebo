# builtin.json — Json.parse

Parses bounded JSON up to 4096 bytes. Duplicate keys, malformed text, trailing data and excessive depth fail before file publication.

```text
Identity: material:Json.parse (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
Parses bounded JSON up to 4096 bytes. Duplicate keys, malformed text, trailing data and excessive depth fail before file publication. Canonical key order; duplicate/trailing/malformed/depth rejection; 4096-byte input; private atomic sink
