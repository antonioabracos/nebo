# builtin.sync — Mutex.new

Creates a unique typed lock owner.

```text
Identity: material:Mutex.new (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
```text
Mutex<Int>.new(Int) -> Mutex<Int>
```

## Bounded behavior

Creates a unique typed lock owner. Typed Int payloads; guard release across scope/return/break/continue; compareExchange; unique ownership and no escape
