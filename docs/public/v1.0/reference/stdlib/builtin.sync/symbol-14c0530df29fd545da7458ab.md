# builtin.sync — AtomicInt.load

Loads the integer; examples use material order code 4.

```text
Identity: material:AtomicInt.load (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
```text
AtomicInt.load(Int order) -> Int
```

## Bounded behavior

Loads the integer; examples use material order code 4. Typed Int payloads; guard release across scope/return/break/continue; compareExchange; unique ownership and no escape
