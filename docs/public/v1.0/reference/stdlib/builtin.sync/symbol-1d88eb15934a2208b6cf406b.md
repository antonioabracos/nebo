# builtin.sync — Mutex.tryLock

Returns an acquisition-bearing guard; isAcquired is false while already held.

```text
Identity: material:Mutex.tryLock (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
```text
Mutex<Int>.tryLock() -> guard
```

## Bounded behavior

Returns an acquisition-bearing guard; isAcquired is false while already held. Typed Int payloads; guard release across scope/return/break/continue; compareExchange; unique ownership and no escape
