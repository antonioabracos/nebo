# builtin.sync — AtomicInt.new

Creates a unique integer atomic owner; copying it is rejected.

```text
Identity: material:AtomicInt.new (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
```text
AtomicInt.new(Int) -> AtomicInt
```

## Bounded behavior

Creates a unique integer atomic owner; copying it is rejected. Typed Int payloads; guard release across scope/return/break/continue; compareExchange; unique ownership and no escape
