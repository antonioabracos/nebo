# builtin.sync — RwLock.read

Allows multiple active read guards; writes require their release.

```text
Identity: material:RwLock.read (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
```text
RwLock<Int>.read() -> read guard
```

## Bounded behavior

Allows multiple active read guards; writes require their release. Typed Int payloads; guard release across scope/return/break/continue; compareExchange; unique ownership and no escape
