# builtin.sync — AtomicInt.store

Stores a typed integer at the explicit material memory order.

```text
Identity: material:AtomicInt.store (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
```text
AtomicInt.store(Int value, Int order)
```

## Bounded behavior

Stores a typed integer at the explicit material memory order. Typed Int payloads; guard release across scope/return/break/continue; compareExchange; unique ownership and no escape
