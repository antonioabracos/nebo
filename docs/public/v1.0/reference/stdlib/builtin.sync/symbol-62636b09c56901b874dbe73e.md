# builtin.sync — guard.get

Reads the held payload; no borrowed guard escape is admitted.

```text
Identity: material:guard.get (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
Reads the held payload; no borrowed guard escape is admitted. Typed Int payloads; guard release across scope/return/break/continue; compareExchange; unique ownership and no escape
...
### sync:sync-lifetime-untaken; expected 23

```nebo
start(){Mutex<Int>.new(17).m;if(false){m.lock().a;}m.lock().g;g.get().console();23.return;}
```
