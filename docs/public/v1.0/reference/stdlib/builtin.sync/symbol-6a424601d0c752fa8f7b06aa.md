# builtin.sync — AtomicInt.compareExchange

Atomically compares then conditionally stores. The result preserves the old value and success bit.

```text
Identity: material:AtomicInt.compareExchange (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
```text
AtomicInt.compareExchange(Int expected, Int desired, Int order) -> compare result
```

## Bounded behavior

Atomically compares then conditionally stores. The result preserves the old value and success bit. Typed Int payloads; guard release across scope/return/break/continue; compareExchange; unique ownership and no escape
