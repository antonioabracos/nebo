# builtin.sync — compareResult.observed

Returns the observed prior payload even when comparison fails.

```text
Identity: material:compareResult.observed (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
```text
compare result.observed() -> Int
```

## Bounded behavior

Returns the observed prior payload even when comparison fails. Typed Int payloads; guard release across scope/return/break/continue; compareExchange; unique ownership and no escape
