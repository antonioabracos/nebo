# builtin.sync — compareResult.swapped

Distinguishes success and failed comparison.

```text
Identity: material:compareResult.swapped (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
```text
compare result.swapped() -> Bool
```

## Bounded behavior

Distinguishes success and failed comparison. Typed Int payloads; guard release across scope/return/break/continue; compareExchange; unique ownership and no escape
