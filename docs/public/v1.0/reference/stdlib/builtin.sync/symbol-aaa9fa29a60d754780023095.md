# builtin.sync — guard.isAcquired

Observes acquisition status without treating an unsuccessful tryLock as a held lock.

```text
Identity: material:guard.isAcquired (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
```text
guard.isAcquired() -> Bool
```

## Bounded behavior

Observes acquisition status without treating an unsuccessful tryLock as a held lock. Typed Int payloads; guard release across scope/return/break/continue; compareExchange; unique ownership and no escape
