# builtin.sync — guard.release

Ends the guard lifetime; implicit lexical cleanup follows the same owner.

```text
Identity: material:guard.release (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
Ends the guard lifetime; implicit lexical cleanup follows the same owner. Typed Int payloads; guard release across scope/return/break/continue; compareExchange; unique ownership and no escape
...
### sync:mutex-17; expected 23

```nebo
start(){Mutex<Int>.new(17).m;m.lock().g;g.get().console();m.tryLock().t;t.isAcquired().console();g.set(53);g.release();m.lock().h;h.get().console();h.release();23.return;}
```

Oracle: {"capabilities": {"console": "RETAINED_DOCUMENT", "filesystem": "EXPLICIT_SCRATCH_EFFECT_ORACLE", "network": "DENIED_BY_SECCOMP"}, "console_text_utf8": "17false53", "filesystem_effects": {}, "independent_builds": 2, "kinds": \[4, 5, 4\], "process_exit": 23, "runtime_determinism": "BYTE_IDENTICAL", "runtime_sha256": "c77de09d8fcba81ddb24e8a4a11349f5da9bf5027a1b791c0bfd9cf1369f5220", "text": {"bytes_hex": "313766616c73653533"}}
