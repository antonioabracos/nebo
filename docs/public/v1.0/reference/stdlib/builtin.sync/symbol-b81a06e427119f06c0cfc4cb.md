# builtin.sync — guard.set

Writes the guarded payload; the next acquisition observes the value.

```text
Identity: material:guard.set (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
```text
mutable guard.set(Int)
```

## Bounded behavior

Writes the guarded payload; the next acquisition observes the value. Typed Int payloads; guard release across scope/return/break/continue; compareExchange; unique ownership and no escape
...
### sync:sync-lifetime-scope; expected 23

```nebo
start(){Mutex<Int>.new(17).m;if(true){m.lock().g;g.set(29);}m.lock().h;h.get().console();23.return;}
```
