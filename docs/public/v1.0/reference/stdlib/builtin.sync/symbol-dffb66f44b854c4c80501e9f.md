# builtin.sync — AtomicInt.fetchAdd

Returns the prior value; the next load observes the increment.

```text
Identity: material:AtomicInt.fetchAdd (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
```text
AtomicInt.fetchAdd(Int delta, Int order) -> Int
```

## Bounded behavior

Returns the prior value; the next load observes the increment. Typed Int payloads; guard release across scope/return/break/continue; compareExchange; unique ownership and no escape
