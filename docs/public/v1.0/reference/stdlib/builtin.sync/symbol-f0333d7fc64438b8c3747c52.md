# builtin.sync — RwLock.new

Creates a unique typed reader/writer lock.

```text
Identity: material:RwLock.new (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
```text
RwLock<Int>.new(Int) -> RwLock<Int>
```

## Bounded behavior

Creates a unique typed reader/writer lock. Typed Int payloads; guard release across scope/return/break/continue; compareExchange; unique ownership and no escape
