# builtin.system — Process

The Process receiver/type supports the individually listed operations under the same bounded ownership and target profile. Use the executed construction form below.

```text
Identity: material-type:Process (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
```text
Process — material receiver/type profile
```

## Bounded behavior

The Process receiver/type supports the individually listed operations under the same bounded ownership and target profile. Use the executed construction form below. Current ID; synthetic NEBO_TEST_* values and bounded arguments only; no arbitrary process spawn
