# builtin.tasks — TaskGroup.spawn

Admits one noncapturing callback and owns its future.

```text
Identity: material:TaskGroup.spawn (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
```text
TaskGroup.spawn((Int)->Int) -> Future<Int>
```

## Bounded behavior

Admits one noncapturing callback and owns its future. Noncapturing Int callback; cooperative deterministic admission; single-consumer Future; callback once; cancellation
