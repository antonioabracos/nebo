# builtin.tasks — TaskGroup.new

Creates an independent admission group. Callback indices follow admission order.

```text
Identity: material:TaskGroup.new (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
Creates an independent admission group. Callback indices follow admission order. Noncapturing Int callback; cooperative deterministic admission; single-consumer Future; callback once; cancellation
