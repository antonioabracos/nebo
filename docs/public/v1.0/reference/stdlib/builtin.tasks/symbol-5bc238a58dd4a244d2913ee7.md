# builtin.tasks — Future.select

Selects the first supplied ready future in the bounded deterministic profile; swapping arguments changes the selected value.

```text
Identity: material:Future.select (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
```text
Future<Int>.select(Future<Int>) -> Future<Int>
```

## Bounded behavior

Selects the first supplied ready future in the bounded deterministic profile; swapping arguments changes the selected value. Noncapturing Int callback; cooperative deterministic admission; single-consumer Future; callback once; cancellation
...
### tasks:task-select-False; expected 23

```nebo
(Int.index)first(){17.return;}(Int.index)second(){71.return;}start(){Task.spawn(first).left;Task.spawn(second).right;left.select(right).winner;winner.await().get().console();23.return;}
```
