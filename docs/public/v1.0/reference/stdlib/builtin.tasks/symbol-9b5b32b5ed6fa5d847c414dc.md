# builtin.tasks — Future.map

Applies the admitted noncapturing callback once, preserving cancellation and source effect order.

```text
Identity: material:Future.map (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
```text
Future<Int>.map((Int)->Int) -> Future<Int>
```

## Bounded behavior

Applies the admitted noncapturing callback once, preserving cancellation and source effect order. Noncapturing Int callback; cooperative deterministic admission; single-consumer Future; callback once; cancellation
