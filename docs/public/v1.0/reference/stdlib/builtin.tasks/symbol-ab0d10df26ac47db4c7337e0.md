# builtin.tasks — Task.spawn

Runs an admitted noncapturing Int callback once; standalone task index is zero. This cooperative profile does not promise parallel OS threads.

```text
Identity: material:Task.spawn (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
```text
Task.spawn((Int)->Int) -> Future<Int>
```

## Bounded behavior

Runs an admitted noncapturing Int callback once; standalone task index is zero. This cooperative profile does not promise parallel OS threads. Noncapturing Int callback; cooperative deterministic admission; single-consumer Future; callback once; cancellation
