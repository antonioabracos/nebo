# builtin.tasks — Future.await

Consumes the active future once; repeat await yields error 12.

```text
Identity: material:Future.await (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
```text
Future<Int>.await() -> Result<Int,Int>
```

## Bounded behavior

Consumes the active future once; repeat await yields error 12. Noncapturing Int callback; cooperative deterministic admission; single-consumer Future; callback once; cancellation
