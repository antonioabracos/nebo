# builtin.tasks — TaskGroup.joinAll

Returns the joined task count; repeat consumption is rejected with the observed error.

```text
Identity: material:TaskGroup.joinAll (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
```text
TaskGroup.joinAll() -> Result<Int,Int>
```

## Bounded behavior

Returns the joined task count; repeat consumption is rejected with the observed error. Noncapturing Int callback; cooperative deterministic admission; single-consumer Future; callback once; cancellation
