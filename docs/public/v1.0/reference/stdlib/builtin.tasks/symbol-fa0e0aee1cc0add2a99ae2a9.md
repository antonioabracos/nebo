# builtin.tasks — TaskGroup.cancel

Cancels subsequent admission. The example verifies that cancelled callbacks produce no Console effects.

```text
Identity: material:TaskGroup.cancel (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
Cancels subsequent admission. The example verifies that cancelled callbacks produce no Console effects. Noncapturing Int callback; cooperative deterministic admission; single-consumer Future; callback once; cancellation
...
### tasks:task-cancel-admission; expected 23

```nebo
(Int.index)work(){index.console();(index+17).return;}(Int.value)scale(){value.console();(value*3).return;}start(){TaskGroup.new().g;g.cancel();g.spawn(work).a;a.map(scale).b;b.await().getErr().console();g.joinAll().get().console();23.return;}
```

Oracle: {"capabilities": {"console": "RETAINED_DOCUMENT", "filesystem": "EXPLICIT_SCRATCH_EFFECT_ORACLE", "network": "DENIED_BY_SECCOMP"}, "console_text_utf8": "70", "filesystem_effects": {}, "independent_builds": 2, "kinds": \[4, 4\], "process_exit": 23, "runtime_determinism": "BYTE_IDENTICAL", "runtime_sha256": "aa074dd6647b0a3c63a506c432c630de388026e0f617bc107f5e574c08225138", "text": {"bytes_hex": "3730"}}
