# builtin.time — Duration.asNanos

Observes nanoseconds: milliseconds multiplied by 1000000 within the checked integer domain.

```text
Identity: material:Duration.asNanos (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
```text
Duration.asNanos() -> Int
```

## Bounded behavior

Observes nanoseconds: milliseconds multiplied by 1000000 within the checked integer domain. Integer nanoseconds; bounded monotonic/sleep observations; no exact wall-clock value promise
