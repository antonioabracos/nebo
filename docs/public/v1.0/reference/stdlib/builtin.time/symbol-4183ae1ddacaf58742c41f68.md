# builtin.time — Duration.fromMillis

Constructs integer milliseconds; the material representation uses nanoseconds.

```text
Identity: material:Duration.fromMillis (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
```text
Duration.fromMillis(Int) -> Duration
```

## Bounded behavior

Constructs integer milliseconds; the material representation uses nanoseconds. Integer nanoseconds; bounded monotonic/sleep observations; no exact wall-clock value promise
...
### system:duration-0; expected 23

```nebo
start(){Duration.fromMillis(0).d;d.asNanos().console();23.return;}
```
