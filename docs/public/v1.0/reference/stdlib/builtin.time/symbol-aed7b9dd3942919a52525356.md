# builtin.time — Instant

The Instant receiver/type supports the individually listed operations under the same bounded ownership and target profile. Use the executed construction form below.

```text
Identity: material-type:Instant (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
```text
Instant — material receiver/type profile
```

## Bounded behavior

The Instant receiver/type supports the individually listed operations under the same bounded ownership and target profile. Use the executed construction form below. Integer nanoseconds; bounded monotonic/sleep observations; no exact wall-clock value promise
