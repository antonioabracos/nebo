# builtin.time — Instant.now

Reads a monotonic instant; no wall-clock timestamp contract.

```text
Identity: material:Instant.now (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
Reads a monotonic instant; no wall-clock timestamp contract. Integer nanoseconds; bounded monotonic/sleep observations; no exact wall-clock value promise
