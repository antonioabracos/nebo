...
```text
Identity: g010-public:Dataset.scan.sink (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### data:dataset-scan-sink-consumed; expected 0

```nebo
(Row.self)observe(){self.get("age").unwrapOr(89).console();true.return;}(Table.self)consume(){self.rowCount().console();self.filter(observe);}start(){Table.fromColumns([Tuple.of("age",Column<Int>.from([17,29]))]).a;Table.fromColumns([Tuple.of("age",Column<Int>.from([71]))]).b;Dataset.fromTables([a,b]).d;d.scan().s;s.sink(consume);s.sink(consume).return;}
```

Oracle: {"capabilities": {"console": "RETAINED_DOCUMENT", "filesystem": "EXPLICIT_SCRATCH_EFFECT_ORACLE", "network": "DENIED_BY_SECCOMP"}, "console_text_utf8": "21729171", "filesystem_effects": {}, "independent_builds": 2, "kinds": \[4, 4, 4, 4, 4\], "process_exit": 0, "runtime_determinism": "BYTE_IDENTICAL", "runtime_sha256": "c36f24df76876f28a72f0d205922471a6f83ad03c5d106be020da5790b1ea501", "text": {"bytes_hex": "3231373239313731"}}

## Rejected examples

### data:dataset-sink-return-type; expected NEBO_TYPE_MISMATCH

```nebo
(Table.self)consume(){17.return;}start(){Table.fromColumns([Tuple.of("age",Column<Int>.from([17]))]).t;Dataset.fromTables([t]).d;d.scan().s;s.sink(consume);23.return;}
```
