...
```text
Identity: g007-public:List.filter (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### sequence:filter-less; expected 23

```nebo
(Int.self)choose(){(self<20).return;}start(){List<Int>.from([29,-17,71]).a;a.filter(choose).b;b.length().console();b.at(0).console();23.return;}
```
...
### sequence:list-filter-type; expected NEBO_TYPE_MISMATCH

```nebo
start(){List<Int>.new().a;a.filter(true);23.return;}
```
