...
Up to 32 nullable Int cells; native missing bitmap, checked aggregates and Int/UInt cast storage; UInt is not an implicit process status; native registry identity and runtime profile are explicit; serialized interface metadata is verified by the separate metadata suite Typed nullable integer rows/columns/tables/datasets/streams; finite schema, rows and stages; lazy callbacks once

```text
Identity: g010-public:Column.min (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
Exact typed owner contract; no copy/borrow/clone capability inferred from spelling Up to 32 nullable Int cells; native missing bitmap, checked aggregates and Int/UInt cast storage; UInt is not an implicit process status; native registry identity and runtime profile are explicit; serialized interface metadata is verified by the separate metadata suite Typed nullable integer rows/columns/tables/datasets/streams; finite schema, rows and stages; lazy callbacks once
...
### data:column-min-changed; expected 23

```nebo
start(){Column<Int>.from([-17,83,7]).c;c.min().console();23.return;}
```

Oracle: {"capabilities": {"console": "RETAINED_DOCUMENT", "filesystem": "EXPLICIT_SCRATCH_EFFECT_ORACLE", "network": "DENIED_BY_SECCOMP"}, "console_text_utf8": "-17", "filesystem_effects": {}, "independent_builds": 2, "kinds": \[4\], "process_exit": 23, "runtime_determinism": "BYTE_IDENTICAL", "runtime_sha256": "4f23f4ead3078ce18b7b420f04b40e84ea911783f0c40e4b4480335bf4042346", "text": {"bytes_hex": "2d3137"}}

## Rejected examples

### data:column-min-arity; expected NEBO_TYPE_MISMATCH

```nebo
start(){Column<Int>.from([17]).c;c.min(17);23.return;}
```
