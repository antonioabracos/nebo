...
```text
Identity: g010-public:Stream.batch.next (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### data:batch-option-consumed; expected 23

```nebo
start(){Stream<Int>.from([17,29,71]).s;s.batch(2).b;b.next().v;v.isSome().console();v.expect("v").a;v.isNone().console();a.at(0).console();23.return;}
```

Oracle: {"capabilities": {"console": "RETAINED_DOCUMENT", "filesystem": "EXPLICIT_SCRATCH_EFFECT_ORACLE", "network": "DENIED_BY_SECCOMP"}, "console_text_utf8": "truetrue17", "filesystem_effects": {}, "independent_builds": 2, "kinds": \[5, 5, 4\], "process_exit": 23, "runtime_determinism": "BYTE_IDENTICAL", "runtime_sha256": "8a88133d6e911f246be39e6f646bfd54f93853b709e8071f2b33e4088308cd38", "text": {"bytes_hex": "74727565747275653137"}}

## Rejected examples

### data:batch-option-copy; expected NEBO_COPY_UNIQUE

```nebo
start(){Stream<Int>.from([17]).s;s.batch(1).b;b.next().v;v.copy;23.return;}
```
