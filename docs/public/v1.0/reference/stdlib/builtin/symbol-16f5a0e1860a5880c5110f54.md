# builtin — Text.isBlank

isBlank(); ASCII SP/TAB/LF/CR; O(n); ; borrowed immutable receiver; public UTF-8 descriptor bridge and distinct bounded function storage; native intrinsic identity is receiver-qualified; no serialized SymbolId or DocRecord is declared by this owner; interface metadata is verified by the separate metadata suite UTF-8 byte/codepoint/grapheme units are distinct; per-member finite profile

```text
Identity: claim:b4e6c473be922721f8308fa8 (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
Exact typed owner contract; no copy/borrow/clone capability inferred from spelling isBlank(); ASCII SP/TAB/LF/CR; O(n); ; borrowed immutable receiver; public UTF-8 descriptor bridge and distinct bounded function storage; native intrinsic identity is receiver-qualified; no serialized SymbolId or DocRecord is declared by this owner; interface metadata is verified by the separate metadata suite UTF-8 byte/codepoint/grapheme units are distinct; per-member finite profile
...
Edition 1; x86_64-systemv-elf-linux; BOUNDED_PUBLIC_EXECUTED_WITHIN_LIMITS. isBlank(); ASCII SP/TAB/LF/CR; O(n); ; borrowed immutable receiver; public UTF-8 descriptor bridge and distinct bounded function storage; native intrinsic identity is receiver-qualified; no serialized SymbolId or DocRecord is declared by this owner; interface metadata is verified by the separate metadata suite UTF-8 byte/codepoint/grapheme units are distinct; per-member finite profile Use the declared operand and receiver domain; source rejection publishes no executable.
...
### text:isBlank-empty; expected 23

```nebo
start(){"".isBlank().console();23.return;}
```
...
### text:arity-isBlank; expected NEBO-TEXT-CHAR-UNICODE-E-BYTES-TEXTUAL-ARGUMENTS-NOT-ALLOWED

```nebo
start(){"Nebo".isBlank(1);23.return;}
```
