...
```text
Identity: g010-public:Table.invalidRows (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### data:quality-default; expected 0

```nebo
start(){List<Int>.new().empty;Table.fromColumns([Tuple.of("age",Column<Int>.from([empty.get(0),empty.get(0)]))]).t;Schema.new([Tuple.of("age",Int,false)]).strict;Schema.new([Tuple.of("age",Int,true)]).relaxed;t.invalidRows().bad;bad.rowCount().return;}
```
...
### data:quality-invalid-rows-arity; expected NEBO_TYPE_MISMATCH

```nebo
start(){Table.fromColumns([Tuple.of("age",Column<Int>.from([17]))]).t;t.invalidRows(17);23.return;}
```
