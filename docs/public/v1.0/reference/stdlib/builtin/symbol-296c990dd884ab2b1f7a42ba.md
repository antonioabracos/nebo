...
```text
Identity: g007-public:List.swapRemove (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### sequence:list-swap-remove; expected 23

```nebo
start(){List<Int>.from([17,29,71]).items;items.swapRemove(0).console();items.at(0).console();23.return;}
```
...
### sequence:list-swapRemove-type; expected NEBO_TYPE_MISMATCH

```nebo
start(){List<Int>.new().a;a.swapRemove(true);23.return;}
```
