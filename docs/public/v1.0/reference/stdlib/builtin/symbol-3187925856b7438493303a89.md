...
```text
Identity: g010-public:Table.filter (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### data:table-callback-native-local; expected 23

```nebo
(Row.self)observe(){List<Int>.from([17,29]).a;a.iterator().it;it.next().expect("v").console();true.return;}start(){Table.fromColumns([Tuple.of("age",Column<Int>.from([71,17,29])),Tuple.of("score",Column<Int>.from([83,7,53]))]).t;t.filter(observe);23.return;}
```

Oracle: {"capabilities": {"console": "RETAINED_DOCUMENT", "filesystem": "EXPLICIT_SCRATCH_EFFECT_ORACLE", "network": "DENIED_BY_SECCOMP"}, "console_text_utf8": "171717", "filesystem_effects": {}, "independent_builds": 2, "kinds": \[4, 4, 4\], "process_exit": 23, "runtime_determinism": "BYTE_IDENTICAL", "runtime_sha256": "2dd516069f0b0383a1a92f00ea8d2b3dbf9cf2bd48e96bd6ad191052beabcc50", "text": {"bytes_hex": "313731373137"}}

## Rejected examples

### data:table-filter-result-type; expected NEBO_TYPE_MISMATCH

```nebo
(Row.self)keep(){17.return;}start(){Table.fromColumns([Tuple.of("age",Column<Int>.from([17]))]).t;t.filter(keep);23.return;}
```
