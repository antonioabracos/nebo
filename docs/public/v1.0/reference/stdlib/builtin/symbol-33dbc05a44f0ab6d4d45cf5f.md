...
```text
Identity: g007-public:List.capacity (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### sequence:list-capacity; expected 5

```nebo
start(){List<Int>.withCapacity(5).items;items.capacity().return;}
```
...
### sequence:list-capacity-arity; expected NEBO_TYPE_MISMATCH

```nebo
start(){List<Int>.new().a;a.capacity(17);23.return;}
```
