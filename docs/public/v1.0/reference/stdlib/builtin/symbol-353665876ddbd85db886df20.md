...
```text
Identity: g010-public:Dataset.collect.at (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### data:collect-row-independent; expected 23

```nebo
start(){Table.fromColumns([Tuple.of("age",Column<Int>.from([17,29]))]).a;Table.fromColumns([Tuple.of("age",Column<Int>.from([71]))]).b;Dataset.fromTables([a,b]).d;d.collect().rows;rows.at(0).first;rows.at(2).last;last.project(["age"]).p;first.get("age").expect("v").console();p.get("age").expect("v").console();23.return;}
```
...
### data:collect-at-type; expected NEBO_TYPE_MISMATCH

```nebo
start(){Table.fromColumns([Tuple.of("age",Column<Int>.from([17]))]).t;Dataset.fromTables([t]).d;d.collect().rows;rows.at(true);23.return;}
```
