# builtin — collection.iteratorMutable
...
```text
Identity: g007-public:collection.iteratorMutable (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### sequence:iterator-mutable; expected 17

```nebo
start(){List<Int>.from([17]).a;a.iteratorMutable().i;i.next().expect("v").return;}
```
...
### sequence:iterator-unique-read; expected NEBO_BORROW_CONFLICT

```nebo
start(){List<Int>.new().a;a.iteratorMutable().i;a.length().return;}
```
