...
```text
Identity: g010-public:Stream.sink (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### data:stream-sink-empty; expected 0

```nebo
(Int.self)emit(){self.console();}start(){Stream<Int>.from([]).s;s.sink(emit).return;}
```
...
### data:stream-sink-return-type; expected NEBO_TYPE_MISMATCH

```nebo
(Int.self)step(){17.return;}start(){Stream<Int>.from([17]).s;s.sink(step);23.return;}
```
