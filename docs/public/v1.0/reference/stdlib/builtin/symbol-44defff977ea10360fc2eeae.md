...
```text
Identity: g010-public:Column.sum (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### data:column-sum-empty; expected 23

```nebo
start(){Column<Int>.from([]).c;c.sum().console();23.return;}
```
...
### data:column-sum-arity; expected NEBO_TYPE_MISMATCH

```nebo
start(){Column<Int>.from([17]).c;c.sum(17);23.return;}
```
