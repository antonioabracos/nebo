...
Spelling alias for g009-public:Graph.dijkstra; no additional API counted; preserve qualified profile limits; native intrinsic identity is receiver-qualified; no serialized SymbolId or DocRecord is declared by this owner; interface metadata is verified by the separate metadata suite Int64 payloads; 32 nodes/256 edges; authenticated handle/owner pair; no stale or foreign handle

```text
Identity: claim:dca093f988973a0a07cdd72b (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
Exact typed owner contract; no copy/borrow/clone capability inferred from spelling Spelling alias for g009-public:Graph.dijkstra; no additional API counted; preserve qualified profile limits; native intrinsic identity is receiver-qualified; no serialized SymbolId or DocRecord is declared by this owner; interface metadata is verified by the separate metadata suite Int64 payloads; 32 nodes/256 edges; authenticated handle/owner pair; no stale or foreign handle
...
Edition 1; x86_64-systemv-elf-linux; BOUNDED_PUBLIC_EXECUTED_WITHIN_LIMITS. Spelling alias for g009-public:Graph.dijkstra; no additional API counted; preserve qualified profile limits; native intrinsic identity is receiver-qualified; no serialized SymbolId or DocRecord is declared by this owner; interface metadata is verified by the separate metadata suite Int64 payloads; 32 nodes/256 edges; authenticated handle/owner pair; no stale or foreign handle Use the declared operand and receiver domain; source rejection publishes no executable.
...
### relation:graph-dijkstra-19-2; expected 23

```nebo
start(){Graph<Int,Int>.directed().g;g.addNode(17).a;g.addNode(29).b;g.addNode(71).c;g.addNode(83).d;g.addEdge(a,b,19);g.addEdge(a,c,2);g.addEdge(b,d,7);g.addEdge(c,d,3);g.dijkstra(a,d).v;v.next().expect("n").value().console();v.next().expect("n").value().console();v.next().expect("n").value().console();23.return;}
```

Oracle: {"capabilities": {"console": "RETAINED_DOCUMENT", "filesystem": "EXPLICIT_SCRATCH_EFFECT_ORACLE", "network": "DENIED_BY_SECCOMP"}, "console_text_utf8": "177183", "filesystem_effects": {}, "independent_builds": 2, "kinds": \[4, 4, 4\], "process_exit": 23, "runtime_determinism": "BYTE_IDENTICAL", "runtime_sha256": "f4b419d1f90c2a1f6611c4ddd8a96a07b6df2c643bae9e53acc0ca06fdda6dc8", "text": {"bytes_hex": "313737313833"}}

## Rejected examples

### relation:graph-dijkstra-nodes-type; expected NEBO_TYPE_MISMATCH

```nebo
start(){Graph<Int,Int>.directed().g;g.dijkstra(17,29);23.return;}
```
