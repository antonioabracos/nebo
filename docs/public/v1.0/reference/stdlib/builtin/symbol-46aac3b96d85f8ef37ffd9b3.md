...
```text
Identity: g010-public:Row.project (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### data:row-project-composition; expected 17

```nebo
start(){Schema.new([Tuple.of("age",Int,false),Tuple.of("score",Int,true)]).s;Row.from(s,[17,71]).r;r.project(["score","age"]).p;p.project(["age"]).q;q.get("age").expect("v").return;}
```
...
### data:row-project-argument-type; expected NEBO_TYPE_MISMATCH

```nebo
start(){Schema.new([Tuple.of("age",Int,false)]).s;Row.from(s,[17]).r;r.project(17);23.return;}
```
