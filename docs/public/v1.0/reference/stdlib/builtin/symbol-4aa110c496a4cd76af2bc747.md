...
```text
Identity: g007-public:iterator.enumerate (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### sequence:iterator-enumerate-empty; expected 23

```nebo
start(){List<Int>.new().a;a.iterator().i;i.enumerate();i.next().isNone().console();23.return;}
```
...
### sequence:iterator-enumerate-conditional; expected NEBO_TYPE_MISMATCH

```nebo
start(){List<Int>.new().a;a.iterator().i;if(true){i.enumerate();}23.return;}
```
