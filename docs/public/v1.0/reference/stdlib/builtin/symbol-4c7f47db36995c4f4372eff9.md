...
```text
Identity: g009-public:Node<Int> value (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### relation:node-prefix-changed; expected 71

```nebo
start(){Node<Int> 71.node;node.value().return;}
```
