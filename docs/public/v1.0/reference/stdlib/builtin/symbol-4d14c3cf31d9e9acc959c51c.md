# builtin — iterator.collect&lt;List&lt;Int&gt;&gt;
...
```text
Identity: g007-public:iterator.collect<List<Int>> (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### sequence:iterator-collect-empty; expected 0

```nebo
start(){List<Int>.new().a;a.iterator().i;i.collect<List<Int>>().b;b.length().return;}
```
...
### sequence:iterator-collect-type; expected NEBO_TYPE_MISMATCH

```nebo
start(){List<Int>.new().a;a.iterator().i;i.collect<List<Text>>();23.return;}
```
