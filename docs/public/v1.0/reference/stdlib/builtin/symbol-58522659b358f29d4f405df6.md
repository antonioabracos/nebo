# builtin — Text.equalsAsciiIgnoreCase

equalsAsciiIgnoreCase(Text); ASCII only; O(n); ; borrowed immutable receiver and argument; public UTF-8 descriptor bridge and distinct bounded function storage; native intrinsic identity is receiver-qualified; no serialized SymbolId or DocRecord is declared by this owner; interface metadata is verified by the separate metadata suite UTF-8 byte/codepoint/grapheme units are distinct; per-member finite profile

```text
Identity: claim:d868cfc43c050cbac67070fb (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
```text
Text.equalsAsciiIgnoreCase
equalsAsciiIgnoreCase(Text); ASCII only; O(n); ; borrowed immutable receiver and argument; public UTF-8 descriptor bridge and distinct bounded function storage; native intrinsic identity is receiver-qualified; no serialized SymbolId or DocRecord is declared by this owner; interface metadata is verified by the separate metadata suite UTF-8 byte/codepoint/grapheme units are distinct; per-member finite profile
```

## Ownership and complexity

Exact typed owner contract; no copy/borrow/clone capability inferred from spelling equalsAsciiIgnoreCase(Text); ASCII only; O(n); ; borrowed immutable receiver and argument; public UTF-8 descriptor bridge and distinct bounded function storage; native intrinsic identity is receiver-qualified; no serialized SymbolId or DocRecord is declared by this owner; interface metadata is verified by the separate metadata suite UTF-8 byte/codepoint/grapheme units are distinct; per-member finite profile
...
Edition 1; x86_64-systemv-elf-linux; BOUNDED_PUBLIC_EXECUTED_WITHIN_LIMITS. equalsAsciiIgnoreCase(Text); ASCII only; O(n); ; borrowed immutable receiver and argument; public UTF-8 descriptor bridge and distinct bounded function storage; native intrinsic identity is receiver-qualified; no serialized SymbolId or DocRecord is declared by this owner; interface metadata is verified by the separate metadata suite UTF-8 byte/codepoint/grapheme units are distinct; per-member finite profile Use the declared operand and receiver domain; source rejection publishes no executable.
...
### text:equals-ascii-1; expected 23

```nebo
start(){"É".equalsAsciiIgnoreCase("é").console();23.return;}
```
...
### text:type-equalsAsciiIgnoreCase; expected NEBO-TEXT-QUERY-ARGUMENT-MUST-BE-TEXT

```nebo
start(){"Nebo".equalsAsciiIgnoreCase(17);23.return;}
```
