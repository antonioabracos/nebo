...
```text
Identity: g007-public:List.contains (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### sequence:contains-present-absent; expected 23

```nebo
start(){List<Int>.from([17,29]).a;a.contains(29).console();a.contains(71).console();23.return;}
```
...
### sequence:list-contains-type; expected NEBO_TYPE_MISMATCH

```nebo
start(){List<Int>.new().a;a.contains(true);23.return;}
```
