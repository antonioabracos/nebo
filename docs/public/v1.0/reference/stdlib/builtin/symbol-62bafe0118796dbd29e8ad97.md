...
```text
Identity: g010-public:Column<Int>.from (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### data:column-return-23; expected 23

```nebo
start(){Column<Int>.from([17,29]).c;23.return;}
```
...
### data:column-from-type; expected NEBO_TYPE_MISMATCH

```nebo
start(){Column<Int>.from(17).c;23.return;}
```
