...
```text
Identity: g010-public:Dataset.repartition (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### data:repartition-arity; expected NEBO_TYPE_MISMATCH

```nebo
start(){Table.fromColumns([Tuple.of("age",Column<Int>.from([17]))]).t;Dataset.fromTables([t]).d;d.repartition();23.return;}
```
