...
```text
Identity: g007-public:List.get (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### sequence:list-get-present; expected 29

```nebo
start(){List<Int>.from([17,29]).items;items.get(1).expect("v").return;}
```
...
### sequence:list-get-type; expected NEBO_TYPE_MISMATCH

```nebo
start(){List<Int>.new().a;a.get(true);23.return;}
```
