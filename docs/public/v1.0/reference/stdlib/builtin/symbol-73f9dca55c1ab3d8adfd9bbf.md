...
```text
Identity: g007-public:Queue<Int>.new (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### sequence:Queue-return-23; expected 23

```nebo
start(){Queue<Int>.new().items;23.return;}
```
...
### sequence:Queue-element-type; expected NEBO_TYPE_MISMATCH

```nebo
start(){Queue<Text>.new().a;23.return;}
```
