...
```text
Identity: g007-public:iterator.next (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### sequence:iterator-empty; expected 83

```nebo
start(){List<Int>.new().a;a.iterator().i;i.next().unwrapOr(83).return;}
```
...
### sequence:iterator-released; expected NEBO_USE_AFTER_MOVE

```nebo
start(){List<Int>.new().a;a.iterator().i;i.release();i.next();23.return;}
```
