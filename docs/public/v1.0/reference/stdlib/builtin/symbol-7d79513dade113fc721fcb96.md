...
Spelling alias for g009-public:Graph.hasCycle; no additional API counted; preserve qualified profile limits; native intrinsic identity is receiver-qualified; no serialized SymbolId or DocRecord is declared by this owner; interface metadata is verified by the separate metadata suite Int64 payloads; 32 nodes/256 edges; authenticated handle/owner pair; no stale or foreign handle

```text
Identity: claim:1069f6270bef9e4c51e0eb71 (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
Exact typed owner contract; no copy/borrow/clone capability inferred from spelling Spelling alias for g009-public:Graph.hasCycle; no additional API counted; preserve qualified profile limits; native intrinsic identity is receiver-qualified; no serialized SymbolId or DocRecord is declared by this owner; interface metadata is verified by the separate metadata suite Int64 payloads; 32 nodes/256 edges; authenticated handle/owner pair; no stale or foreign handle
...
Edition 1; x86_64-systemv-elf-linux; BOUNDED_PUBLIC_EXECUTED_WITHIN_LIMITS. Spelling alias for g009-public:Graph.hasCycle; no additional API counted; preserve qualified profile limits; native intrinsic identity is receiver-qualified; no serialized SymbolId or DocRecord is declared by this owner; interface metadata is verified by the separate metadata suite Int64 payloads; 32 nodes/256 edges; authenticated handle/owner pair; no stale or foreign handle Use the declared operand and receiver domain; source rejection publishes no executable.
...
### relation:graph-acyclic; expected 23

```nebo
start(){Graph<Int,Int>.directed().g;g.addNode(17).a;g.addNode(29).b;g.addNode(71).c;g.addNode(83).d;g.addEdge(a,b,5);g.addEdge(a,c,11);g.addEdge(b,d,7);g.addEdge(c,d,3);g.hasCycle().console();23.return;}
```
...
### relation:graph-hasCycle-arity; expected NEBO_TYPE_MISMATCH

```nebo
start(){Graph<Int,Int>.directed().g;g.hasCycle(17);23.return;}
```
