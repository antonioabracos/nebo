...
```text
Identity: g010-public:Table.rowCount (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### data:table-dimensions-empty; expected 23

```nebo
start(){Table.fromColumns([Tuple.of("age",Column<Int>.from([])),Tuple.of("score",Column<Int>.from([]))]).t;t.rowCount().console();t.columnCount().console();23.return;}
```

Oracle: {"capabilities": {"console": "RETAINED_DOCUMENT", "filesystem": "EXPLICIT_SCRATCH_EFFECT_ORACLE", "network": "DENIED_BY_SECCOMP"}, "console_text_utf8": "02", "filesystem_effects": {}, "independent_builds": 2, "kinds": \[4, 4\], "process_exit": 23, "runtime_determinism": "BYTE_IDENTICAL", "runtime_sha256": "58517d7d720510e6300bbffe63887944e5788c3c5d1a7852e1c8224585383abf", "text": {"bytes_hex": "3032"}}

## Rejected examples

### data:table-row-count-arity; expected NEBO_TYPE_MISMATCH

```nebo
start(){Table.fromColumns([Tuple.of("age",Column<Int>.from([17]))]).t;t.rowCount(17);23.return;}
```
