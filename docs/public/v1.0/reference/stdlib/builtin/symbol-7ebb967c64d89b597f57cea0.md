...
```text
Identity: g007-public:List<Int>.from (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### sequence:from-empty; expected 0

```nebo
start(){List<Int>.from([]).items;items.length().return;}
```
...
### sequence:from-type; expected NEBO_TYPE_MISMATCH

```nebo
start(){List<Int>.from(17).a;23.return;}
```
