# builtin — Text.replaceOnce

replaceOnce(Text,Text); 4096 output bytes; non-overlapping; ; owned caller workspace; public UTF-8 descriptor bridge and distinct bounded function storage; native intrinsic identity is receiver-qualified; no serialized SymbolId or DocRecord is declared by this owner; interface metadata is verified by the separate metadata suite UTF-8 byte/codepoint/grapheme units are distinct; per-member finite profile

```text
Identity: claim:28520f06cc389966c57b1350 (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
Exact typed owner contract; no copy/borrow/clone capability inferred from spelling replaceOnce(Text,Text); 4096 output bytes; non-overlapping; ; owned caller workspace; public UTF-8 descriptor bridge and distinct bounded function storage; native intrinsic identity is receiver-qualified; no serialized SymbolId or DocRecord is declared by this owner; interface metadata is verified by the separate metadata suite UTF-8 byte/codepoint/grapheme units are distinct; per-member finite profile
...
Edition 1; x86_64-systemv-elf-linux; BOUNDED_PUBLIC_EXECUTED_WITHIN_LIMITS. replaceOnce(Text,Text); 4096 output bytes; non-overlapping; ; owned caller workspace; public UTF-8 descriptor bridge and distinct bounded function storage; native intrinsic identity is receiver-qualified; no serialized SymbolId or DocRecord is declared by this owner; interface metadata is verified by the separate metadata suite UTF-8 byte/codepoint/grapheme units are distinct; per-member finite profile Use the declared operand and receiver domain; source rejection publishes no executable.
...
### text:replaceOnce-2; expected 23

```nebo
start(){"ação ação".replaceOnce("ação","é").console();23.return;}
```

Oracle: {"capabilities": {"console": "RETAINED_DOCUMENT", "filesystem": "EXPLICIT_SCRATCH_EFFECT_ORACLE", "network": "DENIED_BY_SECCOMP"}, "console_text_utf8": "é ação", "filesystem_effects": {}, "independent_builds": 2, "kinds": \[2\], "process_exit": 23, "publications": 1, "runtime_determinism": "BYTE_IDENTICAL", "runtime_sha256": "b95f035c439d524d7b7ff85025fb64b65f120f1677986435713d6418bc3213a2", "text": {"bytes_hex": "c3a92061c3a7c3a36f"}}

## Rejected examples

### text:type-replaceOnce; expected NEBO-TEXT-TRANSFORM-TEXT-ARGUMENT

```nebo
start(){"Nebo".replaceOnce(17,"x");23.return;}
```
