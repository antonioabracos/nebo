...
Spelling alias for g009-public:Tree.children; no additional API counted; preserve qualified profile limits; native intrinsic identity is receiver-qualified; no serialized SymbolId or DocRecord is declared by this owner; interface metadata is verified by the separate metadata suite Int64 payloads; 32 nodes/256 edges; authenticated handle/owner pair; no stale or foreign handle

```text
Identity: claim:57b9a3fd10dc01e0aea508b1 (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
Exact typed owner contract; no copy/borrow/clone capability inferred from spelling Spelling alias for g009-public:Tree.children; no additional API counted; preserve qualified profile limits; native intrinsic identity is receiver-qualified; no serialized SymbolId or DocRecord is declared by this owner; interface metadata is verified by the separate metadata suite Int64 payloads; 32 nodes/256 edges; authenticated handle/owner pair; no stale or foreign handle
...
Edition 1; x86_64-systemv-elf-linux; BOUNDED_PUBLIC_EXECUTED_WITHIN_LIMITS. Spelling alias for g009-public:Tree.children; no additional API counted; preserve qualified profile limits; native intrinsic identity is receiver-qualified; no serialized SymbolId or DocRecord is declared by this owner; interface metadata is verified by the separate metadata suite Int64 payloads; 32 nodes/256 edges; authenticated handle/owner pair; no stale or foreign handle Use the declared operand and receiver domain; source rejection publishes no executable.
...
### relation:tree-child-order; expected 23

```nebo
start(){Tree<Int>.new(17).t;t.root().r;t.addChild(r,29);t.addChild(r,71);t.children(r).v;v.length().console();v.next().expect("node").value().console();v.next().expect("node").value().console();v.next().isNone().console();23.return;}
```

Oracle: {"capabilities": {"console": "RETAINED_DOCUMENT", "filesystem": "EXPLICIT_SCRATCH_EFFECT_ORACLE", "network": "DENIED_BY_SECCOMP"}, "console_text_utf8": "22971true", "filesystem_effects": {}, "independent_builds": 2, "kinds": \[4, 4, 4, 5\], "process_exit": 23, "runtime_determinism": "BYTE_IDENTICAL", "runtime_sha256": "c512a8b4fb07cc24284985cfffb53a56b41b5c966da3d77124fe52fb21cfc904", "text": {"bytes_hex": "323239373174727565"}}

## Rejected examples

### relation:tree-children-node-type; expected NEBO_TYPE_MISMATCH

```nebo
start(){Tree<Int>.new(17).t;t.children(17);23.return;}
```
