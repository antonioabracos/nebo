...
Spelling alias for g009-public:Tree.preorder; no additional API counted; preserve qualified profile limits; native intrinsic identity is receiver-qualified; no serialized SymbolId or DocRecord is declared by this owner; interface metadata is verified by the separate metadata suite Int64 payloads; 32 nodes/256 edges; authenticated handle/owner pair; no stale or foreign handle

```text
Identity: claim:45ec8d7b2a67419031f67091 (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
Exact typed owner contract; no copy/borrow/clone capability inferred from spelling Spelling alias for g009-public:Tree.preorder; no additional API counted; preserve qualified profile limits; native intrinsic identity is receiver-qualified; no serialized SymbolId or DocRecord is declared by this owner; interface metadata is verified by the separate metadata suite Int64 payloads; 32 nodes/256 edges; authenticated handle/owner pair; no stale or foreign handle
...
Edition 1; x86_64-systemv-elf-linux; BOUNDED_PUBLIC_EXECUTED_WITHIN_LIMITS. Spelling alias for g009-public:Tree.preorder; no additional API counted; preserve qualified profile limits; native intrinsic identity is receiver-qualified; no serialized SymbolId or DocRecord is declared by this owner; interface metadata is verified by the separate metadata suite Int64 payloads; 32 nodes/256 edges; authenticated handle/owner pair; no stale or foreign handle Use the declared operand and receiver domain; source rejection publishes no executable.
...
### relation:tree-preorder; expected 23

```nebo
start(){Tree<Int>.new(17).t;t.root().r;t.addChild(r,29).a;t.addChild(r,71);t.addChild(a,83);t.preorder().v;v.next().expect("node").value().console();v.next().expect("node").value().console();v.next().expect("node").value().console();v.next().expect("node").value().console();23.return;}
```

Oracle: {"capabilities": {"console": "RETAINED_DOCUMENT", "filesystem": "EXPLICIT_SCRATCH_EFFECT_ORACLE", "network": "DENIED_BY_SECCOMP"}, "console_text_utf8": "17298371", "filesystem_effects": {}, "independent_builds": 2, "kinds": \[4, 4, 4, 4\], "process_exit": 23, "runtime_determinism": "BYTE_IDENTICAL", "runtime_sha256": "dde3da602c5f98b3f7c2b6ee0ded96416679bdf1e0694e655953a20c50c97c55", "text": {"bytes_hex": "3137323938333731"}}

## Rejected examples

### relation:tree-preorder-arity; expected NEBO_TYPE_MISMATCH

```nebo
start(){Tree<Int>.new(17).t;t.preorder(17);23.return;}
```
