...
```text
Identity: g007-public:List.at (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### sequence:from-many; expected 23

```nebo
start(){List<Int>.from([17,29,71]).items;items.at(0).console();items.at(1).console();items.at(2).console();23.return;}
```

Oracle: {"capabilities": {"console": "RETAINED_DOCUMENT", "filesystem": "EXPLICIT_SCRATCH_EFFECT_ORACLE", "network": "DENIED_BY_SECCOMP"}, "console_text_utf8": "172971", "filesystem_effects": {}, "independent_builds": 2, "kinds": \[4, 4, 4\], "process_exit": 23, "runtime_determinism": "BYTE_IDENTICAL", "runtime_sha256": "18092c90e408382784d63a0effceb8e9a49e4bcad8e304f9bc8c2a023d23598c", "text": {"bytes_hex": "313732393731"}}

## Rejected examples

### sequence:list-at-type; expected NEBO_TYPE_MISMATCH

```nebo
start(){List<Int>.new().a;a.at(true);23.return;}
```
