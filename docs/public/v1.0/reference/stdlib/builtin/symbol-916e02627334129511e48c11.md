...
```text
Identity: g007-public:List.reserve (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### sequence:list-reserve; expected 38

```nebo
start(){List<Int>.from([17,29]).items;items.reserve(7);(items.capacity()+items.at(1)).return;}
```

Oracle: {"capabilities": {"console": "NO_DOCUMENT_OBSERVER", "filesystem": "EXPLICIT_SCRATCH_EFFECT_ORACLE", "network": "DENIED_BY_SECCOMP"}, "filesystem_effects": {}, "independent_builds": 2, "process_exit": 38, "runtime_determinism": "BYTE_IDENTICAL", "runtime_sha256": "e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855"}

## Rejected examples

### sequence:list-reserve-type; expected NEBO_TYPE_MISMATCH

```nebo
start(){List<Int>.new().a;a.reserve(true);23.return;}
```
