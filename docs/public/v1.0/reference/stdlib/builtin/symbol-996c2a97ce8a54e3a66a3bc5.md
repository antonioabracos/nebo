...
```text
Identity: g010-public:Stream.map (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### data:stream-lazy; expected 23

```nebo
(Int.self)step(){self.console();(self+7).return;}start(){"ready".console();Stream<Int>.from([17,71]).s;s.map(step).out;23.return;}
```

Oracle: {"capabilities": {"console": "RETAINED_DOCUMENT", "filesystem": "EXPLICIT_SCRATCH_EFFECT_ORACLE", "network": "DENIED_BY_SECCOMP"}, "console_text_utf8": "ready", "filesystem_effects": {}, "independent_builds": 2, "kinds": \[2\], "process_exit": 23, "runtime_determinism": "BYTE_IDENTICAL", "runtime_sha256": "f4873070899794c04dfb3951f56cc350ba2f59e02848efde3c962acade2bf6b5", "text": {"bytes_hex": "7265616479"}}

## Rejected examples

### data:stream-map-type; expected NEBO_TYPE_MISMATCH

```nebo
start(){Stream<Int>.from([17]).s;s.map(7);23.return;}
```
