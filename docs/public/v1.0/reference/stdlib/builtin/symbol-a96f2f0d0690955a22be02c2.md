...
```text
Identity: g010-public:Stream.batch.sink (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### data:batch-sink-empty-2; expected 0

```nebo
(List<Int>.self)emit(){self.length().console();0.i.mutable;while(i<self.length()){self.at(i).console();i+=1;}}start(){Stream<Int>.from([]).s;s.batch(2).b;b.sink(emit).return;}
```
...
### data:batch-sink-scalar-receiver; expected NEBO_TYPE_MISMATCH

```nebo
(Int.self)emit(){self.console();}start(){Stream<Int>.from([17]).s;s.batch(1).b;b.sink(emit);23.return;}
```
