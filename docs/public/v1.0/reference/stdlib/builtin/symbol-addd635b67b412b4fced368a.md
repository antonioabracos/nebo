# builtin — Stream.window

Finite 64-event profile; Event.sequence supplies monotonic logical timestamps; positive Int duration; lazy nonempty windows yield independent owned Lists; no next-window callback executes early; native registry identity and runtime profile are explicit; serialized interface metadata is verified by the separate metadata suite Typed nullable integer rows/columns/tables/datasets/streams; finite schema, rows and stages; lazy callbacks once

```text
Identity: g010-public:Stream.window (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### data:window-return-23; expected 23

```nebo
start(){Stream<Int>.from([17,29,71]).s;s.window(2).w;w.next().expect("v").at(1).console();23.return;}
```

Oracle: {"capabilities": {"console": "RETAINED_DOCUMENT", "filesystem": "EXPLICIT_SCRATCH_EFFECT_ORACLE", "network": "DENIED_BY_SECCOMP"}, "console_text_utf8": "29", "filesystem_effects": {}, "independent_builds": 2, "kinds": \[4\], "process_exit": 23, "runtime_determinism": "BYTE_IDENTICAL", "runtime_sha256": "370d97f6e26f23ed81e9c021bd2ab29ed3ebfce8dd258bf43154946597952637", "text": {"bytes_hex": "3239"}}

## Rejected examples

### data:window-missing-duration; expected NEBO_TYPE_MISMATCH

```nebo
start(){Stream<Int>.from([17,29,71]).s;s.window();23.return;}
```
