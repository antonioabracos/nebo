...
```text
Identity: g007-public:Stack<Int>.new (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### sequence:Stack-return-23; expected 23

```nebo
start(){Stack<Int>.new().items;23.return;}
```
...
### sequence:Stack-element-type; expected NEBO_TYPE_MISMATCH

```nebo
start(){Stack<Text>.new().a;23.return;}
```
