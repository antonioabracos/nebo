...
```text
Identity: g007-public:List.insert (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### sequence:list-insert; expected 71

```nebo
start(){List<Int>.from([17,29]).items;items.insert(1,71);items.at(1).return;}
```
...
### sequence:list-insert-type; expected NEBO_TYPE_MISMATCH

```nebo
start(){List<Int>.new().a;a.insert(0,true);23.return;}
```
