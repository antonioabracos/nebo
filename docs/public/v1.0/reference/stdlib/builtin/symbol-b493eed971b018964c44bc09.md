...
```text
Identity: g007-public:iterator.take (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### sequence:iterator-take-zero; expected 83

```nebo
start(){List<Int>.from([17,29]).a;a.iterator().i;i.take(0);i.next().unwrapOr(83).return;}
```
...
### sequence:iterator-take-type; expected NEBO_TYPE_MISMATCH

```nebo
start(){List<Int>.new().a;a.iterator().i;i.take(true);23.return;}
```
