...
```text
Identity: g007-public:Stack.length (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### sequence:Stack-empty; expected 0

```nebo
start(){Stack<Int>.new().items;items.length().return;}
```
...
### sequence:Stack-copy; expected NEBO_COPY_UNIQUE

```nebo
start(){Stack<Int>.new().a;a.b;23.return;}
```
