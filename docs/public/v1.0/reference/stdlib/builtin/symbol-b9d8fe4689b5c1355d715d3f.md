...
```text
Identity: g007-public:iterator.skip (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### sequence:iterator-skip-past; expected 83

```nebo
start(){List<Int>.from([17,29]).a;a.iterator().i;i.skip(9);i.next().unwrapOr(83).return;}
```
...
### sequence:iterator-skip-type; expected NEBO_TYPE_MISMATCH

```nebo
start(){List<Int>.new().a;a.iterator().i;i.skip(true);23.return;}
```
