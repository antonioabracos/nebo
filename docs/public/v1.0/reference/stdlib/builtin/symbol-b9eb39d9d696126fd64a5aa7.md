...
```text
Identity: g007-public:List.pop (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### sequence:list-pop-empty; expected 23

```nebo
start(){List<Int>.new().a;a.pop().isNone().console();23.return;}
```
...
### sequence:list-pop-arity; expected NEBO_TYPE_MISMATCH

```nebo
start(){List<Int>.new().a;a.pop(17);23.return;}
```
