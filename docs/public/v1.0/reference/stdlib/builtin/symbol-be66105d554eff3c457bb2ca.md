...
```text
Identity: g007-public:List.sort (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### sequence:sort-False; expected 23

```nebo
(Int.self)order(Int.other){(self<=>other).return;}start(){List<Int>.from([29,-17,71]).a;a.sort(order);a.at(0).console();a.at(1).console();a.at(2).console();23.return;}
```

Oracle: {"capabilities": {"console": "RETAINED_DOCUMENT", "filesystem": "EXPLICIT_SCRATCH_EFFECT_ORACLE", "network": "DENIED_BY_SECCOMP"}, "console_text_utf8": "-172971", "filesystem_effects": {}, "independent_builds": 2, "kinds": \[4, 4, 4\], "process_exit": 23, "runtime_determinism": "BYTE_IDENTICAL", "runtime_sha256": "fac900efbade1df2aafb1eee42fd833ad2562e73cad78b779ede2528b96377e2", "text": {"bytes_hex": "2d313732393731"}}

## Rejected examples

### sequence:list-sort-type; expected NEBO_TYPE_MISMATCH

```nebo
start(){List<Int>.new().a;a.sort(true);23.return;}
```
