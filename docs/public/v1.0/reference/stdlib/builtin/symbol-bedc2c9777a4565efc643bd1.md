...
```text
Identity: g007-public:List.map (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### sequence:map-empty; expected 0

```nebo
(Int.self)transform(){self.console();(self+7).return;}start(){List<Int>.new().a;a.map(transform).b;b.length().return;}
```
...
### sequence:list-map-type; expected NEBO_TYPE_MISMATCH

```nebo
start(){List<Int>.new().a;a.map(true);23.return;}
```
