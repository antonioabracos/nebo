...
```text
Identity: claim:7fe225c46198adab3c4282f7 (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### text:normalizeNfd-0; expected 23

```nebo
start(){"".normalizeNfd().console();23.return;}
```
...
### text:arity-normalizeNfd; expected NEBO-TEXT-TRANSFORM-ARITY

```nebo
start(){"Nebo".normalizeNfd(1);23.return;}
```
