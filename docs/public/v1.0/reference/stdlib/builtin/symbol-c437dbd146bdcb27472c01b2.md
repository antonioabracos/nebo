...
```text
Identity: g009-public:Graph.depthFirst (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### relation:graph-depthFirst; expected 23

```nebo
start(){Graph<Int,Int>.directed().g;g.addNode(17).a;g.addNode(29).b;g.addNode(71).c;g.addNode(83).d;g.addEdge(a,b,5);g.addEdge(a,c,11);g.addEdge(b,d,7);g.addEdge(c,d,3);g.depthFirst(a).v;v.next().expect("n").value().console();v.next().expect("n").value().console();v.next().expect("n").value().console();v.next().expect("n").value().console();v.next().isNone().console();v.release();g.addNode(7).n;n.value().console();23.return;}
```

Oracle: {"capabilities": {"console": "RETAINED_DOCUMENT", "filesystem": "EXPLICIT_SCRATCH_EFFECT_ORACLE", "network": "DENIED_BY_SECCOMP"}, "console_text_utf8": "17298371true7", "filesystem_effects": {}, "independent_builds": 2, "kinds": \[4, 4, 4, 4, 5, 4\], "process_exit": 23, "runtime_determinism": "BYTE_IDENTICAL", "runtime_sha256": "69a8dbd0f916c57e1d71cdb0035ad3567f482e761214edd959f28ab6264f961a", "text": {"bytes_hex": "31373239383337317472756537"}}

## Rejected examples

### relation:graph-depthFirst-node-type; expected NEBO_TYPE_MISMATCH

```nebo
start(){Graph<Int,Int>.directed().g;g.depthFirst(17);23.return;}
```
