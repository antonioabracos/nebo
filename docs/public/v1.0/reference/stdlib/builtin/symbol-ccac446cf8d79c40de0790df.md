...
```text
Identity: g009-public:Graph.removeNode (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### relation:graph-directed-remove-node; expected 0

```nebo
start(){Graph<Int,Int>.directed().g;g.addNode(17).a;g.addNode(71).b;g.addEdge(a,b,5).e;g.removeNode(b);g.neighbors(a).length().return;}
```
...
### relation:graph-removeNode-node-type; expected NEBO_TYPE_MISMATCH

```nebo
start(){Graph<Int,Int>.directed().g;g.removeNode(17);23.return;}
```
