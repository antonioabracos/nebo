...
```text
Identity: g007-public:List.push (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### sequence:list-push; expected 71

```nebo
start(){List<Int>.new().items;items.push(17);items.push(71);items.at(1).return;}
```
...
### sequence:List-wrong-value; expected NEBO_TYPE_MISMATCH

```nebo
start(){List<Int>.new().a;a.push(true);23.return;}
```
