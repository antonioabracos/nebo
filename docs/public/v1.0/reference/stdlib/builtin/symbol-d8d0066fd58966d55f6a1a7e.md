...
```text
Identity: g010-public:Row.get (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### data:row-values-17-71; expected 23

```nebo
start(){Schema.new([Tuple.of("age",Int,false),Tuple.of("score",Int,true)]).s;Row.from(s,[17,71]).r;r.get("age").expect("v").console();r.get("score").expect("v").console();23.return;}
```
...
### data:row-get-name-type; expected NEBO_TYPE_MISMATCH

```nebo
start(){Schema.new([Tuple.of("age",Int,false)]).s;Row.from(s,[17]).r;r.get(17);23.return;}
```
