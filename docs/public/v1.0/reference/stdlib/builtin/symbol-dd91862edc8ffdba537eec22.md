...
```text
Identity: g007-public:List.find (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### sequence:find-absent; expected 83

```nebo
(Int.self)choose(){(self>80).return;}start(){List<Int>.from([17,29,71]).a;a.find(choose).unwrapOr(83).return;}
```
...
### sequence:list-find-type; expected NEBO_TYPE_MISMATCH

```nebo
start(){List<Int>.new().a;a.find(true);23.return;}
```
