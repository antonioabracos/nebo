# builtin — Text.normalizeNfc

Text.() -&gt; Text; 4096 public workspace; 15.1 Latin acute canonical pair; caller-owned result; public UTF-8 descriptor bridge and distinct bounded function storage; native intrinsic identity is receiver-qualified; no serialized SymbolId or DocRecord is declared by this owner; interface metadata is verified by the separate metadata suite UTF-8 byte/codepoint/grapheme units are distinct; per-member finite profile

```text
Identity: claim:2bfd213e6c20704403b90651 (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
Exact typed owner contract; no copy/borrow/clone capability inferred from spelling Text.() -&gt; Text; 4096 public workspace; 15.1 Latin acute canonical pair; caller-owned result; public UTF-8 descriptor bridge and distinct bounded function storage; native intrinsic identity is receiver-qualified; no serialized SymbolId or DocRecord is declared by this owner; interface metadata is verified by the separate metadata suite UTF-8 byte/codepoint/grapheme units are distinct; per-member finite profile
...
Edition 1; x86_64-systemv-elf-linux; BOUNDED_PUBLIC_EXECUTED_WITHIN_LIMITS. Text.() -&gt; Text; 4096 public workspace; 15.1 Latin acute canonical pair; caller-owned result; public UTF-8 descriptor bridge and distinct bounded function storage; native intrinsic identity is receiver-qualified; no serialized SymbolId or DocRecord is declared by this owner; interface metadata is verified by the separate metadata suite UTF-8 byte/codepoint/grapheme units are distinct; per-member finite profile Use the declared operand and receiver domain; source rejection publishes no executable.
...
### text:normalizeNfc-0; expected 23

```nebo
start(){"".normalizeNfc().console();23.return;}
```
...
### text:arity-normalizeNfc; expected NEBO-TEXT-TRANSFORM-ARITY

```nebo
start(){"Nebo".normalizeNfc(1);23.return;}
```
