...
```text
Identity: g007-public:List.length (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### sequence:List-empty; expected 0

```nebo
start(){List<Int>.new().items;items.length().return;}
```
...
### sequence:list-length-arity; expected NEBO_TYPE_MISMATCH

```nebo
start(){List<Int>.new().a;a.length(17);23.return;}
```
