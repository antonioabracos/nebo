...
```text
Identity: g010-public:Dataset.collect.next (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### data:collect-next-arity; expected NEBO_TYPE_MISMATCH

```nebo
start(){Table.fromColumns([Tuple.of("age",Column<Int>.from([17]))]).t;Dataset.fromTables([t]).d;d.collect().rows;rows.next(17);23.return;}
```
