# builtin — Text.startsWith

startsWith(Text); literal bytes; O(m); ; borrowed immutable receiver and argument; public UTF-8 descriptor bridge and distinct bounded function storage; native intrinsic identity is receiver-qualified; no serialized SymbolId or DocRecord is declared by this owner; interface metadata is verified by the separate metadata suite UTF-8 byte/codepoint/grapheme units are distinct; per-member finite profile

```text
Identity: claim:7d801be18c4ca393da223ca0 (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
Exact typed owner contract; no copy/borrow/clone capability inferred from spelling startsWith(Text); literal bytes; O(m); ; borrowed immutable receiver and argument; public UTF-8 descriptor bridge and distinct bounded function storage; native intrinsic identity is receiver-qualified; no serialized SymbolId or DocRecord is declared by this owner; interface metadata is verified by the separate metadata suite UTF-8 byte/codepoint/grapheme units are distinct; per-member finite profile
...
Edition 1; x86_64-systemv-elf-linux; BOUNDED_PUBLIC_EXECUTED_WITHIN_LIMITS. startsWith(Text); literal bytes; O(m); ; borrowed immutable receiver and argument; public UTF-8 descriptor bridge and distinct bounded function storage; native intrinsic identity is receiver-qualified; no serialized SymbolId or DocRecord is declared by this owner; interface metadata is verified by the separate metadata suite UTF-8 byte/codepoint/grapheme units are distinct; per-member finite profile Use the declared operand and receiver domain; source rejection publishes no executable.
...
### text:startsWith-2; expected 23

```nebo
start(){"Nebo".startsWith("absent").console();23.return;}
```
...
### text:type-startsWith; expected NEBO-TEXT-QUERY-ARGUMENT-MUST-BE-TEXT

```nebo
start(){"Nebo".startsWith(17);23.return;}
```
