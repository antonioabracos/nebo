...
```text
Identity: g007-public:Queue.dequeue (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### sequence:queue-dequeue-empty; expected 23

```nebo
start(){Queue<Int>.new().a;a.dequeue().isNone().console();23.return;}
```
...
### sequence:Queue-copy; expected NEBO_COPY_UNIQUE

```nebo
start(){Queue<Int>.new().a;a.b;23.return;}
```
