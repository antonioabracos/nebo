...
```text
Identity: g007-public:List.retain (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### sequence:retain-less; expected 23

```nebo
(Int.self)choose(){(self<20).return;}start(){List<Int>.from([29,-17,71]).a;a.retain(choose);a.length().console();a.at(0).console();23.return;}
```

Oracle: {"capabilities": {"console": "RETAINED_DOCUMENT", "filesystem": "EXPLICIT_SCRATCH_EFFECT_ORACLE", "network": "DENIED_BY_SECCOMP"}, "console_text_utf8": "1-17", "filesystem_effects": {}, "independent_builds": 2, "kinds": \[4, 4\], "process_exit": 23, "runtime_determinism": "BYTE_IDENTICAL", "runtime_sha256": "677e33b61808ae19f11279c658be0cc08dfc821a4d6a46e9d3e7efa122b1e825", "text": {"bytes_hex": "312d3137"}}

## Rejected examples

### sequence:list-retain-type; expected NEBO_TYPE_MISMATCH

```nebo
start(){List<Int>.new().a;a.retain(true);23.return;}
```
