...
```text
Identity: g009-public:traversal.path (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### relation:traversal-bfs-path; expected 23

```nebo
start(){Graph<Int,Int>.directed().g;g.addNode(17).a;g.addNode(29).b;g.addNode(71).c;g.addNode(83).d;g.addEdge(a,b,5);g.addEdge(a,c,11);g.addEdge(b,d,7);g.addEdge(c,d,3);g.breadthFirst(a).v;v.next();v.next();v.next();v.next();v.path().p;p.next().expect("n").value().console();p.next().expect("n").value().console();p.next().expect("n").value().console();23.return;}
```

Oracle: {"capabilities": {"console": "RETAINED_DOCUMENT", "filesystem": "EXPLICIT_SCRATCH_EFFECT_ORACLE", "network": "DENIED_BY_SECCOMP"}, "console_text_utf8": "172983", "filesystem_effects": {}, "independent_builds": 2, "kinds": \[4, 4, 4\], "process_exit": 23, "runtime_determinism": "BYTE_IDENTICAL", "runtime_sha256": "7eb8719b7b83e47df497d23e2b5286f43e9ae7ebd166bc78c2f2e68cd4a50168", "text": {"bytes_hex": "313732393833"}}

## Rejected examples

### relation:traversal-path-arity; expected NEBO_TYPE_MISMATCH

```nebo
start(){Graph<Int,Int>.directed().g;g.addNode(17).a;g.breadthFirst(a).v;v.path(17);23.return;}
```
