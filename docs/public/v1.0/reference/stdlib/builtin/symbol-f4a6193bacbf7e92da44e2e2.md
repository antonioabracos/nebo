...
```text
Identity: g010-public:Table.select (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### data:table-select-subset; expected 1

```nebo
(Row.self)observe(){self.get("score").expect("v").console();true.return;}start(){Table.fromColumns([Tuple.of("age",Column<Int>.from([71,17,29])),Tuple.of("score",Column<Int>.from([83,7,53]))]).t;t.select(["score"]).out;out.filter(observe);out.columnCount().return;}
```

Oracle: {"capabilities": {"console": "RETAINED_DOCUMENT", "filesystem": "EXPLICIT_SCRATCH_EFFECT_ORACLE", "network": "DENIED_BY_SECCOMP"}, "console_text_utf8": "83753", "filesystem_effects": {}, "independent_builds": 2, "kinds": \[4, 4, 4\], "process_exit": 1, "runtime_determinism": "BYTE_IDENTICAL", "runtime_sha256": "f3fdac7426139812c453d18bbecd7f825bfc88cf431821c66dcc3b8b862190cc", "text": {"bytes_hex": "3833373533"}}

## Rejected examples

### data:table-select-empty; expected NEBO_LIMIT_EXCEEDED

```nebo
start(){Table.fromColumns([Tuple.of("age",Column<Int>.from([17]))]).t;t.select([]);23.return;}
```
