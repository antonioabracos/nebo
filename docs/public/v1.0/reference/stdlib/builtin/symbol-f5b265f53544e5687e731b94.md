...
```text
Identity: g010-public:Stream.next (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### data:stream-next-arity; expected NEBO_TYPE_MISMATCH

```nebo
start(){Stream<Int>.from([17]).s;s.next(7);23.return;}
```
