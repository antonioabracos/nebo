# std.fs — File.writeText

Writes the requested UTF-8 bytes with truncate, append or atomic mode; returns written byte count. Failure preserves prior published state for the atomic mode.

```text
Identity: qualified:File.writeText (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
