# std.fs — File.readText

Reads validated UTF-8 below the caller-owned directory; rejects symlinks, absent files, invalid encoding and exceeded byte limit. Releases native descriptors.

```text
Identity: qualified:File.readText (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
