...
Combines two typed paths; a Text argument is not a Path.

```text
Identity: qualified:Path.join (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### filesystem:path-join; expected 23

```nebo
import "std.fs" { Path; File; }.fs;
start(){Path.parse("folder").a;Path.parse("child/../third.txt").b;a.join(b).normalize().fileName().console();23.return;}
```

Oracle: {"capabilities": {"console": "RETAINED_DOCUMENT", "filesystem": "EXPLICIT_SCRATCH_EFFECT_ORACLE", "network": "DENIED_BY_SECCOMP"}, "console_text_utf8": "third.txt", "filesystem_effects": {}, "independent_builds": 2, "kinds": \[2\], "process_exit": 23, "publications": 1, "runtime_determinism": "BYTE_IDENTICAL", "runtime_sha256": "9a599df1e273c5cc7f5f60232a9b9ba74d045662b66f2af4ea071e0742f5c6f3", "text": {"bytes_hex": "74686972642e747874"}}
