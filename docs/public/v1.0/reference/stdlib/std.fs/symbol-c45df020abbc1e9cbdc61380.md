...
Eliminates lexical dot segments within the admitted path boundary.

```text
Identity: qualified:Path.normalize (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
### filesystem:path-independent; expected 23

```nebo
import "std.fs" { Path; File; }.fs;
start(){Path.parse("first.txt").a;Path.parse("second.bin").b;a.normalize().fileName().console();23.return;}
```

Oracle: {"capabilities": {"console": "RETAINED_DOCUMENT", "filesystem": "EXPLICIT_SCRATCH_EFFECT_ORACLE", "network": "DENIED_BY_SECCOMP"}, "console_text_utf8": "first.txt", "filesystem_effects": {}, "independent_builds": 2, "kinds": \[2\], "process_exit": 23, "publications": 1, "runtime_determinism": "BYTE_IDENTICAL", "runtime_sha256": "82215f0ee616e5d43a94f4fba878b8dd33a8edc3d44bbf671ae62d4fb9b4688f", "text": {"bytes_hex": "66697273742e747874"}}
