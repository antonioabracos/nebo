...
```text
Identity: NSR-DOM-029 (QUALIFIED_INTRINSIC_OR_REGISTRY_ID)
Edition: 1
...
result_rule: Set
canonical_ascii: symmetricDifference()
```
...
### operator:set-e296b3-4; expected 23

```nebo
start(){Array<Int,0> [].left;Array<Int,0> [].right;(left △ right).cardinality().console();23.return;}
```
...
### operator:wrong-set-e296b3; expected NEBO_TYPE_MISMATCH
