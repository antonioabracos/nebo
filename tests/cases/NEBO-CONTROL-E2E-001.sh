...
source=tests/e2e/core/control/if-true-short-circuit.no
...
grep -q 'jz \.nebo_bool_false_' "$tmp/a.asm"
...
for exe in "$tmp/a" "$tmp/b"; do
  ...
  printf 'MF039_AND_RUN expected=3 observed=%s\n' "$status"
  test "$status" -eq 3; test ! -s "$exe.out"; test ! -s "$exe.err"
  ./scripts/mf037/verify-no-c-structural.sh "$exe" >/dev/null
done
echo NEBO_CONTROL_E2E_001_GREEN
